...
def main(args):
    ...
    for path in paths:
        with np.load(path, allow_pickle=False) as archive:
            a = {key: archive[key].copy(order='K') for key in archive.files}
        ...
        np.testing.assert_array_equal(old[1], (a['expanded_xx'] + a['expanded_xx'].T) * .5)
        ...
        for i, quantity in enumerate(('gradient', 'hessian')):
            ...
            row['entries'][quantity] = dict(
                ...
                archived_grouped_max_difference=float(np.max(np.abs(new[i] - (
                    a['grouped_x'] if i == 0 else (a['grouped_xx'] + a['grouped_xx'].T) * .5)))),
            )
