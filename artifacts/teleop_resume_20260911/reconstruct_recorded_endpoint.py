"""Recover complete native state at the end or a declared boundary of an audited trace."""
...
def main(args):
    ...
    full_trace_controls = count
    if args.prefix_controls is not None:
        if not 0 < args.prefix_controls <= full_trace_controls:
            raise ValueError('Declared prefix must lie within the complete independently audited trace')
        count = args.prefix_controls
    is_prefix = count < full_trace_controls
    ...
    compare(0)
    step = 0
    for target in trace['target'][:count]:
        for _ in range(10):
    ...
    np.savez_compressed(endpoint_path, final_integration=endpoint, integration_state_spec=spec,
        ...
        original_full_trace_controls=full_trace_controls, is_prefix_boundary=is_prefix,
    ...
    report = dict(kind=('independent_native_prefix_integration_endpoint_reconstruction' if is_prefix
                        else 'independent_native_full_integration_endpoint_reconstruction'),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prefix-controls', type=int,
        help='Comparison-only endpoint inside the original complete audited trace; does not qualify a new continuation')
