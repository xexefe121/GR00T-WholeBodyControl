...
parser.add_argument('--run-name', default='hard_feasibility_continuation_3740_v1')
parser.add_argument('--output-name', default='bounded_recovery_plot_v2')
args = parser.parse_args()
new_input = NEW / args.run_name / 'trace.npz'
producer_path = new_input.parent / 'report.json'
...
post = np.arange(3740, 3740 + len(new['qpos']))
source = (post - 350) * .02
frames = post + 10
...
ax[1, 0].set(ylabel='Absolute heading error (degrees)', title='Heading error against original intent')
...
for pane in ax.flat:
    ...
    pane.set(xlim=(67.8, source[-1]), xlabel='Original source time (s)')
    pane.grid(alpha=.18)
requested_seconds = producer['requested_controls'] * .02
completion = ('requested physical segment complete' if producer.get('completed')
              else 'stopped: ' + str(producer.get('failure')))
fig.suptitle(f'Native23 PICO repair: {(len(new["qpos"])-1)*.02:.1f}s / {requested_seconds:.1f}s; {completion}\n'
             'Actual physics; restored initial state at 67.8s. Full-source / live teleoperation unqualified.', fontsize=12)
output = NEW / args.output_name
...
(output / 'report.json').write_text(json.dumps(dict(
    ...
    metric_controls=len(new['qpos'])-1, source_seconds=[67.8, float(source[-1])], initial_sample_excluded_from_aggregate=True,
