"""Prepare immutable, bounded original-BFM entry evidence. No policy or physics calls."""
...
PILOT=NEW/'fast_controller_nominal_pilot_v1'
DEST=NEW/'original_bfm_entry250_v1'
DEST.mkdir(exist_ok=False)
SNAP=DEST/'source_snapshot_v1'
SNAP.mkdir()
sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
old=json.loads((PILOT/'frozen_inputs_v2.json').read_text())
for name,value in old['source_sha256'].items():
    source=PILOT/'source_snapshot_v2'/name
    assert sha(source)==value,name
    target=SNAP/name;target.parent.mkdir(parents=True,exist_ok=True)
    shutil.copy2(source,target)

driver=r'''"""ONE original h8pos1/yaw2 BFM run through canonical initial_entry250."""
import json
import os
import platform
import time
...
'''
ast.parse(driver)
(SNAP/'evaluate_entry250.py').write_text(driver)
inputs=dict(old['input_sha256'])
for name in ('frozen_inputs_v2.json','fit/zero_head.onnx','zero_parity/report.json','zero_parity/trace.npz'):
    p=PILOT/name;inputs[p.as_posix()]=sha(p)
receipt=dict(kind='one_original_BFM_canonical_initial_entry250',
    source_sha256={p.relative_to(SNAP).as_posix():sha(p) for p in sorted(SNAP.rglob('*')) if p.is_file()},
    input_sha256=inputs,requested_controls=250,canonical_control_start=0,canonical_control_end_exclusive=250,
    BFM_goal='unchanged original h8pos1 yaw2',residual_disabled=True,terminal_yaw4_used=False,
    original_prefix100_physics_and_commands_must_match=True,original_sources_byte_exact=True,
    physical_statewrites_after_initialization=0,policy_history_resets=0,optimizer_calls=0,new_expert_queries=0,
    hardware_authorized=False,fit_or_source_acquisition_authorized=False)
(DEST/'frozen_inputs_v2.json').write_text(json.dumps(receipt,indent=2)+'\n')
(DEST/'README.md').write_text('One bounded canonical initial_entry check: 250 controls (5 seconds), original frozen h8pos1/yaw2 BFM, residual disabled. No head switching, acquisition, source rollout, training or expert query. All original 15 pilot source files are byte-exact.\n\nThe first 100 controls must reproduce every saved native2ms position/velocity and commanded torque exactly against the original zero_parity trace. Comparison only; no recorded state or command is injected. The same plant and measured BFM history continue through control249. Native limits/warnings/clock checked every2ms. Full integration, named history, prior actions, final plant/history and all substeps retained. Last3s quiet uses unchanged thresholds. Root independent physical and quiet audits remain required.\n')
print(json.dumps(dict(directory=str(DEST),receipt_sha256=sha(DEST/'frozen_inputs_v2.json'),sources=len(receipt['source_sha256']),inputs=len(inputs))))
