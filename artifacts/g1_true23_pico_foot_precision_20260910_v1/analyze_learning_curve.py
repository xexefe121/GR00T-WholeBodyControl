...
def main(directory):
    ...
    assert outcome["completed"] and outcome["simulator_updates"] == 1000 and outcome["additional_updates"] == 500
    ...
    with np.load(bind(directory / "reward_capture.npz"), allow_pickle=False) as z:
        ...
        foot_bonus = z["foot_precision_bonus"].copy()
    ...
    for begin in range(0, 8000, 1600):
        ...
        windows.append(
            dict(
                updates=[500 + begin // 16 + 1, 500 + end // 16],
                additional_updates=[begin // 16 + 1, end // 16],
                ...
                non_done_foot_precision_bonus_mean=float(foot_bonus[sl][~done[sl]].mean()),
                sampled_source_by_clip=rows,
            )
        )
    report = dict(
        kind="preserved_learner_pico_foot_precision_learning_curve_v1",
        parent_completed_updates=500,
        additional_updates=500,
