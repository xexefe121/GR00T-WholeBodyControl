...
def main():
    output = RUN / "sampled_reference_noise_aware_audit.json"
    if output.exists():
    ...
    cfg_args = SimpleNamespace(**args)
    cfg_args.spans, cfg_args.sim_config = Path(args["spans"]), Path(args["sim_config"])
    cfg = recipe.environment_configuration(cfg_args, spec, spans)
    noise = {}
    for group_name in ("tokenizer", "policy", "root_feedback"):
        group = cfg.observations[group_name]
        configured = {}
        for name, term in group.terms.items():
            if term.noise is not None and group.enable_corruption:
                value = term.noise
                assert type(value).__name__ == "UniformNoiseCfg" and value.operation == "add"
                configured[name] = [float(value.n_min), float(value.n_max)]
        noise[group_name] = configured
    assert noise == {
        "tokenizer": {"motion_anchor_ori_b": [-0.05, 0.05]},
        "policy": {"base_ang_vel": [-0.2, 0.2], "projected_gravity": [-0.05, 0.05]},
        "root_feedback": {},
    }
    bind(HERE / "audit_sampled_references.py")
    bind(RUN / "sampled_reference_failure.json")
    bind(Path(recipe.__file__))
    bind(Path(recipe.ROOT) / "gear_sonic/envs/mjlab/sonic_true23.py")
    ...
    assert orientation_error <= 0.05 + 2e-6
    w, x, y, z = np.moveaxis(measured_quat, -1, 0)
    yaw = np.arctan2(2 * (w * z + x * y), 1 - 2 * (y * y + z * z))
    c, s = np.cos(yaw), np.sin(yaw)
    expected_velocity = (
        native["body_pos_w"][anchors + 1, 0].astype(np.float32)
        - native["body_pos_w"][anchors, 0].astype(np.float32)
    ) / np.float32(0.02)
    ...
    assert gravity_error <= 0.05 + 2e-6
    ...
    report = dict(
        kind="actual_sampled_pico_reference_and_declared_noise_consistency_v2",
        original_strict_noiseless_audit_failed=True,
        original_failure_explained_by_existing_additive_training_noise=True,
        configured_noise=noise,
        orientation_and_gravity_match_within_configured_noise_not_bit_exact=True,
        no_training_or_evaluation_configuration_changed=True,
        passed=True,
