...
def main():
    ...
    diagnostic = json.loads(bind(HERE / "diagnostic_v1/report.json").read_text())
    assert diagnostic["correction_hypothesis_preflight_justified"]
    for path, digest in diagnostic["inputs"].items():
        bind(path, digest)
    import gear_sonic.utils.g1_true23_momentum_source_model as corrected_implementation
    ...
    report = dict(
        kind="momentum_preserving_internal_source29_prediction_preflight_v1",
        records=records,
        inputs=inputs,
