...
OUT = Path(__file__).parent / "actual_v1"
...
def main():
    ...
    materials = {
        ...
        ).as_source_files(ROOT).values()
    }
    materials.update(
        {
            str(path): sha256_file(path)
            for path in (SOURCE, OLD, preflight_path, Path(__file__), Path(__file__).parent / "EXPERIMENT.md")
        }
    )
    ...
    for case in CASES:
        ...
        try:
            ...
            context.term()
    result = dict(
        kind="native23_prepared_reference_fixed_six_case_pacing_v1",
        cases=summary,
