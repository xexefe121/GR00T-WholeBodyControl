...
from gear_sonic.utils.g1_true23_current_momentum_observer import CurrentMomentumSourceModel as MomentumSourceModel
...
def main():
    ...
    after_directory = HERE.parent / "g1_true23_momentum_update_20260910_v1/native_actual_v1"
    after_report = read(after_directory / "report.json")
    bind(after_directory / "trace.npz", after_report["inputs"][str(after_directory / "trace.npz")])
    after_trace = arrays(after_directory / "trace.npz")
    ...
    for i in range(count):
        current_state = predictor.prepare_current(attempt["measured_qpos"][i], attempt["measured_qvel"][i])
        np.testing.assert_array_equal(current_state, attempt["current_assimilated_missing_state12"][i])
        current_row = rows[-1].copy()
        current_row[:12] = current_state
        rows[-1] = current_row
    ...
    all_traces = dict(
        momentum_model=trace,
        after_inference_momentum_model=after_trace,
        previous_velocity_copy_model=old_trace,
        zero_model_baseline=baseline,
    )
    ...
    result = dict(
        kind="current_momentum_source_native23_history_and_tracking_audit_v1",
        inputs=inputs,
        ...
        all_current_assimilated_virtual_states_bit_exact=True,
