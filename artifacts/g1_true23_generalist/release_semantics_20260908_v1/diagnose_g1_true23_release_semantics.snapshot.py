...
class ReferenceAblation:
    def __init__(self, motion, mode):
        if mode not in ("causal_past", "released_future_preview"):
    ...
    def infer(self, policy, encoder, history, *, control_index, **unused):
        if self.mode == "released_future_preview":
    ...
    def contract(self):
        return dict(
            ...
            future_reference_consumed=self.mode == "released_future_preview",
...
def main(argv=None):
    ...
    with ieee_training_precision() as (precision, guard):
        ...
        for mode in ("causal_past", "released_future_preview"):
            guard()
            ...
            trace = output / f"{mode}.npz"
            with trace.open("xb") as stream:
                np.savez_compressed(stream, **arrays)
            row = dict(mode=mode, result=result, lifecycle=lifecycle, trace_sha256=sha256_file(trace))
            write_json(output / f"{mode}.json", row)
            rows.append(row)
            print(json.dumps(dict(
                mode=mode, completed=result["completed_controls"], available=result["available_controls"],
        ...
        for field in ("initial_state_and_history_sha256", "compiled_model_sha256", "motion_sha256",
                      "available_controls", "kp_hardware", "kd_hardware", "effort_limit_hardware_nm"):
            if rows[0]["result"][field] != rows[1]["result"][field]:
        ...
        write_json(output / "report.json", dict(
            ...
            learned_parameter_updates=0, paired_initial_states_and_physics_equal=True,
