...
class ReferenceAblation:
    def __init__(self, motion, mode, virtual_vr=None, action_convention="native_tanh", case="nominal"):
...
def main(argv=None):
    ...
    with ieee_training_precision() as (precision, guard):
        ...
        for profile, mode, case in (
            ...
            guard()
            adapter = ReferenceAblation(motion, mode, virtual_vr, args.action_convention, case)
        ...
        write_json(
            output / "report.json",
            dict(
                kind="zero_training_native23_sonic_semantic_ablation_v1",
                rows=rows,
                inputs=inputs,
                policy=core.artifact_contract(),
                timeline=timeline,
                precision=precision,
                learned_parameter_updates=0,
