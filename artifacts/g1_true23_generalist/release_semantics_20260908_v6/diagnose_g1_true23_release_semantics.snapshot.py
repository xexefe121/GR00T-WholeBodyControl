...
class ReferenceAblation:
    def __init__(
        ...
        self.root_adapter = RootFeedbackRuntimeAdapter() if root_feedback else None
...
def main(argv=None):
    ...
    parser.add_argument(
        "--action-convention", choices=("native_tanh", "released_bounded_linear"), default="native_tanh"
    )
    parser.add_argument(
        "--profiles", choices=("native_model", "historical_released_gains"), nargs="+", default=["native_model"]
    )
    ...
    with ieee_training_precision() as (precision, guard):
        ...
        if args.root_feedback_manifest is None:
            ...
            policy, identity = load_root_feedback_pair(args.root_feedback_manifest, session_options=options)
