...
### 2026-09-20 — Fix 15: live PICO to BFM packet bridge

**The bridge produces admissible BFM packets: the recorded PICO-derived full run admitted 6,541 of 6,541 packets with zero validation failures, capture-to-publication p95 `15.0 ms`, and G1/G3/G4 plus the completed recorded-segment simulation evidence are proven against recordings, not a live headset.** Nothing in this fix opens DDS, LowCmd, or any robot endpoint.

**Design.** `gear_sonic/scripts/stream_g1_true23_pico_bfm_packets.py` is a standalone PUB publisher for the exact six-field BFM contract and envelope accepted by `run_g1_true23_bfm_teleop_sim consume`: `joint_pos`, `joint_vel`, `body_pos_w`, `body_quat_w`, `body_lin_vel_w`, `body_ang_vel_w`, plus `sequence`, `source_time`, `epoch`, and `final`. It subscribes to the existing `stream_g1_23dof_pico_causal_zmq.py --native23-body-packets` output, so the existing capture worker, measured-frame selector, and pinned IK24/SOMA solver stay authoritative. It uses the existing `Native23PicoBodyAdapter` for the original29-to-current-native23 body mapping; it does not retarget or change solver behaviour. The recorded-source mode turns a saved PICO-derived native23 motion into the same current-body input only for hardware-free testing.

The bridge applies the accepted v4 floor transform causally on every current pose: exact 50 Hz critically damped state-space filter, omega `15/s`, fixed `20 mm` buffer, and `1 um` explicit guard. It recalculates joint, body-linear, and body-angular velocities only from the corrected current and immediately preceding poses; the first packet has zero derivative because no earlier pose exists. Quaternions are renormalized and then checked by the unchanged `validate_fields` tolerance. It reports one JSON record per published packet with `sequence`, `epoch`, capture-to-publication time, and post-filter floor clearance, followed by a summary containing the published/failed counters, reasons, epoch, latency percentiles, and minimum floor clearances.

**G1 — packet admission, proven on recording.** `fix15_recorded_bridge.json` is a complete 6,541-frame recorded PICO-derived run. `validate_fields` passed before every publication; `PacketGate.receive` admitted `6541/6541` (`100.0%`); there were zero validation failures and zero gate rejections. Rejection reasons seen: none. This directly exercises the exact BFM `FIELD_SHAPES` and its unchanged admission gate.

**G2 — simulation.** The bridge, rather than the built-in recorded-clip publisher, drove the unchanged consumer. A 1,417-frame recorded PICO-derived segment completed normally: leg RMSE `0.11052627355650949`, arm RMSE `0.1673062531364588`, root p95 `0.2574394699341393`, and no physical failure (`fix15_bridge_pico_segment_sim/report.json`). The bridge-preserved `walk002` comparison also completed all 1,417 source controls with no physical failure: leg `0.13094901951844776`, arm `0.12524876421702724`, root p95 `0.3472248674682699` (`fix15_bridge_walk002_sim/report.json`). Against the existing ideal `walk002` baseline (`0.18202273382760834`, `0.04273216819201739`, `0.36741054963315306`), the leg/root errors remain comparable and arm error is higher but not an order-of-magnitude regression. The entire 6,541-frame PICO-derived lifecycle is **not** qualified: it reached the existing simulation's `physical_limit_diagnostic_stop` at control `5130`, range excess `0.012359621633028062 rad` (`fix15_bridge_sim_c/report.json`). That result is retained as a failure, not hidden by the successful segment.

**G3 — floor discipline, proven on recording.** The full recorded PICO-derived run reached raw minimum foot clearance `-0.0582099326566423 m`; after the causal v4 correction its minimum was `+0.0016487721316078398 m`, with `0` published frames below floor. The `walk002` bridge comparison was `-0.0206741895562939 m` before and `+0.009552737763519854 m` after. These values use the exact native foot collision spheres and native floor plane, not body-origin height.

**G4 — latency, proven on recording.** Across all 6,541 recorded packets, capture-to-publication p50/p95/maximum were `8.0/15.0/30.0 ms`. The p95 leaves `85.0 ms` of the policy's 100 ms staleness bound; the observed maximum leaves `70.0 ms`. This is a recording-host timing measurement, not a headset/network latency qualification.

**G5 — regressions.** `python -m pytest -q gear_sonic/tests/test_stream_g1_true23_pico_bfm_packets.py gear_sonic/tests/test_g1_true23_pico_body_adapter.py gear_sonic/tests/test_g1_true23_bfmzero_stream.py` passed `49` tests. The old `run_g1_true23_bfm_teleop_sim publish` implementation and its packet contract were not changed. The first paired simulation attempt deliberately recorded a strict `packet_sequence_gap_or_reorder` fault because its subscriber had not finished initialization and missed packets 0–2; the gate was not relaxed. Starting the consumer first and allowing the documented subscription warmup produced the zero-rejection runs above.

**Tomorrow's headset command.** Start the existing BFM consumer first and wait until it has completed its normal model startup. Then start the bridge before the PICO publisher, using an endpoint already supplied to the policy as `--teleop-endpoint`:

```powershell
python -m gear_sonic.scripts.stream_g1_true23_pico_bfm_packets --upstream-endpoint tcp://127.0.0.1:5557 --bind tcp://127.0.0.1:5591 --subscriber-warmup-s 2 --output artifacts/bfm_teleop_20260917/fix15_live_bridge.json
```

Finally start the existing capture/solver publisher with its verified APK hash and the native-body extension:

```powershell
python -m gear_sonic.scripts.stream_g1_23dof_pico_causal_zmq --native23-body-packets --bind tcp://127.0.0.1:5557 --packets 1000000 --timeout-seconds 21600 --subscriber-warmup-s 2 --evidence artifacts/bfm_teleop_20260917/fix15_live_pico_upstream.jsonl --pico-client-apk-sha256 <verified-64-hex-apk-sha256>
```

The bridge should print one 50 Hz JSON record per frame with increasing `sequence`, `epoch: 0`, finite capture-to-publication time under the 100 ms budget, and non-negative `floor_after_m`; its final summary should report zero validation failures. It publishes only the policy endpoint. It does not arm, release motion control, publish LowCmd, or make a hardware decision.

**Still unproven until the headset is connected.** The XRT capture-worker-to-pinned-solver-to-bridge process boundary has not been exercised with this bridge on a live PICO. Therefore live capture timestamp age, live PICO tracking/dropout behaviour, real end-to-end latency, live packet admission, and live policy/robot behaviour remain unproven. The completed 1,417-frame recorded PICO-derived segment is not evidence that the full PICO-derived lifecycle is dynamically qualified, because the full recorded lifecycle still reaches the simulator range diagnostic at control 5,130. No physical robot test is authorized by this fix.
