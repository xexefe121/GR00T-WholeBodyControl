...
def audit(directory):
    ...
    assert outcome["completed"] and outcome["simulator_updates"] == args["updates"]
    ...
    checkpoints = [
        torch.load(
            bind(directory / "checkpoints" / f"normal_lora_model_{i}.pt"), map_location="cpu", weights_only=True
        )
    ...
    close("returned", rewards["returned_reward"], rewards["base_reward"] + shaping + bonus)
    close(
        "PPO_stored",
    ...
    report = dict(
        kind="existing_pico_normal_training_arithmetic_audit_v1",
        passed=True,
