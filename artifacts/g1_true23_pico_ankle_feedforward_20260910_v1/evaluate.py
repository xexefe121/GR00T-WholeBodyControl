...
from gear_sonic.utils.g1_true23_ankle_feedforward_benchmark import MODEL, PHYSICS, run_reference_diagnostic
from gear_sonic.utils.g1_true23_generalist_corpus import sha256_file
from gear_sonic.utils.g1_true23_ankle_feedforward import AnkleFeedforwardRangePreview
...
class AnkleFeedforwardAdapter(PicoFootPrecisionAdapter):
    def __init__(self, motion, virtual_vr, *, root, assets):
        super().__init__(motion, virtual_vr, root=root, assets=assets)
        self.preview = AnkleFeedforwardRangePreview(model_path=assets / MODEL, physics_path=root / PHYSICS)

    def current_actuator_feedforward23(self):
        if self.preview.feedforward23 is None:
            raise ValueError("no verified actuator feedforward for current control")
        return self.preview.feedforward23.copy()

    def contract(self):
        result = super().contract()
        result.update(
            kind="native23_offline_explicit_ankle_feedforward_adapter_v1",
            extra_actuator_torque_logged_separately=True,
            raw_action_history_is_position_target_not_feedforward_torque=True,
        )
        return result
...
def main():
    ...
    if resolved.get("pico_used_for_training") is not True or policy.completed_updates != 1000:
        raise ValueError("ankle feedforward evaluation requires fixed final1000 milestone")
    ...
    for name, case in cases.items():
        ...
        adapter = AnkleFeedforwardAdapter(case["motion"], case["reference"].virtual_vr21, root=ROOT, assets=ASSETS)
