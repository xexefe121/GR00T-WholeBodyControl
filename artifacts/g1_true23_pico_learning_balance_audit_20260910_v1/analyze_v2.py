...
def main():
    ...
    bind(__file__)
    bind(ROOT / "gear_sonic/envs/mjlab/sonic_true23.py")
    ...
    for control in (1830, 1853, 1880, 1900, 1910, 1920):
        ...
        current = failure["history930"][control, 30:320].reshape(10, 29)[-1]
        current = current[list(NATIVE_IL23_TO_CANONICAL_IL29)][list(ISAACLAB_TO_MUJOCO_DOF)].astype(float)
        current += np.asarray(SONIC_HARDWARE_DEFAULT_Q, dtype=np.float32).astype(float)
        np.testing.assert_allclose(current, failure["measured_qpos"][control, 7:], rtol=0, atol=5e-7)
        dq = failure["history930"][control, 320:610].reshape(10, 29)[-1]
        dq = dq[list(NATIVE_IL23_TO_CANONICAL_IL29)][list(ISAACLAB_TO_MUJOCO_DOF)]
        np.testing.assert_array_equal(dq, failure["measured_qvel"][control, 6:].astype(np.float32))
        f = features(
