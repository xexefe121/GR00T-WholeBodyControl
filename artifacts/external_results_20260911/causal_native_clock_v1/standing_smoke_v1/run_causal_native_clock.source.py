"""One received-only ONNX controller on the unchanged independent native plant.
...
class Bridge:
    """Existing C ABI; no changes to the compiled physical plant."""
...
def import_applied_history(controller, values):
...
def producer(packets, epoch_ns, start, stop, motion, original, controls, standing, fault_control, output):
    rows = []
    try:
        start.wait()
...
def worker(library, name, actor, model, contract, options, initial_packets, packets,
           epoch_ns, start, ready, stop, output, affinity):
    bridge = None
    rows, statuses = [], []
    last = -1
    try:
...
def main():
    ...
    if controls < 1 or controls > 5000:
        raise ValueError('Unchanged native clock currently limited to 1..5000 controls; long Pico clock recurrence remains pending')
    ...
    library = NEW / 'native_clock_v1/libtrue23clock.so'
    ...
    controller = context.Process(target=worker, args=(library, bridge.shm.name, args.actor, model, contract,
        options, initial_packets, packets, epoch_ns, start, ready, stop, args.output, args.affinity))
    sender = context.Process(target=producer, args=(packets, epoch_ns, start, stop, motion, original,
        controls, args.standing, args.fault_control, args.output))
    ...
    try:
        ...
        arrays = [np.ascontiguousarray(contract[k], np.float64) for k in
    ...
    report = dict(actor=str(args.actor), clip=args.clip, standing_diagnostic=args.standing,
        ...
        native_clock_long_pico_recurrence_unresolved=True, general_live_teleop_qualified=False, hardware_authorized=False)
