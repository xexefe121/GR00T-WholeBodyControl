...
def main():
 ...
 rows=[];completed=0;roundtrip=np.empty(291);source_before=np.empty(291);source_after=np.empty(291)
 try:
  for i in range(232):
   state,target=cases['integration'][i],cases['target'][i];controls=int(cases['horizon_controls'][i])
