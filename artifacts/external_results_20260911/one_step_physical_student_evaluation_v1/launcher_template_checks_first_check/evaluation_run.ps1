...
$bindingPath = 'Y:\Temp\tmpihp4xtil\evaluation_binding.json'
...
if (Test-Path -LiteralPath 'Y:\Temp\tmpihp4xtil\nominal') { throw 'Existing attempt output must be preserved.' }
if (Test-Path -LiteralPath 'Y:\Temp\tmpihp4xtil\post_lifecycle_hold_5s') { throw 'Existing attempt output must be preserved.' }
if (Test-Path -LiteralPath 'Y:\Temp\tmpihp4xtil\pilot_outcome.json') { throw 'Existing attempt output must be preserved.' }
$executionLock = [System.IO.File]::Open('Y:\Temp\tmpihp4xtil\evaluation_process\execution.lock',[System.IO.FileMode]::CreateNew,[System.IO.FileAccess]::Write,[System.IO.FileShare]::None)
$executionLock.Close()
$arguments = @(
    '-d',
    'Ubuntu-22.04',
    ...
    'MKL_NUM_THREADS=1',
    'PYTHONPATH=/mnt/e/codex-artifacts/sonic23_teleop_resume_20260911/tmpihp4xtil/source_snapshot_v1',
    '/mnt/e/codex_sonic_runtime/mjbatch323_20260910/venv/bin/python',
    '/mnt/e/codex-artifacts/sonic23_teleop_resume_20260911/tmpihp4xtil/source_snapshot_v1/evaluate_physical_response_student.py'
)
...
$result = Get-Content -Raw -LiteralPath 'Y:\Temp\tmpihp4xtil\pilot_outcome.json' | ConvertFrom-Json
...
$parity = Get-Content -Raw -LiteralPath 'Y:\Temp\tmpihp4xtil\canonical_prefix250_parity.json' | ConvertFrom-Json
if (-not $parity.passed) { throw 'Canonical transition parity failed.' }
$parity = Get-Content -Raw -LiteralPath 'Y:\Temp\tmpihp4xtil\actual_query250_input_parity.json' | ConvertFrom-Json
if (-not $parity.passed) { throw 'Canonical transition parity failed.' }
$parity = Get-Content -Raw -LiteralPath 'Y:\Temp\tmpihp4xtil\actual_query250_ownexport_output_parity.json' | ConvertFrom-Json
if (-not $parity.passed) { throw 'Canonical transition parity failed.' }
exit 0
