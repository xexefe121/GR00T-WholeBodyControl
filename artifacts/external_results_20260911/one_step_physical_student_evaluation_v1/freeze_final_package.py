...
FIT=BASE.parent/'one_step_physical_student_v1'
...
def main():
    ...
    final={name:item(path) for name,path in dict(head=FIT/'fit/student_head.onnx',checkpoint=FIT/'fit/student_head.pt',
        ...
        centers=CENTERS,query250_labels=BASE.parent/'bfm_entry250_labels_v1/labels/labels.npz').items()}
    ...
    for key in ('head','checkpoint','fit_report','training_manifest'):
        assert config['selected_subject_sha256'][key]==final[key]['sha256'],key
    ...
    driver=SOURCE/('head_activation_witness.py' if args.mode=='witness' else 'evaluate_physical_response_student.py')
    ...
    for path in (Path(__file__),BASE/'prepare_bound_launcher.py',BASE/'prepare_binding_tools.py',BASE/'binding_tools_derivation.json',
                 BASE/'source_preparation.json',BASE/'source_only_check.json',BASE/'source_freeze.json',BASE/'freeze_sources.py',
                 BASE/'test_binding_workflow.py',BASE/'binding_workflow_tests.json',BASE/'binding_workflow_tests.log',args.reviews):
    ...
    binding=dict(**final,reviews=reviews,input_files=[dict(path=path,sha256=digest) for path,digest in sorted(pins.items())],
        onnx_dependencies=DEPS.as_posix(),training_dataset_sha256=[CENTERS_SHA,final['physical_manifest']['sha256'],final['physical_report']['sha256']],
        ordinary_final_step=75000,hardware_authorized=False)
    if args.mode=='witness':
        ...
        write_new(frozen,dict(kind='one_final75000_canonical_unfiltered_evaluation',
