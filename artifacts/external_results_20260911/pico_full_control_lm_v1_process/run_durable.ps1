...
$processDirectory = Join-Path $base 'pico_full_control_lm_v1_process'
$launcher = Join-Path $base 'launch_pico_full_control_lm_v1.ps1'
...
try {
    ...
    foreach ($name in @('pico_full_control_lm_v1','pico_full_control_lm_v1.stdout.log','pico_full_control_lm_v1.stderr.log')) {
        if (Test-Path -LiteralPath (Join-Path $base $name)) { throw ('Output exists; refusing duplicate run: ' + $name) }
    }
    @{state='running';pid=$PID;started_utc=$started;launcher=$launcher;requested_controls=6530} | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $processDirectory 'running.json')
    ...
    $reportPath = Join-Path $base 'pico_full_control_lm_v1\report.json'
    $result = @{state='exited';pid=$PID;started_utc=$started;finished_utc=[DateTime]::UtcNow.ToString('o');exit_code=$exitStatus;error=$errorMessage;report_exists=(Test-Path -LiteralPath $reportPath);completed=$false;requested_controls=6530}
    if ($result.report_exists) {
        $report = Get-Content -LiteralPath $reportPath -Raw | ConvertFrom-Json
        $result.completed = $report.probe_completed -and $report.completed_controls -eq 6530 -and $null -eq $report.failure
