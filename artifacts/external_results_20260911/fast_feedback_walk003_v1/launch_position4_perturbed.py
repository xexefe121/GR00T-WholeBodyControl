...
process=BASE/'position4_perturbed_process_v1';process.mkdir(exist_ok=False)
output=BASE/'position4_perturbed_v1';assert not output.exists()
def posix(p):
...
command=['C:/Windows/System32/wsl.exe','-d','Ubuntu-22.04','--cd','/','--','bash',boot,'env',
    ...
    '/mnt/e/codex_sonic_runtime/mjbatch323_20260910/venv/bin/python','-B','-u',posix(BASE/'run_position4_perturbed.py'),'--output',posix(output)]
write(process/'start.json',dict(utc=datetime.now(timezone.utc).isoformat(),command=command,
    source_sha256=hashlib.sha256((BASE/'run_position4_perturbed.py').read_bytes()).hexdigest(),automatic_retry=False))
