...
def main():
    ...
    write(a.output/'request.json',dict(controller='BFM startup; original81000 head at250; compiled state feedback251..1268; BFM terminal and hold',
        original_timing=True,requested_main_controls=1569,conditional_hold_controls=1500,
    ...
    # Warm the policy before the timed epoch; no history commit or plant step.
    ...
    # CountedSession captured this dry startup call's immutable inference inputs.
    ...
    def segment(name,start,count):
        ...
        shapes=dict(qpos=(count+1,30),qvel=(count+1,29),target=(count,23),
            ...
            inference_ms=(count,),control_loop_ms=(count,),deadline_finish_ms=(count,),start_lateness_ms=(count,),feedback_correction_max=(count,),
        ...
        result=dict(full_segment_completed=complete,requested_controls=count,completed_controls=int(np.sum(arrays['physics_substeps']==10)),
            ...
            trace_sha256=sha(dest/'trace.npz'),motion_specific=True,real_time_paced=True,live_teleoperation_qualified=False,
    ...
    continuous=segment('continuous',0,3069)
