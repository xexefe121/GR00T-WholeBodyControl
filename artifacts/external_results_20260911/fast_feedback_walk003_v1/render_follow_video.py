"""Render the complete fast feedback motion and hold from its fresh native trace."""
...
OUT = BASE / 'fast_feedback_walk003_v1/follow_video_v1'
...
def main():
    run = BASE / 'fast_feedback_walk003_v1/baseline_v1'
    ...
    assert q.shape == (18191, 30) and dq.shape == (18191, 29)
    np.testing.assert_allclose(times, np.arange(18191) * .002, atol=2e-10, rtol=0)
    ...
    steps = np.unique(np.r_[np.arange(0, 18191, 50), 11690, 15690, 18190])
    ...
    for path in (reference_path, receipt_path, bundle / 'manifest.json', bundle / 'native_prepared.xml',
                 bundle / 'prepared_model_arrays.npz', bundle / 'walk003/timeline.json'):
        inputs[str(path)] = sha(path)
    timeline = json.loads((bundle / 'walk003/timeline.json').read_text())
    ...
    contact_steps = (0, 3500, 5500, 7500, 9500, 11690, 15690, 18190)
    ...
    try:
        for slot, step in enumerate(steps):
            ...
            draw.text((14, 35), 'Shared moving camera; original source timing', font=small, fill='#cbd5e1')
            draw.text((WIDTH + 14, 35), 'Full motion and separate quiet hold pass', font=small, fill='#cbd5e1')
            camera.lookat[:] = (desired[slot, :3] + actual[slot, :3]) / 2
            camera.lookat[2] = .75
            camera.distance = 2.8
            ...
            seconds = step * .002
            if step >= 15690:
                phase = f'separate hold +{seconds - 31.38:.1f}/5.0 s'
            else:
                control = max(0, (step - 1) // 10)
                phase = next(p['name'].replace('_', ' ') for p in timeline['phases'] if p['control_start'] <= control < p['control_stop'])
            source_seconds = np.clip(seconds - 7., 0, 16.38)
            draw.text((14, TOP + HEIGHT + 5), f'WALK003 | simulation {seconds:.1f}/36.38 s | source {source_seconds:.1f}/16.38 s | {phase}', font=small, fill='white')
            speed = np.abs(dq[step, 6:]).max()
            root_error = np.linalg.norm(actual[slot, :3] - desired[slot, :3])
            draw.text((14, TOP + HEIGHT + 34), f'18,190 fresh physics steps | joint speed now {speed:.3f} rad/s | root error now {root_error:.3f} m', font=small, fill='#b4edc0')
            draw.text((14, TOP + HEIGHT + 64), 'FAST STATE FEEDBACK | prepared motion | live teleoperation and real Pico remain unqualified', font=small, fill='#ffd0a3')
    ...
    video = OUT / 'full_walk003_and_continuous_hold.shared_follow_camera.mp4'
    ...
    report = dict(kind='full_fast_feedback_walk003_and_continuous_hold_saved_state_visual', inputs=inputs,
                  video_sha256=sha(video), video=str(video), contact_sheet_sha256=sha(OUT / 'contact_sheet.png'),
                  source_controls=819, lifecycle_controls=1569, separate_hold_controls=250,
                  physical_steps=18190, display_seconds=36.38, final_display_hold_seconds=.002,
                  ...
                  camera_mode='same moving camera for both panels; no state alignment',
                  final_camera=dict(lookat=camera.lookat.tolist(), distance=camera.distance,
                                    azimuth=camera.azimuth, elevation=camera.elevation),
                  static_overview_projection_ratio=float(ratio), no_new_dynamics=True,
