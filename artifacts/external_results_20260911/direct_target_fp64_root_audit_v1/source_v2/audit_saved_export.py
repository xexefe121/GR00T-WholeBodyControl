...
def run(experiment, output):
    ...
    try:
        ...
        direct.update(head=sha(graph_path),export_report=sha(export/'report.json'),export_request=sha(request_path),export_manifest=sha(frozen_path),output_manifest=sha(export/'manifest.json'),root_training_audit=sha(subjects['training_audit']))
