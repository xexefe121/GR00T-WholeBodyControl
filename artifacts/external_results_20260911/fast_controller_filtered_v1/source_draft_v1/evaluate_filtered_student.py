"""One synchronous native private-admission lifecycle with frozen65000 weights."""
import json
...
from ordered_admission import admit,CANDIDATE_NAMES
...
def main():
    ...
    common=dict(kind='native23_fixed65000_private_admission_student',clip='walk003',motion_override=override,
    ...
    def segment(dest,start,count,expected_time):
        ...
        ledger=[];private_copies=[];private_meta=[];failure=None;selected_forecast_checks=0
        for control in range(start,start+count):
            ...
            try:
                ...
                if control<250:
                    ...
                    def predict(target):
                        ...
                        first_index=next(i for i,value in enumerate(candidates) if np.array_equal(value,target))
                        private_meta.append(dict(control=control,candidate=first_index,target=target.copy()))
                        forecast_count+=1
                        return dict(report,forecast_index=index)
            ...
            values=dict(qpos=data.qpos.copy(),qvel=data.qvel.copy(),source_frame=frame,global_control=control,
                ...
                selected_candidate=candidate,candidate_count=len(records),forecast_count=forecast_count,admission_ms=admission_ms,**proposed)
