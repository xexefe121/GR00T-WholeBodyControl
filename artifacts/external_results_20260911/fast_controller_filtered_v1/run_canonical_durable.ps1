...
$runRoot='E:\codex-artifacts\sonic23_teleop_resume_20260911\fast_controller_filtered_v1'
foreach($existing in @('canonical_process_status.json','canonical_stdout.log','canonical_stderr.log','nominal','post_lifecycle_hold_5s')){
    if(Test-Path -LiteralPath (Join-Path $runRoot $existing)){throw 'Existing filtered canonical attempt must remain preserved.'}
}
...
if((Read-Sha256 $clearancePath) -ne $ClearanceSha256){throw 'Filtered runtime clearance identity mismatch.'}
$clearance=Get-Content -LiteralPath $clearancePath -Raw | ConvertFrom-Json
if($clearance.canonical_rollout_authorized -ne $true -or $clearance.canonical_trials -ne 1 -or $clearance.private_horizon_controls -ne 5 -or $clearance.actual_commit_controls -ne 1){throw 'Filtered runtime clearance scope mismatch.'}
...
foreach($required in @('frozen_inputs_v2.json','run_canonical_durable.ps1','proposal_snapshot.md','source_snapshot_v1\evaluate_filtered_student.py','source_snapshot_v1\native_forecast.py','source_snapshot_v1\transactional_student.py','source_snapshot_v1\ordered_admission.py')){
...
foreach($item in $clearance.bound_files){if((Read-Sha256 $item.path) -ne $item.sha256){throw ('Reviewed filtered runtime input changed: '+$item.path)}}
if((Read-Sha256 (Join-Path $runRoot 'frozen_inputs_v2.json')) -ne $clearance.frozen_sources_sha256){throw 'Frozen filtered sources changed.'}
$statePath=Join-Path $runRoot 'canonical_process_status.json'
$status=[ordered]@{kind='one_synchronous_filtered65000_student';state='RUNNING';powershell_pid=$PID;started_utc=[DateTime]::UtcNow.ToString('o');requested_controls=1569;separate_hold_controls=250;private_horizon_controls=5;actual_commit_controls=1;initial_BFM_controls=250;head_sha256=$clearance.head_sha256;clearance_sha256=$ClearanceSha256;additional_fitting=$false;expert_query=$false;hardware_authorized=$false}
