...
GEOMETRY_FIELDS=(
    'body_mass','body_inertia','body_pos','body_quat','jnt_range','jnt_pos','jnt_axis',
    'dof_armature','dof_damping','dof_frictionloss','geom_type','geom_size','geom_pos','geom_quat',
    'geom_friction','geom_contype','geom_conaffinity','actuator_forcerange','actuator_forcelimited',
    'actuator_ctrlrange','actuator_ctrllimited','actuator_gear','actuator_gaintype','actuator_gainprm',
    'actuator_biastype','actuator_biasprm','actuator_dyntype','actuator_dynprm','actuator_trntype','actuator_trnid')
OPTION_FIELDS=('timestep','integrator','solver','iterations','tolerance','gravity','density','viscosity','wind')
...
def arrays_hash(values):
    ...
    for key in sorted(values):
        ...
        h.update(json.dumps([key,value.dtype.str,list(value.shape)],separators=(',',':')).encode())
...
class NativeStepper:
    def __init__(self,model,data,api,contract,expected_model_arrays,expected_options,expected_model_sha256):
        ...
        self.initialized=False;self.restore_attempted=False
        ...
        if set(expected_model_arrays)!=set(GEOMETRY_FIELDS) or set(expected_options)!=set(OPTION_FIELDS):
            raise ValueError('complete pinned native geometry/options required')
        if re.fullmatch('[0-9a-f]{64}',str(expected_model_sha256)) is None:
            raise ValueError('literal selected native model-array SHA256 required')
        selected={key:np.asarray(value) for key,value in expected_model_arrays.items()}
        selected.update({'opt.'+key:np.asarray(value) for key,value in expected_options.items()})
        if arrays_hash(selected)!=expected_model_sha256:raise ValueError('pinned geometry/options digest mismatch')
        for key,value in expected_model_arrays.items():
            if not exact(np.asarray(getattr(model,key)),np.asarray(value)):raise ValueError('native geometry changed: '+key)
        for key,value in expected_options.items():
            if not exact(np.asarray(getattr(model.opt,key)),np.asarray(value)):raise ValueError('native option changed: '+key)
    ...
    def _fail(self,reason,exc=None,evidence=None):
        if self.failure is None:
            if evidence is None:
                # Exception evidence is never credited as a returned/captured step.
                try:
                    evidence=owned_evidence(dict(qpos=np.asarray(self.data.qpos).copy(),qvel=np.asarray(self.data.qvel).copy(),
                        time=float(self.data.time),ctrl=np.asarray(self.data.ctrl).copy(),
                        warnings=np.asarray(self.data.warning.number).copy(),lastinfo=np.asarray(self.data.warning.lastinfo).copy()))
                except Exception:
                    evidence=b'exception evidence unavailable; no capture credit'
            self.failure=Fault(reason,self.stage,self.attempted,self.returned,self.captured,
                               '' if exc is None else str(exc),bytes(evidence))
    ...
    def restore_initial(self,integration,warning_counts,warning_lastinfo):
        if self.restore_attempted or self.returned or self.attempted:raise NativeAdapterError('initial restoration is allowed once; no rearm/reset')
    ...
    def _ready(self):
        if not self.initialized or self.failure is not None:raise NativeAdapterError('native adapter uninitialized or fault latched')
    ...
    def counters(self):
        return dict(attempted=self.attempted,returned=self.returned,capture_attempts=self.capture_attempts,
                    ...
                    mutation_uncertain=self.attempted>self.returned,stage=self.stage)
