"""Independent literal identities for causal71000 saved execution evidence."""
...
def binding_identity(b,check):
    ...
    check.require(b['ordinary_final_step']==71000 and b['release_kind']=='ordinary71000_warm_balanced_context_same_weight_fp64_export','71000 causal release kind')
...
def warm_identity(fit,energy,check):
    ...
    check.require(fit['ordinary_start_step']==68000 and fit['ordinary_final_step']==71000 and fit['additional_updates']==3000,'fixed ordinary warm continuation')
    check.require(fit['optimizer_start_step']==3000 and fit['optimizer_step']==6000 and fit['fresh_optimizer'] is False,'retained warm optimizer protocol')
    ...
    check.require(fit['source_checkpoint_sha256']=='10d238198e2d7826c35eaa9bd4a13ff2e05050a7f601c1f212b585c4a90eaefd','actual causal68000 source')
...
def check_release(p,check):
    ...
    check.require(fit['completed'] is True and fit['numerical_gate_passed'] is True and fit['export_parity_passed'] is True and fit['ordinary_final_step']==71000,'actual completed71000 fit')
    ...
    for name in ('main','hold'):
        ...
        check.require(request['head_sha256']==expected['head'] and request['evaluation_binding_sha256']==sha(p['binding']) and request['ordinary_final_step']==71000 and request['context_condition']=='causal',name+' model activation subject')
