...
from context_math import features
...
def synthetic(control,substeps=10,previous=None,history=None):
    ...
    a=dict(target=target[None],qpos=np.stack([q,q]),qvel=np.stack([dq,dq]),global_control=np.array([control],np.int64),
        ...
        previous_action=previous[None],history=flat(history)[None],state=sensed[None],features=features(np.zeros(1000,np.float32),previous,history,control)[None],
    ...
    if substeps<10:
        cap=dict(integration_at_failure=end,qpos_at_failure=q,qvel_at_failure=dq,previous_action_after=action,previous_action_before=previous,
            ...
            global_control=np.array(control),recorded_controls_after=np.array(control+1),head_input_features=a['features'].copy(),head_output_0=head[None],
...
class SavedChecks(unittest.TestCase):
    ...
    def test_forbidden_model_native_imports(self):
        for name in ('saved_common.py','release_checks.py','audit_saved.py','fixed_maps.py','prepare_request.py','context_math.py'):
