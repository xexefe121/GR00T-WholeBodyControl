"""Frozen retry-aware source-draft-v2 launcher metadata only; dispatch is always separate."""
...
SOURCE_PREPARATION=SOURCE.parent/'source_preparation_v2.json'
...
def validate_source_review(request,path,digest,review,source_map):
...
def main():
    ...
    q=read(request);assert q['kind']=='independent_saved_clock_audit'
    ...
    for path in (request,BASE/'helper_preparation.json',BASE/'preserved_saved_audit_template.ps1.txt',
                 BASE/'template_parse.json',source_review,SOURCE_PREPARATION,Path('C:/Windows/System32/wsl.exe')):add(path)
