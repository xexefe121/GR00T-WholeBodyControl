...
import torch
from student_linear_runtime import BASE,KIND,FEATURES,assert_frozen,archive,sha
...
def main():
    ...
    request=dict(kind=KIND,configuration=CONFIG,labels_sha256=sha(BASE/'labels/labels.npz'),
        frozen_sources_sha256=sha(BASE/'frozen_inputs.json'),joint_span=span.tolist(),samples=1269,
    ...
    export(actor,mean,std,span,dest/'zero_head.onnx')
