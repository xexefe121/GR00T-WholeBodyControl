...
def main():
    ...
    common_request=dict(kind=KIND,clip='walk003',motion_override=override,model_manifest=manifest,
        ...
        frozen_sources_sha256=sha(BASE/'frozen_inputs.json'),teacher_audit_sha256=sha(TEACHER/'recorded_source_audit_v2.json'),
    ...
    def segment(dest,start,count,expected_time):
        ...
        quiet=None
        if start>0 or complete==count:
            windows=standing_windows(arrays,motion,original,original29);quiet=quiet_diagnostic(windows,strict)
        result=dict(kind=KIND,clip='walk003',segment=request['segment'],requested_controls=count,completed_full_controls=complete,
            ...
            full_segment_completed=complete==count and strict,failure=failure,
            range_excess_max=float(np.max(arrays['range_excess'])),velocity_ratio_max=float(np.max(arrays['velocity_ratio'])),effort_ratio_max=float(np.max(arrays['effort_ratio'])),
            engine_warning_counts=data.warning.number.tolist(),engine_warning_lastinfo=data.warning.lastinfo.tolist(),
            independent_accumulated_clock_max_error=float(np.max(arrays['clock_error'])),
            policy_ms_p50_p95_max=np.percentile(arrays['inference_ms'],[50,95,100]).tolist(),policy_20ms_deadline_misses=int(np.sum(arrays['inference_ms']>20)),
            source_metrics=source_metrics(native,arrays,motion,original29,audit),quiet_standing_diagnostic=quiet,
