...
from saved_math import (GROUPS, KEPT, original_difference, expected_radii, exact,
                        check_probe, map_values, duplicate_conflicts, validate_producer_report)
...
def run(request_path, generation, training_request_path, original_feature_source, output):
    ...
    try:
        ...
        validate_producer_report(report, sha(request_path))
        ...
        manifest = read(bind(generation / 'manifest.json', report['manifest_sha256']))
        summary = read(bind(generation / 'group_cell_summary.json', report['group_cell_summary_sha256']))
        check(manifest['complete'] is True and manifest['request_sha256'] == sha(request_path), 'Manifest request identity')
        check(manifest['centers_sha256'] == report['output_sha256']['centers.npz'], 'Manifest center identity')
        check(manifest['group_cell_summary_sha256'] == report['group_cell_summary_sha256'], 'Manifest summary identity')
        ...
        for name, (width, dtype) in shapes.items():
            ...
            check(item['path'] == name+'.npy' and item['shape'] == list(expected_shape) and item['dtype'] == str(np.dtype(dtype)) and item['sha256'] == report['output_sha256'][name+'.npy'], 'Manifest array identity: ' + name)
        ...
        check(len(summary['cells']) == 54, 'Exactly54 producer summaries')
        summary_keys = set()
        for item in summary['cells']:
            cell, group = item['cell'], item['group']
            check((cell, group) not in summary_keys and 0 <= cell < 9 and 0 <= group < 6, 'Unique summary cell/group')
            summary_keys.add((cell, group))
            for field, expected in [('requested_signed_rows', group_counts[cell, group]),
                                    ('feedback_clipped_rows', group_feedback_clips[cell, group]),
                                    ('native_clipped_rows', group_native_clips[cell, group]),
                                    ('zero_response_rows', group_zero_response[cell, group])]:
                check(item[field] == int(expected), 'Independent producer summary: ' + field)
