"""One K0 solve with algebraically factored value updates; no new regularization."""
import hashlib
import json
...
OUT = BASE / 'pico_final3800_factored_k0_v1'
...
def factored_backward(A, B, lx, lxx, lu, luu, lo, hi, mu):
    ...
    for t in reversed(range(T)):
        ...
        quu_reg = luu[t] + B[t].T @ reg @ B[t]
        qux_reg = B[t].T @ reg @ A[t]
        # Preserve the exact original feasibility and box-QP logic. No jitter,
        # eigenvalue clipping, altered mu, or changed stage/control cost.
...
report = dict(kind='one_private_guided_to_K0_factored_value_solve', actual_control=3800, actual_controls_executed=0,
              ...
              backward_override='algebraically equivalent factored closed-loop vx/vxx update only',
              eigenvalue_clipping=False, extra_diagonal_or_jitter=False,
