"""Freeze the selected direct-target continuation; only synthetic tests and saved arrays."""
import hashlib
import json
...
def main():
    ...
    names=['direct_contract.py','direct_model.py','direct_objective.py','direct_data.py','direct_diagnostics.py','train_direct.py','test_direct.py','continuation_contract.py','test_continuation.py']
    ...
    command=[str(PYTHON),'-m','unittest','-v','test_direct','test_continuation']
    ...
    original=BASE.parent/'direct_target_student_v1';original_fit=original/'fit'
    restoration=dict(checkpoint_path=(original_fit/'student_head.pt').as_posix(),checkpoint_sha256=sha(original_fit/'student_head.pt'),
        normalization_path=(original_fit/'normalization.npz').as_posix(),normalization_sha256=sha(original_fit/'normalization.npz'),
        predictions={key:(original_fit/('final_GPU_'+key+'.npy')).as_posix() for key in ('nominal','velocity','physical')},
        metrics_path=(original_fit/'final_metrics.json').as_posix(),ordinary_start_step=5000,
        all_six_AdamW_states=True,CPU_CUDA_NumPy_Python_RNG=True,initial_GPU_exact_before_updates=True)
    prior_paths=[original_fit/name for name in ('student_head.pt','student_head.onnx','normalization.npz','report.json','final_metrics.json','request.json','runtime.json','optimization_completed.json','export_parity.json')]
    prior_paths += [Path(value) for value in restoration['predictions'].values()]
    prior_paths += [original/name for name in ('training_request.json','training_frozen_inputs.json','training_clearance.json','owner_completion_verification.json')]
    prior_paths += [BASE.parent/'direct_target_final_fit_review_v1/review.json',BASE.parent/'direct_target_root_audit_v1/results_v1/report.json',BASE.parent/'direct_target_saved_outcome_review_v1/report.json',BASE.parent/'direct_target_independent_physics_v1/report.json']
    for path in prior_paths:pins[path.as_posix()]=sha(path)
    request=dict(kind='one_fixed_direct_absolute_target_continuation',root_selected=True,updates=50000,ordinary_final_step=55000,restored_start_step=5000,seed=20260911,
        features=1000,retained_indices=[[0,52],[75,1023]],head_output='normalized_target',architecture=[1000,256,256,23],activation='ELU',
        initialization='exact ordinary5000 actor/AdamW/RNG; fresh construction overwritten before the fixed initial pass',restoration=restoration,paths=paths,
        normalization=dict(restored_byte_exact=True,population='original all9904 nominal only',equal_cells=15,mean_variance='float64 global mean and between-cell population variance',
        ...
        objective=dict(nominal_weight=1,velocity_weight=1,physical_weight=1,nominal_cells=15,velocity_cells=9,physical_cells=9,
            ...
            sampler='exact saved original5000x576 rows and axes repeated ten times via index modulo5000; zero new sample draws; differentiable nominal center reuse'),
        optimizer=dict(name='AdamW',lr_start=3e-5,lr_end=3e-6,schedule='inclusive cosine over50000 additional updates',weight_decay=1e-5,
        ...
        budgets=dict(training_head_rows=705500000,diagnostic_torch_rows=460740,ORT_calls=601,native_steps=0,BFM_calls=0),
    ...
    receipt=dict(kind='frozen_direct_target_continuation',source_directory=snapshot.as_posix(),source_sha256=sources,input_sha256=dict(sorted(pins.items())),
    ...
    write(BASE/'training_clearance_draft.json',dict(approved=False,request_sha256=sha(BASE/'training_request.json'),
        ...
        required_review_fields=['training_request_sha256','frozen_receipt_sha256'],additional_updates=50000,ordinary_final_step=55000,
