"""One fixed direct5000->55000 continuation; no automatic retry or physical calls."""
import os
...
import numpy as np
import torch
from direct_contract import RETAINED,PHYSICAL_COUNTS,SEED,normalized_labels
...
from continuation_contract import (source_row,continuation_rate,
                                   exact_saved,restore_rng,verify_start)
...
def gate():
    ...
    if request['kind']!='one_fixed_direct_absolute_target_continuation' or not request['root_selected'] or request['updates']!=50000 or request['seed']!=SEED or request['ordinary_final_step']!=55000:raise ValueError('Unexpected selected fit.')
    expected=dict(training_head_rows=705500000,diagnostic_torch_rows=460740,ORT_calls=601,native_steps=0,BFM_calls=0)
...
def main():
    ...
    try:
        ...
        state['stage']='load_exact_ordinary5000'
        start_checkpoint=request['restoration']['checkpoint_path']
        if sha(start_checkpoint)!=request['restoration']['checkpoint_sha256']:raise ValueError('Restored checkpoint hash differs.')
        ...
        for key,actual in [('joint_span',data['span']),('runtime_joint_span',data['span'].astype(np.float64)),('default_q',data['default']),('joint_limits',data['limits']),('retained_feature_indices',RETAINED)]:
            exact_saved(torch.from_numpy(actual),saved[key],key)
        normalization_source=request['restoration']['normalization_path']
        if sha(normalization_source)!=request['restoration']['normalization_sha256']:raise ValueError('Restored normalization archive differs.')
        with np.load(normalization_source,allow_pickle=False) as oldnorm:
            exact_saved(mean,oldnorm['feature_mean'],'old feature mean');exact_saved(std,oldnorm['feature_std'],'old feature std')
        ...
        model=DirectTarget(mean,std)
        if any(p.dtype!=torch.float32 for p in model.parameters()):raise ValueError('Nonfloat32 parameter.')
        model.actor.load_state_dict(saved['actor_state'],strict=True);model=model.to('cuda')
        optimizer=torch.optim.AdamW(model.parameters(),lr=3e-5,weight_decay=1e-5,foreach=False,fused=False)
        optimizer.load_state_dict(saved['optimizer_state']);restore_rng(saved['rng'])
        restored_rng=rng_save();restoration=verify_start(model,optimizer,saved,restored_rng)
        torch.save(dict(seed=SEED,actor_state=cpu_tree(model.actor.state_dict()),optimizer_state=cpu_tree(optimizer.state_dict()),rng_after_restoration=restored_rng,
            source_checkpoint_sha256=sha(start_checkpoint),ordinary_start_step=5000,fresh_initialization=False),dest/'initialization.pt')
        write(dest/'restoration_state.json',dict(**restoration,source_checkpoint_sha256=sha(start_checkpoint),normalization_archive_exact=sha(dest/'normalization.npz')==sha(normalization_source)))
        ...
        for corpus,actual in initial.items():
            prior_prediction=np.load(request['restoration']['predictions'][corpus],mmap_mode='r',allow_pickle=False)
            exact_saved(np.asarray(actual),np.asarray(prior_prediction),'restored GPU predictions '+corpus)
        exact_saved(initial_metrics,read(request['restoration']['metrics_path']),'restored initial metrics')
        write(dest/'restoration.json',dict(**restoration,source_checkpoint_sha256=sha(start_checkpoint),normalization_archive_exact=True,
            initial_GPU_predictions_exact=True,initial_metrics_exact=True,all_restoration_checks_passed=True,additional_model_calls=0))
        ...
        used_rows=np.lib.format.open_memmap(dest/'replayed_center_rows.npy',mode='w+',dtype=np.int32,shape=(50000,576));used_rows[:]=-1
        used_axes=np.lib.format.open_memmap(dest/'replayed_axes.npy',mode='w+',dtype=np.int8,shape=(50000,576));used_axes[:]=-1
        model.train();state['stage']='fixed_50000_additional_updates'
        for index in range(50000):
            state.update(attempted_step=5000+index+1,additional_update=index+1,optimizer_call_started=False,optimizer_call_returned=False,optimizer_synchronized=False)
            schedule_index=source_row(index)
            rows=data['sampled_rows'][schedule_index].astype(np.int64);axes=data['sampled_axes'][schedule_index].astype(np.int64)
            probe_rows=((rows*23+axes)[:,None]*2+np.arange(2)[None]).reshape(-1)
            mapped=data['center_map'][rows];active.clear();active.update(step=5000+index+1,schedule_index=schedule_index,center_rows=rows.copy(),axes=axes.copy(),probe_rows=probe_rows.copy(),nominal_center_indices=mapped.copy())
            rate=continuation_rate(index)
            ...
            used_rows[index]=data['sampled_rows'][schedule_index];used_axes[index]=data['sampled_axes'][schedule_index]
            ...
            if (index+1)%50==0:
                ...
                atomic_write(dest/'progress.json',dict(completed_updates=index+1,requested_updates=50000,optimizer_step=5000+index+1,losses=losses[-1],counters=counters,elapsed_seconds=time.perf_counter()-started))
                print(json.dumps(dict(step=5000+index+1,nominal=losses[-1][0],velocity=losses[-1][1],physical=losses[-1][2])),flush=True)
        ...
        if counters['training_head_rows_returned']!=705500000 or counters['training_head_rows_attempted']!=705500000:raise ValueError('Training row budget mismatch.')
        for repeat in range(10):
            if not np.array_equal(used_rows[repeat*5000:(repeat+1)*5000],data['sampled_rows']) or not np.array_equal(used_axes[repeat*5000:(repeat+1)*5000],data['sampled_axes']):raise ValueError('Exact repeated schedule mismatch.')
        checkpoint=dict(kind='direct_absolute_native23_target',ordinary_final_step=55000,additional_updates=50000,seed=SEED,
        ...
        write(dest/'optimization_completed.json',dict(optimization_completed=True,ordinary_final_step=55000,checkpoint_sha256=sha(dest/'student_head.pt'),export_validation_pending=True))
        ...
        report=dict(completed=bool(numerical['passed']),optimization_completed=True,final_export_diagnostics_completed=True,
            numerical_gate_passed=bool(numerical['passed']),export_parity_passed=bool(numerical['passed']),ordinary_final_step=55000,additional_updates=50000,
            features=1000,head_output='normalized_target',
            fresh_initialization=False,restored_start_step=5000,restoration_sha256=sha(dest/'restoration.json'),seed=SEED,nominal_rows=9904,velocity_probe_rows=140622,physical_rows=3054,
        ...
        state['stage']='COMPLETE';atomic_write(dest/'progress.json',dict(completed_updates=50000,optimizer_step=55000,stage='COMPLETE',counters=counters));print(json.dumps(dict(completed=True,checkpoint_sha256=report['checkpoint_sha256'],onnx_sha256=report['onnx_sha256'])),flush=True)
