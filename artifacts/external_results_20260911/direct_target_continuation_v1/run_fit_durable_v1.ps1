...
try{
    ...
    if($clear.approved -ne $true -or $clear.additional_updates -ne 50000 -or $clear.ordinary_final_step -ne 55000 -or $clear.automatic_retry -ne $false){throw 'Selected direct fit scope differs.'}
    ...
    if($request.kind -ne 'one_fixed_direct_absolute_target_continuation' -or $request.root_selected -ne $true -or $request.updates -ne 50000 -or $request.seed -ne 20260911){throw 'Unexpected request.'}
    ...
    $snapshot=Join-Path $runRoot 'source_snapshot_v1';$driver=Join-Path $snapshot 'train_direct.py'
    ...
    Write-NewJson (Join-Path $processRoot 'start.json') ([ordered]@{
        kind='one_fixed_direct_target_continuation';wrapper_pid=$PID;started_utc=[DateTime]::UtcNow.ToString('o');
        ...
        updates=50000;ordinary_final_step=55000;budgets=$request.budgets;automatic_retry=$false
    })
    ...
    if($report.ordinary_final_step -ne 55000 -or $report.additional_updates -ne 50000 -or $report.features -ne 1000 -or $report.head_output -ne 'normalized_target'){throw 'Final direct head metadata differs.'}
    if($report.counters.training_head_rows_returned -ne 705500000 -or $report.counters.diagnostic_torch_rows_returned -ne 460740 -or $report.counters.ORT_calls_returned -ne 601){throw 'Completed diagnostic counts differ.'}
    ...
    try{Write-NewJson (Join-Path $processRoot 'exit.json') ([ordered]@{
        kind='one_fixed_direct_target_continuation_exit';ended_utc=[DateTime]::UtcNow.ToString('o');wrapper_pid=$PID;
