...
def assert_pins(request):
    ...
    for p,h in request['runtime_sha256'].items():
        if sha(local(p))!=h:raise ValueError('runtime changed: '+p)
...
def run(request_path,dest):
    ...
    if platform.python_version()!=identity['python_version'] or str(Path(sys.executable).resolve())!=identity['python_executable']:raise ValueError('actual Python identity')
    if str(Path(np.__file__).resolve())!=identity['numpy_module'] or str(Path(scipy.__file__).resolve())!=identity['scipy_module']:raise ValueError('actual package identity')
    assert_pins(request)
    own=Path(__file__).resolve()
    if str(own) not in {str(local(p).resolve()) for p in request['source_sha256']}:raise ValueError('executing source not pinned')
    for name in ('direct_features.py','secant_math.py','input_schema.py'):
    ...
    dest.mkdir(exist_ok=False);arrays={};active={};progress={'phase':'centers','centers_verified':0,'overlap_committed':0,'added_committed':0,
        'feature_attempted':0,'feature_returned':0,'map_attempted':0,'map_returned':0,'model_calls':0,'physics_steps':0}
    try:
        contract=read(p['contract']);loaded,schemas=validate_inputs(p,contract)
        if schemas!=request['archive_schemas']:raise ValueError('frozen archive schema changed')
        write(dest/'input_schema_gate.json',{'passed':True,'archive_schemas':schemas,'task_feature_calls':0,'task_map_calls':0})
        ...
        for i in range(ROWS):
            ...
            progress['feature_attempted']+=1
            x=builder(q,v,int(c['source_frame'][i]));progress['feature_returned']+=1;active['features']=x.copy()
            progress['map_attempted']+=1
            target,raw,correction,preclip=committed_target(difference,c['planned_state'][i],c['planned_target'][i],c['gain'][i],q,v,limits)
            progress['map_returned']+=1
        ...
        np.savez_compressed(dest/'centers.npz',features=nominal_features,target=c['expert_target'],qpos=c['qpos'],qvel=c['qvel'],
            ...
            plan_control=c['plan_control'],plan_local=c['plan_local'],source_center_index=np.arange(ROWS,dtype=np.int64))
            ...
            for i in range(ROWS):
                ...
                for axis in axes:
                    for sign_index,sign in enumerate(SIGNS):
                        ...
                        progress['feature_attempted']+=1
                        x=builder(q,v,int(c['source_frame'][i]));progress['feature_returned']+=1;active.update(features=x.copy(),tangent=tangent,tangent_change=delta)
                        progress['map_attempted']+=1
                        target,raw,correction,preclip=committed_target(difference,c['planned_state'][i],c['planned_target'][i],c['gain'][i],q,v,limits)
                        progress['map_returned']+=1
        ...
        summary=[]
        zero_gain=np.all(c['gain']==0,axis=(1,2))
        for cell_id in range(9):
            ids=np.flatnonzero(cell==cell_id)
            for group_id,(name,start,end,unit) in enumerate(GROUPS):
                f=arrays['feedback_clipped'][ids,start:end];n=arrays['native_clipped'][ids,start:end];change=arrays['target_change'][ids,start:end]
                summary.append({'cell':cell_id,'group':group_id,'name':name,'centers':len(ids),'axes':end-start,'requested_signed_rows':int(len(ids)*(end-start)*2),
                    'feedback_clipped_rows':int(np.any(f,axis=-1).sum()),'feedback_clipped_components':int(f.sum()),
                    'native_clipped_rows':int(np.any(n,axis=-1).sum()),'native_clipped_components':int(n.sum()),
                    'zero_response_rows':int(np.all(change==0,axis=-1).sum()),'zero_response_components':int((change==0).sum()),
                    'zero_gain_centers':int(zero_gain[ids].sum()),'zero_gain_signed_rows':int(zero_gain[ids].sum()*(end-start)*2)})
        write(dest/'group_cell_summary.json',{'groups':GROUPS,'cells':summary,'source':'saved target_change and clip arrays only'})
        output_specs={key:{'path':key+'.npy','shape':list(value.shape),'dtype':str(value.dtype),'sha256':sha(dest/(key+'.npy'))} for key,value in arrays.items()}
        manifest={'complete':True,'request_sha256':request_sha,'arrays':output_specs,'centers_sha256':sha(dest/'centers.npz'),
            'group_cell_summary_sha256':sha(dest/'group_cell_summary.json'),'input_schema_gate_sha256':sha(dest/'input_schema_gate.json'),
            'center_gate_sha256':sha(dest/'center_gate.json'),'overlap_gate_sha256':sha(dest/'overlap_gate.json')}
        if any(progress[key]!=357669 for key in ('feature_attempted','feature_returned','map_attempted','map_returned')):raise ValueError('fixed pure call counts')
        assert_pins(request)
        ...
        write(dest/'report.json',{'complete':True,'request_sha256':request_sha,'centers':ROWS,'signed_rows':TOTAL,'overlap_rows':140622,'new_rows':213990,
            ...
            'manifest_sha256':sha(dest/'manifest.json'),'group_cell_summary_sha256':sha(dest/'group_cell_summary.json'),
            ...
            'all_runtime_and_sources_unchanged':True,'pure_feature_calls':357669,'pure_committed_map_calls':357669,
            'call_accounting':{key:progress[key] for key in ('feature_attempted','feature_returned','map_attempted','map_returned')},
