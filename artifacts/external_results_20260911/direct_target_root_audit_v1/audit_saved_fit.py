...
def run(experiment, output):
    ...
    sources=[Path(__file__),Path(__file__).with_name('audit_math.py')]
    ...
    try:
        ...
        check('ordinary_final_completed',report['completed'] and report['optimization_completed'] and report['ordinary_final_step']==5000)
        ...
        exact('all_2880000_sampled_centers',npy(fit/'replayed_center_rows.npy'),npy(paths['sampled_rows']))
        exact('all_2880000_sampled_axes',npy(fit/'replayed_axes.npy'),npy(paths['sampled_axes']))
        progress=npy(fit/'training_progress.npy');check('5000_complete_finite_loss_records',progress.shape==(5000,6) and np.isfinite(progress).all())
        check('nonnegative_objectives_and_gradient_norm',bool((progress[:,:4]>=0).all() and (progress[:,5]>=0).all()))
        for index in range(5000):
            rate=3e-5+.5*(3e-4-3e-5)*(1+math.cos(math.pi*index/4999))
        ...
        check('saved_cell_shapes',nc.shape==(5000,15) and vc.shape==pc.shape==(5000,9))
        ...
        for key,value in {'training_head_rows_attempted':70550000,'training_head_rows_returned':70550000,'diagnostic_torch_rows_attempted':460740,'diagnostic_torch_rows_returned':460740,'ORT_calls_attempted':601,'ORT_calls_returned':601}.items():check('counter.'+key,report['counters'][key]==value)
        import torch
        import onnx
        from onnx import numpy_helper
        checkpoint=torch.load(bind(fit/'student_head.pt'),map_location='cpu',weights_only=True)
        check('checkpoint_direct_output_contract',checkpoint['kind']=='direct_absolute_native23_target' and checkpoint['output']=='normalized_absolute_target' and checkpoint['seed']==20260911 and checkpoint['ordinary_final_step']==checkpoint['additional_updates']==5000)
        compare('checkpoint_request',checkpoint['request'],request)
        optimization=read(bind(fit/'optimization_completed.json'))
        check('optimization_receipt_subject',optimization['optimization_completed'] is True and optimization['ordinary_final_step']==5000 and optimization['checkpoint_sha256']==sha(fit/'student_head.pt') and optimization['export_validation_pending'] is True)
        ...
        for key,state in optimizer['state'].items():
            check('optimizer_step:'+str(key),float(state['step'])==5000)
            check('optimizer_moments_finite:'+str(key),all(bool(torch.isfinite(state[name]).all()) for name in ('exp_avg','exp_avg_sq')))
        group=optimizer['param_groups'][0]
        check('optimizer_contract',len(optimizer['param_groups'])==1 and group['lr']==3e-5 and group['weight_decay']==1e-5 and group['foreach'] is False and group['fused'] is False)
        ...
        result=dict(passed=True,checks=len(checks),initial_metrics=calculated['initial_GPU'],final_metrics=calculated['final_GPU'],maximum_preclamp_export_error_rad=maximum,
                    input_sha256=tracked,task_model_calls=0,ORT_calls=0,optimizer_updates=0,native_steps=0,
                    limitations=['saved losses cannot reconstruct unsaved gradients or prove connected stability','nominal float32 mean independently checked with3e-7 relative roundoff tolerance','RNG initialization and CUDA execution flags covered by trainer source/runtime review, not reproduced by this array audit'])
    ...
    print(json.dumps({'passed':True,'checks':len(checks),'report_sha256':sha(output/'report.json')}))
