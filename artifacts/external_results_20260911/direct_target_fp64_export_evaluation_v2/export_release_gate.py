...
def validate_release(binding,paths,source,purpose,*,read,sha,bound_file,field,has_hash):
    ...
    root_entry=binding['root_training_audit'];root=read(bound_file(root_entry))
    assert field(root,root_entry['pass_field']) is True
    assert root['evidence_audit_passed'] is True
    assert root['export_qualified'] is False and root['canonical_cleared'] is False
    # The immutable root audit exposes actual consumed file paths, not the
    # reviewer's logical subject-map schema. Match path AND digest explicitly.
    def path_key(value):return str(value).replace('\\','/').casefold()
    consumed={}
    for name,digest in root['input_sha256'].items():
        key=path_key(name)
        assert key not in consumed or consumed[key]==digest,'ambiguous audit path'
        consumed[key]=digest
    for name,digest in original.items():
        assert consumed.get(path_key(binding[name]['path']))==digest,('root_training_audit',name)
