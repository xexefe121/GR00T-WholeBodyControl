...
def fixture():
    ...
    records['root_training_audit']=dict(passed=True,evidence_audit_passed=True,export_qualified=False,canonical_cleared=False,
        input_sha256={k:digest(k) for k in ('fit_report','checkpoint','source_head','normalization','training_manifest','training_request')})
...
def test_direct_training_subject_cannot_hide_elsewhere():
    f=fixture();receipt=f[2]['root_training_audit'];receipt['other_metadata']=receipt['input_sha256'].pop('checkpoint')
    with pytest.raises(AssertionError):run(f)
...
def test_actual_root_schema_needs_no_invented_direct_map():
    f=fixture();assert 'direct_subject_sha256' not in f[2]['root_training_audit'];run(f)

@pytest.mark.parametrize('key,value',[('evidence_audit_passed',False),('export_qualified',True),('canonical_cleared',True)])
def test_root_evidence_and_original_failure_roles(key,value):
    f=fixture();f[2]['root_training_audit'][key]=value
    with pytest.raises(AssertionError):run(f)

def test_root_input_digest_on_different_path_does_not_count():
    f=fixture();r=f[2]['root_training_audit']['input_sha256'];r['different_checkpoint']=r.pop('checkpoint')
    with pytest.raises(AssertionError):run(f)

def test_training_review_binds_actual_root_audit():
    f=fixture();f[2]['training']['direct_subject_sha256']['root_training_audit']='a'*64
    with pytest.raises(AssertionError):run(f)
