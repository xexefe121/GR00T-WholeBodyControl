...
def test_helper_review_cannot_omit_source(tmp_path):
    ...
    prep=tmp_path/'launch_helper_preparation_v2.json';prep.write_text(json.dumps({'helper_sha256':{'helper.py':sha(code)}}))
...
def test_exact_condition_path_and_all23_roles(tmp_path,condition):
    from freeze_final_package import actual_paths,SUBJECTS
    request={'subjects':{key:{'path':str(tmp_path/key)} for key in ('checkpoint','coefficient_source','full_state_root_audit','full_state_root_owner','energy_source','recovery_rows','collection_report','collection_request','collection_qualification','collection_source_review','consistency_report','warm_restore_review')},
    ...
    assert len(SUBJECTS)==23 and set(SUBJECTS)<=set(paths)
...
def test_context_binding_keeps_scope_and_no_condition_default(condition):
    ...
    assert b['context_condition']==condition and b['architecture']==[1323,512,512,23] and b['ordinary_final_step']==91000
