...
def main():
    ...
    if absence['expected_pids']!=[exit_report['wrapper_pid'],exit_report['child_pid']]:raise ValueError('Absence receipt does not identify actual wrapper/child')
    ...
    mapping.update(head=sha(dest/'student_head_fp64.onnx'),export_report=sha(dest/'report.json'),export_request=sha(BASE/'export_request.json'),export_manifest=sha(BASE/'export_frozen_inputs.json'),output_manifest=sha(dest/'manifest.json'))
    ...
    with (BASE/'owner_completion_verification_v2.json').open('x',encoding='utf-8') as f:json.dump(result,f,indent=2,allow_nan=False);f.write('\n')
    print(json.dumps(dict(owner_verification_passed=True,sha256=sha(BASE/'owner_completion_verification_v2.json'),direct_subject_sha256=mapping)))
