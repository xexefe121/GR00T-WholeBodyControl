...
def main():
    assert not (BASE/'generation_frozen_inputs_v2.json').exists()
    draft=BASE/'source_draft_v2';dest=BASE/'source_snapshot_v2'
    ...
    oldtree=ast.parse((BASE/'source_snapshot_v1/generate_velocity_chords.py').read_text());newtree=ast.parse((dest/'generate_velocity_chords.py').read_text())
    fn=lambda tree,name:next(n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name==name)
    assert ast.dump(fn(oldtree,'reconstruct_centers'))==ast.dump(fn(newtree,'reconstruct_centers'))
    ...
    for rel in ['learner_stability_design_v1/proposal.json','learner_stability_design_v1/PROPOSAL.md',
        ...
        add(NEW/rel)
    add(BASE/'generation_preservation_tests_v2.json');add(BASE/'generation_frozen_inputs.json');add(BASE/'saved_center_pure_test_v2.json');add(BASE/'run_generation_durable_v2.ps1');add(Path(__file__))
    ...
    result=dict(kind='frozen_one_velocity_chord_generation',source_directory=str(dest),
        ...
        input_sha256=inputs,selected_proposal_sha256=expected,original18_phase_sources_byteexact=True,pure_center_reconstruction_AST_unchanged_from_passing_v1=True,atomic_progress_and_failure_snapshot=True,final_source_input_rehash_required=True,
    ...
    write(BASE/'generation_frozen_inputs_v2.json',result)
    write(BASE/'generation_clearance_DRAFT_v2.json',dict(approved=False,generation_only=True,
        selected_proposal_sha256=expected,frozen_receipt_sha256=sha(BASE/'generation_frozen_inputs_v2.json'),
        launcher_sha256=sha(BASE/'run_generation_durable_v2.ps1'),review_path=None,review_sha256=None))
    print(json.dumps(dict(sources=len(result['source_sha256']),inputs=len(inputs),receipt_sha256=sha(BASE/'generation_frozen_inputs_v2.json'))))
