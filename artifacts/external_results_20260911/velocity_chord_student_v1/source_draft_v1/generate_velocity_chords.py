...
from chord_common import (BASE,NEW,KEYS,ROWS,AXES,SIGNS,ACTOR_BUDGET,BACKWARD_BUDGET,
    read,write,sha,archive,exact,assert_clearance,dataset_configs)
...
def bfm_actor(seed,qpos,qvel,previous,history,latent):
...
def main():
    ...
    try:
        ...
        for i in range(ROWS):
            ...
            raw,base,state=bfm_actor(seed,q,v,previous,history,z)
        ...
        for i in range(ROWS):
            ...
            for j in range(AXES):
                ...
                for sign_index,sign in enumerate(SIGNS):
                    ...
                    raw,base,state=bfm_actor(seed,q,changed,previous,history,centers['goal_latent'][i])
        ...
        result=dict(kind='one_exact_bounded_velocity_chord_generation',complete=True,centers=ROWS,axis_pairs=ROWS*AXES,
            ...
            input_frozen_receipt_sha256=sha(BASE/'generation_frozen_inputs.json'),clearance_sha256=sha(args.clearance),
        ...
        progress['failure']=dict(type=type(exc).__name__,message=str(exc));persist('FAILED')
        raise
