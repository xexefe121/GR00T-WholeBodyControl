...
def assert_frozen():
    receipt = read(BASE/'generation_frozen_inputs_v2.json')
...
def assert_clearance(path):
    ...
    assert clearance['frozen_receipt_sha256'] == sha(BASE/'generation_frozen_inputs_v2.json')
    assert clearance['launcher_sha256'] == sha(BASE/'run_generation_durable_v2.ps1')
