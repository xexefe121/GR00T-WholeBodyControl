"""Audit saved D3 warm512 ordinary91000 evidence. No forwards, gradients, optimizers or physics."""
import argparse
import hashlib
import json
...
def run(request_path,request_sha,output):
    ...
    def compare(name,a,b):
        ...
        elif isinstance(b,float):close(name,a,b,nominal=name.endswith(('normalized_MSE','nominal_objective','weighted_objective','recovery_objective')))
    ...
    try:
        ...
        check('audit_kind',ar['kind']=='saved_width251_recovery_warm_only')
        import sys
        ...
        expected_budget=dict(training_forward_rows=157040000,training_forward_calls=40000,training_updates=10000,
        ...
        # The concrete review binds the frozen receipt by hash; the source map
        ...
        manifest(shared,shared/'output_manifest.json',['normalization.npz','nominal_context.npy','center_context.npy','physical_context.npy','data_identities.npz','context_alignment.json','schedule_centers.npy','schedule_axes.npy','source_source_schedule_centers.npy','source_source_schedule_axes.npy','schedule_lineage.json','reused_inputs.json','runtime.json','recovery_rows.npz','recovery_evidence.json'])
        ...
        for key,value in [('centers',sc),('axes',sa)]:
            ...
            exact('copied_prior_schedule:'+key,npy(shared/('source_'+Path(request['context_paths']['schedule_'+key]).name)),old)
        ...
        recovery=archive(bind(request['subjects']['recovery_rows']['path'],request['subjects']['recovery_rows']['sha256']))
        ...
        original_context_receipt=source['request']['subjects']['source_frozen_inputs']
        ...
        check('qualified_source_report',source_report['completed'] is True and source_report['ordinary_final_step']==81000 and source_report['checkpoint_sha256']==request['subjects']['checkpoint']['sha256'])
        ...
        for condition in ('causal',):
            ...
            for key,value in dict(condition=condition,ordinary_final_step=91000,additional_updates=10000,optimizer_step=26000,fresh_optimizer=False,features=1323,context_features=323,head_output='normalized_target',fixed_full_state_coefficient=WEIGHT,execution_dtype='float64',public_input_dtype='float32',public_output_dtype='float32',training_first_layer_execution='split_old256_new256_original1000_plus323',export_first_layer_execution='monolithic_float64_1323',parity_tolerance_rad=1e-5,all_frozen_inputs_unchanged=True,checkpoint_selection=False,native_steps=0,BFM_calls=0).items():compare(condition+'_report.'+key,report[key],value)
            for field,value in dict(ordinary_start_step=81000,optimizer_start_step=16000,hidden_width=512,architecture=[1323,512,512,23],expansion_performed=False,recovery_rows=1018,recovery_phase_counts=[99,819,100],recovery_coefficient=.2,context_and_normalization_reused=True,source_checkpoint_sha256=request['subjects']['checkpoint']['sha256'],energy_source_sha256=request['subjects']['energy_source']['sha256']).items():compare('report_extra.'+field,report[field],value)
            ...
            check('warm_full512_initial_actor_optimizer_RNG',not warm_initial_errors(init,source))
            tree('source_expansion_provenance',checkpoint['source_expansion_generator_state'],source['expansion_generator_state'])
            compare('source_expansion_seed',checkpoint['source_expansion_seed'],source['expansion_seed'])
            for key,value in dict(ordinary_start_step=81000,optimizer_start_step=16000,hidden_width=512,expansion_performed=False,condition=condition,source_checkpoint_sha256=request['subjects']['checkpoint']['sha256']).items():compare(condition+'_init.'+key,init[key],value)
            ...
            for key,value in dict(kind='direct_absolute_native23_target',condition=condition,ordinary_final_step=91000,additional_updates=10000,optimizer_step=26000,fresh_optimizer=False,hidden_width=512,architecture=[1323,512,512,23],expansion_performed=False,recovery_coefficient=.2,context_order='previous_action23_then_incoming_history300',context_blinded=condition=='blinded',full_state_coefficient=WEIGHT,source_checkpoint_sha256=request['subjects']['checkpoint']['sha256']).items():compare(condition+'_checkpoint.'+key,checkpoint[key],value)
            ...
            for index,shape in enumerate(shapes):
                state=optimizer['state'][index];check(condition+'_optimizer_step:'+str(index),state['step'].dtype==torch.float32 and state['step'].shape==torch.Size([]) and float(state['step'])==26000)
            ...
            check(condition+'_optimization_receipt',optimization['optimization_completed'] is True and optimization['condition']==condition and optimization['ordinary_final_step']==91000 and optimization['optimizer_step']==26000 and optimization['checkpoint_sha256']==sha(folder/'student_head.pt') and optimization['export_validation_pending'] is True)
            ...
            cell_values={}
            for name,width in zip(OLD_CORPORA,(15,54,9)):
            ...
            dc=npy(folder/'recovery_cell_losses.npy');dl=npy(folder/'recovery_objectives.npy')
            ...
            initial_outputs[condition]=outputs['initial_GPU32'];restoration=read(folder/'restoration.json');restored={corpus:drift(outputs['initial_GPU32'][corpus],npy(request['restoration_predictions'][corpus]),span) for corpus in OLD_CORPORA}
            maximum=max(v['max_preclip_error_rad'] for v in restored.values())
            compare(condition+'_restoration',restoration,dict(source_checkpoint_sha256=request['subjects']['checkpoint']['sha256'],**restoration_fields(),initial_drift=dict(passed=maximum<=1e-5,tolerance_rad=1e-5,max_preclip_error_rad=maximum,corpora=restored,source_ordinary_step=81000,changed_MatMul_dimension=False,stored_first_layer_width=1323,stored_hidden_width=512,old_block_contractions_preserved=True,first_layer_execution='split_old256_new256_original1000_plus323',byte_gate_required=False)))
            ...
            for label in BACKENDS:
                m=recovery_metrics(outputs[label],data,group_weights,energy,.2);calculated[condition][label]=m;saved=read(folder/(label+'_metrics.json'));compare(condition+'_metrics:'+label,saved,m);compare(condition+'_report_metrics:'+label,report['metrics'][label],saved)
            comparisons=numerical_comparisons(outputs,data);maximum=max(comparisons.values());qualification[condition]=maximum<=1e-5
            parity=read(folder/'export_parity.json');compare(condition+'_twelve_parity',parity['comparisons'],comparisons)
            ...
            drift_report=read(folder/'drift.json');check(condition+'_twelve_drifts',len(drift_report)==12)
            ...
            count=report['counts'];compare(condition+'_training_count',count['training'],counts(40000,157040000))
            for label in BACKENDS:compare(condition+'_diagnostic_count:'+label,count['diagnostics'][label],counts(1441,368588))
            ...
            for label in BACKENDS:compare(condition+'_checkpoint_diagnostics:'+label,checkpoint['counters']['diagnostics'][label],counts(1441,368588) if label=='initial_GPU32' else counts(0,0))
            ...
            check(condition+'_7205_call_partitions',ledger==expected and len(ledger)==7205)
        ...
        launch_path=bind(base/'launch_receipt.json');launch_sha=sha(launch_path);launch=read(launch_path)
        ...
        result=dict(passed=qualified,evidence_audit_passed=True,optimization_complete=True,export_qualified=qualified,
            ...
            limitations=['Saved predictions and training cells are checked for algebra and provenance without recomputing model authenticity or optimizer gradients.',
                ...
                'Nominal and D3 float32 reductions use3e-7 relative tolerance; other saved metric reductions5e-12. Original three-corpus initial drift retains the1e-5rad gate; recovery has no prior prediction byte gate.',
