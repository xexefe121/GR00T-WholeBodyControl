...
def validate_process_chain(base,request,clearance,launch_sha,start,child,end,owner,owner_expected,read,bind,sha,check,compare,canonical):
    ...
    attempt=read(bind(process/'dispatch_attempt.json'))
    for key in ('dispatcher_pid','executable','arguments','clearance_sha256','launch_receipt_sha256','automatic_retry'):
        compare('dispatch_attempt.'+key,attempt[key],dispatch[key])
    ...
    process_files=('dispatch_attempt.json','dispatch.json','start.json','child.json','exit.json','prerun_pins.json','postrun_pins.json','process_absence.json','stdout.log','stderr.log','wrapper_stdout.log','wrapper_stderr.log')
    expected_paths={canonical(p) for p in paths.values()}|{canonical(process/name) for name in process_files}
