...
if($receipt.selected_single_saved_audit -ne $true -or $receipt.kind -ne 'saved_width251_recovery_warm_only' -or $receipt.request_sha256 -ne (Get-TaskSha (Join-Path $runRoot 'audit_request.json'))){throw 'Wrong selected saved audit.'}
...
try{
    ...
    $source=Join-Path $runRoot 'source_prepared_v1'
    ...
    $arguments='-B -u "'+(Join-Path $source 'audit_saved_warm.py')+'" --request "'+(Join-Path $runRoot 'audit_request.json')+'" --request-sha256 '+$receipt.request_sha256+' --output "'+(Join-Path $runRoot 'results_v1')+'"'
    $child=Start-Process -FilePath $receipt.python_path -ArgumentList $arguments -WorkingDirectory $source -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $processRoot 'stdout.log') -RedirectStandardError (Join-Path $processRoot 'stderr.log')
    $handle=$child.Handle
