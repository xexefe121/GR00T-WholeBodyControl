...
class PlantFoundation:
    def __init__(self, *, clock, stepper, epoch_ns, steps, binding, initial_command,
        ...
        self.active_admitted_ns = self.clock.now_ns()
    ...
    def _boundary(self):
        ...
        activation_ns = self.clock.now_ns()
    ...
    def tick(self):
        ...
        try:
            ...
            if not isinstance(snapshot, CapturedStep) or not isinstance(snapshot.state, bytes) or not isinstance(snapshot.torque, bytes) or not isinstance(snapshot.warnings, tuple):
                raise ValueError('immutable captured step payload required')
    ...
    def summary(self):
        return dict(requested=self.requested_steps, attempted=self.attempted, returned=self.returned,
            ...
            unexecuted=self.requested_steps-self.returned, current_debt=self.debt(self.clock.now_ns()),
            max_debt=self.maximum_debt, cumulative_wake_debt=self.cumulative_wake_debt,
            deadline_misses=self.deadline_misses, first_deadline_failure=self.first_deadline_failure,
