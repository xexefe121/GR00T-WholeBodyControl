# Independent plant clock foundation â€” source preparation only
...
Run existing Python and pytest from `source_draft_v2`:

```powershell
python -m pytest test_foundation.py -q
```

The final preparation receipt binds 41 passing tests, all core/test sources, the design note, preserved earlier test reports, and two fake-only example ledgers. No actual controller, MuJoCo, ORT or optimizer calls are made.
