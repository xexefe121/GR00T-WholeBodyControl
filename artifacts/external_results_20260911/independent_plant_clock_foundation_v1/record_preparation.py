...
BASE=Path(__file__).resolve().parent;NEW=BASE.parent;SOURCE=BASE/'source_draft'
...
test=ET.parse(BASE/'tests_final.xml').getroot().find('testsuite')
assert int(test.attrib['tests'])==40 and all(int(test.attrib[k])==0 for k in ('errors','failures','skipped'))
...
for label in ('hung_worker','plant_stall23ms'):
    ...
    path=BASE/(label+'_fake_example.json')
...
evidence={str(p).replace('\\','/'):sha(p) for p in (
    BASE/'README.md',BASE/'tests_v1.xml',BASE/'tests_v2.xml',BASE/'tests_v3.xml',BASE/'tests_final.xml',
    Path(__file__),design/'NOTE.md',design/'source_pins.json')}
report=dict(passed=True,source_preparation_only=True,tests=40,failures=0,errors=0,skips=0,
    ...
    limitations_path=(BASE/'README.md').as_posix())
write(BASE/'preparation_report.json',report)
print(json.dumps(dict(report_sha256=sha(BASE/'preparation_report.json'),source_sha256=source_hashes,tests=40)))
