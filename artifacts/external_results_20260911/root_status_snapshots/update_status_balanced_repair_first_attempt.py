...
for name in ('CURRENT.md','SIM_RESULT.md'):
    with (OUT/(name.replace('.md','')+'_before_balanced_repair.md')).open('x',encoding='utf-8') as f:f.write((ART/name).read_text())
s=(ART/'CURRENT.md').read_text();start=s.index('## Active work');end=s.index('## Latest controller:',start)
...
r=(ART/'SIM_RESULT.md').read_text();start=r.index('## Work now underway');end=r.index('## Existing qualified offline evidence',start)
