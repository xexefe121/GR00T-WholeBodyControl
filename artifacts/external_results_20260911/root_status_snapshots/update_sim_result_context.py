...
path=repo/'artifacts/teleop_resume_20260911/SIM_RESULT.md'
with (base/'root_status_snapshots/SIM_RESULT_before_causal68000_failure.md').open('x',encoding='utf-8') as f:
    f.write(path.read_text(encoding='utf-8'))
text='''# Native23 simulation result

Updated 2026-09-11 18:16 UTC. Work continues. Native 23-DOF G1 U2 simulation runs, but the fast full-body controller is not yet qualified. Latest trained controller fails a native joint-speed limit before source motion begins. Real Pico and robot operation remain the next stage after simulation passes.
...
- direct_target_causal_context_evaluation_v2/nominal/trace.npz —589cbab8051ab9c1257049d62098248e7866b10e6c0eb085e73cbaf32d79082d.
- direct_target_causal_context_evaluation_v2/evaluation_completion_verification.json —c1e578569bd9f7e1bb0c86342f196440573ecbfc2f574e6fa3dce7fafc32f97e. RawPython0; diagnostic/wrapper2; all5241 inputs unchanged; processes stopped.
- direct_target_causal_context_independent_physics_v1/report.json —fccc2ec215c8536529161eb98982038232ac9116bcfdbec685233807b60f0753.
- direct_target_causal_context_independent_intent_v1/report.json —source and quiet gates false. First invocation failed before main because PYTHONPATH was missing; corrected invocation completed without any repeated task calculation. Launch failure preserved separately.
- direct_target_context_saved_semantics_review_v1/results_v1/report.json —e633e27d39cde9b24026519c2aeae68f44d22450cc70e33cb55b1c292ed492ae; owner512bed04e808a1a3fd1659ca6838c852e3a80a85ead7afe56f2d614b12a453ee. Saved-only audit; no new native/model calls.
...
A source-only fixed3000-update continuation toordinary71000 is being prepared. It restores the causal68000 actor, normalization, all AdamW moments/step3000 and RNG. It retains the same data, schedule, architecture, original nominal/physical objectives and export gates. The proposed change weights each response group by the six-group mean teacher energy divided by that group's energy. Both original and reweighted losses remain reported. Root-reviewed math passed16 synthetic gradient/value tests. Warm continuation uses the specified1e-5→1e-6 cosine schedule; the separate earlier lower-rate proposal is preserved and unselected. No new training has launched as of this update. A single engineering continuation cannot isolate effects from extra epochs or optimizer choices.

One corrected recorded-command clock benchmark has been selected. Its native-step count18190, fourMJB serializations,2ms plant,20ms command interval, original strict limits,60s elapsed/debt100 bounds remain fixed. Setup, plant and evidence-preservation timeouts are now separate. The earlier whole-process120s attempt timed out without final counters; that attempt remains preserved with actual step count unknown. The corrected benchmark is not an online-policy qualification. Heavy training/tests are paused while it runs.
