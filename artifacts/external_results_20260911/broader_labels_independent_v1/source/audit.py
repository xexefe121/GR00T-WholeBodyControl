...
def main():
    ...
    try:
        ...
        for case in request['cases']:
            ...
            for control,q,dq,previous,state,flat,named in actual_observations(trace,contract,stop):
                ...
                if args.phase=='inference':
                    ...
                    proof.context.update(inferred_raw=raw,inferred_base=inferred_base,inferred_features=inferred_features,goal=goal)
        ...
        report=dict(kind='independent_broader_label_'+args.phase+'_audit',pass_all=True,
            ...
            producer_checks_imported=False,request_sha256=sha(args.request),source_sha256=sha(__file__),
        ...
        write(folder/'failure.json',dict(pass_all=False,error=repr(error),traceback=traceback.format_exc(),
            ...
            physics_steps=0,labels_written=False,fitting_launched=False,request_sha256=sha(args.request)))
        raise
