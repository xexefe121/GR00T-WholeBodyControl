...
def test_generic_inference_fault_preserves_staged_inputs_outputs(tmp_path):
    proof=Proof(tmp_path);proof.clip='walk002';proof.control=250
    proof.context=dict(integration=np.arange(291,dtype=np.float64),history=np.zeros(300,np.float32),
        goal=np.ones((1,32),np.float32),inferred_raw=np.full(23,np.nan,np.float32))
    proof.preserve_exception(ValueError('nonfinite feature computation'))
    with np.load(tmp_path/'exception_context.npz',allow_pickle=False) as data:
        assert same(data['history'],proof.context['history']) and same(data['goal'],proof.context['goal'])
        assert np.isnan(data['inferred_raw']).all() and data['control'].item()==250
