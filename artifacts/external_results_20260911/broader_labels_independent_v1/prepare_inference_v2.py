...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--inference',action='store_true',default=True);args=parser.parse_args()
    pins={}
    ...
    for path in (BASE/'source_inference_v2').rglob('*.py'):pin(path)
    ...
    if args.inference:
        ...
        pin(BASE/'focused_inference_v2.xml')
        pin(BASE/'source/audit.py')
    result['input_hashes']=pins
    path=BASE/'inference_request_v2.json'
