...
def outcome(end,report,failure,request_sha):
    ...
    assert type(end['exit_code']) is int
    if report is None:
        assert failure is not None and failure['passed'] is False and failure['request_sha256']==request_sha
        assert failure['model_calls']==failure['native_steps']==0
        assert end['exit_code']!=0 and raw!=0
        return False
    assert failure is None
    assert report['request_sha256']==request_sha and report['evidence_audit_passed'] is True
    assert report['context_condition']=='causal' and report['features']==1323
    assert report['original_requested_controls']==1569
    assert report['model_calls']==report['BFM_calls']==report['native_steps']==report['optimizer_updates']==0
    assert report['behavioral_qualification'] is False and report['all_inputs_unchanged'] is True
...
def main():
    ...
    if positive:
        assert report['input_sha256']==read(BASE/'request.json')['input_sha256']
        assert report['actual_rows_sha256']==sha(BASE/'results_v1/actual_rows.npz')
        assert report['fixed_maps']['report_sha256']==sha(BASE/'results_v1/fixed_map_report.json')
        assert read(BASE/'results_v1/fixed_map_report.json')['arrays_sha256']==sha(BASE/'results_v1/fixed_map_arrays.npz')
    assert end['all_postrun_pins_exact'] is True
    output_paths=[BASE/'request.json',BASE/'launch_receipt.json',BASE/'dispatch.json',BASE/'source_preparation.json',
        BASE/'stage_source_preparation.json',clearance_path,Path(clearance['review']['path']),Path(__file__)]
    output_paths+=list((BASE/'process_v1').glob('*.json'))+list((BASE/'process_v1').glob('*.log'))+list((BASE/'results_v1').glob('*'))
    result=dict(completion_accounting_passed=True,evidence_audit_passed=positive,behavioral_qualification=False,
        ...
        source_review_sha256=receipt['source_review_sha256'],canonical_owner_sha256=read(BASE/'request.json')['owner_sha256'],
    ...
    print(json.dumps(dict(completion_accounting_passed=True,evidence_audit_passed=positive,behavioral_qualification=False,owner_sha256=sha(BASE/'owner_completion.json'),report_sha256=result['report_sha256'])))
