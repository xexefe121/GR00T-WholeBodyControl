...
def validate_dispatch_correction(dispatch):
    ...
    assert utc_order_key(dispatch['utc'])>utc_order_key(stop['utc'])
...
def main():
    ...
    output_paths += [BASE/'verify_completion_dispatch_v2.py',BASE/'derive_owner_dispatch_v3.py',BASE/'owner_dispatch_v3.diff',BASE/'test_owner_timestamp.py',BASE/'owner_timestamp_tests.log']
    ...
    with (BASE/'owner_completion_dispatch_v3.json').open('x',encoding='utf-8') as f:json.dump(result,f,indent=2);f.write('\n')
    print(json.dumps(dict(completion_accounting_passed=True,evidence_integrity_passed=positive,behavioral_qualification=False,owner_sha256=sha(BASE/'owner_completion_dispatch_v3.json'),report_sha256=result['report_sha256'])))
