...
def main():
    old=NEW/'independent_plant_clock_saved_actual_v1'
    ...
    assert len(source_map)==17 and review['source_sha256']==source_map
    assert review['required_clock_request_contract']=='pending_BUSY_same_job_max10_before_original_activation'
    diff=''
    for name in HELPERS:
    ...
    report=dict(passed=True,preparation_only=True,helper_sha256={name:sha(BASE/name) for name in HELPERS},
        ...
        changes=['Frozen retry-aware source_draft_v2 and its exact 17-file root review replace old one-publication v3 source.',
