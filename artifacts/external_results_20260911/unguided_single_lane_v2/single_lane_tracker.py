...
class SingleLaneNative23Tracker(Native23Tracker):
    ...
    def advance(self,x,u):
        ...
        active=(np.flatnonzero(self._rollout_valid) if self.feasibility is not None
                else np.arange(len(self._state)))
        if self.feasibility is not None and self._rollout_knot==0:
            # Defer the rollout clock override until it is consumed by a step.
            self._state[active,self._override_slices['TIME']]=0.0
        ...
        for name,value in (('QPOS',x[:,:self.nq]),('QVEL',x[:,self.nq:]),('CTRL',u),('WARMSTART',0.0)):
            # A rejected lane is not stepped; its public inputs may be assigned
            # by frozen advance, but authoritative integration writes stay deferred.
            self._state[active,self._override_slices[name]]=(value[active] if isinstance(value,np.ndarray) else value)
