...
def report_fixture():
    counts=dict(training=expected(30000,146860000),diagnostics={name:expected(1437,367570) for name in
    ...
    return dict(completed=True,optimization_completed=True,final_export_diagnostics_completed=True,numerical_gate_passed=True,
        ...
        condition='causal',ordinary_start_step=71000,ordinary_final_step=81000,additional_updates=10000,
        optimizer_start_step=6000,optimizer_step=16000,fresh_optimizer=False,features=1323,context_features=323,
        ...
        checkpoint_selection=False,native_steps=0,BFM_calls=0,training_first_layer_execution='split_old256_new256_original1000_plus323',hidden_width=512,expansion_seed=20260912,architecture=[1323,512,512,23],
