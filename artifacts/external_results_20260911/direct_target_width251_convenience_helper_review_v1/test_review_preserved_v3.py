...
class LauncherReview(unittest.TestCase):
    ...
    def test_new_dependency_and_exact_boot_permit_one_fake_call(self):
        ...
        self.assertTrue(end['accounting_passed']);self.assertTrue(end['raw_exit_known']);self.assertEqual(end['raw_python_exit_code'],0)
    ...
    def test_corrected_postread_error_preserves_known_exit(self):
        ...
        self.assertTrue(end['raw_exit_known']);self.assertTrue(end['child_reaped']);self.assertEqual(end['raw_python_exit_code'],0)
    ...
    def test_report_hash_error_preserves_known_exit(self):
        ...
        self.assertEqual(end['raw_python_exit_code'],0);self.assertTrue(end['raw_exit_known']);self.assertFalse(end['accounting_passed'])
    ...
    def test_interrupted_wait_never_claims_return(self):
        ...
        self.assertFalse(end['raw_exit_known']);self.assertFalse(end['child_reaped']);self.assertIsNone(end['raw_python_exit_code'])
