...
RUN=NEW/'direct_target_causal_response_evaluation_v1'
...
assert sha(prep_path)=='f92270c7e6a61eeec05c496860ce0f40ddf36b79ee9a4cb53644c3df2e72e148'
prep=read(prep_path)
assert prep['source_preparation_pass'] is True and prep['tests_run']==56
assert prep['failures']==prep['errors']==prep['skips']==0
assert len(prep['source_sha256'])==15
...
runtime_review=NEW/'direct_target_causal_response_evaluation_independent_review_v1/review.json'
assert sha(runtime_review)=='e40aca20c419219a8e6d1e4630ef5e4442ac7c1f492d85b009bc94b1460a0121'
...
assert sha(stage)=='29d74e990eca9aa861459ffb337bb140dfdafd338db1838e42f995d8494ff0d4'
...
assert result.wasSuccessful() and result.testsRun==56 and not result.skipped,stream.getvalue()
for n,h in prep['source_sha256'].items():assert sha(BASE/n)==h,n
report=dict(passed=True,source_review_pass=True,helper_review_pass=True,reviewer='root',reviewed_utc=datetime.now(timezone.utc).isoformat(),
 ...
 original_sources_exact=True,common_math_AST_identical=True,actual_runtime_sources_exact=37,
 segment_context_fixed_map_files_byte_identical=True,synthetic_tests=56,failures=0,errors=0,skips=0,
 ...
 'Sixteen actual71000 warm release roles; source68000, optimizer3000 to6000, unchanged coefficient/context; distinct producer and energy rule labels.',
...
print(json.dumps(dict(path=path.as_posix(),sha256=sha(path),tests=56,passed=True)))
