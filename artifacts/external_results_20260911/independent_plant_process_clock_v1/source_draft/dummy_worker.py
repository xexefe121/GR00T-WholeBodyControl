...
class WorkerLifecycle:
    ...
    def __init__(self,process,ready,stop):self.process,self.ready,self.stop=process,ready,stop;self.records=[];self.started=False
    def start_ready(self):
        if self.started:raise ValueError('no worker restart')
        self.started=True;self.process.start();self.records.append(dict(event='STARTED',pid=self.process.pid))
        if not self.ready.wait(10) or not self.process.is_alive():raise RuntimeError('worker failed pre-epoch readiness')
        self.records.append(dict(event='READY',pid=self.process.pid))
    def stop_join(self):
        if not self.started:raise ValueError('worker not started')
        self.stop.set();self.process.join(5);terminated=False
        ...
        if not live:self.process.close()
