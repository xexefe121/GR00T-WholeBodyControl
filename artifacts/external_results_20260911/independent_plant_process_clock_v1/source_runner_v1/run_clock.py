...
import numpy as np
from packet_io import local,sha,read,write,lossless,ready,pin_check,save_owned,worker_verdict
...
def imported_source_paths(pins):
    ...
    for name in ('packet_io','counted_api','native_loader','model_identity','native_stepper',
                 ...
                 'recorded_protocol','evidence','capture_schema','bfm_observations','oracle_source'):
...
def run(request,request_sha,precheck,output,request_path):
...
def main():
    ...
    args=parser.parse_args();request,request_sha,checks=ready(args.request,args.clearance)
    return run(request,request_sha,checks,local(request['output_directory']),local(args.request))
