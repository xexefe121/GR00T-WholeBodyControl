...
def arguments(stage):
    ...
    return ['-d','Ubuntu-22.04','--cd','/','--','bash',linux(template.BOOT),
        ...
        'PYTHONDONTWRITEBYTECODE=1','PYTHONPATH='+linux(BASE/'source_runner_v1'),template.PY,'-B',
        linux(BASE/'source_runner_v1/run_clock.py'),'--request',linux(BASE/'clock_request.json'),
...
def main():
    ...
    if args.final_review:
        ...
        if (folder/'started.lock').exists() or (BASE/'run').exists():raise ValueError('preserve prior attempt')
    ...
    if folder.exists() or (BASE/'run').exists():raise ValueError('fresh stage output required')
    request=read(request_path);checks=pin_check(request['input_files'])
