...
def protocol(a,metadata,jobs,published_list,worker,request,full,hold,full_sha,hold_sha,reference_sha,contract):
    ...
    for event in events:
        ...
        if reason=='JOB_PUBLICATION':
            ...
            if len(matches)!=1 or matches[0]['status']!=event['status']:raise AssertionError('Job mailbox/publication ledger differs')
    ...
    if set(publication_events)!=set(identities) and metadata.get('event_ledger_overflow') is None and metadata['summary']['foundation']['failure'] is None:
        raise AssertionError('Unexplained missing publication evidence')
    if published_seen!=set(published_list) and metadata.get('event_ledger_overflow') is None:raise AssertionError('Published set lacks matching status')
    ...
    return dict(issued_jobs=len(identities),published_jobs=len(published_list),admitted_results=len(sealed),
        rejected_results=rejections,held_controls=held_events,worker_recorded_reply_mismatches=mismatches,
        worker_reply_bytes_exact=not mismatches,recorded_schedule_sha256=schedule,
        complete_protocol_coverage=len(identities)==1818 and len(published_list)==1818 and len(sealed)==1818 and not rejections and not held_events and not mismatches)
