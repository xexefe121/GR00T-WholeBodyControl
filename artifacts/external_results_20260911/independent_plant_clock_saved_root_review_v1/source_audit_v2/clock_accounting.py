...
def counts(a,metadata,report,capsules):
    ...
    if api['step_attempted']!=native['attempted'] or api['step_returned']!=native['returned']:raise AssertionError('Actual native API counters differ')
    ...
    for _ in range(core['returned']):expected+=.002
    ...
    return {'foundation':{k:core[k] for k in ('attempted','returned','captured','verified','committed','unexecuted')},
