...
def array(value, dtype, shape, name, finite=True):
...
def physical_arrays(a, initial, contract):
    ...
    packed = array(a['packed_capture'], np.float64, (n, 373), 'Packed373',finite=False)
    ...
    for key, width in schema.items():
        array(a[key], np.float64, (n, width), key,finite=key in ('step_command','step_target'))
    ...
    force_finite=np.isfinite(a['step_actuator_force']).all(axis=1)
    effort = (np.abs(a['step_actuator_force'])/contract['native_effort']).max(axis=1)
    tilt = np.arccos(np.clip(1-2*(q[:,4]**2+q[:,5]**2),-1,1))
    state_finite=np.isfinite(q).all(axis=1)&np.isfinite(v).all(axis=1)
    excess=np.where(state_finite,excess,np.inf);speed=np.where(state_finite,speed,np.inf);tilt=np.where(state_finite,tilt,np.inf)
    bad = dict(nonfinite_state=~state_finite,joint_bound=excess>1e-6, joint_speed=speed>1., effort=(~force_finite)|(effort>1+1e-9),
        quaternion=state_finite&(np.abs(np.linalg.norm(q[:,3:7],axis=1)-1)>1e-10),
    ...
    if failed[:-1].any():raise AssertionError('Native stepping continued after a strict failed capture')
...
def initial_strict(initial,warning_counts,contract):
    """Exact original step-zero distinction: force values do not enter its oracle."""
    array(initial,np.float64,(373,),'Initial373',finite=False)
    q,v=initial[291:321],initial[321:350];limits=contract['joint_limits']
    if not np.isfinite(initial[0]) or not np.isfinite(q).all() or not np.isfinite(v).all():raise AssertionError('Initial finite state/clock')
    tilt=np.arccos(np.clip(1-2*(q[4]**2+q[5]**2),-1.,1.))
    if np.any(initial[112:291]) or np.any(warning_counts) or np.max(np.maximum(np.maximum(limits[:,0]-q[7:],q[7:]-limits[:,1]),0.))>1e-6 or np.max(np.abs(v[6:])/contract['native_velocity'])>1. or abs(np.linalg.norm(q[3:7])-1)>1e-10 or q[2]<.25 or tilt>1.2:
        raise AssertionError('Initial canonical strict state/force-assistance gate')
    return {'original_step0_force_exception_preserved':True,'initial_state_passed':True}


def timing_arrays(a, outer_records, epoch_ns, returned_steps=None):
    ...
    outer_indices, outer_misses, uncommitted_cycles = [], [], []
    returned_steps=n if returned_steps is None else returned_steps
    if type(returned_steps) is not int or not n<=returned_steps<=n+1:
        raise AssertionError('At most one final uncommitted returned native step')
    for record in outer_records:
        ...
        nominal_end=epoch_ns+(i+1)*DT_NS
        if record['fixed_nominal_end_ns'] != nominal_end:
            raise AssertionError('Outer original deadline/counter')
        if i==n:
            if n==TOTAL_STEPS or record['returned']!=returned_steps or record is not outer_records[-1]:
                raise AssertionError('Only final outer cycle may lack committed capture')
            # poll_results precedes the first wait: a failed input/logger call
            # can return before epoch without any attempted physics.
            minimum=0 if n==0 else int(actual_end[-1]);uncommitted_cycles.append(i)
        else:
            if record['returned']!=i+1:raise AssertionError('Outer actual return counter')
            minimum=int(actual_end[i])
        if type(record['cycle_return_ns']) is not int or record['cycle_return_ns'] < minimum:
            raise AssertionError('Outer time precedes verified native return')
        if record['cycle_return_ns'] > nominal_end: outer_misses.append(i)
    ...
    return dict(captured_step_deadline_misses=np.flatnonzero(actual_end>ends).tolist(),
        ...
        final_uncommitted_cycle_indices=uncommitted_cycles,
        complete_outer_coverage=len(outer_indices)==n and not uncommitted_cycles,
        fixed_epoch_timing_pass=n==TOTAL_STEPS and len(outer_indices)==n and not uncommitted_cycles and not np.any(actual_end>ends) and not outer_misses)
...
def compare_expert_prefix(a, full, hold):
    ...
    expected_time=np.concatenate([z['physics_time'][1:] for z in (full,hold)])
    fields['step_time']=first_difference(a['step_integration'][:,0],expected_time[:n])
    c=len(a.get('control_integration',()))
    if c:
        expected=np.concatenate([z['control_integration_before'] for z in (full,hold)])
        fields['control_full291']=first_difference(a['control_integration'],expected[:c])
        for key,source in [('control_incoming_raw','previous_action'),('control_flat_history_before','control_history_before')]:
            expected=np.concatenate([z[source] for z in (full,hold)])
            fields[key]=first_difference(a[key],expected[:c])
    if n==15690:fields['main_final_full291']=0 if a['step_integration'][-1].tobytes()!=full['final_integration'].tobytes() else None
    if n==18190:fields['hold_final_full291']=0 if a['step_integration'][-1].tobytes()!=hold['final_integration'].tobytes() else None
