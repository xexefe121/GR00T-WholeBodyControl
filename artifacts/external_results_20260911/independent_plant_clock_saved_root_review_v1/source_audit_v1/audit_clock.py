...
def run_audit(request,roles,proof):
    ...
    if report['session'] is None:
        ...
        return dict(evidence_integrity_passed=True,physical_pass=False,timing_pass=False,command_pass=False,component_qualified=False,
    ...
    if not np.isfinite(initial).all() or np.any(initial[112:291]) or np.any(np.asarray(warnings['counts'])) or np.max(np.maximum(np.maximum(limits[:,0]-q[7:],q[7:]-limits[:,1]),0.))>1e-6 or np.max(np.abs(v[6:])/contract['native_velocity'])>1. or abs(np.linalg.norm(q[3:7])-1)>1e-10 or q[2]<.25 or tilt>1.2:
        raise AssertionError('Initial canonical state strict gate differs (original step0 force exception retained)')
    ...
    if type(epoch) is not int or epoch-chosen!=200000000 or finished<chosen or report['epoch_rebased'] is not False:raise AssertionError('One fixed epoch construction differs')
    ...
    if len(a['control_control']) and not chosen<=int(a['control_admitted_ns'][0])<=finished:
    ...
    protocol_result=protocol.protocol(a,metadata,jobs,published,worker,run_request,full,hold,sha(roles['expert_main']),sha(roles['expert_hold']),sha(roles['reference']),contract)
    ...
    timing_pass=timing_result['fixed_epoch_timing_pass'] and setup_timely
    ...
    return dict(evidence_integrity_passed=True,physical_pass=physical_pass,timing_pass=timing_pass,command_pass=command_pass,
        ...
        physical=physical_result,counters=counter_result,timing=timing_result,controls=control_result,reserved_control=reserved_result,reserved_capture=reserved_capture,protocol=protocol_result,
