...
for path in paths:
    z=np.load(path);q0=z['quu'];q=.5*(q0+q0.T);rhs=z['B'].T@z['next_value']@z['A']
    ...
    row=dict(knot=int(z['knot']),source_sha256=sha(path),source_name=path.name,
        ...
        archived_state_regularized_norm_inf=float(np.linalg.norm(z['regularized_quu'],np.inf)),
    ...
    try:
        ...
        arrays[f'knot{int(z["knot"]):02d}_Qsym']=q
        arrays[f'knot{int(z["knot"]):02d}_Qreg']=reg
        arrays[f'knot{int(z["knot"]):02d}_Qux']=rhs
        arrays[f'knot{int(z["knot"]):02d}_all_control_linear_solution']=sol
