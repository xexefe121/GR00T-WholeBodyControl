...
SOURCE=Path('E:/codex-artifacts/sonic23_teleop_resume_20260911/direct_target_width512_expert_recovery_v1/verify_recovery_completed_v2.py')
...
class OwnerReceipts(unittest.TestCase):
    def setUp(self):
        ...
        self.base=Path(self.temp.name);self.process=self.base/'recovery_process_v2';self.process.mkdir()
        self.write('run_recovery_durable_v2.ps1','synthetic launcher only',raw=True)
        ...
        receipt=dict(request_sha256=self.sha('execution_request.json'),frozen_receipt_sha256=self.sha('frozen_inputs.json'),
                     launcher_sha256=self.sha('run_recovery_durable_v2.ps1'),wsl_arguments=args,input_sha256={})
        self.write('launch_receipt_v2.json',receipt)
        review=dict(passed=True,request_sha256=receipt['request_sha256'],frozen_receipt_sha256=receipt['frozen_receipt_sha256'],
                    launch_receipt_sha256=self.sha('launch_receipt_v2.json'))
        ...
        self.write('execution_clearance_v2.json',clearance)
        start=dict(wrapper_pid=111,request_sha256=receipt['request_sha256'],frozen_receipt_sha256=receipt['frozen_receipt_sha256'],
                   launch_receipt_sha256=self.sha('launch_receipt_v2.json'),clearance_sha256=self.sha('execution_clearance_v2.json'),arguments=args)
        self.write('recovery_process_v2/start.json',start)
        self.write('recovery_process_v2/child.json',dict(wrapper_pid=111,child_pid=222,handle_acquired=True,
            command_arguments=' '.join('"'+x+'"' for x in args)))
        self.write('recovery_process_v2/linux_process.json',dict(linux_pid=333,process_identity='bash exec replaced by selected Python'))
        self.write('recovery_process_v2/dispatch.json',dict(wrapper_pid=111,clearance_sha256=self.sha('execution_clearance_v2.json'),
            launcher_sha256=receipt['launcher_sha256'],arguments=['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',
                (self.base/'run_recovery_durable_v2.ps1').as_posix(),'-ClearanceSha256',self.sha('execution_clearance_v2.json')]))
        self.write('recovery_process_v2/process_absence.json',dict(windows_expected_pids=[111,222],linux_expected_pids=[333],
                                                                 windows_all_absent=True,linux_all_absent=True))
        self.write('recovery_process_v2/exit.json',dict(wrapper_pid=111,child_pid=222,raw_exit_known=True,raw_python_exit_code=0,
            exit_code=0,error=None,all_postrun_pins_exact=True,final_work_counters_present=True,requested_branch_completed=True))
        pins={str(self.base/name):self.sha(name) for name in ('launch_receipt_v2.json','execution_clearance_v2.json','root_review.json')}
        pin_report=dict(all_exact=True,files={p:dict(expected=h,actual=h,matched=True) for p,h in pins.items()})
        self.write('recovery_process_v2/prerun_pins.json',pin_report);self.write('recovery_process_v2/postrun_pins.json',pin_report)
    ...
    def test_wrong_child_parent_rejected(self):
        self.mutate('recovery_process_v2/child.json','wrapper_pid',444)
    ...
    def test_missing_linux_identity_rejected_for_completed_child(self):
        ...
        self.mutate('recovery_process_v2/process_absence.json','linux_expected_pids',[])
        with self.assertRaises(AssertionError):owner.verify(self.base)

    def test_changed_actual_start_arguments_rejected(self):
        self.mutate('recovery_process_v2/start.json','arguments',['unselected'])
        with self.assertRaises(AssertionError):owner.verify(self.base)

    def test_raw_zero_wrapper_failure_not_complete_accounting(self):
        self.mutate('recovery_process_v2/exit.json','exit_code',1)
