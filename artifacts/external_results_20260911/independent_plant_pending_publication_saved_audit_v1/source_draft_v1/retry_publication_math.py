...
def check(identities,jobs,events,transports,summary,epoch,arrays,published,
          event_overflow=None,transport_overflow=None):
    ...
    last=summary['last_job_publication'];failed=summary['failure'] is not None
    ...
    attempted=returned=recorded=expired=transport_index=0;uncertain=[];previous_physics=-1;prior_transport_end=-1
    for event,committed in sequence:
        ...
        if event['reason']=='PENDING_JOB_EXPIRED':
            ...
            if now<deadline or physics<=pending['last_attempt_index']:raise AssertionError('Premature pending expiry')
        ...
        if status is None:uncertain.append(dict(activation=k,physics=physics,transport_status=None if transport is None else transport['status']))
        ...
        if status=='PUBLISHED':published_expected.add(k)
    ...
    return dict(attempted=attempted,returned=returned,recorded_events=recorded,expired=expired,
                ...
                fixed_deadline_checks_exact=True,published_set_exact=True,
