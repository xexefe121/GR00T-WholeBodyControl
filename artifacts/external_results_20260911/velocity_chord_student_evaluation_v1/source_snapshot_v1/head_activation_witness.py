...
def require_witness_ready(base):
    ...
    if not path.is_file():raise ValueError('Final70000 head witness is not bound; no call is permitted.')
    binding=read(path)
...
def main():
    ...
    try:
        ...
        report=dict(kind='one_separately_selected_WSL_final70000_head_activation_witness',pass_all=True,
            head_sha256=binding['head']['sha256'],witness_sha256=sha(output/'witness.npz'),
            ordinary_final_step=70000,control=250,source_frame=261,centers_row=2038,
