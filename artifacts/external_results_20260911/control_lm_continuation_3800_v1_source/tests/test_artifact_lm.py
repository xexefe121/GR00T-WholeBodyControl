"""Scope and delegation tests for the private continuation only."""
...
from gear_sonic.utils import g1_true23_mjbatch_restoration_lm as helper


class ScalarPlanner:
    feasibility = None
    T, nu, nx, nq, nv = 2, 1, 1, 1, 0
    lo, hi = np.array([-2.]), np.array([2.])

    def rollout(self, x0, us, gains=None):
        states = np.empty((3, 9, 1)); controls = np.empty((2, 9, 1))
        states[0] = x0
        for t in range(2):
            controls[t] = us[t]
        ...
        costs = ((states-1)**2).sum((0, 2)) + .1*(controls**2).sum((0, 2))
    ...
    def expand(self, states, targets):
        return 2*(states-1), np.full((3, 1, 1), 2.), .2*targets, np.full((2, 1, 1), .2)
...
def test_default_solver_bitexact_original():
    ...
    spec = importlib.util.spec_from_file_location('old_exact_solver', old/'g1_true23_mjbatch_ilqr_core_snapshot.py')
    original = importlib.util.module_from_spec(spec); spec.loader.exec_module(original)
    actual = core.ilqr(ScalarPlanner(), np.array([0.]), np.zeros((2, 1)), iters=5)
    expected = original.ilqr(ScalarPlanner(), np.array([0.]), np.zeros((2, 1)), iters=5)
    for a, b in zip(actual, expected):
        np.testing.assert_array_equal(a, b)


def test_override_cannot_affect_main_hard_solver():
    planner = ScalarPlanner(); planner.feasibility = object()
    with pytest.raises(ValueError, match='cannot alter the hard main solver'):
        core.ilqr(planner, np.array([0.]), np.zeros((2, 1)), backward_override=lambda *x: None)
...
def test_control_LM_uses_control_identity_and_unregularized_cross():
    ...
    d = (np.array([[[2.]]]), np.array([[[3.]]]), np.zeros((2, 1)),
         np.array([[[0.]], [[4.]]]), np.zeros((1, 1)), np.array([[[.5]]]),
         np.full((1, 1), -10.), np.full((1, 1), 10.), 1.)
...
def test_prior_success_returns_exact_tuple_without_third(monkeypatch, enabled):
    best = (1., 'restoration', object(), object())
    result = {'accepted': True, 'attempts': [], 'elapsed_ms': 0.}
    ...
    out = helper.restore_feasible_seed(None, None, None, None, None, np.zeros((30,23)), 3810,
                                       retry_control_lm=enabled)
...
def test_unavailable_or_nonfinite_guided_skips_third(monkeypatch, guided):
    def fake(*args, **kwargs):
        ...
        return None, {'accepted': False, 'attempts': [], 'elapsed_ms': 0.}, object()
    ...
    best, result, _ = helper.restore_feasible_seed(None, None, None, None, None, np.zeros((30,23)), 3810,
                                                  retry_control_lm=True)
...
def test_third_initial_iterate_is_guided_but_anchor_is_original(monkeypatch):
    warm, guided, solution = np.zeros((30, 23)), np.ones((30, 23)), np.full((30, 23), 2.)
    restorer = SimpleNamespace(original_targets=None, zero_rollout_feedback=False,
                               window=lambda start: None)
    restorer.rollout = lambda x,u: (np.zeros((31,9,59)), np.repeat(u[:,None],9,axis=1), np.ones(9))
    def old(*args, **kwargs):
        kwargs['save_proposal']('guided', guided)
        kwargs['save_proposal']('zero_feedback', np.full((30,23),3.))
        return None, {'accepted':False, 'attempts':[{'mode':'guided'},{'mode':'zero_feedback'}],
                      'elapsed_ms':0.}, restorer
    def solve(p,x,u,**kwargs):
        ...
        return None,solution,None,.1
    checked_states=np.full((31,9,59),4.)
    tracker=SimpleNamespace(rollout=lambda x,u:(checked_states,np.repeat(u[:,None],9,axis=1),np.full(9,8.)),
                            last_rollout_feasibility={'feasible':[True]*9})
    monkeypatch.setattr(helper,'original_restore',old); monkeypatch.setattr(helper,'ilqr',solve)
    ...
    best,result,_=helper.restore_feasible_seed(tracker,None,data,{},None,warm,3810,retry_control_lm=True)
    assert result['accepted'] and best[0]==8.
    np.testing.assert_array_equal(best[2],checked_states[:,0]); np.testing.assert_array_equal(best[3],solution)
    np.testing.assert_array_equal(warm,0.)
