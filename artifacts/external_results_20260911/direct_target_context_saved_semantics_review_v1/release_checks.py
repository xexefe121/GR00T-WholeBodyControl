"""Independent literal identities for causal68000 saved execution evidence."""
...
def binding_identity(b,check):
    ...
    check.require(b['ordinary_final_step']==68000 and b['release_kind']=='ordinary68000_matched_context_same_weight_fp64_export','68000 causal release kind')
...
def check_release(p,check):
    ...
    check.require(fit['completed'] is True and fit['numerical_gate_passed'] is True and fit['export_parity_passed'] is True and fit['ordinary_final_step']==68000,'actual completed68000 fit')
    ...
    for name in ('main','hold'):
        ...
        check.require(request['head_sha256']==expected['head'] and request['evaluation_binding_sha256']==sha(p['binding']) and request['ordinary_final_step']==68000 and request['context_condition']=='causal',name+' model activation subject')
