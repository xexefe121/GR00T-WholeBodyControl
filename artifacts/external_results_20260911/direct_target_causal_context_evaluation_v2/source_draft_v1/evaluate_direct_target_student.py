...
def main():
    ...
    assert fit['features']==1323 and fit['head_output']=='normalized_target'
    condition=evaluation_binding['context_condition'];context_mean=evaluation_binding['context_mean']
    runtime=DirectStudentRuntime.from_native(native,c,original,motion,original29,HEAD,evaluation_binding['span'],timeline,condition,context_mean)
    ...
    common_request=dict(kind=KIND,clip='walk003',motion_override=override,model_manifest=manifest,
        ...
        ordinary_final_step=fit.get('ordinary_final_step'),context_condition=condition,
        context_order='original1000_then_previous_action23_then_incoming_history300',
        evaluation_binding_sha256=evaluation_binding['binding_sha256'],
    ...
    def verify_generated_prefix250(trace):
        fields=('qpos','qvel','target','source_frame','global_control','controller_mode','joint_error','root_error',
            ...
            'physics_warning_counts','physics_warning_lastinfo','physics_substeps','range_excess','velocity_ratio','effort_ratio','clock_error',
            'control_integration_before','control_history_before','control_previous_action_before')
        expected_context=np.concatenate((baseline['previous_action'],baseline['history']),axis=1)
        if condition=='blinded':expected_context=np.broadcast_to(context_mean,expected_context.shape)
        expected_features=np.concatenate((baseline['features'][:,np.r_[0:52,75:1023]],expected_context),axis=1)
        pairs=[(key,np.asarray(trace[key])[:len(baseline[key])],expected_features if key=='features' else baseline[key]) for key in fields]
    ...
    def verify_actual_query250(proposed):
        ...
        expected_context=np.concatenate((labels['previous_action'][0],labels['history'][0]))
        if condition=='blinded':expected_context=context_mean
        expected_features=np.concatenate((labels['features'][0,np.r_[0:52,75:1023]],expected_context))
        pairs.append(('features1323',proposed['features'],expected_features))
