"""Observation-only proposal diagnostics around the unchanged runtime."""
...
class ProposalEvidence:
    ...
    def accepted_proposal(self,proposed):
        ...
        # Record the unchanged proposal and applied inverse, then independently
        # check that only the selected clipped components enter feedback.
        ...
        learned=250<=int(self.current['global_control'])<1269
        feedback,native_clip,mask=clipped_component_feedback(proposed['action'],raw,proposed['target'],self.contract,learned)
        for key,wanted in (('raw_combined_action',proposed['action']),('feedback_action',feedback),
                           ('native_target_clip_mask',native_clip),('feedback_clip_mask',mask)):
            if not exact(proposed[key],wanted):raise ValueError('Feedback diagnostic mismatch: '+key)
        if not exact(self.runtime.seed.previous_action,feedback):raise ValueError('Committed feedback differs from selected components.')
...
TRAILING_SHAPES=dict(qpos=(30,),qvel=(29,),target=(23,),state=(52,),history=(300,),
    previous_action=(23,),action=(23,),base_target=(23,),delta=(23,),features=(1069,),
...
TRAILING_SHAPES.update(raw_combined_action=(23,),feedback_action=(23,),native_target_clip_mask=(23,),feedback_clip_mask=(23,))
EMPTY_DTYPES={key:np.float32 for key in ('state','history','previous_action','action','delta','features',
    'actual_normalized_action','control_history_before','control_previous_action_before')}
EMPTY_DTYPES.update(raw_combined_action=np.float32,feedback_action=np.float32,native_target_clip_mask=np.bool_,feedback_clip_mask=np.bool_)
...
def trace_arrays(trace):
    ...
    for key in ('source_frame','global_control','controller_mode','state','history','previous_action','action',
                'base_target','delta','features','raw_proposal','actual_normalized_action','physics_substeps',
                'raw_combined_action','feedback_action','native_target_clip_mask','feedback_clip_mask'):
