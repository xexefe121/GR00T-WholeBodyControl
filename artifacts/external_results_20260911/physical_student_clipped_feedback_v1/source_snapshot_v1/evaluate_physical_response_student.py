"""One selected final75000 test of learned clipped-component feedback."""
import json
...
from feedback_parity import FeedbackParity,ORIGINAL_TRACE_SHA
...
def main():
    ...
    original_raw_path=evaluation_binding['original_raw_trace']
    assert sha(original_raw_path)==ORIGINAL_TRACE_SHA
    feedback_parity=FeedbackParity(archive(original_raw_path),c)
    ...
    common_request=dict(kind=KIND,clip='walk003',motion_override=override,model_manifest=manifest,
        controller='originalBFM yaw2 disabledresidual0..249; clipped-component feedback linearspan residual250..1268; originalBFM yaw4 terminal1269 onward',
        head_sha256=sha(HEAD),baseline250_trace_sha256=sha(BASE.parent/'original_bfm_entry250_v1/entry250/trace.npz'),
        ordinary_final_step=75000,evaluation_binding_sha256=evaluation_binding['binding_sha256'],
        proposal_diagnostics='raw position, raw combined action, applied normalized action, feedback action and both clip masks are retained separately',
        ...
        prior_action='rawBFM startup/terminal; learned native-clipped components use inverse applied target; all other float32 raw combined components unchanged',
        original_raw_trace_sha256=ORIGINAL_TRACE_SHA,reused_75000_witness_no_new_call=True,
        original_prefix265_native_steps_required=2650,first_changed_prior_control=265,first_changed_lag_action_control=266,
    ...
    def segment(dest,start,count,expected_time):
        ...
        for key in ('raw_combined_action','feedback_action','native_target_clip_mask','feedback_clip_mask'):trace[key]=[]
        ...
        for control in range(start,start+count):
            ...
            try:
                ...
                gate=feedback_parity.before(control,trace,trace['control_integration_before'][-1],data.qpos,data.qvel,
                    trace['control_history_before'][-1],runtime.seed.previous_action,runtime.seed.recorded_controls,
                    data.warning.number,data.warning.lastinfo,expected_time)
                if gate is not None:record_parity(*gate)
                ...
                gate=feedback_parity.after(control,proposed)
                if gate is not None:record_parity(*gate)
        ...
        result=dict(kind=KIND,clip='walk003',mujoco=mujoco.__version__,segment=request['segment'],requested_controls=count,completed_full_controls=complete,completed_controls=complete,
            ...
            raw_proposal_and_actual_normalized_action_recorded=True,actual_normalized_action_used_for_history='learned_native_clipped_components_only',
            raw_combined_action_retained_separately=True,unclipped_float32_feedback_preserved=True,
            initial_and_terminal_BFM_feedback_unchanged=True,
    ...
    (BASE/'pilot_outcome.json').write_text(json.dumps(dict(nominal=result,extension=extended,new_expert_query_launched=False,new_fit_launched=False,one_component_selective_feedback_test=True,ordinary_final_global_step=75000),indent=2,allow_nan=False)+'\n')
