"""One earlier H30 completion and one continuous2s BFM recovery witness."""
import json
...
def main():
    ...
    cases=[]
    for mode,control,horizon,prefix_count in [('3770_shift15_bfm15',3770,30,15),('3740_continuous_bfm2s',3740,100,0)]:
        ...
        for local in range(horizon):
            ...
            if local<prefix_count:
                target=source['final_warm_targets'][local]
        ...
        if control==3740 and steps>=300:
            golden=archive(RECOVERY/'3740_source_goals_yaw2.npz')
            for key in ('physics_qpos','physics_qvel','physics_torque'):
                np.testing.assert_array_equal(arrays[key][:len(golden[key])],golden[key])
        if control==3770 and steps>=150:
            golden=archive(BASE.parent/'tail_completion_3770_v1/hold_last_accepted_target.npz')
            for key,count in [('physics_qpos',151),('physics_qvel',151),('physics_torque',150)]:
                np.testing.assert_array_equal(arrays[key][:count],golden[key][:count])
        path=BASE/(mode+'.npz');np.savez_compressed(path,**arrays,initial_integration=vector,integration_state_spec=np.asarray(spec))
        result=dict(mode=mode,initial_control=control,requested_controls=horizon,retained_shifted_prefix=prefix_count,
            ...
            retained_or_golden_physics_prefix_bitexact=True)
        cases.append(result);print(json.dumps(result),flush=True)
    result=dict(kind='two_predeclared_earlier_BFM_recovery_witnesses',cases=cases,source_goal_unchanged=True,no_optimizer=True,
        ...
        hashes={str(p):sha256(p) for p in (Path(__file__),SOURCE/'trace.npz',SOURCE/'report.json',RECOVERY/'actual_3740_integration_state.npz',RECOVERY/'actual_3740_bfm_history.npz',REFERENCE,BUNDLE/'contract.json')})
