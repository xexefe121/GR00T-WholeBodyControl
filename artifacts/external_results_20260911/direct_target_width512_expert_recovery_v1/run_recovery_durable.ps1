...
$processRoot=Join-Path $runRoot 'recovery_process_v1'
...
$clearancePath=Join-Path $runRoot 'execution_clearance.json'
...
try{
    ...
    $arguments=($receipt.wsl_arguments | ForEach-Object {if($_ -match '["\r\n]'){throw 'Invalid fixed WSL argument.'};'"'+$_+'"'}) -join ' '
