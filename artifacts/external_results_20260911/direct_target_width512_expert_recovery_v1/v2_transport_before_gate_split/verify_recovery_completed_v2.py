...
def verify(base):
    ...
    receipt=read(base/'launch_receipt_v2.json');clearance=read(base/'execution_clearance.json');request=read(base/'execution_request.json')
    ...
    assert dispatch['arguments']==['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',
        str(base/'run_recovery_durable_v2.ps1').replace('\\','/'),'-ClearanceSha256',sha(base/'execution_clearance.json')]
    ...
    files=[base/'execution_request.json',base/'frozen_inputs.json',base/'launch_receipt_v2.json',base/'execution_clearance.json',process/'exit.json']
