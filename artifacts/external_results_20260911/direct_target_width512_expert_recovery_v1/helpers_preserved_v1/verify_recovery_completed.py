...
def verify(base):
    process=base/'recovery_process_v1'
    receipt=read(base/'launch_receipt.json');clearance=read(base/'execution_clearance.json');request=read(base/'execution_request.json')
    ...
    assert start['launch_receipt_sha256']==clearance['launch_receipt_sha256']==sha(base/'launch_receipt.json')
    assert start['clearance_sha256']==sha(base/'execution_clearance.json')
    ...
    assert dispatch['launcher_sha256']==receipt['launcher_sha256']==sha(base/'run_recovery_durable.ps1')
    assert dispatch['arguments']==['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',
        str(base/'run_recovery_durable.ps1').replace('\\','/'),'-ClearanceSha256',sha(base/'execution_clearance.json')]
    extra={str(base/'launch_receipt.json'):sha(base/'launch_receipt.json'),
           str(base/'execution_clearance.json'):sha(base/'execution_clearance.json'),
    ...
    files=[base/'execution_request.json',base/'frozen_inputs.json',base/'launch_receipt.json',base/'execution_clearance.json',process/'exit.json']
...
def main():
    ...
    with (args.base/'owner_completion.json').open('x') as f:json.dump(result,f,indent=2)
