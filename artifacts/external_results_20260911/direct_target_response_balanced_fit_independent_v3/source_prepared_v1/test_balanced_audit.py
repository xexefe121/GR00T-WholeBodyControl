...
def test_exact_single_release_roles():
    request=dict(subjects={key:dict(path='/synthetic/'+key) for key in ('coefficient_source','checkpoint','full_state_root_audit','full_state_root_owner','energy_source')},full_state_paths=dict(request='/synthetic/generation_request',report='/synthetic/generation_report'))
    roles=release_paths(Path('/synthetic/study'),request)
    assert set(roles)==set('fit_report checkpoint head normalization training_manifest training_request export_manifest coefficient source_checkpoint full_state_generation_request full_state_generation_report full_state_data_audit full_state_data_owner shared_manifest context_alignment energy_source'.split())
    assert len(roles)==16 and roles['checkpoint'].name=='student_head.pt' and roles['fit_report'].parent.name=='fit'
