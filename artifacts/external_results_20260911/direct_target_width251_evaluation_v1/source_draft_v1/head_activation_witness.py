...
def main():
    ...
    try:
        ...
        assert len(head.get_inputs())==1 and head.get_inputs()[0].name=='features' and head.get_inputs()[0].shape[-1]==1323
        ...
        np.savez_compressed(output/'witness.npz',features=features,normalized_target=normalized,
            ...
            context_condition=np.asarray(ready['context_condition']),actual_context=ready['center']['actual_context'],
        ...
        report=dict(kind='one_WSL_direct_absolute_target_activation_witness',pass_all=True,
            ...
            exact_extended_query250_features=True,context_condition=ready['context_condition'],
            features=1323,centers_row=2038,control=250,source_frame=261,
