...
class DirectStudentRuntime:
    ...
    @classmethod
    def from_native(cls,native,c,original,motion,original29,head_path,span,timeline,context_condition,context_mean):
        ...
        if len(head.get_inputs())!=1 or head.get_inputs()[0].name!='features' or head.get_inputs()[0].shape[-1]!=1323:
            raise ValueError('causal features1323 ONNX input required')
        ...
        result=cls(seed,head,CausalFeatures(DirectFeatures(motion,original29,c),seed,context_condition,context_mean),c,span,timeline,infer_base)
    ...
    def propose(self,control,qpos,qvel):
        ...
        if self.current_mode==1:
            ...
            x=np.zeros(1323,np.float32) if self.current_mode==2 else self.features(qpos,qvel,frame)
            raw=base+delta
