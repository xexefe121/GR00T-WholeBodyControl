...
@pytest.mark.parametrize('condition',[None,'','both','best','CAUSAL',0,False])
...
@pytest.mark.parametrize('condition',['blinded','causal'])
def test_exact_condition_path_and_all18_roles(tmp_path,condition):
    from freeze_final_package import actual_paths,SUBJECTS
    request={'subjects':{key:{'path':str(tmp_path/key)} for key in ('checkpoint','coefficient_source','full_state_root_audit','full_state_root_owner')},
    ...
    assert len(SUBJECTS)==18 and set(SUBJECTS)<=set(paths)
    assert paths['head']==tmp_path/'fit'/condition/'student_head.onnx'
    ...
    assert paths['fit_report']==paths[condition+'_fit_report']
...
def test_context_binding_keeps_scope_and_no_condition_default(condition):
    ...
    assert b['context_condition']==condition and b['architecture']==[1323,256,256,23] and b['ordinary_final_step']==68000
