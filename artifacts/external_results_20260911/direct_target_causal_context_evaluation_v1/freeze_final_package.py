"""Future explicitly selected context68000 release binding. Requires actual positive receipts first."""
import argparse,json,sys
from pathlib import Path
BASE=Path(__file__).resolve().parent;SOURCE=BASE/'source_draft_v1';NEW=BASE.parent
FIT=NEW/'direct_target_causal_context_study_v1'
...
def require_helper_review(spec):
    entry=role(spec);review=read(entry['path']);prep=read(BASE/'launch_helper_preparation_v1.json')
...
def require_condition(condition):
    if type(condition) is not str or condition not in ('blinded','causal'):
        raise ValueError('An explicitly selected blinded or causal condition is required.')
    return condition


def actual_paths(condition):
    condition=require_condition(condition)
    request=read(FIT/'training_request.json');dest=FIT/'fit'/condition;shared=FIT/'fit/shared'
    return dict(head=dest/'student_head.onnx',checkpoint=dest/'student_head.pt',fit_report=dest/'report.json',
        ...
        paired_report=FIT/'fit/paired_report.json',shared_manifest=shared/'output_manifest.json',
...
def base_binding(config,final,inventory):
    ...
    return dict(**final,reviews={name:role(config['reviews'][name]) for name in ('release','source')},
        ...
        onnx_dependencies=inventory['onnx_dependencies'],ordinary_final_step=68000,context_condition=condition,
        release_kind='ordinary68000_matched_context_same_weight_fp64_export',export_input_dtype='float32',export_output_dtype='float32',export_internal_dtype='float64',
        controller='direct_absolute_target_1323_causal_context_study',architecture=[1323,256,256,23],hidden_activation='ELU',head_output='normalized_target',
...
def main():
    ...
    for name in ('freeze_final_package.py','prepare_bound_launcher.py','diagnostic_verdict.py','verify_completed_stage.py',
                 'prepare_review_configuration.py','test_launch_helpers.py','test_release_helpers.py','launch_helper_tests_v2.xml',
                 'runtime_inventory.json','source_preparation.json','launch_helper_preparation_v1.json','powershell_path_normalization_v1.json'):
        add(pins,BASE/name)
    ...
    if args.mode=='evaluation':
        ...
        frozen=BASE/'frozen_inputs_v1.json';write_new(frozen,dict(kind='one_ordinary68000_context_canonical_evaluation',context_condition=b['context_condition'],
