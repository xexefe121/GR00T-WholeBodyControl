...
Use the existing 232 cases and the newly frozen 132 filtered-branch cases, preserving their fixed targets and horizons. No fresh policy or expert query is needed. Compare all native qpos/qvel/command/actual-force/time/warning-number/lastinfo samples and first-failure indices/reasons against their saved Python reference traces. Compare full metrics, initial and final integration, computed and checked counts, and actual caller state/warnings unchanged. Reuse the same private MjData across alternating successful/failed cases to expose stale ctrl/warmstart/warning state. Existing trace archives cover the seven sampled fields; they do not necessarily contain the full private 291-element final integration for every forecast. Use saved reference fields directly, and rerun the same bounded Python reference cases only where final-integration comparison or a discrepancy requires it. Such a rerun adds no new candidate, policy or parameter choice.
