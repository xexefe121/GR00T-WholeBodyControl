...
if($receipt.selected_single_saved_audit -ne $true -or $receipt.kind -ne 'saved_matched_context_pair_only' -or $receipt.request_sha256 -ne (Get-TaskSha (Join-Path $runRoot 'audit_request.json'))){throw 'Wrong selected saved audit.'}
...
try{
    ...
    $source=Join-Path $runRoot 'source_draft_v1'
    ...
    $arguments='-B -u "'+(Join-Path $source 'audit_saved_pair.py')+'" --request "'+(Join-Path $runRoot 'audit_request.json')+'" --request-sha256 '+$receipt.request_sha256+' --output "'+(Join-Path $runRoot 'results_v1')+'"'
