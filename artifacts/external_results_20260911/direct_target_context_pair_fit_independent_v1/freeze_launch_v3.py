...
def main(args):
    ...
    request=read(request_path);assert request['kind']=='saved_matched_context_pair_only'
    ...
    for p in [request_path,BASE/'run_audit_durable_v3.ps1',BASE/'prepare_audit_request_v3.py',BASE/'source_preparation_v3.json',BASE/'verify_completion_v3.py',Path(request['python_path']),Path(__file__)]:pins[p.resolve().as_posix()]=sha(p)
    for p,d in pins.items():assert sha(p)==d
    receipt=dict(selected_single_saved_audit=True,kind='saved_matched_context_pair_only',request_sha256=args.request_sha256,
