...
def run(request_path,request_sha,output):
    ...
    try:
        ...
        start=read(bind(base/'fit_process/start.json'));child=read(bind(base/'fit_process/child.json'));exit_record=read(bind(base/'fit_process/exit.json'))
        ...
        for filename in ('prerun_pins.json','postrun_pins.json'):
            p=read(bind(base/'fit_process'/filename));check(filename+'_all_exact',p['all_exact'] is True);actual={canonical(k):v for k,v in p['files'].items()};check(filename+'_membership',set(actual)==set(expected_pins))
        ...
        owner_path=base/'owner_completion_verification_v2.json';literal(ar['subjects']['owner_completion'],owner_path,'owner_completion')
        ...
        owner_expected=dict(training_request=training_sha,training_frozen_inputs=frozen_sha,frozen=frozen_sha,
            process_exit=sha(base/'fit_process/exit.json'),source_checkpoint=request['subjects']['checkpoint']['sha256'],
