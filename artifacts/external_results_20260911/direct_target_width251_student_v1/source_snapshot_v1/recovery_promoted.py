...
def from_checkpoint(saved,expected_step):
    if type(expected_step) is not int or not 81000<expected_step<=91000:raise ValueError("Selected ordinary endpoint required")
    if saved['kind']!='direct_absolute_native23_target' or saved['ordinary_final_step']!=expected_step:raise ValueError('Exact selected ordinary recovery endpoint required')
...
def export_onnx(model,path):
    ...
    graph=helper.make_graph(nodes,'selected_recovery_width512_context_weights_fp64_execution',[helper.make_tensor_value_info('features',TensorProto.FLOAT,[None,len(model.feature_mean)])],
