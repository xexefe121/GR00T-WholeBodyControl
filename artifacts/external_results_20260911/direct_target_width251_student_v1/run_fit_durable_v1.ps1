...
foreach($name in @('running.lock','start.json','child.json','exit.json','stdout.log','stderr.log','prerun_pins.json','postrun_pins.json','process_absence.json')){
...
try{
    ...
    $launchReceiptPath=Join-Path $runRoot 'launch_receipt.json'
    ...
    if($clear.approved -ne $true -or $clear.updates -ne 10000 -or $clear.ordinary_final_step -ne 91000 -or $clear.condition -ne 'causal' -or $clear.optimizer_start_step -ne 16000 -or $clear.optimizer_final_step -ne 26000 -or $clear.automatic_retry -ne $false){throw 'Selected direct fit scope differs.'}
    ...
    if($request.kind -ne 'qualified_width251_recovery_warm512_fit' -or $request.root_selected -ne $true -or $request.updates -ne 10000 -or $request.condition -ne 'causal' -or $request.fresh_optimizer -ne $false -or $request.ordinary_final_step -ne 91000 -or $request.coefficient -ne 1.8188207859141674d -or $request.recovery_coefficient -ne 0.2d -or $request.expansion_performed -ne $false -or $request.recovery_rows -ne 1018 -or $request.learning_rate_values.Count -ne 10000){throw 'Unexpected request.'}
    ...
    if($review.($clear.review_pass_field) -ne $true -or $review.training_request_sha256 -ne $clear.request_sha256 -or $review.frozen_receipt_sha256 -ne $clear.frozen_receipt_sha256 -or $review.launch_receipt_sha256 -ne $LaunchReceiptSha256 -or $review.launcher_sha256 -ne $clear.launcher_sha256){throw 'Review does not clear this exact request and receipt.'}
    ...
    $snapshot=Join-Path $runRoot 'source_snapshot_v1';$driver=Join-Path $snapshot 'train_recovery.py'
    ...
    Write-NewJson (Join-Path $processRoot 'start.json') ([ordered]@{
        kind='qualified_width251_recovery_warm512_fit';wrapper_pid=$PID;started_utc=[DateTime]::UtcNow.ToString('o');
        clearance_sha256=$ClearanceSha256;launch_receipt_sha256=$LaunchReceiptSha256;request_sha256=$clear.request_sha256;frozen_receipt_sha256=$clear.frozen_receipt_sha256;
        ...
        updates=10000;ordinary_start_step=81000;ordinary_final_step=91000;condition='causal';optimizer_start_step=16000;optimizer_final_step=26000;budgets=$request.budgets;automatic_retry=$false
    })
    ...
    if($report.condition -ne 'causal' -or $report.ordinary_start_step -ne 81000 -or $report.ordinary_final_step -ne 91000 -or $report.additional_updates -ne 10000 -or $report.optimizer_start_step -ne 16000 -or $report.optimizer_step -ne 26000 -or $report.fresh_optimizer -ne $false -or $report.features -ne 1323 -or $report.head_output -ne 'normalized_target'){throw 'Final warm continuation metadata differs.'}
    if($report.hidden_width -ne 512 -or $report.expansion_performed -ne $false -or $report.recovery_rows -ne 1018 -or $report.recovery_coefficient -ne 0.2d -or $report.training_first_layer_execution -ne 'split_old256_new256_original1000_plus323'){throw 'Width512 geometry differs.'}
    ...
    if($report.counts.training.rows_verified -ne 157040000 -or $report.counts.training.calls_verified -ne 40000 -or $report.counts.calibration_forward_calls -ne 0 -or $report.counts.calibration_gradient_calls -ne 0 -or $report.counts.BFM_calls -ne 0 -or $report.counts.native_calls -ne 0){throw 'Training counts differ.'}
    foreach($backend in @('initial_GPU32','final_GPU32','CPU64','GPU64','ORT64')){if($report.counts.diagnostics.$backend.rows_verified -ne 368588 -or $report.counts.diagnostics.$backend.calls_verified -ne 1441){throw "Incomplete backend counts: $backend"}}
    ...
    try{Write-NewJson (Join-Path $processRoot 'exit.json') ([ordered]@{
        kind='qualified_width251_recovery_warm512_fit_exit';ended_utc=[DateTime]::UtcNow.ToString('o');wrapper_pid=$PID;
        child_pid=$(if($null -ne $child){$child.Id}else{$null});child_started=$started;raw_python_exit_code=$rawExit;
        exit_code=$runExit;exit_known=($null -ne $rawExit);error=$errorText;clearance_sha256=$ClearanceSha256;launch_receipt_sha256=$LaunchReceiptSha256;
