...
def run(args):
    ...
    if args.single_policy:
        ...
        actor=SinglePolicy(weights,env.c,neutral,env.expert_features.mean(0).cpu().numpy(),
            env.expert_features.std(0).cpu().numpy()).cuda()
    else:
    ...
    optimizer=torch.optim.Adam([{'params':actor.parameters(),'lr':args.actor_lr},{'params':critic.parameters(),'lr':3e-4}])
    ...
    request=dict(kind='full_range_causal_native23_dynamics_ppo',initial_checkpoint=str(args.initial),
        ...
        initial_actor_learning_rate=args.actor_lr,
