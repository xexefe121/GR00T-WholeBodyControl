...
class FocusedDirectBodyNative23Env(DirectBodyNative23Env):
    def __init__(self,bundle,bank,count=64,focus_clip='walk002',settle_objective=False,**kwargs):
        ...
        self.settle_objective=bool(settle_objective)
    ...
    def reset(self,ids,canonical=False):
        ...
        # 10% lifecycle starts; 75% real moving expert states; 15% braking or
        ...
        if not canonical:
            roles=torch.randint(20,(n,),device=self.device)
            moving_stop=12 if self.settle_objective else 17
            moving=ids[(roles>=2)&(roles<moving_stop)]
            ...
            recovery=ids[roles>=moving_stop]
