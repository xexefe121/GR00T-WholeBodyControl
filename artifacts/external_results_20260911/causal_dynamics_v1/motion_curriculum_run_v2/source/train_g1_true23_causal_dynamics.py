...
def validate_initialization(checkpoint,args,input_width,*,initial_only=False):
    ...
    if bool(metadata.get('task_closure_stand',False))!=args.task_closure_stand:
        raise ValueError('checkpoint task-closure semantics differ; explicitly convert an initialization-only checkpoint first')
...
def generalized_advantages(reward,value,done,timeout_value,next_value,gamma=.995,lam=.95):
    """Time limits bootstrap the actual final state; resets never share GAE."""
    advantage=torch.zeros_like(reward);carry=torch.zeros_like(next_value)
    for t in range(len(reward)-1,-1,-1):
        alive=1-done[t]
        delta=reward[t]+gamma*(next_value*alive+timeout_value[t])-value[t]
        carry=delta+gamma*lam*alive*carry;advantage[t]=carry;next_value=value[t]
    return advantage
...
def run(args):
    ...
    env_options={}
    if args.direct_body_goal:env_options['task_closure_stand']=args.task_closure_stand
    if args.focus_clip:
    ...
    request=dict(kind='full_range_causal_native23_dynamics_ppo',initial_checkpoint=str(args.initial),
        ...
        task_closure_stand=args.task_closure_stand,
        motion_curriculum=args.motion_curriculum,moving_noise_rad=args.moving_noise_rad,
        standing_noise_rad=.03,checkpoint_interval=args.checkpoint_interval,
    ...
    if args.motion_curriculum:
        request.update(reset_schedule='fixed75pct source,12.5pct transitions,12.5pct standing environments',
            curriculum_seconds=[2,4,8,'full source'],curriculum_fixed_evaluation_starts=64,
            curriculum_advance_fraction=.9,time_limit_bootstrap='actual final pre-reset observation',
    ...
    def save(label,update):
        ...
        np.savez(path.with_suffix('.normalization.npz'),span=cpu.span.numpy(),default=cpu.default.numpy(),limits=cpu.limits.numpy(),task_closure_stand=np.bool_(args.task_closure_stand))
    ...
    if args.wait_initial_evaluation:
        ...
        while not (args.output/'CONTINUE').exists():
            if (args.output/'STOP').exists():
                raise RuntimeError('initial evaluation stopped training before PPO')
    ...
    buffers={k:torch.empty(shape,device='cuda') for k,shape in dict(
        ...
        mean=(args.steps,args.num_envs,23),std=(args.steps,args.num_envs,23),logprob=(args.steps,args.num_envs),value=(args.steps,args.num_envs),
        reward=(args.steps,args.num_envs),done=(args.steps,args.num_envs),timeout_value=(args.steps,args.num_envs),policy_weight=(args.steps,args.num_envs)).items()}
        ...
        for update in range(1,args.updates+1):
            ...
            with torch.no_grad():
                for t in range(args.steps):
                    mean=actor(obs)
                    std=(env.noise_radians(args.moving_noise_rad)/actor.span if args.motion_curriculum else .03/actor.span)
                    ...
                    buffers['timeout_value'][t]=0
                    if 'final_observation' in info:
                        final=info['final_observation'];truncated=info['truncated']
                        buffers['timeout_value'][t,truncated]=critic((final[truncated,:1323]-actor.mean)/actor.scale).squeeze(-1)
                    weight=(info['motion_fraction']>0).float() if not args.single_policy and 'motion_fraction' in info else torch.ones_like(reward)
                ...
                advantage=generalized_advantages(buffers['reward'],buffers['value'],buffers['done'],buffers['timeout_value'],next_value,gamma,lam)
            ...
            metric=dict(update=update,global_update=args.prior_updates+update,elapsed_s=time.monotonic()-started,iteration_s=time.monotonic()-tick,
                ...
                curriculum_stage=env.stage if args.motion_curriculum else None,
            ...
            milestone=(update in (args.updates//2,args.updates) or
                       (args.checkpoint_interval and update%args.checkpoint_interval==0))
            if milestone:
                save(f'{update:05d}',update)
                if args.motion_curriculum:
                    evaluation=env.evaluate_segments(actor)
                    write(args.output/f'curriculum_{update:05d}.json',evaluation)
                    print(json.dumps({k:v for k,v in evaluation.items() if k!='cases'}),flush=True)
                    obs=env.observe()
            if (args.output/'STOP').exists() or time.monotonic()-started>args.wall_hours*3600:
                if not milestone:save(f'{update:05d}',update)
                break
...
def main():
    ...
    p.add_argument('--motion-curriculum',action='store_true')
    p.add_argument('--moving-noise-rad',type=float,default=.06)
    ...
    if args.task_closure_stand and not args.direct_body_goal:raise ValueError('--task-closure-stand requires --direct-body-goal')
    if args.motion_curriculum and not (args.focus_clip and args.task_closure_stand):raise ValueError('motion curriculum requires focused clip and task closure')
    if not 0<args.moving_noise_rad<=.1:raise ValueError('moving noise must be in (0,.1] radians')
    if args.checkpoint_interval<0:raise ValueError('checkpoint interval cannot be negative')
