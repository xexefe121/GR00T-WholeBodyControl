...
class CausalNative23Env:
    ...
    def step(self, targets):
        ...
        for _ in range(10):
            self.sim.data.ctrl.copy_(torch.clamp(self.kp*(targets-self.q[:,7:])-self.kd*self.v[:,6:],-self.effort,self.effort))
            self.sim.step()
        ...
        quiet=(r['root_velocity'].norm(dim=-1)<.02)&(r['joint_velocity'].abs().amax(-1)<.05)
        ...
        if getattr(self,'quota_ready',False):
            ...
            truncated=cutoff&~(self.bad|tracking_failed|complete)
        done=self.bad|tracking_failed|complete|truncated
        ...
        info=dict(failed=self.bad.clone(),complete=complete.clone(),age=self.age.clone(),
                  ...
                  terminated=self.bad|tracking_failed|complete,truncated=truncated,
