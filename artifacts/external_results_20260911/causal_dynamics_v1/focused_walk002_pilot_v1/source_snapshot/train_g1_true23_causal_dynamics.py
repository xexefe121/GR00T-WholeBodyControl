...
def run(args):
    ...
    if args.single_policy:
        ...
        actor=SinglePolicy(weights,env.c,neutral,env.expert_features[:,:1323].mean(0).cpu().numpy(),
            env.expert_features[:,:1323].std(0).cpu().numpy(),smooth_action_tau=args.smooth_action_tau).cuda()
    else:
    ...
    with (args.output/'metrics.jsonl').open('x') as log:
        for update in range(1,args.updates+1):
            tick=time.monotonic();failures=successes=0;age_sum=[];root=[];legs=[];active=[];source_motion=[];tracking_failures=0
            with torch.no_grad():
