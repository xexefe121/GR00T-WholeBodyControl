...
class SinglePolicy(nn.Module):
    def __init__(self,weights,contract,neutral_motion,feature_mean,feature_scale,*,smooth_action_tau=0.):
        super().__init__()
    ...
    def forward(self,features):
        ...
        if self.smooth_action_tau:
            ...
            raw=(baseline-self.limits[:,0])/self.span+self.range_adapter(hidden)
        ...
        logits=torch.log(fraction)-torch.log1p(-fraction)+self.range_adapter(hidden)
