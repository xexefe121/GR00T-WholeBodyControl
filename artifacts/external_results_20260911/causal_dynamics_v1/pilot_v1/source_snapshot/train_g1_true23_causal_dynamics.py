...
def run(args):
    ...
    env=CausalNative23Env(ROOT/'artifacts/teleop_six_hour_20260910/mjbatch_native23_inputs_v1',args.bank,args.num_envs)
    ...
    buffers={k:torch.empty(shape,device='cuda') for k,shape in dict(
        ...
        reward=(args.steps,args.num_envs),done=(args.steps,args.num_envs)).items()}
    rollout_started=time.monotonic();completed=0
    with (args.output/'metrics.jsonl').open('x') as log:
        for update in range(1,args.updates+1):
            tick=time.monotonic();failures=successes=0;age_sum=[];root=[];legs=[]
            with torch.no_grad():
            ...
            for epoch in range(4):
                for ix in torch.randperm(count,device='cuda').chunk(4):
                    ...
                    policy=-torch.minimum(ratio*adv[ix],ratio.clamp(.8,1.2)*adv[ix]).mean()
                    ...
                    with torch.no_grad():kl=float(.5*((actor(x)-flat['mean'][ix])/std).square().sum(-1).mean())
