...
def run(args):
    ...
    while done < args.controls:
        ...
        if best is None and args.restoration:
            ...
            restoration = dict(trigger="all standard seeds infeasible", attempts=[])
            modes = (False, True) if args.restoration_zero_feedback_retry else (False,)
            for zero_feedback in modes:
                ...
                attempt = dict(mode="zero_feedback" if zero_feedback else "guided",
                               contract=restorer.merit_contract(),
                ...
                if np.isfinite(proposal_costs[0]):
                    ...
                    restoration_initial = np.empty_like(initial_integration)
                    mujoco.mj_getState(native, data, restoration_initial, state_spec)
                    proposal_path = args.output / ("restoration_%05d_%s.npz" % (control, attempt["mode"]))
                    np.savez_compressed(
                        proposal_path, targets=proposal_targets, initial_integration=restoration_initial,
                        integration_state_spec=state_spec, original_warm_targets=warm,
                        previous_action=fresh.previous_action, recorded_controls=fresh.recorded_controls,
                        **{"history_" + key: value for key, value in fresh.history.data.items()},
                    )
                    attempt.update(final_merit=float(proposal_cost),
                                   nominal_feasibility=tracker.last_rollout_feasibility,
                                   proposal_file=proposal_path.name, proposal_sha256=sha256(proposal_path))
                ...
                attempt.update(accepted=best is not None,
                               elapsed_ms=(time.perf_counter() - attempt_tick) * 1000)
                restoration["attempts"].append(attempt)
                if best is not None:
                    break
        ...
        for local in range(min(5, args.controls - done)):
            ...
            for substep in range(10):
                ...
                substeps += 1
                if failure:
                    failure.update(control=3740 + done, substep=substeps)
                    break
    ...
    result = finite_json(dict(
        ...
        final_control=3740 + int(np.count_nonzero(np.asarray(trace["physics_substeps"]) == 10)),
        final_attempted_slot_boundary=3740 + done,
        final_source_seconds=(3740 - 350) * .02 + len(trace["physics_torque"]) * .002,
