...
for path in (fit/'report.json',checkpoint,head,BASE/'training_request.json',BASE/'training_frozen_inputs.json'):
    assert audit['input_sha256'][path.resolve().as_posix()]==sha(path),path
...
metric_names=('nominal_objective','full_velocity_objective','physical_objective','nominal_equal_cell_preclip_RMSE_rad','nominal_clipped_rows')
