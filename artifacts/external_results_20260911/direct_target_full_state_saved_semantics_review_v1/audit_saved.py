...
def segment(a,r,start,count,previous,history,goals,terms_fn,contract,span,check,cap=None):
    ...
    sub=a['physics_substeps'];check.require(sub.shape==(n,) and np.all((sub>=1)&(sub<=10)) and int(sub.sum())==steps,tag+'issued native substep count')
    ...
    for i,control in enumerate(range(start,start+n)):
        ...
        wanted_features=np.zeros(1000,np.float32) if phase(control)==2 else goals(q,dq,control+11)
        check.exact(wanted_features,a['features'][i],prefix+'features1000')
...
def main():
    ...
    try:
        ...
        check.exact(a['features'][:prefix],b['features'][:prefix,kept],'original BFM actual prefix features1000')
        if n>250:
            ...
            check.exact(a['features'][250],labels['features'][0,kept],'query250 pure features')
        ...
        j=np.arange(len(selected));teacher_features=labels['features'][j][:,kept];teacher_target=labels['expert_target'][j]
        ...
        maps=run_maps(a,selected,centers,c,p['map_core'],p['map_original'],output,check)
        ...
        report=dict(passed=True,evidence_audit_passed=True,actual_controls=n,moving_controls=len(selected),original_requested_controls=1569,
            original_lifecycle_completed=r['full_segment_completed'],hold_completed=hold['full_segment_completed'],release_subjects=release,
            ...
            limitations=['Saved semantic consistency and previously independent native replay do not qualify stability or real-time plant behavior.',
                ...
                'The direct1000 head omits action/history; history remains original BFM startup/terminal handoff bookkeeping.'])
