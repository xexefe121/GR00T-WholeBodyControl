...
from capture_checks import exact,original_warning,compare_sample,snapshot_execution_flags

def test_partial_snapshot_flags_separate_returned_verified_and_started():
    flags=snapshot_execution_flags([0,1,2],16,15,16)
    assert flags['snapshot_native_steps_completed'].tolist()==[10,6,0]
    assert flags['snapshot_native_steps_verified'].tolist()==[10,5,0]
    assert flags['snapshot_control_was_started'].tolist()==[True,True,False]
    assert flags['snapshot_control_fully_verified'].tolist()==[True,False,False]

def test_unclassified_native_attempt_keeps_attempt_count():
    flags=snapshot_execution_flags([0,1],10,10,11)
    assert flags['snapshot_native_steps_completed'].tolist()==[10,0]
    assert flags['snapshot_native_steps_attempted'].tolist()==[10,1]
    assert flags['snapshot_control_was_started'].tolist()==[True,True]
    assert flags['snapshot_control_fully_verified'].tolist()==[True,False]
