...
from audit_saved import identity,api_counts,serialization_records,process_accounting
...
class SavedAuditTests(unittest.TestCase):
    ...
    def test_process_owner_schema_without_exit_pids(self):
        pins={'source.py':'a'*64};absence=dict(wrapper_pid=10,child_pid=11,wrapper_absent=True,child_absent=True)
        values=dict(owner=dict(passed=True,owner_completion_accounting_passed=True,stage='replay',input_hashes=pins,process_absence=absence),
            launch=dict(stage='replay',input_hashes=pins),start=dict(wrapper_pid=10),child=dict(wrapper_pid=10,child_pid=11,handle_acquired=True),absence=absence,
            raw=dict(known=True,raw_error=None,raw_python_exit_code=0),verdict=dict(passed=True,raw_python_exit_code=0,diagnostic_exit_code=0),
            exit_receipt=dict(known=True,error=None,all_postrun_hashes_exact=True,exit_code=0,raw_python_exit_code=0,diagnostic_exit_code=0,automatic_retry=False),stage='replay')
        process_accounting(**values)
        for obj,key,new in [('raw','known',False),('child','wrapper_pid',12),('absence','child_absent',False),('exit_receipt','raw_python_exit_code',None),('launch','input_hashes',{})]:
            bad=copy.deepcopy(values);bad[obj][key]=new
            with self.assertRaises(AssertionError):process_accounting(**bad)
