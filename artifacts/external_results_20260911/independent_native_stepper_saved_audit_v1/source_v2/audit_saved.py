...
def process_accounting(owner,launch,start,child,absence,raw,verdict,exit_receipt,stage):
    """Match actual owner wrapper schema; PIDs live in start/child, not exit."""
    assert owner['passed'] is True and owner['owner_completion_accounting_passed'] is True
    assert owner['stage']==launch['stage']==stage
    assert owner['input_hashes']==launch['input_hashes']
    assert absence['wrapper_absent'] is True and absence['child_absent'] is True
    assert absence['wrapper_pid']==start['wrapper_pid']==child['wrapper_pid']
    assert absence['child_pid']==child['child_pid'] and child['handle_acquired'] is True
    ...
    assert exit_receipt['known'] is True and exit_receipt['error'] is None and exit_receipt['all_postrun_hashes_exact'] is True
    assert exit_receipt['exit_code']==exit_receipt['raw_python_exit_code']==exit_receipt['diagnostic_exit_code']==0
    assert exit_receipt['automatic_retry'] is False
...
def run(request_path,output):
    ...
    try:
        ...
        for phase,steps,serializations in [('witness',0,2),('replay',21348,8)]:
            ...
            launch_path=role(spec['launch_receipt']);launch=read(launch_path)
            assert owner['launch_receipt_sha256']==sha(launch_path)
            assert launch['request_sha256']==sha(producer_request_path)
            absence_path=role(spec['process_absence']);absence=read(absence_path)
            assert owner['process_absence_sha256']==sha(absence_path)
            output_pins={str(local(path).resolve()):digest for path,digest in owner['output_hashes'].items()}
            process_records={}
            for filename in ('start.json','child.json','raw_exit.json','diagnostic_verdict.json'):
                path=(exit_path.parent/filename).resolve();bind(path,output_pins[str(path)]);process_records[filename]=read(path)
            process_accounting(owner,launch,process_records['start.json'],process_records['child.json'],absence,
                process_records['raw_exit.json'],process_records['diagnostic_verdict.json'],exit_receipt,phase)
            assert process_records['start.json']['receipt_sha256']==sha(launch_path)
            assert process_records['start.json']['clearance_sha256']==sha(clearance_path)
            assert process_records['start.json']['review_sha256']==clearance['review']['sha256']
            ...
            for phase_name in ('pre_hashes','post_hashes'):
                ...
                assert output_pins[str(ledger_path)]==sha(ledger_path)
