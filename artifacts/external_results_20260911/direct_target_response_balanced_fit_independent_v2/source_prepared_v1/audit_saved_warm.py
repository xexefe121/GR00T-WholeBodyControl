"""Audit saved warm71000 evidence. No forwards, gradients, optimizers or physics."""
import argparse
import hashlib
import json
...
from audit_balanced_math import energy_weights,balanced_metrics,warm_initial_errors,ledger_expectations,PRODUCER_WEIGHT_RULE
...
def run(request_path,request_sha,output):
    ...
    try:
        ...
        check('audit_kind',ar['kind']=='saved_response_balanced_warm_only')
        import sys
        ...
        expected_budget=dict(training_forward_rows=44058000,training_forward_calls=9000,training_updates=3000,
        ...
        for key,value in [('centers',sc),('axes',sa)]:
            old=npy(request['context_paths']['schedule_'+key]);check('old_schedule_shape:'+key,old.shape==(3000,864));exact('reused_schedule:'+key,old,value);exact('shared_schedule:'+key,npy(shared/('schedule_'+key+'.npy')),value)
        ...
        check('source68000',source['ordinary_final_step']==68000 and request['subjects']['checkpoint']['sha256']=='10d238198e2d7826c35eaa9bd4a13ff2e05050a7f601c1f212b585c4a90eaefd')
        source_report=read(bind(request['subjects']['fit_report']['path']))
        check('qualified_source_report',source_report['completed'] is True and source_report['ordinary_final_step']==68000 and source_report['checkpoint_sha256']==request['subjects']['checkpoint']['sha256'])
        ...
        for condition in ('causal',):
            ...
            for key,value in dict(condition=condition,ordinary_final_step=71000,additional_updates=3000,optimizer_step=6000,fresh_optimizer=False,features=1323,context_features=323,head_output='normalized_target',fixed_full_state_coefficient=WEIGHT,execution_dtype='float64',public_input_dtype='float32',public_output_dtype='float32',training_first_layer_execution='split_contiguous_1000_plus_323',export_first_layer_execution='monolithic_float64_1323',parity_tolerance_rad=1e-5,all_frozen_inputs_unchanged=True,checkpoint_selection=False,native_steps=0,BFM_calls=0).items():compare(condition+'_report.'+key,report[key],value)
            for field,value in dict(ordinary_start_step=68000,optimizer_start_step=3000,context_and_normalization_reused=True,source_checkpoint_sha256=request['subjects']['checkpoint']['sha256'],energy_source_sha256=request['subjects']['energy_source']['sha256']).items():compare('report_extra.'+field,report[field],value)
            ...
            check('warm_initial_actor_optimizer_RNG',not warm_initial_errors(init,source))
            ...
            for key,value in dict(kind='direct_absolute_native23_target',condition=condition,ordinary_final_step=71000,additional_updates=3000,optimizer_step=6000,fresh_optimizer=False,context_order='previous_action23_then_incoming_history300',context_blinded=condition=='blinded',full_state_coefficient=WEIGHT,source_checkpoint_sha256=request['subjects']['checkpoint']['sha256']).items():compare(condition+'_checkpoint.'+key,checkpoint[key],value)
            ...
            for index,shape in enumerate(shapes):
                state=optimizer['state'][index];check(condition+'_optimizer_step:'+str(index),state['step'].dtype==torch.float32 and state['step'].shape==torch.Size([]) and float(state['step'])==6000)
            ...
            check(condition+'_optimization_receipt',optimization['optimization_completed'] is True and optimization['condition']==condition and optimization['ordinary_final_step']==71000 and optimization['optimizer_step']==6000 and optimization['checkpoint_sha256']==sha(folder/'student_head.pt') and optimization['export_validation_pending'] is True)
            ...
            compare(condition+'_restoration',restoration,dict(source_checkpoint_sha256=request['subjects']['checkpoint']['sha256'],actor_exact=True,learned_context_columns_preserved=True,normalization_exact=True,warm_optimizer_exact=True,optimizer_state_count=6,optimizer_start_step=3000,fresh_optimizer=False,RNG_exact=True,learning_rate_restart_after_restoration=[1e-5,1e-6],initial_drift=dict(passed=maximum<=1e-5,tolerance_rad=1e-5,max_preclip_error_rad=maximum,corpora=restored,source_ordinary_step=68000,changed_MatMul_dimension=False,stored_first_layer_width=1323,first_layer_execution='split_contiguous_1000_plus_323',byte_gate_required=False)))
