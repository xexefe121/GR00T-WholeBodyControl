...
def run(args):
    ...
    restoration_control_lm = bool(getattr(args, "restoration_control_lm_retry", False))
    if restoration_control_lm and not restoration_enabled:
        raise ValueError("third control-LM retry requires explicit guided/K0 restoration")
    ...
    if restoration_control_lm:
        inputs.append(utility / "g1_true23_mjbatch_restoration_lm.py")
    request = dict(
    ...
    if restoration_control_lm:
        request["restoration"]["third_control_lm_retry"] = dict(
            enabled=True, trigger="only after original guided and original-warm K0 attempts both fail",
            initial_iterate="ordinary final proposal of the first guided attempt",
            regularization_anchor="unchanged original shifted warm targets",
            rollout_feedback="K0 only inside the third private restoration attempt",
            ...
            acceptance="ordinary final proposal; strict nominal and full native-MjData 300-step oracle",
        )
        request["restoration"]["backward_and_alpha_schedule_unchanged"] = False
        request["restoration"]["original_two_attempts_unchanged"] = True
        request["restoration"]["policy"] += "; then one control-LM K0 retry from first guided final"
        request["hard_feasibility"]["fallback_policy"] = request["restoration"]["policy"]
    ...
    while completed < requested:
        ...
        if restoration_enabled:
            ...
            if not np.isfinite(cached_rollout[1]):
                ...
                def save_diagnostics(mode, arrays):
                    path = args.output / ("restoration_search_%05d_%s.npz" % (completed, mode))
                    np.savez_compressed(path, **arrays, control=np.asarray(completed),
                                        request_sha256=np.asarray(sha256(args.output / "request.json")))
                    return dict(searches_file=path.name, searches_sha256=sha256(path))

                restoration_options = ({"retry_control_lm": True, "save_diagnostics": save_diagnostics}
                                       if restoration_control_lm else {})
        ...
        if args.checkpoint_controls and completed and (completed >= next_checkpoint or failure):
            continuity.checkpoint(trace, plans, data, fresh_seed, warm_targets,
                                  expected_physics_time, completed, finite_json(failure))
            next_checkpoint = completed + args.checkpoint_controls
        if failure:
            break
    return continuity.finalize(trace, plans, data, fresh_seed, warm_targets,
                               expected_physics_time, completed, finite_json(failure),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--restoration-control-lm-retry", action="store_true",
                        help="Opt-in third private control-LM K0 attempt after guided/K0 restoration both fail")
    parser.add_argument(
        "--fd-epsilon",
        type=float,
        default=EPS,
