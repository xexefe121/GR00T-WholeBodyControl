...
$runRoot = 'E:\codex-artifacts\sonic23_teleop_resume_20260911\bfm_entry250_actual_oracle_v1'
foreach ($existing in @('process_status.json', 'stdout.log', 'stderr.log', 'initial_seed', 'nominal', 'post_lifecycle_hold_5s', 'initial_restore_preflight.json')) {
...
if ((Read-Sha256 $clearancePath) -ne $ClearanceSha256) { throw 'Expert branch clearance identity mismatch.' }
$clearance = Get-Content -LiteralPath $clearancePath -Raw | ConvertFrom-Json
if ($clearance.one_expert_branch_authorized -ne $true -or $clearance.requested_controls -ne 1569 -or $clearance.initial_global_control -ne 250) { throw 'Clearance does not authorize the bounded BFM250 expert transition.' }
foreach ($required in @('frozen_inputs.json', 'run_expert_durable.ps1')) {
...
foreach ($item in $clearance.bound_files) {
    if ((Read-Sha256 $item.path) -ne $item.sha256) { throw ('Reviewed expert branch input changed: ' + $item.path) }
}
$statePath = Join-Path $runRoot 'process_status.json'
$status = [ordered]@{ kind='one_original_BFM250_actual_expert_transition'; state='RUNNING'; powershell_pid=$PID; started_utc=[DateTime]::UtcNow.ToString('o'); requested_controls=1569; initial_global_control=250; BFM_prefix_controls=250; separate_hold_controls=250; learned_head_loaded=$false; physical_state_resets_after_initialization=0; hardware_authorized=$false; clearance_sha256=$ClearanceSha256 }
$status | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $statePath -Encoding UTF8
$runExit = -1
$runError = $null
try {
    & wsl.exe -d Ubuntu-22.04 --cd /mnt/z/codex/GR00T-WholeBodyControl-sonic-transfer-23dof -- bash /mnt/z/codex/GR00T-WholeBodyControl-sonic-transfer-23dof/artifacts/teleop_six_hour_20260910/RUN_PASSING_WALK_WSL.sh env OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 /mnt/e/codex_sonic_runtime/mjbatch323_20260910/venv/bin/python -u /mnt/e/codex-artifacts/sonic23_teleop_resume_20260911/bfm_entry250_actual_oracle_v1/source_snapshot_v1/run_bfm250_actual_oracle.py 1> (Join-Path $runRoot 'stdout.log') 2> (Join-Path $runRoot 'stderr.log')
