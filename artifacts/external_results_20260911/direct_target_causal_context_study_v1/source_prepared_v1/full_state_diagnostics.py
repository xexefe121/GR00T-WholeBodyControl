...
FEATURE_KEYS = ('features','full_state_features','physical_features')
...
def run_backend(label, model, session, data, dest, counters, active, ledger, outputs):
    ...
    for corpus,size,key in zip(CORPORA,SIZES,FEATURE_KEYS):
        features=data[key]
        if features.shape!=(size,1000) or features.dtype!=np.float32:raise ValueError('Fixed corpus input schema.')
