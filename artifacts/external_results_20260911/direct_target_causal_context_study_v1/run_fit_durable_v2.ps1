...
$processRoot=Join-Path $runRoot 'fit_process_v2'
...
try{
    ...
    if($request.kind -ne 'matched_causal_context_utility_study' -or $request.root_selected -ne $true -or $request.updates_per_condition -ne 3000 -or (@($request.conditions) -join ',') -ne 'blinded,causal' -or $request.coefficient -ne 1.8188207859141674d){throw 'Unexpected request.'}
