"""Future actual65000 selection/review configuration, no placeholder subjects."""
import argparse,json
from pathlib import Path
from freeze_final_package import BASE,SOURCE,item,role,read,sha,field,bound_file,has_hash,actual_paths,base_binding,write_new,require_helper_review
from export_release_gate import SUBJECTS,validate_release


def configuration(*,root_selected,release,root_audit,fit_owner,source_review,helper_review):
    if root_selected is not True:raise ValueError('parent has not selected this actual witness/canonical pipeline')
    paths=actual_paths();final={name:item(path) for name,path in paths.items()}
    config={'root_selected':True,'selected_witness_calls':1,'selected_main_controls':1569,'conditional_hold_controls':250,
...
def main():
    ...
    config=configuration(root_selected=args.root_selected,release=spec('release'),root_audit=spec('root_audit'),
