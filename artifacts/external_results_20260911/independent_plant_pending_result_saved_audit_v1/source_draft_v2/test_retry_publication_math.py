...
def fixture(statuses=('BUSY','PUBLISHED')):
    ...
    summary=dict(failure=None,returned=len(statuses),last_job_publication=last,pending_publication=last if statuses[-1]=='BUSY' else None,
    ...
    arrays=dict(step_actual_start=epoch+np.arange(len(statuses))*2000000,step_actual_end=epoch+np.arange(len(statuses))*2000000+1000000)
...
def test_ten_busy_attempts_then_expiry():
    ...
    f[4]['failure']={'reason':'stop at expiry before next native step'}
...
def test_event_overflow_retains_returned_success_without_committed_event():
    ...
    f[4]['returned']=1;f[6]={k:v[:1].copy() for k,v in f[6].items()}
...
def test_duplicate_tick_after_pending_expiry_rejected():
    ...
    f[4]['failure']={'reason':'stopped after expiry before native step'}
