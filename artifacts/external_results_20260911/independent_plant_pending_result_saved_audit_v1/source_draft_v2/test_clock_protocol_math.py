...
def small_case():
    ...
    identity=dict(binding=binding,sequence=0,snapshot_control=0,snapshot_physics=0,activation=1,window_id=windows[1],
        input_digest=digest(payload),history_digest=digest(a['control_flat_history_before'][0].tobytes()),schedule_digest=digest(encoded(commands[0])),created_ns=epoch+1,deadline_ns=epoch+20000000)
    ...
    metadata['summary']['foundation'].update(returned=1,job_publication_attempted=1,job_publication_returned=1,
    ...
    worker=make_worker(identity,job,commands[1],schedule,epoch)
...
class ProtocolTests(unittest.TestCase):
    def test_outer_interrupt_after_returned_publish_before_native_preserved(self):
        args=list(small_case());a,metadata=args[:2]
        a['step_actual_start']=np.zeros(0,np.int64);a['step_actual_end']=np.zeros(0,np.int64)
        metadata['foundation_events']=[];metadata['transport_records']=metadata['transport_records'][:1]
        f=metadata['summary']['foundation'];f['returned']=0;f['job_publication_events']=0
        f['last_job_publication']['event_recorded']=False;args[4]=None
        r=protocol(*args,terminal_interruption={'type':'StageTimeout','detail':'interrupted before native step'})
        self.assertEqual(r['publication_attempts']['successful_publications_without_committed_event'],[1])
        self.assertEqual(r['admitted_results'],0);self.assertFalse(r['complete_protocol_coverage'])
    ...
    def test_retry_then_sealed_uses_original_identity(self):
        ...
        f=metadata['summary']['foundation'];f.update(returned=3,job_publication_attempted=2,job_publication_returned=2,job_publication_events=2)
        ...
        args[4]=make_worker(result['identity'],args[2]['1'],result['command'],args[5]['command_table_sha256'],epoch,shift)
        r=protocol(*args)
    ...
    def test_worker_publication_must_match_taken_bytes(self):
        args=list(small_case());args[4]['events'][2]['payload_sha256']='0'*64
        with self.assertRaisesRegex(AssertionError,'transport'):protocol(*args)
