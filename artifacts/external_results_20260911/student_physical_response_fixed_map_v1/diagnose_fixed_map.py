"""All seventy actual learned controls against matching-clock saved committed maps; pure arrays."""
...
def paths():return dict(actual=NEW/'one_step_physical_student_evaluation_v1/nominal/trace.npz',
    ...
    prior_outcome_audit=NEW/'student_physical_response_saved_outcome_v1/report.json',
...
def freeze():
    ...
    assert sha(p['actual'])=='38428ae27055be7c2e771c5a23056c858f004ef38f260a0b45a57662d8d0aae3'
    ...
    write(BASE/'request.json',dict(source_sha256=sha(__file__),input_sha256={str(v):sha(v) for v in p.values()},
        paths={k:str(v) for k,v in p.items()},controls=list(range(250,320)),
        nominal_map_checks=70,actual_state_map_evaluations=70,model_inference_calls=0,physics_steps=0,optimizer_updates=0,new_queries=0,
        map_selection='Matching global control from qualified query250 branch: dataset2 centers2038..2107; planned state/target/gain and both clips fixed.'))
def main():
    ...
    report=dict(passed=True,request_sha256=sha(BASE/'request.json'),source_sha256=sha(__file__),rows=records,
        nominal_map_checks_exact=70,actual_state_map_evaluations=70,feedback_clip_rows=sum(bool(r['feedback_clipped_joints']) for r in records),
