...
for control in CONTROLS:
 ...
 d=x-reference;average_J=np.zeros_like(J)
 ...
 delta_difference=prediction-teacher_prediction;integrated_sum=sum(block_contributions.values())
 closure=float(np.max(np.abs(integrated_sum-delta_difference)));assert closure<2e-4,(control,closure)
 ...
 result=dict(control=control,teacher_row=row,mathematical_float64_vs_recorded_ONNX_max_rad=actual_max,
  ...
  integrated_sameclock_teacher_to_actual=dict(quadrature_points=64,closure_max_rad=closure,
