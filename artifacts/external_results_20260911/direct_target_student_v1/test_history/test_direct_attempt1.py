...
class ObjectiveTests(unittest.TestCase):
    def test_nominal_equal15_cell_loss(self):
        ...
        self.assertAlmostEqual(float(loss),float(np.mean(np.arange(1,16)**2)),places=5)
    ...
    def test_velocity_sign_axis_cell_reduction(self):
        ...
        np.testing.assert_array_equal(per.detach().numpy(),wanted);self.assertEqual(float(loss),float(wanted.mean()))
    ...
    def test_physical_fixed_requested_denominators_and_live_centers(self):
        ...
        self.assertAlmostEqual(float(loss),float(wanted.mean()),places=14);loss.backward();self.assertTrue((centers.grad!=0).any())
