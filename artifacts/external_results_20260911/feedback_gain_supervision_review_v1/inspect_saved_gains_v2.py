...
CONTRACT=ARRAYS.parent/'contract.json'
limits=np.asarray(json.loads(CONTRACT.read_text())['joint_limits'])
plans=json.loads((EXPERT/'nominal/plans.json').read_text())
rows=[]; files=[CONTRACT,CORE,SRC/'run_actual_student_oracle.py',EXPERT/'nominal/trace.npz',EXPERT/'nominal/plans.json',ARRAYS,Path(__file__),NEW/'expert_target_saturation_diagnostic_v1/report.json',NEW/'fast_controller_evidence_review_v1/README.md',NEW/'expert_resumed_root_qualification_v1/qualification.json',NEW/'fast_controller_aggregate_fit_v1/source_snapshot_v2/student_linear_runtime.py']
no_update=[]
for plan in plans:
