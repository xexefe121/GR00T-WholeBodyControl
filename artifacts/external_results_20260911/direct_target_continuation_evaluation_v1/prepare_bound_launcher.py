...
def wsl_arguments(base,mode):
    ...
    return ['-d','Ubuntu-22.04','--cd','/','--','bash','/mnt/z'+BOOTSTRAP[2:],'env',
        ...
        'PYTHONPATH='+linux+'/source_draft_v1','/mnt/e/codex_sonic_runtime/mjbatch323_20260910/venv/bin/python',
...
$reviewText = Get-Content -Raw -LiteralPath $clearance.review.path
if (-not $reviewText.Contains($clearance.launch_receipt_sha256)) { throw 'Final review does not bind this launch.' }
...
def main():
    ...
    if a.final_review:
        ...
        assert field(review,a.pass_field) is True and has_hash(review,sha(receipt)) and has_hash(review,sha(binding_path))
    ...
    write_json(folder/'launch_receipt.json',dict(kind='one_selected_direct_target_'+a.mode,
        binding_sha256=sha(binding_path),exact_wsl_arguments=wsl_arguments(BASE,a.mode),input_hashes=pins,
