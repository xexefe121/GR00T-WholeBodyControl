...
def require_model_ready(base,purpose):
    ...
    assert binding['ordinary_final_step']==55000
    assert binding['controller']=='direct_absolute_target_1000_applied_inverse_learned_prior'
    assert binding['architecture']==[1000,256,256,23]
    ...
    names=('head','checkpoint','fit_report','training_manifest','centers','query250_labels','contract')
    ...
    script='head_activation_witness.py' if purpose=='witness' else 'evaluate_direct_target_student.py'
    subjects=dict(fit=binding['fit_report']['sha256'],export=binding['head']['sha256'],source=sha(source/script))
    for role in ('dataset','fit','source','export'):
        entry=binding['reviews'][role];review=read(bound_file(entry))
        assert field(review,entry['pass_field']) is True,role
        expected=binding['training_dataset_sha256'] if role=='dataset' else [subjects[role]]
        assert expected and all(re.fullmatch('[0-9a-f]{64}',digest) and has_hash(review,digest) for digest in expected),role
    ...
    center['features']=center['features'][INDICES].copy()
