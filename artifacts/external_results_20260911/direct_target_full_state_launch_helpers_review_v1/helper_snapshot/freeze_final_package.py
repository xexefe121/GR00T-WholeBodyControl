"""Future unified65000 release binding. Requires actual positive receipts first."""
import argparse,json,sys
from pathlib import Path
BASE=Path(__file__).resolve().parent;SOURCE=BASE/'source_draft_v1';NEW=BASE.parent
FIT=NEW/'direct_target_full_state_student_v1'
...
from export_release_gate import validate_release,SUBJECTS
...
def require_helper_review(spec):
    entry=role(spec);review=read(entry['path']);prep=read(BASE/'launch_helper_preparation.json')
...
def actual_paths():
    request=read(FIT/'training_request.json')
    return dict(head=FIT/'fit/student_head.onnx',checkpoint=FIT/'fit/student_head.pt',fit_report=FIT/'fit/report.json',
        ...
        export_manifest=FIT/'fit/output_manifest.json',coefficient=FIT/'fit/coefficient.json',
...
def base_binding(config,final,inventory):
    ...
    return dict(**final,reviews={name:role(config['reviews'][name]) for name in ('release','source')},
        ...
        onnx_dependencies=inventory['onnx_dependencies'],ordinary_final_step=65000,
        release_kind='ordinary65000_full58_same_weight_fp64_export',export_input_dtype='float32',export_output_dtype='float32',export_internal_dtype='float64',
        controller='direct_absolute_target_1000_applied_inverse_learned_prior',architecture=[1000,256,256,23],hidden_activation='ELU',head_output='normalized_target',
...
def main():
    ...
    paths=actual_paths();final={name:add(pins,path) for name,path in paths.items()}
    ...
    for name in ('freeze_final_package.py','prepare_bound_launcher.py','diagnostic_verdict.py','verify_completed_stage.py',
                 'prepare_review_configuration.py','test_launch_helpers.py','test_release_helpers.py','launch_helper_tests.xml',
                 'runtime_inventory.json','source_preparation.json','launch_helper_preparation.json'):
        add(pins,BASE/name)
    ...
    if args.mode=='evaluation':
        ...
        frozen=BASE/'frozen_inputs_v1.json';write_new(frozen,dict(kind='one_ordinary65000_full58_canonical_evaluation',
