...
class TimingProbe:
    def __init__(self,*,wall_ns,thread_cpu_ns,process_cpu_ns,thread_id,span_capacity=MAX_SPANS,gc_capacity=MAX_GC_EVENTS):
        ...
        self.last_wall=self.last_process=-1
    ...
    def begin(self,phase,tick,control):
        ...
        try:self.spans[offset+14]=int(self._sample(self.spans,offset+6,token))
    ...
    def end(self,token):
        ...
        try:
            ...
            if okay and self.spans[offset+14]==1:
                if self.spans[offset+11]<self.spans[offset+7]:okay=self._fault('CLOCK_REGRESSION',token)
            self.spans[offset+15]=int(okay)
        finally:self.in_probe=prior
        self.depth-=1;self.stack[self.depth]=-1
        if okay:self.spans_closed+=1
        return okay
    ...
    def gc_event(self,phase,info):
        ...
        try:
            ...
            if code==1:
                ...
                if self.gc[opening+2]!=thread or self.gc[offset+8]<self.gc[opening+8]:self._fault('GC_PAIR',row)
