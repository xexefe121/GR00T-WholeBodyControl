...
START, BOUNDARY, EXTENSION = 6500, 6530, 250
TRACE_SHA = "438e6420685666b104d9f944620bc9df6745763727848d8709fb223eb47f36cb"
ENDPOINT_SHA = "7741b36128e30c342ad0cf79a2b73cbd2b7697b5b714dde84eed8ed219d131a7"
...
class Continuity:
    def __init__(self, args, native, fresh, timeline):
        ...
        required = dict(clip="pico", probe="full-lifecycle", horizon=30, commit=5, iterations=5,
        ...
        if (self.metadata["completed_controls"] != START or self.metadata["failure"] is not None
            ...
            raise ValueError("wrong6500 checkpoint or request binding")
        ...
        if self.warm.shape != (30, 23) or self.metadata["plans"][-1]["control"] != START - 5:
            raise ValueError("checkpoint warm plan is not6495/H30")
        ...
        with np.load(self.source / "trace.npz", allow_pickle=False) as archive:
            ...
            exact(fresh.previous_action, actions[START], "checkpoint previous action6500")
            exact(history_flat(fresh), histories[START], "checkpoint named history6500")
    ...
    def initialize(self):
        ...
        atomic(self.args.output / "checkpoint6500_restored.npz", dict(
    ...
    def request_fields(self):
        return dict(kind="separate_same_mpc_terminal_hold_after_exact_tail_reexecution",
                    ...
                    physical_initialization="checkpoint6500 integration/history/last full6495 warm plan",
    ...
    def verify_boundary(self, trace, plans, data, fresh, warm, expected_time):
        ...
        (self.args.output / "reexecuted_tail_plans.json").write_text(json.dumps(finite_json(plans), indent=2, allow_nan=False))
        checks = {}
        try:
        ...
        atomic(self.args.output / "boundary6530.npz", dict(
        ...
        (self.args.output / "boundary_verification.json").write_text(json.dumps(dict(
            verified=True, compared_controls=30, compared_physics_steps=300, all_saved_fields_bitexact=checks,
            ...
            boundary_sha256=sha(self.args.output / "boundary6530.npz"),
    ...
    def finalize(self, trace, plans, data, fresh, warm, expected_time, completed, failure, restoration_events):
        ...
        result = dict(kind="separate_same_mpc_terminal_hold", completed_controls=count, requested_controls=EXTENSION,
                      ...
                      reexecution_controls=30 if self.boundary_verified else len(trace["target"]),
