...
def authorize(request_path,clearance_path):
    ...
    output=local(request['output_directory']);stages=local(request['stage_directory'])
    if not output.is_absolute() or not stages.is_absolute() or stages.resolve()!=output.parent.resolve()/'stage_receipts':
        raise ValueError('durable stage directory must be adjacent to the reviewed output')
    return request,request_sha


def ready(request_path,clearance_path):
    request,request_sha=authorize(request_path,clearance_path)
