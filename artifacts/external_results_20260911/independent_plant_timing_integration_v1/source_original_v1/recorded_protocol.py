...
def decode_job(payload,binding,table):
    ...
    for key in ['sequence','snapshot_control','snapshot_physics','activation','created_ns','deadline_ns']:
    ...
    expected_incoming=np.zeros(23,np.float32).tobytes() if c==0 else table.commands[c-1].raw_action
    if incoming!=expected_incoming:raise ValueError('incoming actual action differs from selected recorded predecessor')
    ...
    job=Job(binding,c,c,c*10,a,value['window_id'],body,value['history_digest'],value['schedule_digest'],value['created_ns'],value['deadline_ns'])
