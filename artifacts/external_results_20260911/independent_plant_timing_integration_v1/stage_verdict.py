...
def main():
    ...
    output=args.base/'run';process=args.base/'clock_process';passed=False;error=None;report=None;sidecars=None
    try:
        raw=read(process/'raw_exit.json');report=read(output/'report.json')
        sidecars=check_sidecars(output,report,read(output/'output_manifest.json'))
        ...
        passed=bool(raw['known'] is True and raw['raw_python_exit_code']==0 and raw['raw_error'] is None and
                    preservation_complete and sidecars['instrumentation_complete'] is True and report['component_preliminary_pass'] is True and report['worker_pass'] is True and
    ...
    write(process/'diagnostic_verdict.json',{'diagnostic_passed':passed,'component_qualified':False,
        'root_saved_audit_pending':True,'timing_sidecar_accounting':sidecars,'error':error,'request_sha256':sha(args.base/'clock_request.json'),
