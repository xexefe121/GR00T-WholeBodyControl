...
from prepare_concrete_packet import (RETRY_CONTRACT,RETRY_DETAILS,RESULT_CONTRACT,RESULT_DETAILS,
    DEADLINE_CONTRACT,TIMING_CONTRACT,validate_retry_contract,check_review,check_subject)
...
class HelperTests(unittest.TestCase):
    ...
    def test_every_contract_literal(self):
        for key in ('job_publication_retry_contract','worker_result_retry_contract','job_deadline_contract','timing_probe_contract'):
    ...
    def test_review_requires_exact_named_subject(self):
        ...
        for field in ('source_preparation_subject','preparation_subject','source_preparation_sha256'):
            good=dict(passed=True,source_review_pass=True,source_sha256=prep['source_sha256'])
            good[field]='b'*64 if field=='source_preparation_sha256' else {'path':path.as_posix(),'sha256':'b'*64}
    ...
    def test_absolute_bootstrap_and_fixed_budget(self):
        ...
        self.assertIn('PYTHONPATH=/mnt/e/codex-artifacts/sonic23_teleop_resume_20260911/independent_plant_timing_integration_v1/source_draft_v1',args)
