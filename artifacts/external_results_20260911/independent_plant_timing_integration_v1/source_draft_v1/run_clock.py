...
def imported_source_paths(pins):
    ...
    for name in ('packet_io','counted_api','native_loader','model_identity','native_stepper',
                 ...
                 'recorded_protocol','pending_result','timing_probe','timing_hooks','timing_preservation','evidence','capture_schema','bfm_observations','oracle_source','stage_watchdog'):
...
def run(request,request_sha,precheck,output,request_path,watchdog,journal):
    ...
    if request.get('timing_probe_contract')!='preallocated_wall_thread_process_GC_v2':
    ...
    try:
        ...
        # Allocate all probe rows/scopes and attach before the one epoch choice.
        ...
        # Main owned evidence above is preserved first. Sidecar errors cannot
        ...
        result={'kind':'recorded_command_process_clock_component','request_sha256':request_sha,
                ...
                'stage_watchdog':watchdog.evidence(),'timing_instrumentation':timing_result,
