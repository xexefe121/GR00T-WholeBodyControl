...
class HookFailures(unittest.TestCase):
    ...
    def test_capture_end_interrupt_retains_owned_capture(self):
        class Interrupt(TimingProbe):
            def end(self,token):
                if self.spans[token*len(SPAN_FIELDS)]==10:raise StageTimeout('capture end injected',100,101)
    ...
    def test_verifier_end_interrupt_retains_verified_credit(self):
        class Interrupt(TimingProbe):
            def end(self,token):
                if self.spans[token*len(SPAN_FIELDS)]==12:raise StageTimeout('verify end injected',100,101)
                return super().end(token)
        parts=setup();h=hooks(cls=Interrupt);attach(parts,h)
        with self.assertRaises(StageTimeout):parts[-1].tick_once()
    ...
    def test_ownership_begin_interrupt_keeps_adapter_only_capture(self):
        class Interrupt(TimingProbe):
            def begin(self,phase,*args):
                if phase==11:raise StageTimeout('ownership begin',100,101)
                return super().begin(phase,*args)
        parts=setup();h=hooks(cls=Interrupt);attach(parts,h)
        original=parts[1].capture_step
        def capture():
            value=original();parts[1].last_capture=value;return value
        parts[1].capture_step=capture
        with self.assertRaises(StageTimeout):parts[-1].tick_once()
        self.assertEqual((parts[1].captured,parts[-1].foundation.captured),(1,0))
        self.assertIsNone(parts[-1].foundation.last_capture)
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertFalse(result['complete']);self.assertIn('last_capture',saved['adapter'])
    ...
    def test_fatal_end_does_not_replace_original_measured_exception(self):
        class Broken(TimingProbe):
            def end(self,token):raise StageTimeout('secondary deadline',100,101)
        h=hooks(cls=Broken)
        with self.assertRaisesRegex(ValueError,'measured first'):
            with h.span(1,0,0):raise ValueError('measured first')
        self.assertTrue(h.failed);self.assertEqual(h.depth,0)
    def test_fatal_begin_propagates_without_body(self):
        class Broken(TimingProbe):
            def begin(self,*args):raise StageTimeout('begin deadline',100,101)
        h=hooks(cls=Broken);body=[]
        with self.assertRaises(StageTimeout):
            with h.span(1,0,0):body.append(1)
        self.assertEqual(body,[]);self.assertEqual(h.depth,0);self.assertTrue(h.failed)
...
class NativeMethodFakes(unittest.TestCase):
    ...
    def test_native_end_interrupt_keeps_returned_counter(self):
        class Interrupt(TimingProbe):
            def end(self,token):
                if self.spans[token*len(SPAN_FIELDS)]==9:raise StageTimeout('native end deadline',100,101)
                return super().end(token)
