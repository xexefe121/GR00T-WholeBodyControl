...
class PendingPublication:
    ...
    retry_checked_ns: int | None = None
    ...
    def evidence(self):
        ...
        return dict(identity=self.job.identity(), payload=b64(self.payload),
                    ...
                    retry_checked_ns=self.retry_checked_ns)
...
class PlantFoundation:
    ...
    def _attempt_job_publication(self, pending):
        # Called once at the original boundary, or once on a later eligible tick.
        with self.timing.span(7,self.returned,self.returned//CONTROL_STEPS):
            index = self.returned
            ...
            self._event('JOB_PUBLICATION', activation=pending.job.activation, status=status,
                        ...
                        retry_checked_ns=pending.retry_checked_ns,
    ...
    def _expire_pending_publication(self, reason):
        ...
        self._event('PENDING_JOB_EXPIRED', activation=pending.job.activation,
                    ...
                    observed_ns=self._last_clock_ns,
    ...
    def _retry_pending_publication(self):
        ...
        self._attempt_job_publication(replace(pending, retry_checked_ns=now))
    ...
    def _boundary(self):
        ...
        with self.timing.span(5,self.returned,control):
        ...
        if control + 1 < len(self.window_ids) and self.input_fault is None:
            with self.timing.span(6,self.returned,control):
                ...
                pending = PendingPublication(job, job.to_bytes(),
                                             job.deadline_ns)
    ...
    def tick(self):
        ...
        try:
            ...
            with self.timing.span(10,index,index//CONTROL_STEPS):
                ...
                with self.timing.span(11,index,index//CONTROL_STEPS):
            ...
            with self.timing.span(12,index,index//CONTROL_STEPS):
                ...
                with self.timing.span(13,index,index//CONTROL_STEPS):
            ...
            with self.timing.span(14,index,index//CONTROL_STEPS):
