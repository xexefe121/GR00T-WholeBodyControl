...
def benchmark_pass(result):
    ...
    return bool(result['physical_complete'] and tracking and result['main_quiet'] and
        ...
        result['plant_finishes_over2ms_late']==0 and
...
class Bridge:
    def __init__(self,libpath,name=None):
        ...
        l.clock_set_spin_ns.argtypes=[ct.c_int64]
...
def main():
    ...
    if not 0<=a.spin_us<=500:raise ValueError('spin window must be 0..500 microseconds')
    ...
    bridge.lib.clock_set_spin_ns(a.spin_us*1000)
    ...
    try:
        ...
        result=dict(physical_complete=bool(complete),requested_controls=a.controls,completed_physics_steps=count,
            ...
            plant_finishes_over2ms_late=int(np.count_nonzero(timing[:count,3]-timing[:count,0]>2_000_000)),
            maximum_plant_finish_lateness_ms=float(np.max(timing[:count,3]-timing[:count,0])*1e-6),
            ...
            wake_spin_microseconds=a.spin_us,
