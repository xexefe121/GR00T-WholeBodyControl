...
def worker(libpath,name,standing,ready,stop,out,optimize,braking):
    bridge=None;rows=[]
    try:
...
def main():
    ...
    p.add_argument('--initial-velocity',type=float,default=0.);a=p.parse_args()
    ...
    try:
        for standing in (False,True):
            ready=context.Event();process=context.Process(target=worker,args=(libpath,bridge.shm.name,standing,ready,stop,a.output,a.optimize,a.braking))
