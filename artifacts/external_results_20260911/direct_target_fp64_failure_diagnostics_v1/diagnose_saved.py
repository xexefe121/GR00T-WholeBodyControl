...
def main():
    ...
    qualification = read(paths[-3] if False else NEW/'bfm250_expert_root_qualification_v1/qualification.json')
...
def plot(traces,series,result,out,velocity,hip):
    ...
    scope.set(xlim=(0,31.38),ylim=(0,1),yticks=[],xlabel='Original lifecycle time (s) — full 31.38 s requested; conditional 5 s hold unrun')
    ...
    fig.suptitle('Native23 walk003 — saved failure comparison\nNew FP64 head: right hip-roll speed −22.399 rad/s > 20 limit; earlier head: ankle range failure at 6.316 s',fontsize=16)
