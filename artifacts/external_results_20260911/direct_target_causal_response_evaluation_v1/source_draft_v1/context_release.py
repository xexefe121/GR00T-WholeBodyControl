"""Literal single warm71000 release checks; no runtime control change."""
...
SOURCE_CHECKPOINT='10d238198e2d7826c35eaa9bd4a13ff2e05050a7f601c1f212b585c4a90eaefd'
...
def condition_report(report,condition):
    ...
    expected_fields(report,dict(completed=True,optimization_completed=True,final_export_diagnostics_completed=True,
        numerical_gate_passed=True,export_parity_passed=True,condition=condition,ordinary_final_step=71000,
        additional_updates=3000,optimizer_step=6000,fresh_optimizer=False,features=1323,context_features=323,
...
def validate_release(binding,paths,source,purpose,*,read,sha,bound_file,field,has_hash):
    expected_fields(binding,dict(release_kind='ordinary71000_warm_balanced_context_same_weight_fp64_export',
    ...
    expected_fields(request,dict(kind='causal_response_balanced_continuation',root_selected=True,condition='causal',
        updates=3000,ordinary_start_step=68000,ordinary_final_step=71000,optimizer_start_step=3000,optimizer_final_step=6000,
        fresh_optimizer=False,coefficient=COEFFICIENT,learning_rate=[1e-5,1e-6],features=1323,
