...
if($receipt.selected_single_saved_audit -ne $true -or $receipt.request_sha256 -ne '4257b57c529aea3ca2bfa5ab0230e10a089ba13e6b3004a28398eaf6afdf6103'){throw 'Wrong selected saved audit.'}
...
try{
    ...
    $arguments='-B -u "'+(Join-Path $source 'audit_saved_fit.py')+'" --request "'+(Join-Path $runRoot 'audit_request.json')+'" --request-sha256 '+$receipt.request_sha256+' --output "'+(Join-Path $runRoot 'results_v1')+'"'
