...
class SidecarIO(unittest.TestCase):
    ...
    def test_actual_reserved_capture_without_verification_credit(self):
        ...
        self.assertFalse(r['actual_native_verification_attempted']);self.assertIsNone(r['actual_native_verification_returned'])
        self.assertEqual(r['new_verification_credit'],0)
        self.assertFalse(r['full_scope_credit'])
    def test_attempted_verifier_interruption_does_not_establish_return(self):
        args=list(reserved());args[2]['session']['stepper']['verification_attempts']=3
        args[2]['first_error']={'type':'StageTimeout','detail':'inside assess_capture before return'}
        r=reserved_capture(*args)
        self.assertTrue(r['actual_native_verification_attempted'])
        self.assertIsNone(r['actual_native_verification_returned'])
        self.assertTrue(r['verification_return_not_inferred_from_attempt_counter'])
        self.assertEqual(r['new_verification_credit'],0)
