...
def job_expected(raw,a,metadata,binding,commands,windows):
    ...
    if type(c) is not int or type(activation) is not int or not 0<=c<len(a['control_control']) or activation!=c+1 or activation>=1819:
...
def protocol(a,metadata,jobs,published_list,worker,request,full,hold,full_sha,hold_sha,reference_sha,contract,terminal_interruption=None):
    ...
    worker_audit=check_worker(worker,identities,jobs,commands,epoch,schedule,result_hashes)
    ...
    return dict(issued_jobs=len(identities),published_jobs=len(published_list),admitted_results=len(sealed),
        ...
        worker_reply_bytes_exact=not mismatches,worker_result_retry=worker_audit,recorded_schedule_sha256=schedule,
        complete_protocol_coverage=publication_audit['full_publication_coverage'] and len(identities)==1818 and len(published_list)==1818 and len(sealed)==1818 and not rejections and not held_events and not mismatches and worker_audit['full_worker_coverage'])
