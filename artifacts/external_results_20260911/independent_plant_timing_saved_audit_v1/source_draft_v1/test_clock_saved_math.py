...
import unittest
import numpy as np
from clock_saved_math import physical_arrays,timing_arrays,compare_expert_prefix,repeated_time,first_difference,initial_strict
...
class SavedClockTests(unittest.TestCase):
    ...
    def test_initial_force_exception_does_not_add_gate(self):
        a,initial,c,_,_=fixture(1);raw=a['packed_capture'][0].copy();raw[:291]=initial
        raw[350:]=np.nan
        self.assertTrue(initial_strict(raw,np.zeros(8,np.int32),c)['original_step0_force_exception_preserved'])
        raw[293]=.1
        with self.assertRaisesRegex(AssertionError,'strict state'):initial_strict(raw,np.zeros(8,np.int32),c)
    def test_actual_nonfinite_force_is_preserved_strict_failure(self):
        a,initial,c,_,_=fixture(1);a['packed_capture'][0,350]=np.nan;a['step_actuator_force'][0,0]=np.nan;a['step_verified'][0]=False
        r=physical_arrays(a,initial,c);self.assertEqual(r['strict_failures']['effort'],[0]);self.assertFalse(r['all_actual_saved_states_strict'])
    def test_actual_nonfinite_state_is_preserved_strict_failure(self):
        a,initial,c,_,_=fixture(1);a['packed_capture'][0,8]=np.nan;a['packed_capture'][0,298]=np.nan
        a['step_integration']=a['packed_capture'][:,:291].copy();a['step_qpos']=a['packed_capture'][:,291:321].copy();a['step_verified'][0]=False
        r=physical_arrays(a,initial,c);self.assertEqual(r['strict_failures']['nonfinite_state'],[0]);self.assertEqual(r['strict_failures']['fall'],[0])
