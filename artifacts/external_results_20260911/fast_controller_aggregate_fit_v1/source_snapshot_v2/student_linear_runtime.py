...
FROZEN_RECEIPT=BASE/'frozen_inputs_v2.json'
...
def assert_frozen():
    ...
    for name,expected in receipt['source_sha256'].items():
        if sha(Path(__file__).parent/name)!=expected:raise ValueError('frozen pilot source changed: '+name)
...
def infer_base(seed,qpos,qvel,previous_action,history,frame,terminal=False):
    ...
    if not np.isfinite(raw).all() or not np.isfinite(target).all():
        raise ValueError('nonfinite frozen BFM output')
