...
def paths(physics=None):
    ...
    p=dict(trace=run/'nominal/trace.npz',report=run/'nominal/report.json',failure=run/'nominal/strict_failure_state.npz',
        ...
        physics=physics if physics is not None else NEW/'direct_target_fp64_independent_physics_v1/report.json',contract=bundle/'contract.json',original29=bundle/'walk003/original29.npz',
        ...
        norm=NEW/'direct_target_continuation_v1/fit/normalization.npz',feature_source=src/'direct_features.py',
    ...
    return p
def freeze(physics,physics_sha):
    p=paths(physics);assert sha(p['trace'])=='c5829d6e54bc5791bf9e9409d055496f7e94402c9af8e39ec83cf3b654ac4cae'
    assert sha(p['physics'])==physics_sha
    ...
    write(BASE/'request.json',dict(kind='one_pure_direct_target_saved_outcome_audit',source_sha256=sha(__file__),
        ...
        model_calls=0,BFM_calls=0,physics_steps=0,optimizer_updates=0,scope='292 actual controls,42 learned, no recovered trajectory'))
def pure(p):
...
def audit():
    ...
    p={k:local(v) for k,v in req['paths'].items()};release=check_release(p)
    ...
    assert len(a['target'])==292 and len(a['qpos'])==293 and r['attempted_controls']==292 and r['physics_steps']==2916
    exact(a['global_control'],np.arange(292,dtype=np.int64),'control clock');exact(a['source_frame'],np.arange(11,327,dtype=np.int64),'source clock')
    exact(a['controller_mode'],np.r_[np.zeros(250,np.int64),np.ones(42,np.int64)],'phase schedule')
    exact(a['physics_substeps'],np.r_[np.full(291,10,np.int64),np.array([6],np.int64)],'actual substeps')
    expected=0.;times=[expected]
    for _ in range(2916):expected+=.002;times.append(expected)
    ...
    for i in range(292):
        ...
        previous=outgoing
    exact(previous,a['final_previous_action'],'final prior');assert int(a['final_recorded_controls'])==292
    ...
    for mode in range(3):
        for graph in ('backward','actor','head'):
            count=250 if mode==0 and graph in ('backward','actor') else 42 if mode==1 and graph=='head' else 0
    ...
    exact(cap['integration_at_failure'],a['final_integration'],'failure full291');exact(cap['head_input_features'][0],a['features'][291],'failure head input')
    exact(cap['head_output_0'][0],a['normalized_head'][291],'failure returned head output');exact(cap['previous_action_after'],previous,'failure prior after')
    for key in history:
        exact(cap['history_before_'+key],named[key][291],'failure prehistory '+key);exact(cap['history_after_'+key],history[key],'failure final history '+key)
    selected=np.arange(250,292);features=a['features'][250:];teacher_features=labels['features'][:42,kept];teacher_target=labels['expert_target'][:42]
    ...
    np.savez_compressed(BASE/'actual_rows.npz',control=selected,features=features,teacher_features=teacher_features,teacher_target=teacher_target,
        ...
        action=a['action'][250:],state=a['state'][250:],history=a['history'][250:],qpos=a['qpos'][250:292],qvel=a['qvel'][250:292])
    for path,digest in req['input_sha256'].items():assert sha(local(path))==digest,path
    assert sha(BASE/'request.json')==reqsha and sha(__file__)==req['source_sha256']
    report=dict(passed=True,moving_controls=42,actual_controls=292,original_requested_controls=1569,original_lifecycle_completed=False,release_subjects=release,exact_checks=len(checks),checks=checks,
