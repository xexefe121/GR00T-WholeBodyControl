...
def main():
    ...
    assert not (DEST / 'request.json').exists(), 'Existing fit attempt must remain preserved.'
    ...
    request = dict(kind=KIND, experiment='one_qualified_actual_state_aggregate_continuation',
        ...
        optimizer=dict(name='AdamW', learning_rate=3e-4, weight_decay=1e-5,
            gradient_clip_norm=10., loss='mean squared residual divided by native joint span',
            threads=1, device='cpu', batch_size=256, samples_per_dataset_per_phase=32),
        original_normalization_refitted=False, full_final20000_model_optimizer_rng_restored=True,
    ...
    write('optimization_completed.json', dict(global_step=60000, additional_updates=40000,
