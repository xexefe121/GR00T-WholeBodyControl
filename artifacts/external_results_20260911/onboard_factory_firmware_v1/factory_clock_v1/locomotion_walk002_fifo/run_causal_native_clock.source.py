...
def main():
    ...
    if args.factory_locomotion and args.native_conditioned:raise ValueError('select only one factory controller')
    factory_enabled=args.factory_locomotion or args.native_conditioned
    if args.factory_config is None:
        cfg='human_loco/fsm_human_loco_config.yaml' if args.factory_locomotion else 'mimic_test/fsm_mimic_test.yaml'
    ...
    report = dict(actor=str(args.actor), clip=args.clip, standing_diagnostic=args.standing,
        native_factory_conditioned=args.native_conditioned,factory_locomotion=args.factory_locomotion,native_limit_brake=factory_enabled,
