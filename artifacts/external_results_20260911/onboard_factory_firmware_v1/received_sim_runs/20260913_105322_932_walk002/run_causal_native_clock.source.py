...
def worker(library, name, actor, model, contract, options, initial_packets, packets,
    ...
    try:
        ...
        if affinity and hasattr(os, 'sched_setaffinity'):
            ...
            if options.get('native_preview_library'):selected.add(available[min(3,len(available)-1)])
...
def main():
    ...
    report = dict(actor=str(args.actor), clip=args.clip, standing_diagnostic=args.standing,
        ...
        native_preview_controller_affinity_cores=2 if args.affinity and args.native_preview_library else (1 if args.affinity else None),
