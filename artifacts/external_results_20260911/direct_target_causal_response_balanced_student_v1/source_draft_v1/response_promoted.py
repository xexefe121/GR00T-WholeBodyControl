...
def from_checkpoint(saved):
    if saved['kind']!='direct_absolute_native23_target' or saved['ordinary_final_step']!=71000:raise ValueError('Exact warm ordinary71000 subject required')
    ...
    if [w.shape for w in weights]!=[(256,1323),(256,256),(23,256)]:raise ValueError('Causal context architecture changed')
...
def export_onnx(model,path):
    ...
    graph=helper.make_graph(nodes,'same_71000_context_weights_fp64_execution',[helper.make_tensor_value_info('features',TensorProto.FLOAT,[None,len(model.feature_mean)])],
