...
try{
    ...
    if($clear.approved -ne $true -or $clear.updates -ne 3000 -or $clear.ordinary_final_step -ne 71000 -or $clear.condition -ne 'causal' -or $clear.optimizer_start_step -ne 3000 -or $clear.optimizer_final_step -ne 6000 -or $clear.automatic_retry -ne $false){throw 'Selected direct fit scope differs.'}
    ...
    if($request.kind -ne 'causal_response_balanced_continuation' -or $request.root_selected -ne $true -or $request.updates -ne 3000 -or $request.condition -ne 'causal' -or $request.fresh_optimizer -ne $false -or $request.ordinary_final_step -ne 71000 -or $request.coefficient -ne 1.8188207859141674d){throw 'Unexpected request.'}
    ...
    Write-NewJson (Join-Path $processRoot 'start.json') ([ordered]@{
        kind='causal_response_balanced_continuation';wrapper_pid=$PID;started_utc=[DateTime]::UtcNow.ToString('o');
    ...
    try{Write-NewJson (Join-Path $processRoot 'exit.json') ([ordered]@{
        kind='causal_response_balanced_continuation_exit';ended_utc=[DateTime]::UtcNow.ToString('o');wrapper_pid=$PID;
