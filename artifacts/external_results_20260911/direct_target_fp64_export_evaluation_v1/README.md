# Same55000 checkpoint, separately qualified higher-precision export
...
training request and training manifest are individually pinned. A positive
training-only audit/qualification must directly name their hashes. A separate
training review binds that qualification. A dataset review directly binds the
...
and export_owner_completion. The positive training-only qualification may itself
bind an immutable underlying evidence audit that reports the original export
failure. An unrelated digest elsewhere in a receipt cannot satisfy these named
...
tests and27 new release checks. One old helper test was adapted because subject
