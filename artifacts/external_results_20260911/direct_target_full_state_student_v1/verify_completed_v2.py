...
def main():
    ...
    direct_subject_sha256=dict(fit_report=sha(fit/'report.json'),checkpoint=sha(fit/'student_head.pt'),head=sha(fit/'student_head.onnx'),
        ...
        training_manifest=sha(BASE/'training_frozen_inputs.json'),export_manifest=sha(fit/'output_manifest.json'),
        source_checkpoint=sha(request['subjects']['checkpoint']['path']),
        full_state_generation_request=sha(request['full_state_paths']['request']),full_state_generation_report=sha(request['full_state_paths']['report']),
        full_state_data_audit=sha(request['subjects']['full_state_root_audit']['path']),full_state_data_owner=sha(request['subjects']['full_state_root_owner']['path']))
    result=dict(owner_verification_passed=True,report_sha256=direct_subject_sha256['fit_report'],checkpoint_sha256=report['checkpoint_sha256'],onnx_sha256=report['onnx_sha256'],
        ...
        expected_pids=pids,processes_absent=True,raw_exit_known=True,raw_python_exit_code=0,exit_code=0,postpins_exact=True,all_postrun_pins_exact=True,
