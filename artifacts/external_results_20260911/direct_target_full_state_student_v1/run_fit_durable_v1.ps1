...
try{
    ...
    if($clear.approved -ne $true -or $clear.additional_updates -ne 10000 -or $clear.ordinary_final_step -ne 65000 -or $clear.automatic_retry -ne $false){throw 'Selected direct fit scope differs.'}
    ...
    if($request.kind -ne 'one_full_state_finite_feedback_fit' -or $request.root_selected -ne $true -or $request.updates -ne 10000 -or $request.sampler_seed -ne 20260911){throw 'Unexpected request.'}
    ...
    $snapshot=Join-Path $runRoot 'source_snapshot_v1';$driver=Join-Path $snapshot 'train_full_state.py'
    ...
    Write-NewJson (Join-Path $processRoot 'start.json') ([ordered]@{
        kind='one_full_state_finite_feedback_fit';wrapper_pid=$PID;started_utc=[DateTime]::UtcNow.ToString('o');
        ...
        updates=10000;ordinary_final_step=65000;budgets=$request.budgets;automatic_retry=$false
    })
    ...
    foreach($field in @('completed','optimization_completed','final_export_diagnostics_completed','numerical_gate_passed','export_parity_passed')){if($report.$field -ne $true){throw "Incomplete report: $field"}}
    if($report.ordinary_final_step -ne 65000 -or $report.additional_updates -ne 10000 -or $report.features -ne 1000 -or $report.head_output -ne 'normalized_target'){throw 'Final direct head metadata differs.'}
    if($report.counters.training.rows_verified -ne 146860000 -or $report.counters.training.calls_verified -ne 30000 -or $report.counters.calibration.rows_verified -ne 14686 -or $report.counters.gradients.verified -ne 3 -or $report.optimizer_step -ne 10000){throw 'Completed fit counts differ.'}
    foreach($backend in @('initial_GPU32','final_GPU32','CPU64','GPU64','ORT64')){if($report.counters.diagnostics.$backend.rows_verified -ne 367570 -or $report.counters.diagnostics.$backend.calls_verified -ne 1437){throw "Incomplete backend counts: $backend"}}
    if((Read-SharedSha (Join-Path $runRoot 'fit/student_head.pt')) -ne $report.checkpoint_sha256 -or (Read-SharedSha (Join-Path $runRoot 'fit/student_head.onnx')) -ne $report.onnx_sha256){throw 'Output identities differ.'}
    $runExit=0
    ...
    try{Write-NewJson (Join-Path $processRoot 'exit.json') ([ordered]@{
        kind='one_full_state_finite_feedback_fit_exit';ended_utc=[DateTime]::UtcNow.ToString('o');wrapper_pid=$PID;
