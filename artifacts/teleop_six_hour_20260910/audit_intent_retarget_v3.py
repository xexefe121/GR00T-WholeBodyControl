...
def audit(clip):
    directory = BASE / 'intent_retarget_v3' / clip
...
if __name__ == '__main__':
    ...
    output = dict(kind='independent_read_only_retarget_v3_audit', source_file_sha256=sha(__file__), checks=rows,
                  export_integrity_and_joint_step_limits_passed=integrity_passed,
                  dynamics_qualified=False, hardware_authorized=False)
    (BASE / 'intent_retarget_v3_independent_audit.json').write_text(json.dumps(output, indent=2, allow_nan=False))
