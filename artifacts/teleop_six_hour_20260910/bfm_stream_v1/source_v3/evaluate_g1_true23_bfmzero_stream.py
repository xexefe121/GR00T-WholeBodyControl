...
def run(args):
    ...
    if args.pacing_clock != "sleep" and not args.paced:
        raise ValueError("pacing clock requires --paced")
    ...
    request = {
        ...
        "paced": args.paced,
        "timing_environment": args.timing_environment,
        "pacing_clock": args.pacing_clock if args.paced else "unpaced",
        "position_gain": args.position_gain,
        "yaw_gain": args.yaw_gain,
        ...
        "inputs": {
            ...
            for path in (
                ...
                ROOT / "gear_sonic/utils/g1_true23_bfmzero_stream_clock.py",
    ...
    durations, lateness, deadline_misses, receive_durations, receiver_times = [], [], [], [], []
    cursor = 0
    rearm_events = []
        ...
        for control in range(requested_controls):
            ...
            while cursor < len(schedule) and schedule[cursor][0] <= now + 1e-7:
                ...
                if kind == 0:
                    ...
                    rearm_events.append(
                        {"scheduled_time": delivery_time, "received_time": now, "accepted": accepted}
                    )
                    continue
            ...
            receive_durations.append((received - tick) * 1000)
    ...
    arrays.update(
        ...
        receive_ms=np.asarray(receive_durations),
        control_receiver_time=np.asarray(receiver_times),
    )
    ...
    result.update(
        {
            ...
            "phase_ms_p50_p95_max": {
                name: np.percentile(arrays[name], [50, 95, 100]).tolist()
                for name in (
                    "receive_ms",
                    "reference_window_ms",
                    "goal_feedback_ms",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pacing-clock", choices=("sleep", "windows-high-resolution"), default="sleep")
    parser.add_argument("--timing-environment", default="unqualified_shared_machine_load")
