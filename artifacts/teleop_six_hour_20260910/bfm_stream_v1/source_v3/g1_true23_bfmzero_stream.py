...
def received_goal(policy, samples, measured_qpos, position_gain, yaw_gain):
    ...
    with torch.inference_mode():
        latent = policy.backward(torch.from_numpy(states), torch.from_numpy(privileged)).mean(0, keepdim=True)
...
class BFMStreamSimulator:
    ...
    def step(self, now):
        ...
        goal, feedback_ms, backward_ms = received_goal(
            self.policy, samples, self.data.qpos, self.position_gain, self.yaw_gain
        )
