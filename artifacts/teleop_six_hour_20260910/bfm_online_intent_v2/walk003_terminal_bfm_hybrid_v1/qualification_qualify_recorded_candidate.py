...
from artifacts.teleop_six_hour_20260910.inspect_bfm_tracking import inspect, resolve_motion_override
...
def audit(directory, output, motion_override=None):
    if output.exists():
        raise ValueError("qualification outputs are immutable")
    report = json.loads((directory / "report.json").read_text())
    intent = inspect(directory, output_name=None, motion_override=motion_override)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--require-pass", action="store_true",
                        help="Exit with status 1 when any recorded-source gate fails; never implies online qualification")
    args = parser.parse_args()
    result = audit(args.directory, args.output, motion_override=args.motion_override)
    if args.require_pass and not result["recorded_source_tracking_pass"]:
        raise SystemExit(1)
