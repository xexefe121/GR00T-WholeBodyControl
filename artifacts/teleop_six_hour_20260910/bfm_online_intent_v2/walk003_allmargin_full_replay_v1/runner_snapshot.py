...
def load_plan(plan_dir, model, motion_path, contract_path):
    ...
    source_feedback_clip = float(request.get("feedback_correction_clip_rad", 0.))
    if not np.isfinite(source_feedback_clip) or source_feedback_clip < 0:
        raise ValueError("invalid producer feedback correction clip")
    ...
    for control in range(count):
        ...
        correction = plan["feedback_gain"][control] @ difference
        if source_feedback_clip > 0:
            correction = np.clip(correction, -source_feedback_clip, source_feedback_clip)
        target = np.clip(plan["planned_target"][control] + correction,
...
def run(args):
    ...
    feedback_gain = plan["feedback_gain"]
    gain_regularization = None
    if getattr(args, "feedback_noise_gain_cap", None) is not None:
        if args.mode != "feedback" or args.feedback_clip is None:
            raise ValueError("noise-scaled gain ablation requires feedback mode and explicit correction clip")
        from gear_sonic.utils.g1_true23_mpc_robust_feedback import NoiseCappedFeedback
        capped = NoiseCappedFeedback(feedback_gain, noise_gain_cap=args.feedback_noise_gain_cap,
                                     correction_clip=args.feedback_clip)
        feedback_gain = capped.gains
        gain_regularization = capped.report()
        helper = ROOT / "gear_sonic/utils/g1_true23_mpc_robust_feedback.py"
        gain_regularization["helper_sha256"] = sha256(helper)
        (args.output / "gain_regularization_snapshot.py").write_bytes(helper.read_bytes())
    ...
    provenance = dict(mujoco=mujoco.__version__, source_mujoco=request["mujoco"],
                      ...
                      feedback_gain_regularization=gain_regularization,
    ...
    for control in range(len(plan["target"])):
        ...
        if args.mode == "feedback":
            ...
            correction = feedback_gain[control] @ error
        ...
        for _ in range(plan["physics_substeps"][control]):
            ...
            elif abs(float(data.time) - len(trace["physics_torque"]) * model.opt.timestep) > 1e-8 or np.any(data.warning.number):
                failure = dict(kind="engine_warning_or_clock_reset", control=control, substep=substeps,
    ...
    result = dict(clip=clip, mode=args.mode, mujoco=mujoco.__version__, source_mujoco=request["mujoco"],
                  ...
                  source_plan_is_final_result=source_report.get("is_final_result", True),
                  source_plan_kind=source_report.get("kind"),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--feedback-noise-gain-cap", type=float)
