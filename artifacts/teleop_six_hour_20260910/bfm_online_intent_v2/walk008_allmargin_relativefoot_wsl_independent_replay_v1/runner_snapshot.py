...
def main(args):
    ...
    assert request["clip"] == "walk008" and request["physical_initialization"] == "optimization_reference_frame10"
    ...
    for name in ("manifest.json", "contract.json", "walk008/native_original.npz", "walk008/timeline.json"):
    ...
    assert sha(args.bundle / "walk008/native_original.npz") == override["base_native_reference_sha256"]
    assert sha(args.bundle / "walk008/original29.npz") == override["original29_sha256"]
    timeline = json.loads((args.bundle / "walk008/timeline.json").read_text())
    ...
    assert count == 1114 == request["requested_controls"] == len(motion["joint_pos"]) - 11
    assert phase["requested_controls"] == 364
    ...
    relevant += [args.bundle / name for name in ("native_prepared.xml", "prepared_model_arrays.npz",
                 "manifest.json", "contract.json", "walk008/native_original.npz", "walk008/original29.npz", "walk008/timeline.json")]
    ...
    provenance = dict(kind="independent_continuous_recorded_target_native323_replay", clip="walk008",
        ...
        original29_path=str((args.bundle / "walk008/original29.npz").resolve()),
        ...
        warning_evidence_scope="new independent replay every2ms; producer reports final zero counters without per-step ledger",
    ...
    for control, target in enumerate(source["target"]):
        ...
        for sub in range(10):
            ...
            elif abs(float(data.time)-(control*10+sub+1)*.002)>1e-9:
                failure = dict(kind="physics_clock_mismatch", control=control, substep=sub+1,
                               actual_time=float(data.time),expected_time=(control*10+sub+1)*.002)
    ...
    report = dict(kind=provenance["kind"], clip="walk008", mode="executed-target", mujoco=mujoco.__version__,
