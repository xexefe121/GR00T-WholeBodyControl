...
def audit(clip):
    directory = BASE / 'intent_retarget_v3_multistart' / clip
    ...
    result = dict(clip=clip, frames=count, source_frames=phase['requested_controls'], all_scalar_checks=scalar,
                  ...
                  forward_interval_leg_velocity_ratio_max=float(np.max(np.abs(forward[:, :12]) / vlim[:12])),
                  configured_leg_step_bound_violations=int(np.sum(np.abs(np.diff(motion['joint_pos'][:, :12], axis=0)) > np.asarray(report['leg_max_step_rad_per_control']) + 1e-12)),
                  reported_adjacent_ratio_matches=abs(float(np.max(np.abs(forward) / vlim)) - report['adjacent_velocity_ratio_max']) < 1e-12,
                  ...
                  snapshot_hash_matches=dict(retarget=sha(directory / 'retarget_runner_snapshot.py') == report['source_provenance'][str(ROOT / 'gear_sonic/utils/g1_true23_intent_retarget.py')],
                                             ...
                                             hand_geometry=sha(directory / 'hand_geometry_snapshot.py') == report['source_provenance'][str(ROOT / 'gear_sonic/utils/g1_true23_hand_frame_tasks.py')],
                                             physics=sha(directory / 'physics_contract_snapshot.json') == report['source_provenance'][str(ROOT / PHYSICS)]),
                  geometry_helper_current_hash=sha(ROOT / 'gear_sonic/utils/g1_true23_hand_frame_tasks.py'),
...
if __name__ == '__main__':
    ...
    integrity_passed=all(row['timeline_dict_matches_original'] and row['export_finite'] and row['output_hash_matches']
                         and all(row['snapshot_hash_matches'].values()) and all(row['report_source_hashes_current'].values())
                         and row['native_range_excess_max_rad']<=1e-12 and row['forward_interval_velocity_ratio_max']<=1.+1e-9
                         and row['forward_interval_leg_velocity_ratio_max']<=.8+1e-9 and row['configured_leg_step_bound_violations']==0
                         and row['reported_adjacent_ratio_matches'] and row['all_scalar_checks']['body_position_fk_max_m']<=1e-12
                         and row['all_scalar_checks']['body_rotation_fk_max_rad']<=1e-12
                         and row['all_scalar_checks']['torso_rotation_original_max_rad']<=1e-12
                         and row['all_scalar_checks']['torso_xy_original_max_m']<=1e-12
                         and row['all_scalar_checks']['torso_height_original_max_m']<=.060000001
                         and row['all_scalar_checks']['world_angular_interval_reconstruction_max_rad']<=1e-12 for row in rows)
    output = dict(kind='independent_read_only_retarget_v3_multistart_audit', source_file_sha256=sha(__file__), checks=rows,
                  export_integrity_and_joint_step_limits_passed=integrity_passed,
                  dynamics_qualified=False, hardware_authorized=False)
    (BASE / 'intent_retarget_v3_multistart_independent_audit.json').write_text(json.dumps(output, indent=2, allow_nan=False))
    ...
    assert integrity_passed, 'Export integrity or joint step limit audit failed; inspect written JSON.'
