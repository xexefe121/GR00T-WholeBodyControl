...
def main():
    output=Path('C:\\Users\\camer\\.codex\\visualizations\\2026\\09\\10\\01a08b79-d89d-7d13-bcf8-33de331c0024\\mpc_failed_replay_v1')
    ...
    inputs=[CASE/'report.json',CASE/'provenance.json',CASE/'trace.npz',CASE/'runner_snapshot.py',motion_path,
            ROOT.parent/'GR00T-WholeBodyControl'/MODEL,ROOT/PHYSICS,Path(__file__),output/'prior_renderer_snapshot.py']
            ...
            if path.exists():
                try:
                    with Image.open(path) as saved:
                        saved.load()
                        assert saved.size==(2*WIDTH,TOP+HEIGHT+BOTTOM)
    ...
    receipt=dict(kind='native323_failed_mpc_feedback_replay_vs_original_native_requested_reference',inputs=input_hashes,
                 ...
                 resumed_after_workspace_disk_full=True,reused_frame_indices=reused,
                 prior_frame_renderer_sha256=sha(output/'prior_renderer_snapshot.py'),
                 resume_changes='Root/output paths and verified-PNG reuse only; geometry, camera and frame labels unchanged',
