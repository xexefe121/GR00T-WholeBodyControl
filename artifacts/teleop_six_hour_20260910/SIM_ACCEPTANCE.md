...
September13 extension: every complete source motion must be followed by30
continuous seconds of quiet standing from the same controller. Assess all15000
physics samples, not only the final3 seconds. Existing quiet limits remain:
horizontal position error p95<=0.05m, heading p95<=5deg, root speed p95<=0.05m/s,
joint speed p95<=0.5rad/s and maximum<=2rad/s, and maximum tilt<=0.15rad.
Require the same30-second hold after input-loss braking, measured against its
declared standing goal, with explicit rearm still required. Full runs also need
declared initial-velocity perturbations including both+0.03 and-0.03m/s.
