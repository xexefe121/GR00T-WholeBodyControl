...
def run(args):
    ...
    if args.target_seed is not None:
        ...
        with np.load(seed_trace, allow_pickle=False) as seed:
            ...
            np.testing.assert_allclose(seed["qpos"][0], reference_states[10, :30], atol=1e-12, rtol=0)
            np.testing.assert_allclose(seed["qvel"][0], reference_states[10, 30:], atol=1e-12, rtol=0)
            ...
            np.testing.assert_allclose(
                seed["joint_error"], seed["qpos"][1:, 7:] - motion["joint_pos"][11:], atol=1e-12, rtol=0
            )
            np.testing.assert_allclose(
                seed["root_error"], seed["qpos"][1:, :3] - motion["body_pos_w"][11:, 0], atol=1e-12, rtol=0
            )
    ...
    while completed < requested:
        ...
        if seed_targets is not None:
            ...
            _, _, recorded_costs = planner.rollout(actual_state, recorded_targets)
            _, _, warm_costs = planner.rollout(actual_state, warm_targets)
