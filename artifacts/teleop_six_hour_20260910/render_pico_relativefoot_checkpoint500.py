...
def main():
    output=ARCHIVE/'visual_pico_relativefoot_checkpoint500_v3'
    ...
    metadata=json.loads(trace['checkpoint_metadata'].item())
    ...
    inputs=[REVIEW,CHECKPOINT,CASE/'request.json',motion_path,original_path,BASE_RENDERER,
    ...
    def combined(i):
        ...
        draw.text((WIDTH+16,10),'PHYSICAL MPC CHECKPOINT: PICO',font=f24,fill='#ffb65b')
        ...
        draw.text((16,TOP+HEIGHT+37),f'INCOMPLETE: 150 / 5780 source controls only | adapted root error {drift:.3f} m | leg RMSE {leg:.3f} rad',font=f20,fill='#ffb6a4')
        ending='CHECKPOINT END: remaining112.600s source + recovery absent from this view' if i==len(actual)-1 else 'Full7s entry preserved | cyan spheres: ORIGINAL29 world hand/head goals | fixed0.5m grid'
        draw.text((16,TOP+HEIGHT+69),ending,font=f18,fill='#c0ccd7')
        draw.text((16,TOP+HEIGHT+99),f'OFFLINE | >=740ms reference preview | planning p95 {planning_p95_seconds:.2f}s per100ms block | no realtime/full-source pass',font=f18,fill='#ffb6a4')
        return canvas
    contact=[0,250,350,400,450,len(actual)-1]
    ...
    receipt=dict(kind='native323_PICO_relative_foot_MPC_incomplete_checkpoint_vs_declared_v4_reference',inputs=input_hashes,
                 ...
                 clip='pico',recorded_controls=len(actual)-1,full_control_slots=int(np.sum(trace['physics_substeps']==10)),
                 final_control_substeps=10,recorded_physics_steps=int(steps[-1]),recorded_seconds=float(times[-1]),
                 initialization_seconds=source_start_s,actual_source_seconds=float(times[-1]-source_start_s),
                 full_requested_source_seconds=source['requested_controls']*.02,source_controls_shown=150,source_controls_requested=5780,
                 ...
                 declared_reference_packet_preview_seconds=.74,minimum_reference_preview_seconds=.74,raw_pose_derivative_preview_review_pending=True,
