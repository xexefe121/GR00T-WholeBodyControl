...
def run(args):
    ...
    for root in args.roots:
        for request_path in sorted(root.rglob("request.json")):
            ...
            audited_hash = audited.get(request["clip"])
            if base_hash is not None and audited_hash is not None and base_hash != audited_hash:
            ...
            receipt = dict(
                ...
                original_reference_hash_checked_against_derivative_audit=(
                    base_hash is not None and audited_hash is not None
                ),
                raw_pose_bound_evidence_scope=(
                    "common reference generator source; numeric audit additionally covers its listed clips"
                ),
                clip=request["clip"],
                source_clock_hz=50,
            ...
            output = request_path.parent / "preview_support_supplement_v2.json"
