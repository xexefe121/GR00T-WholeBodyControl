...
def run(args):
    ...
    if args.motion_override:
        motion, override = load_motion_override(args.motion_override, args.bundle, args.clip,
                                                native, contract, motion, timeline, manifest)
    ...
    spheres = [i for i in range(native.ngeom) if native.geom_bodyid[i] in feet
               and native.geom_type[i] == mujoco.mjtGeom.mjGEOM_SPHERE and native.geom_contype[i] != 0]
    ...
    for qposes in (reference_qpos, actual_qpos):
        ...
        for qpos in qposes:
            ...
            clearance.append((data.geom_xpos[spheres] - data.geom_xpos[floor]) @ normal - native.geom_size[spheres, 0])
    ...
    report = dict(
        kind="source_control_endpoints_native_geometry_decomposition", clip=args.clip,
        trace_sha256=sha256(args.trace), code_sha256=sha256(__file__), motion_override=override,
        mujoco=mujoco.__version__, source_controls=len(frames),
        floor_position_m=data.geom_xpos[floor].tolist(), floor_unit_normal=normal.tolist(),
        ...
        feet_root_relative_error_p95_m=np.percentile(np.linalg.norm(relative_foot_error, axis=-1), 95, axis=0).tolist(),
        feet_root_relative_abs_components_p95_m=np.percentile(np.abs(relative_foot_error), 95, axis=0).tolist(),
        reference_lowest_collision_clearance_min_p50_p95_max_m=np.percentile(clearances[0].min(axis=1), [0, 50, 95, 100]).tolist(),
        actual_lowest_collision_clearance_min_p50_p95_max_m=np.percentile(clearances[1].min(axis=1), [0, 50, 95, 100]).tolist(),
        reference_frames_all_spheres_above_10mm=int(np.sum(clearances[0].min(axis=1) > .01)),
        selected=[dict(source_frame=int(frames[i]), source_elapsed_seconds=float((frames[i] - phase["control_start"] - 10) * .02),
                       root_reference=reference[i, 0].tolist(), root_actual=actual[i, 0].tolist(),
        ...
        interpretation="Endpoint geometry on unchanged source clock. Clearance alone is not dynamic-feasibility proof; original29 intent is independently assessed by the referee.",
    )
