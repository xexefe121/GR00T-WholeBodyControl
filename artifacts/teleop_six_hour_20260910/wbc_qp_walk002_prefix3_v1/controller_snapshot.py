...
class Native23WBCQP:
    ...
    def point(self, data, body, local=None):
        ...
        return point, jac, rot, dot @ data.qvel, rot_dot @ data.qvel
    ...
    def contacts(self, data, frame):
        ...
        for i in range(data.ncon):
            ...
            if normal[2] < .99:
                continue
            j=np.zeros((3,29)); jd=np.zeros((3,29))
            mujoco.mj_jac(self.model,data,j,None,c.pos,foot)
            ...
            contacts.append(dict(index=i,foot=int(foot),pos=np.asarray(c.pos).copy(),jac=j,
                                 jdot_v=jd @ data.qvel,dist=float(c.dist),
    ...
    def solve(self, data, frame):
        ...
        eq_rhs=data.qfrc_passive-data.qfrc_bias
        tasks=[]; goals=[]
        ...
        for i,(body,local,weight,rweight) in enumerate([
            ...
            body_task(body,self.original["source_task_position_w"][frame,i],
                      ...
                      self.motion["body_ang_vel_w"][frame,body-1],self.task_acceleration[frame,i],
                      np.zeros(3),weight,rweight,local)
