"""Recheck exact recovery evidence after host Git verifies the pushed commit."""
...
assert audit["checked_files"] == 1373 and audit["mismatches"] == []
assert audit["regression_tests"] == 641 and audit["actual_all_physics_steps"] == 491405
inputs = dict(audit["inputs"])
for path, expected in inputs.items():
    if digest(path) != expected:
        raise ValueError(f"post-commit recovery source/evidence mismatch: {path}")
post = json.loads((HERE / "post_validation_report.json").read_text())
for row in post["stages"]:
    assert row["return_code"] == 0 and digest(row["log"]) == row["log_sha256"]
    inputs[row["log"]] = row["log_sha256"]
for path in (HERE / "post_validation_report.json", HERE / "integrity_report.json", Path(__file__),
             ROOT / "PROGRESS.md", ROOT / "docs/source/getting_started/g1_true23_frozen_platform_lora_comparison.md"):
    inputs[str(path)] = digest(path)
result = dict(
    kind="g1_true23_recovery_window_post_commit_v1", host_verified_local_and_remote_commit=commit,
