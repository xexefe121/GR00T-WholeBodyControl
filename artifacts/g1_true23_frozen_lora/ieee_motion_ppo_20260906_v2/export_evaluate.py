...
if len(experiment["stages"]) != 3 or any(row["return_code"] for row in experiment["stages"]):
...
for label, pair_directory, destination in (
    ("ieee", output, output / "recorded_evaluation"),
    ("tf32", previous / "model_100", HERE / "tf32_recorded_evaluation"),
):
    stage(
        f"record_inputs.{label}",
        "record_g1_true23_motion_ppo_inputs.py",
        [
            "--encoder-report",
            bind(pair_directory / "model_100.diagnostic.encoder.json"),
            "--decoder-report",
            bind(pair_directory / "model_100.diagnostic.decoder.json"),
            "--full-request-report",
            bind(OLD / "report.json"),
            "--stationary-report",
            bind(OLD / "stationary/report.json"),
