...
REFINED = ARTIFACTS / "contact_force_trajectory_20260906_v3"
ENVELOPE = ARTIFACTS / "contact_force_trajectory_envelope_20260906_v3"
...
def main():
    ...
    interrupted = read(ARTIFACTS / "contact_force_trajectory_20260906_v2/interrupted.json")
    assert interrupted["full_corpus_completed"] is False
    for snapshot in interrupted["source_snapshots"]:
        bind(ROOT / snapshot["archive"], snapshot["sha256"])
    started = read(REFINED / "started.json")
    bind_map(started["inputs"])
            ...
            for attempt in iteration["attempts"]:
                ...
                for trial in attempt["trials"]:
                    if trial["accepted"]:
                        ...
                        assert trial["frozen_patch_within_candidate_band"] is True
                        assert trial["frozen_patch_guard_consumed_m"] <= 0.00005 - 2e-7
