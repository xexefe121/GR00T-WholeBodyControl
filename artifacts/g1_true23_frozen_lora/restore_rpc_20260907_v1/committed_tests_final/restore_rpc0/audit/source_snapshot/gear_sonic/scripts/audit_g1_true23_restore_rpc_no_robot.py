...
SPANS = (
    (
        "inline constexpr int kMotionRestoreStableSamples =",
        "inline constexpr std::int64_t kMaximumFirstHoldWriteDelayNs =",
    ),
...
def has_physical_gate(source: str) -> bool:
    return "json PhysicalStateEvidence(" in source
...
SCENARIOS = {
    ...
    "select_error_but_applied": {
        "service": "",
        "fsm_id": 801,
        "select_result": 7002,
        "select_applies_despite_error": True,
    },
    ...
    "transient_disabled_during_kick": {
        "fsm_id": 0,
        "disabled_at_ns": 1_100_000_000,
        "disabled_until_ns": 1_300_000_000,
    },
}
...
def extract_runtime(source: str) -> tuple[str, list[dict]]:
    pieces = []
    bindings = []
    for start, end in SPANS if has_physical_gate(source) else LEGACY_SPANS:
...
def summarize(result: dict) -> dict:
    ...
    return {
        ...
        "stand_commands_while_mock_disabled": sum(
            row["args"][0] in (4, 801) and row["mock_disabled"] for row in fsm_calls
        ),
        "health_reads_observing_mock_disabled": sum(
            row["method"] == "LatestPhysicalState" and row["mock_disabled"] for row in calls
        ),
...
def run_audit(
    repository: Path, output: Path, compiler: str = "g++", *, runtime_source_root: Path | None = None
) -> dict:
    repository = repository.resolve()
    runtime_source_root = (runtime_source_root or repository).resolve()
    output = output.resolve()
    executable = shutil.which(compiler)
    if executable is None:
        raise FileNotFoundError(compiler)
    inputs = [(runtime_source_root / SOURCE, SOURCE)]
    inputs += [(runtime_source_root / RUNTIME / "include" / name, RUNTIME / "include" / name) for name in HEADERS]
    inputs += [(repository / path, path) for path in (FIXTURE, DRIVER)]
    inputs += [(Path(__file__).resolve(), Path(__file__).resolve().relative_to(repository))]
    source_hashes = {str(path): sha256(path) for path, _ in inputs}
    source = (runtime_source_root / SOURCE).read_text(encoding="utf-8")
    generated, spans = extract_runtime(source)
    ...
    (output / "generated_features.hpp").write_text(
        f"#define TRUE23_RESTORE_HAS_PHYSICAL {int(has_physical_gate(source))}\n", encoding="utf-8"
    )
    ...
    (output / "build.json").write_text(
        json.dumps(
            dict(command=command, returncode=build.returncode, stdout=build.stdout, stderr=build.stderr), indent=2
        ),
        encoding="utf-8",
    )
    if build.returncode:
    ...
    for name, config in SCENARIOS.items():
        run = subprocess.run(
            [str(binary), json.dumps(config)], capture_output=True, text=True, timeout=10, check=False
        )
        if run.returncode:
    ...
    report = {
        ...
        "runtime_has_physical_gate": has_physical_gate(source),
        "source_files": source_hashes,
        "extracted_spans": spans,
        "artifacts": {
            str(path.relative_to(output)): sha256(path) for path in sorted(output.rglob("*")) if path.is_file()
        },
        "scenarios": summaries,
        "scope": (
            "Actual runtime restore algorithm with fake RPC responses, fake LowState and virtual time; "
            "not firmware or plant simulation."
        ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--runtime-source-root", type=Path, help="Optional source/header snapshot; never a robot path"
    )
    args = parser.parse_args()
    report = run_audit(
        args.repository_root, args.output_directory, args.compiler, runtime_source_root=args.runtime_source_root
    )
    print(
        json.dumps(
            {
                ...
                for key in (
                    ...
                    "no_damp_rpc_contract_met_for_all_cases",
                    "deployment_ready",
                    "scenarios",
                )
            }
