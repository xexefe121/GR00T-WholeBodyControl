...
// Unitree locomotion FSM ids used by the internal-control recovery sequence.
inline constexpr int kUnitreeDampFsmId = 1;
inline constexpr int kUnitreeWalkrunFsmId = 500;
// Upright AI stand. This is the state the robot idles in before a session and
// the only one observed to actually straighten the legs after external control.
inline constexpr int kUnitreeAiStandFsmId = 801;
inline constexpr int kMotionRestoreStandModePolls = 20;
inline constexpr auto kMotionRestoreStandModePollInterval =
    std::chrono::milliseconds(250);
// Settle windows mirror the reviewed g1_restore_walkrun recovery timings.
inline constexpr auto kMotionRestoreDampSettle = std::chrono::milliseconds(400);
inline constexpr auto kMotionRestoreStandSettle =
    std::chrono::milliseconds(3000);
inline constexpr auto kMotionRestoreWalkrunSettle =
    std::chrono::milliseconds(350);
// Wait one second of polling after the handback RPC is accepted before
// concluding that internal control will not re-engage on its own.
// Consecutive causal-join failures tolerated while armed before treating the
// input as genuinely lost. At a 20 ms control period this stays well inside
// the policy-freshness watchdog, which remains the real backstop.
// Bounded wait for a fresh policy when preparing the pre-arm hold. 50 attempts
// at 10 ms is 0.5 s, far shorter than the state-gate timeout, and every attempt
// still enforces the same policy-freshness requirement.
inline constexpr int kPreArmHoldPrepareAttempts = 50;
inline constexpr auto kPreArmHoldPrepareRetryInterval =
    std::chrono::milliseconds(10);
inline constexpr int kArmedCausalJoinSkipLimit = 10;
inline constexpr int kMotionRestoreCheckModeAttempts = 20;
inline constexpr auto kMotionRestoreCheckModeRetryInterval =
    std::chrono::milliseconds(500);
inline constexpr int kMotionRestoreKickAfterPolls = 10;
inline constexpr int kMotionRestoreKickLimit = 2;
...
std::string Usage(std::string_view executable) {
  return
      ...
      " <causal-wireless:20..30|frozen-live:1..10|direct-dance:1..11>"
      " --gantry-authorize " +
...
Arguments ParseArguments(int argc, char** argv) {
  ...
  const int maximum_duration =
      direct_dance
          ? active::kMaximumDirectDancePostArmSeconds
  ...
  if (result.post_arm_duration_seconds < minimum_duration ||
    ...
    throw std::runtime_error(
        direct_dance
            ? "--post-arm-duration-seconds must be within dance-reviewed 1..11 range"
            : (result.frozen_lora_policy
...
json PhysicalStateEvidence(const std::optional<active::StateSample>& state,
                           std::int64_t now_ns) {
  if (!state.has_value()) {
    return {{"verified", false}, {"fresh", false},
            {"sampled_controlled_joints", 0}};
  }
  const auto health = active::EvaluatePhysicalMotorHealth(*state, now_ns);
  std::array<double, 23> q{};
  std::array<unsigned int, 23> modes{};
  std::array<std::uint32_t, 23> statuses{};
  for (std::size_t compact = 0; compact < 23; ++compact) {
    const auto slot = static_cast<std::size_t>(true23::kHardwareJointIds[compact]);
    q[compact] = state->q[slot];
    modes[compact] = state->motor_mode[slot];
    statuses[compact] = state->motor_status[slot];
  }
  return {{"verified", health.verified}, {"fresh", health.fresh},
          {"sampled_controlled_joints", 23},
          {"enabled_motor_count", health.enabled_motor_count},
          {"observed_latched_disabled_motor_count",
           health.observed_latched_disabled_motor_count},
          {"maximum_abs_tau_est_nm", health.maximum_abs_tau_est_nm},
          {"q_hardware_rad", q}, {"motor_modes_hardware", modes},
          {"motor_status_hardware", statuses},
          {"knee_q_rad", std::array<double, 2>{q[3], q[9]}},
          {"imu_rpy_rad", state->imu_rpy},
          {"mode_machine", state->mode_machine}, {"crc_valid", state->crc_valid},
          {"tick", state->tick},
          {"sample_check_monotonic_ns", now_ns},
...
class StateMonitor {
 public:
  ...
  void WithCore(const auto& function) const {
    std::lock_guard lock(mutex_);
    function(core_);
  }
  ...
  [[nodiscard]] int fault_joint() const {
    std::lock_guard lock(mutex_);
    return core_.fault_joint();
  }

  [[nodiscard]] double fault_value() const {
    std::lock_guard lock(mutex_);
    return core_.fault_value();
  }
 ...
 private:
  void OnMessage(const void* message) {
    ...
    for (std::size_t slot = 0; slot < active::kMotorSlotCount; ++slot) {
      ...
      sample.motor_mode[slot] = motor.mode();
      sample.motor_status[slot] = motor.motorstate();
    }
    std::copy_n(state.imu_state().rpy().begin(), 3, sample.imu_rpy.begin());
    // Keep observing even after the policy core faults: handback must verify
    // live motor telemetry, never the core's last pre-fault state.
    physical_state_ = sample;
  ...
  std::optional<active::StateSample> physical_state_;
...
void RestoreReleasedMotionModeAfterFailure(const ReleasedMotionMode& released,
...
ReleasedMotionMode ReleaseMotionModeAfterGate(const StateMonitor& monitor) {
  auto client =
  ...
  // Capture supported start states. Restoration always targets completed 801;
  // FSM 4 is only an intermediate posture, not successful finished handback.
  if (fsm_id != 500 && fsm_id != 801 &&
      fsm_id != active::kUnitreeLockedStandFsmId) {
    throw std::runtime_error(
        "pre-release locomotion FSM is not powered standing (expected 500, 801, "
        "or 4)");
  }
  const auto physical_state =
      PhysicalStateEvidence(monitor.LatestPhysicalState(), NowNs());
  if (!physical_state.value("verified", false)) {
    throw std::runtime_error("pre-release physical motor telemetry not verified");
  }
  ...
  active::PerformMotionReleaseTransaction(
      ...
      [&] { RestoreReleasedMotionModeAfterFailure(released, monitor); });
  return released;
}

struct MotionRestoreResult {
  int check_mode_retries = 0;
  int select_mode_attempts = 0;
  int internal_control_attempts = 0;
  int fsm_kick_attempts = 0;
  int poll_attempts = 0;
  int stable_samples = 0;
  std::string observed_form;
  std::string observed_name;
  int observed_fsm_id = -1;
  int observed_fsm_mode = -1;
  json physical_state = {{"verified", false}, {"fresh", false}};
};

// SwitchToInternalCtrl(WALKRUN) returns success but leaves this firmware in
// zero-torque FSM 0 after external LowCmd control. Unitree's own recovery path
// re-engages the motors with an explicit damp/stand FSM sequence, so drive the
// same sequence here once the handback RPC has been accepted and the FSM is
// observed to be non-standing. This is only ever issued against a positively
// observed non-standing FSM, so it cannot drop a robot that is already holding
// itself up.
bool KickInternalControlToStand(unitree::robot::g1::LocoClient& locomotion) {
  if (locomotion.SetFsmId(kUnitreeDampFsmId) != 0) {
    return false;
  }
  std::this_thread::sleep_for(kMotionRestoreDampSettle);
  // FSM 4 is a *crouched* stand: its stand-up transition ends with the knees
  // near 0.63 rad, not the ~0.29 rad of a finished stand. Wait for the
  // transition to report complete (fsm_mode returns to 0) before asking for
  // the upright AI stand, or the next command lands mid-transition.
  if (locomotion.SetFsmId(active::kUnitreeLockedStandFsmId) != 0) {
    return false;
  }
  std::this_thread::sleep_for(kMotionRestoreStandSettle);
  bool stand_transition_complete = false;
  for (int attempt = 0; attempt < kMotionRestoreStandModePolls; ++attempt) {
    int fsm_id = -1;
    int fsm_mode = -1;
    if (locomotion.GetFsmId(fsm_id) == 0 &&
        locomotion.GetFsmMode(fsm_mode) == 0 &&
        fsm_id == active::kUnitreeLockedStandFsmId && fsm_mode == 0) {
      stand_transition_complete = true;
      break;
    }
    std::this_thread::sleep_for(kMotionRestoreStandModePollInterval);
  }
  if (!stand_transition_complete) {
    return false;
  }
  // Drive the captured upright standing FSM. Observing the operator recover
  // this robot showed 4 -> 801 is what actually straightens the legs; walk/run
  // FSM 500 is never reached with the feet unloaded on a gantry, so asking for
  // 500 alone leaves the robot powered but crouched, which reads as limp.
  if (locomotion.SetFsmId(kUnitreeAiStandFsmId) != 0) {
    return false;
  }
  std::this_thread::sleep_for(kMotionRestoreStandSettle);
  // Deliberately end in the upright AI stand rather than walk/run FSM 500.
  // Handing back to 500 leaves the robot balance-armed with unloaded feet on a
  // gantry; observed behaviour is that it enters stepping (fsm_mode 1), then
  // drops to zero torque after some minutes and refuses further FSM commands.
  // Upright 801 held indefinitely across the same session. Command walk/run
  // explicitly when the robot is actually meant to walk.
  return true;
}

void RestoreMotionModeAfterNormalHold(
    const ReleasedMotionMode& released, MotionRestoreResult& result,
    const StateMonitor& monitor) {
  auto client =
      std::make_unique<unitree::robot::b2::MotionSwitcherClient>();
  client->SetTimeout(3.0F);
  ...
  std::string form;
  std::string name;
  // The motion switcher can be briefly unresponsive right after the writer
  // quiesces, especially following a fault-terminated session. Giving up on a
  // single failed CheckMode leaves the service released and the robot limp in
  // external-control limbo, needing hands-on recovery, so retry before failing.
  bool pre_restore_mode_read = false;
  for (int attempt = 0; attempt < kMotionRestoreCheckModeAttempts; ++attempt) {
    form.clear();
    name.clear();
    if (client->CheckMode(form, name) == 0) {
      pre_restore_mode_read = true;
      break;
    }
    result.check_mode_retries = attempt + 1;
    std::this_thread::sleep_for(kMotionRestoreCheckModeRetryInterval);
  }
  if (!pre_restore_mode_read) {
    throw std::runtime_error(
        "motion-mode pre-restore CheckMode RPC failed after bounded retries");
  }
  bool mode_selected = false;
  bool select_rpc_accepted = name == released.name;
  bool internal_control_accepted = false;
  int polls_since_handoff = 0;
  ...
  for (int attempt = 0; attempt < kMotionRestorePollLimit; ++attempt) {
    ...
    if (mode_read) {
      result.observed_form = form;
      result.observed_name = name;
    }
    if (mode_read && name == released.name) {
      ...
      if (!internal_control_accepted) {
        ...
        internal_control_accepted =
            locomotion.SwitchToInternalCtrl(
                unitree::robot::g1::InternalFsmMode::WALKRUN) == 0;
      }
      int fsm_id = -1;
      int fsm_mode = -1;
      const bool fsm_read = locomotion.GetFsmId(fsm_id) == 0 &&
                            locomotion.GetFsmMode(fsm_mode) == 0;
      if (fsm_read) {
        result.observed_fsm_id = fsm_id;
        result.observed_fsm_mode = fsm_mode;
      }
      const bool walkrun_restored =
          internal_control_accepted && fsm_read &&
          active::WalkrunMotionModeRestored(
              released.name, released.locomotion_fsm_id, name, fsm_id,
              fsm_mode);
      const auto physical_state = monitor.LatestPhysicalState();
      const auto physical_check_ns = NowNs();
      result.physical_state = PhysicalStateEvidence(physical_state, physical_check_ns);
      if (stability_gate.ObservePhysical(walkrun_restored, physical_state,
                                        physical_check_ns)) {
      ...
      result.stable_samples = stability_gate.consecutive_samples();
      ++polls_since_handoff;
      // A failed health check at 801 is not permission to command damp. Only
      // an observed zero-torque/damped FSM uses the full recovery sequence;
      // completed intermediate stand or walk-ready asks directly for 801.
      if (internal_control_accepted && fsm_read && !walkrun_restored &&
          polls_since_handoff >= kMotionRestoreKickAfterPolls &&
          result.fsm_kick_attempts < kMotionRestoreKickLimit) {
        if (fsm_id == 0 || fsm_id == kUnitreeDampFsmId) {
          ++result.fsm_kick_attempts;
          KickInternalControlToStand(locomotion);
          polls_since_handoff = 0;
        } else if ((fsm_id == active::kUnitreeLockedStandFsmId && fsm_mode == 0) ||
                   fsm_id == kUnitreeWalkrunFsmId) {
          ++result.fsm_kick_attempts;
          locomotion.SetFsmId(kUnitreeAiStandFsmId);
          polls_since_handoff = 0;
        }
      }
    } else {
  ...
  // Report the failure that actually occurred. SelectMode can return non-zero
  // while the motion service still reaches the captured mode, so a stale
  // select_rpc_accepted must never mask a later, more specific failure.
  if (!mode_selected) {
    throw std::runtime_error(
        select_rpc_accepted
  ...
  if (!internal_control_accepted) {
    throw std::runtime_error(
        "SwitchToInternalCtrl(WALKRUN) RPC failed after motion-mode selection");
  }
  throw std::runtime_error(
      "internal control did not prove completed FSM 801 with fresh enabled "
      "motor telemetry for 100 advancing samples");
}

void RestoreReleasedMotionModeAfterFailure(const ReleasedMotionMode& released,
                                          const StateMonitor& monitor) {
  // Run has not enabled its LowCmd writer yet. Recover here before propagating
  // the startup exception, retaining the captured service even when the
  // post-release CheckMode RPC failed after a successful ReleaseMode.
  MotionRestoreResult result;
  monitor.WithCore([](active::GantrySafetyCore& core) { core.BeginMotionHandoff(); });
  RestoreMotionModeAfterNormalHold(released, result, monitor);
  std::cerr << "[motion-release-recovery] "
            << json{{"restored_fsm_id", result.observed_fsm_id},
                    {"physical_state", result.physical_state}}.dump() << '\n';
}
...
active::ActiveArtifactBinding ParseFrozenLoraActivePromotion(
  ...
  if (envelope.value("action_fraction", 0.0) !=
      ...
      envelope.value("maximum_post_arm_duration_seconds", 0) !=
          (direct_dance ? active::kMaximumDirectDancePostArmSeconds : 10) ||
...
int Run(const Arguments& arguments) {
  ...
  std::atomic<std::uint64_t> armed_causal_join_skips{0};
  ...
  // Measured joint excursion while armed. A session can otherwise satisfy every
  // gate - frames written, timings met, mode restored - on a robot whose motors
  // are disabled and which never moves, because nothing verifies actual motion.
  std::array<double, 23> armed_q_minimum{};
  std::array<double, 23> armed_q_maximum{};
  std::array<double, 23> armed_q_first{};
  std::array<double, 23> armed_q_last{};
  bool armed_q_seen = false;
  ...
  MotionRestoreResult motion_restore_result;
  ...
  std::jthread inference_thread([&](std::stop_token stop_token) {
    try {
      ...
      int consecutive_causal_join_failures = 0;
      while (!stop_token.stop_requested() && !stop_threads.load()) {
        ...
        if (causal_reference_artifact) {
          ...
          if (attempt.status != CausalJoinStatus::Ready ||
            ...
            // Armed: a brief coverage gap is not a fault. Skip the frame and
            // hold the last commanded target rather than tearing down a live
            // session. This cannot mask a real input loss - the safety core's
            // policy-freshness watchdog still latches if no fresh policy
            // arrives, which is the correct guard. Only a sustained run of
            // failures falls through to the fault path below.
            if (++consecutive_causal_join_failures <=
                kArmedCausalJoinSkipLimit) {
              armed_causal_join_skips.fetch_add(1, std::memory_order_relaxed);
              continue;
            }
            inference_error =
          ...
          consecutive_causal_join_failures = 0;
  ...
  std::jthread writer_thread([&](std::stop_token stop_token) {
    ...
    try {
      ...
      while (!stop_token.stop_requested() && !stop_threads.load()) {
        ...
        bool armed_state_for_excursion = false;
        monitor.WithCore([&](active::GantrySafetyCore& value) {
          ...
          armed_state_for_excursion =
              armed_before_build && value.armed() &&
              value.fault() == active::Fault::None;
          policy_command =
              armed_before_build && value.armed() &&
        ...
        // Record measured joint travel while the policy is actually armed, so
        // the terminal record can prove the robot moved rather than only that
        // commands were emitted.
        if (armed_state_for_excursion && command_state.has_value()) {
          for (std::size_t compact = 0; compact < 23; ++compact) {
            const auto slot = static_cast<std::size_t>(
                gear_sonic::true23::kHardwareJointIds[compact]);
            const double q = command_state->q[slot];
            if (!armed_q_seen) {
              armed_q_minimum[compact] = q;
              armed_q_maximum[compact] = q;
              armed_q_first[compact] = q;
            } else {
              armed_q_minimum[compact] =
                  std::min(armed_q_minimum[compact], q);
              armed_q_maximum[compact] =
                  std::max(armed_q_maximum[compact], q);
            }
            armed_q_last[compact] = q;
          }
          armed_q_seen = true;
        }
  ...
  // PreparePreArmHold requires a policy fresher than kPolicyFreshnessNs. Under
  // host load the policy can age past that in the gap between the readiness
  // flag being set and this call, so a single attempt aborts the session before
  // motion-mode release - observed costing roughly half of all launches, always
  // failing closed. Retry while the prewarm preconditions still hold: this
  // waits for the next fresh policy rather than treating a scheduling hiccup as
  // a fault. Nothing is relaxed - the same freshness requirement must be met,
  // and stop/fault/signal still break out immediately.
  bool hold_prepared = false;
  int hold_prepare_attempts = 0;
  for (int attempt = 0; attempt < kPreArmHoldPrepareAttempts; ++attempt) {
    hold_prepare_attempts = attempt + 1;
    const auto hold_prepare_ns = NowNs();
    monitor.WithCore([&](active::GantrySafetyCore& value) {
      hold_prepared = value.PreparePreArmHold(hold_prepare_ns);
    });
    if (hold_prepared || stop_threads.load() || g_stop_requested != 0 ||
        monitor.fault() != active::Fault::None) {
      break;
    }
    std::this_thread::sleep_for(kPreArmHoldPrepareRetryInterval);
  }
  ...
  execution_evidence.AppendEvent(
      ...
      {{"sampled_hardware_joints", 23},
       {"physical_state", PhysicalStateEvidence(monitor.LatestPhysicalState(), NowNs())},
       {"pre_arm_hold_prepare_attempts", hold_prepare_attempts},
  ...
  const auto released_motion_mode = ReleaseMotionModeAfterGate(monitor);
  ...
  execution_evidence.AppendEvent(
      ...
      {{"post_release_mode_name_empty", true},
       ...
       {"physical_state", released_motion_mode.physical_state},
       {"pre_release_lowcmd_writes", 0},
    ...
    if (emergency_mode_restore_requested.load(std::memory_order_acquire) &&
      ...
      try {
        ...
        execution_evidence.AppendEvent(
            ...
            {{"restored_form", motion_restore_result.observed_form},
             {"physical_state", motion_restore_result.physical_state},
             {"restored_name", motion_restore_result.observed_name},
             {"restored_fsm_id", motion_restore_result.observed_fsm_id},
             {"restored_fsm_mode", motion_restore_result.observed_fsm_mode},
             ...
             {"lowcmd_publisher_closed_before_select",
              ...
              {"select_mode_attempts",
               motion_restore_result.select_mode_attempts},
              {"internal_control_handoff", "walkrun"},
              {"internal_control_attempts",
               motion_restore_result.internal_control_attempts},
              {"internal_control_fsm_kick_attempts",
               motion_restore_result.fsm_kick_attempts},
              {"restore_poll_attempts",
               motion_restore_result.poll_attempts},
              {"stable_restore_samples",
               motion_restore_result.stable_samples},
  ...
  // Largest measured travel of any controlled joint while armed. Near zero
  // means the robot did not move, whatever the other gates report.
  double measured_armed_excursion_rad = 0.0;
  // Net bilateral knee flexion from first to last armed sample. Large travel
  // alone cannot distinguish dancing from collapse. This endpoint screen is
  // specific to the reviewed standing dance, not arbitrary teleop trajectories.
  double armed_knee_flexion_delta_rad = 0.0;
  if (armed_q_seen) {
    for (std::size_t compact = 0; compact < 23; ++compact) {
      measured_armed_excursion_rad = std::max(
          measured_armed_excursion_rad,
          armed_q_maximum[compact] - armed_q_minimum[compact]);
    }
    armed_knee_flexion_delta_rad = std::min(
        armed_q_last[3] - armed_q_first[3],
        armed_q_last[9] - armed_q_first[9]);
  }
  const bool enough_policy_commands =
      policy_command_frames >= static_cast<std::uint64_t>(
                                   active::kMinimumPromotedShadowActionFrames);
  const bool direct_dance_terminal_posture_screen_passed =
      active::DirectDanceTerminalPostureAcceptable(
          armed_q_seen, armed_q_first, armed_q_last);
  const bool normal_return_completed =
      normal_return_hold_frames.load() >=
          static_cast<std::uint64_t>(kNormalReturnHoldCycles) &&
      writer_quiesced_before_restore && motion_mode_restored.load() &&
      lowcmd_publisher_closed_before_restore &&
      motion_restore_result.internal_control_attempts >= 1 &&
      motion_restore_result.stable_samples >= kMotionRestoreStableSamples &&
      motion_restore_result.physical_state.value("verified", false) &&
  ...
  const bool passed =
      ...
      (!direct_dance || active::DirectDanceMotionObserved(measured_armed_excursion_rad)) &&
      (!direct_dance || direct_dance_terminal_posture_screen_passed) &&
  ...
  json terminal = {
      ...
      {"restored_motion_mode_form", motion_restore_result.observed_form},
      {"restored_motion_mode_name", motion_restore_result.observed_name},
      {"restored_locomotion_fsm_id", motion_restore_result.observed_fsm_id},
      {"restored_locomotion_fsm_mode", motion_restore_result.observed_fsm_mode},
      {"pre_release_physical_state", released_motion_mode.physical_state},
      {"restored_physical_state", motion_restore_result.physical_state},
      {"maximum_target_delta_from_state_rad",
       maximum_target_delta_from_state_rad},
      ...
      {"maximum_abs_feedforward_tau_nm",
       maximum_abs_feedforward_tau_nm},
      {"measured_armed_excursion_rad", measured_armed_excursion_rad},
      {"armed_knee_flexion_delta_rad", armed_knee_flexion_delta_rad},
      {"armed_q_seen", armed_q_seen},
      {"armed_q_first_rad", armed_q_first},
      {"armed_q_last_rad", armed_q_last},
      {"direct_dance_terminal_posture_screen_passed",
       direct_dance_terminal_posture_screen_passed},
      {"maximum_direct_dance_terminal_knee_flexion_rad",
       active::kMaximumDirectDanceTerminalKneeFlexionRad},
      {"final_fault", std::string(active::FaultName(fault))},
      {"fault_joint", monitor.fault_joint()},
      // Units depend on the fault: Nm for joint_effort_limit, rad for
      // joint_position_limit.
      {"fault_value", monitor.fault_value()},
      ...
      {"restore_internal_control_attempts",
       motion_restore_result.internal_control_attempts},
      {"restore_internal_control_fsm_kick_attempts",
       motion_restore_result.fsm_kick_attempts},
      {"restore_check_mode_retries", motion_restore_result.check_mode_retries},
      {"armed_causal_join_skips", armed_causal_join_skips.load()},
      {"restore_poll_attempts", motion_restore_result.poll_attempts},
      {"stable_restore_samples", motion_restore_result.stable_samples},
