...
// Stage-one amplitude/speed envelope. 0.10 / 0.25 rad/s throttles the saved
// choreography to an imperceptible creep: the rate limiter saturates every
// 2 ms step, so the robot never reaches the first pose. Stepped up under
// explicit operator authorization, on a gantry, one increment at a time.
inline constexpr double kStageOneActionFraction = 0.60;
inline constexpr double kStageOneTargetRateRadPerSecond = 5.0;
...
// Reviewed direct-dance armed window. The saved SONIC routine is 535 packets
// at a 20 ms control period (~10.7 s), so this covers one complete routine.
inline constexpr int kMaximumDirectDancePostArmSeconds = 11;

// Consecutive over-limit measured-torque samples required before the effort
// fault latches. tau_est carries brief single-sample transients during fast
// motion; a genuine overload is sustained and still latches within a few
// samples, while a one-sample spike is ignored.
inline constexpr int kEffortLimitDebounceSamples = 3;

// Overload protection. kHardwareEffortLimitNm is the URDF *continuous* rating;
// a whole-body dynamic routine produces brief reaction peaks above it that are
// not a fault (measured: tapering the ankle's own command 30% moved its peak
// torque by 0.3 Nm, proving the load is body-reaction, not command).
//
// A flat instantaneous limit cannot distinguish a 6 ms dynamic peak from a
// sustained stall, so protect on both magnitude and duration:
//   * anything above kEffortAbsoluteCeilingMultiplier x continuous latches
//     immediately - that is a genuine fault, not choreography;
//   * between continuous and that ceiling, integrate the excess. Sustained
//     overload exhausts the budget and latches; brief peaks are absorbed and
//     the budget recovers while the joint is back within its rating.
// Measured-position tolerance. A joint resting against its mechanical stop
// reads a hair past the URDF bound (observed L ankle roll: limit 0.26180,
// measured 0.26186 - 0.003 deg over) because the stop, not the encoder,
// defines end of travel. With the ankle-roll command zeroed the joint still
// reached that stop, proving it is driven there by body reaction rather than
// by choreography, so tapering cannot avoid it. Tolerate touching the stop;
// still fault on genuinely exceeding it.
inline constexpr double kMeasuredPositionToleranceRad = 0.02;

// These existing thresholds remain unvalidated against a manufacturer torque
// envelope. A training effort limit is not evidence of hardware capability.
inline constexpr double kEffortAbsoluteCeilingMultiplier = 1.2;
inline constexpr double kEffortOverloadBudgetNmSeconds = 4.0;
inline constexpr double kEffortOverloadRecoveryNmPerSecond = 8.0;
inline constexpr double kEffortOverloadMaximumStepSeconds = 0.05;

// Existing all-one joint taper, applied after the pinned safe-target transform.
// Previous nonuniform ankle tapers were removed during the gantry experiments.
// Neither uniform nor per-joint attenuation guarantees balance: both change
// the closed-loop dynamics learned by SONIC. The current 0.60 fraction, gains
// and slew limiter fail weight-bearing MuJoCo replay (PROGRESS.md, Sep 5).
// These constants are not a qualified full-body deployment profile. Preserve
// the failed experiment for diagnosis; do not infer readiness from a shadow
// pass or fix it by increasing action/effort limits on the physical robot.
inline constexpr std::array<double, 23> kStageOneJointAmplitudeScale = {
...
// Compact hardware order. Absolute reject limits, not clipping envelopes.
//
// The existing ankle 50 Nm values came from training configs, replacing the
// earlier 35 Nm table. This is an UNVERIFIED hardware assumption: simulation
// limits and a parallel mechanism do not establish the motor/joint torque
// envelope. Preserve this pending explicit hardware validation; do not treat
// passing the software checks as validation of these physical limits.
...
inline std::string_view FaultName(Fault fault) {
  switch (fault) {
    ...
    case Fault::MotorDisabled: return "motor_disabled";
...
struct StateSample {
  ...
  std::array<std::uint8_t, kMotorSlotCount> motor_mode{};
  std::array<std::uint32_t, kMotorSlotCount> motor_status{};
  std::array<double, 3> imu_rpy{};
};

// Bit 30 accompanied the observed latched-disabled incidents. Do not infer a
// meaning for other firmware status bits. Raw status is retained in evidence.
inline constexpr std::uint32_t kObservedLatchedDisabledStatusMask = 1U << 30U;

struct PhysicalMotorHealth {
  bool verified = false;
  bool fresh = false;
  int enabled_motor_count = 0;
  int observed_latched_disabled_motor_count = 0;
  double maximum_abs_tau_est_nm = 0.0;
};

inline PhysicalMotorHealth EvaluatePhysicalMotorHealth(
    const StateSample& sample, std::int64_t now_monotonic_ns) {
  PhysicalMotorHealth result;
  const auto age = now_monotonic_ns - sample.received_monotonic_ns;
  result.fresh = sample.received_monotonic_ns > 0 &&
                 age >= -kFutureClockToleranceNs && age <= kStateFreshnessNs;
  bool finite = std::all_of(sample.imu_rpy.begin(), sample.imu_rpy.end(),
                            [](double value) { return std::isfinite(value); });
  for (const int included : kHardwareJointIds) {
    const auto slot = static_cast<std::size_t>(included);
    result.enabled_motor_count += sample.motor_mode[slot] == 1 ? 1 : 0;
    result.observed_latched_disabled_motor_count +=
        (sample.motor_status[slot] & kObservedLatchedDisabledStatusMask) != 0
            ? 1 : 0;
    finite = finite && std::isfinite(sample.q[slot]) &&
             std::isfinite(sample.dq[slot]) && std::isfinite(sample.tau_est[slot]);
    result.maximum_abs_tau_est_nm =
        std::max(result.maximum_abs_tau_est_nm, std::abs(sample.tau_est[slot]));
  }
  // This proves fresh, enabled telemetry only. It does not prove balance,
  // weight bearing, correct posture, or the physical torque envelope.
  result.verified = result.fresh && sample.crc_valid &&
                    sample.mode_machine == kRequiredModeMachine && finite &&
                    result.enabled_motor_count == 23 &&
                    result.observed_latched_disabled_motor_count == 0 &&
                    result.maximum_abs_tau_est_nm > 0.0;
  return result;
}

inline constexpr double kMinimumDirectDanceArmedExcursionRad = 0.03;
inline bool DirectDanceMotionObserved(double excursion_rad) {
  return std::isfinite(excursion_rad) &&
         excursion_rad >= kMinimumDirectDanceArmedExcursionRad;
}

// Evidence-only conservative screen for the reviewed standing dance. This is
// not a general fall detector, crouch restriction, or a hardware safety rating.
// It cannot authorize motion and never changes the recovery sequence.
inline constexpr double kMaximumDirectDanceTerminalKneeFlexionRad = 0.5;
inline bool DirectDanceTerminalPostureAcceptable(
    bool seen, const std::array<double, 23>& first,
    const std::array<double, 23>& last) {
  if (!seen) return false;
  for (std::size_t joint = 0; joint < 23; ++joint) {
    if (!std::isfinite(first[joint]) || !std::isfinite(last[joint])) return false;
  }
  return std::min(last[3] - first[3], last[9] - first[9]) <=
         kMaximumDirectDanceTerminalKneeFlexionRad;
}

// Retain recovery ownership across both ReleaseMode and its post-release
// verification. An RPC error does not prove the service stayed selected.
// Callers capture the original service/FSM before entering this transaction.
template <typename Release, typename Verify, typename Recover>
void PerformMotionReleaseTransaction(Release release, Verify verify,
                                     Recover recover) {
  try {
    release();
    verify();
  } catch (...) {
    const auto release_error = std::current_exception();
    try {
      recover();
    } catch (const std::exception& error) {
      throw std::runtime_error(
          std::string("motion release failed and recovery failed: ") + error.what());
    }
...
// FSM 4 is an intermediate crouched stand. The observed gantry recovery ends
// at completed upright AI stand 801; neither id alone proves motor health.
inline constexpr int kUnitreeLockedStandFsmId = 4;
inline constexpr int kUnitreeUprightAiStandFsmId = 801;
...
inline bool WalkrunMotionModeRestored(
  ...
  return !expected_service.empty() && observed_service == expected_service &&
         (captured_fsm_id == 500 || captured_fsm_id == 801 ||
          captured_fsm_id == kUnitreeLockedStandFsmId) &&
         observed_fsm_mode == 0 &&
         observed_fsm_id == kUnitreeUprightAiStandFsmId;
}
...
class MotionRestoreStabilityGate {
 public:
  ...
  bool ObservePhysical(bool completed_fsm,
                       const std::optional<StateSample>& state,
                       std::int64_t now_ns) {
    const bool advancing = state.has_value() &&
        (!last_physical_tick_.has_value() ||
         static_cast<std::int32_t>(state->tick - *last_physical_tick_) > 0);
    if (state.has_value()) {
      last_physical_tick_ = state->tick;
    }
    return Observe(completed_fsm && advancing &&
                   EvaluatePhysicalMotorHealth(*state, now_ns).verified);
  }
 ...
 private:
  ...
  std::optional<std::uint32_t> last_physical_tick_;
};
...
class GantrySafetyCore {
 public:
  ...
  void ObserveOperator(const OperatorInput& input,
    ...
    if (input.arm_edge) {
      ...
      if (fault_ != Fault::None || !operator_arming_enabled_ ||
          ...
          !input.deadman_held || !policy_.has_value() || !state_.has_value() ||
          !EvaluatePhysicalMotorHealth(*state_, now_monotonic_ns).verified) {
        return;
      }
  ...
  // Runtime calls only after its writer has quiesced. Unitree's subsequent
  // service/FSM transitions may temporarily disable motors; restoration has
  // its own fresh physical-evidence gate and cannot reuse the policy gate.
  void BeginMotionHandoff() {
    motion_handoff_started_ = true;
    ...
    for (std::size_t compact = 0; compact < 23; ++compact) {
      const double raw_target =
          ...
          static_cast<double>(hardware_action[compact]) *
              ...
              kStageOneJointAmplitudeScale[compact];
      if (!std::isfinite(raw_target) ||
          ...
          raw_target > live::kHardwareUpperLimit[compact] -
                           kTargetLimitMarginRad) {
        // Record which joint saturated and by how much, so a raised stage-one
        // amplitude can be diagnosed instead of guessed at.
        fault_joint_ = static_cast<int>(compact);
        fault_value_ = raw_target;
  ...
  [[nodiscard]] int fault_joint() const { return fault_joint_; }
  [[nodiscard]] double fault_value() const { return fault_value_; }
 ...
 private:
  ...
  bool ValidateTelemetry(const StateSample& sample) {
    ...
    if (!motion_handoff_started_ && (armed_ || pre_arm_hold_prepared_) &&
        !EvaluatePhysicalMotorHealth(sample, sample.received_monotonic_ns)
             .verified) {
      Latch(Fault::MotorDisabled);
      return false;
    }
    // Timestep for the overload integrator, clamped so a stalled or jumped
    // clock cannot inflate or freeze the budget.
    const double effort_dt_seconds =
        last_effort_sample_ns_ > 0
            ? std::clamp(
                  static_cast<double>(sample.received_monotonic_ns -
                                      last_effort_sample_ns_) /
                      1e9,
                  0.0, kEffortOverloadMaximumStepSeconds)
            : 0.0;
    last_effort_sample_ns_ = sample.received_monotonic_ns;
    for (std::size_t compact = 0; compact < 23; ++compact) {
      const auto slot =
          static_cast<std::size_t>(kHardwareJointIds[compact]);
      if (sample.q[slot] < live::kHardwareLowerLimit[compact] -
                               kMeasuredPositionToleranceRad ||
          sample.q[slot] > live::kHardwareUpperLimit[compact] +
                               kMeasuredPositionToleranceRad) {
        // Measured joint left its hardware range. Record which one and the
        // observed angle; this is distinct from the target-side check above.
        fault_joint_ = static_cast<int>(compact);
        fault_value_ = sample.q[slot];
        Latch(Fault::JointPositionLimit);
        return false;
      }
      ...
      // Measured tau_est is noisy and shows brief single-sample transients
      // during fast motion that momentarily exceed the URDF continuous-effort
      // limit without being a real overload (observed sustained dance torques
      // stay under ~12% of the limit). Debounce: latch only after several
      // consecutive over-limit samples on the same joint. A genuine overload
      // is sustained and still latches within a few milliseconds, while a
      // one-sample spike no longer tears down the session. Any in-limit sample
      // resets the streak.
      const double measured_effort = std::abs(sample.tau_est[slot]);
      const double continuous_limit = kHardwareEffortLimitNm[compact];
      // Hard ceiling: never tolerated, however brief.
      if (measured_effort >
          kEffortAbsoluteCeilingMultiplier * continuous_limit) {
        fault_joint_ = static_cast<int>(compact);
        fault_value_ = measured_effort;
        Latch(Fault::JointEffortLimit);
        return false;
      }
      if (measured_effort > continuous_limit) {
        if (++effort_over_streak_[compact] >= kEffortLimitDebounceSamples) {
          effort_overload_[compact] +=
              (measured_effort - continuous_limit) * effort_dt_seconds;
          if (effort_overload_[compact] > kEffortOverloadBudgetNmSeconds) {
            fault_joint_ = static_cast<int>(compact);
            fault_value_ = measured_effort;
            Latch(Fault::JointEffortLimit);
            return false;
        ...
        effort_over_streak_[compact] = 0;
        effort_overload_[compact] = std::max(
            0.0, effort_overload_[compact] -
                     kEffortOverloadRecoveryNmPerSecond * effort_dt_seconds);
      }
    }
    return true;
  }
  ...
  std::array<int, 23> effort_over_streak_{};
  std::array<double, 23> effort_overload_{};
  std::int64_t last_effort_sample_ns_ = 0;
  ...
  bool motion_handoff_started_ = false;
};
