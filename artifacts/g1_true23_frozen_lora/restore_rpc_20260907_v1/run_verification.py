...
def run(name, command, expected, extra_env=None):
    ...
    with (HERE / f"{name}.log").open("x") as stream:
    ...
    stages.append(dict(name=name, command=command, returncode=result.returncode,
                       ...
                       log_sha256=sha(HERE / f"{name}.log")))
...
def main():
    ...
    run("current_contract_tests", [sys.executable, "-m", "pytest", "-q", "-x", "--tb=short",
        "--basetemp=" + str(HERE / "current_tests_contract_v2"), TEST], 0)
    run("committed_contract_tests", [sys.executable, "-m", "pytest", "-q", "-x", "--tb=short",
        "--basetemp=" + str(HERE / "committed_tests_contract_v2"), TEST], 0,
    ...
    run("qualifier_before", [sys.executable, str(HERE / "qualifier_before.py"),
        "--repository-root", str(ROOT), "--output", str(HERE / "qualification_before.json")], 0)
    run("qualifier_after", [sys.executable, "-m", "gear_sonic.scripts.qualify_g1_true23_active_lifecycle_no_robot",
        "--repository-root", str(ROOT), "--output", str(HERE / "qualification_after.json")], 2)
    before = json.loads((HERE / "qualification_before.json").read_text())
    after = json.loads((HERE / "qualification_after.json").read_text())
