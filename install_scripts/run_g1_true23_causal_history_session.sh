...
run_dir="/root/g1_true23_runs/causal_history_recovery_stand_transition_dance_v1"
...
mkdir -p "${log_dir}"
log_path="${log_dir}/causal_history_v1_$(printf '%04d' "${start_update}")_$(printf '%04d' "${stop_update}").log"
if [[ -e "${log_path}" ]]; then
...
exec /root/.venvs/g1_true23_mjlab/bin/python -u \
  -m gear_sonic.scripts.train_g1_23dof_mjlab_causal_history \
  train \
  --run-dir "${run_dir}" \
  --num-envs 128 \
  --iterations 5001 \
