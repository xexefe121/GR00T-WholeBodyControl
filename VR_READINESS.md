...
CURRENT (2026-09-15): **simulated teleop bring-up on this workstation is done and
passing.** The operator reported teleop would not run at all here. Seven distinct
environment faults were found and fixed; none of them were controller problems.
A full 23-DoF teleop session now runs from a recorded PICO source, across the
real localhost ZMQ transport, through the frozen paired policy, into MuJoCo, for
the complete 535-control clip, with the safety fallback never engaging:
`passed: true`, `completed_live_transitions: 535/535`, `maximum_reference_age_ns`
28.25 ms against the 100 ms gate, `fallback_transition_count: 0`,
`live_transport_proven: true`.

Scope, unchanged by this: `live_headset_source_proven: false` — no PICO has ever
connected, and `pico_manager_thread_server.py` cannot be driven from recordings
because it hard-rejects non-XRT input. `tracking_fidelity_qualified: false` and
`deployment_ready: false`. No DDS, no robot channel, no robot command, no
checkpoint promoted, no gain or limit changed. `free_standing_authorized: false`
still stands.

Which policy pair matters here: the clean `causal_model_250` + `steptouch
alpha010` pair falls at transition 189 of 535 (`base_tilt` fallback at 72, gate
failure at `height=0.410189, tilt=0.573767`). The paired `breadth25` pair
completes the clip upright. Both were run on the same plant and input bundle.

The seven environment faults, in the order they blocked: `Z:` full at 447 MB so
no venv could be built in the repo; no teleop venv existed for this worktree;
`xrobotoolkit_sdk` only ever built against the sibling checkout; `artifacts/g1_true23`
absent from this worktree so every clean run died in `sha256_file` before physics;
`onnxruntime` missing; `E:` never mounted inside WSL, so runs writing "to E:"
went into the WSL root filesystem invisibly — **earlier sessions were affected
too, see `/mnt/e_stray`**; and a ZMQ consumer poll timeout shorter than the
publisher warm-up, which made the intuitive fix (raising warm-up) worse.

Full detail, exact commands and evidence:
`artifacts/teleop_sim_bringup_20260915/BRINGUP.md`. Reports under
`E:\codex-artifacts	eleop_sim_bringup_20260915\`.
