...
It adapts all nine decoder affines alongside the root branch while preserving
source references, observation timing, rewards, physical limits and physics.
The critic retains the previously tested unclipped value loss.
"""

from pathlib import Path

from gear_sonic.scripts import train_g1_true23_original_intent as intent
from gear_sonic.trl.mjlab.native23_decoder_lora_actor import ARCHITECTURE, TRAINABLE
...
def validate_frozen_recipe(args):
    ...
    if args.optimizer_profile != "feedback_priority" or args.ppo_auxiliary_objective != "none":
        raise ValueError("pose-LoRA recipe requires fixed root-priority rates and no PPO auxiliary loss")
    if not 1 <= args.iterations <= 100 or not 1 <= args.session_updates <= 100:
        raise ValueError("pose-LoRA fresh experiment is capped at100 updates")
...
def main(argv=None):
    ...
    def feedback(args, curriculum):
        ...
        result.update(
            ...
            checkpoint_pattern="decoder_lora_model_N.pt",
            decoder_lora_training=training_contract(),
        )
        return result
    ...
    def dispatch(remaining):
        ...
        def install_hooks(args, inputs, curriculum, precision, guard):
            ...
            native23_root_feedback_runner.Native23RootFeedbackRunner = Native23DecoderLoraRunner
            try:
            ...
            def agent():
                ...
                cfg.actor.class_name = (
                    "gear_sonic.trl.mjlab.native23_decoder_lora_actor:True23DecoderLoraActorModel"
                )
                cfg.experiment_name = "sonic_native23_decoder_lora_original_intent_simulation"
                if cfg.algorithm.use_clipped_value_loss is not True or cfg.algorithm.clip_param != 0.2:
                    raise ValueError("unexpected predecessor critic/actor clipping configuration")
            ...
            def resolved(*values, **kwargs):
                ...
                result["native23_decoder_lora"] = training_contract()
            ...
            def sources():
                ...
                roots = [Path(__file__)] + [
                    ...
                    for name in (
                        "gear_sonic/trl/mjlab/native23_decoder_lora_actor.py",
                        "gear_sonic/trl/mjlab/native23_decoder_lora_runner.py",
                    )
                ]
                result.update(
