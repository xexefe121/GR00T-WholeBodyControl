"""Verify frozen decoder/encoder and actual root-only PPO updates, not readiness."""
...
from gear_sonic.trl.mjlab.native23_frozen_decoder_runner import validate_frozen_decoder_checkpoint
from gear_sonic.utils.g1_true23_frozen_decoder_checkpoint import construct_actor, validate_semantics
...
def audit(initial_path, trained_path, warm_start_path, source_checkpoint_path):
    ...
    from gear_sonic.utils import g1_true23_frozen_decoder_checkpoint as reader
    ...
    validate_frozen_decoder_checkpoint(initial, actor=actor, lineage=initial["lineage"])
    validate_frozen_decoder_checkpoint(trained, actor=actor, lineage=initial["lineage"], minimum_update_count=1)
    ...
    checks.update(
        ...
        == ["root_conditioner", "exploration", "critic"],
        only_root_and_exploration_actor_trainable={n for n, p in actor.named_parameters() if p.requires_grad}
        == {"root_conditioner.weight", "distribution.raw_std"},
    ...
    return dict(
        kind="native23_frozen_decoder_original_intent_actual_update_audit_v1",
        checks=checks,
