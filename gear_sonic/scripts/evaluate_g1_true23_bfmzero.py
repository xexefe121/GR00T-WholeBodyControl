...
def run(args):
    ...
    for control in range(requested):
        ...
        for key, value in values.items():
            traces[key].append(value)
        if abs(float(data.time) - (control + 1) * physics.decimation * model.opt.timestep) > 1e-8 or np.any(data.warning.number):
            failure = dict(kind="engine_warning_or_clock_reset", control=control,
    ...
    result = dict(clip=args.clip, completed=completed, requested=requested, available=available, failure=failure,
                  ...
                  simulated_seconds=float(data.time), engine_warning_counts=data.warning.number.tolist(),
