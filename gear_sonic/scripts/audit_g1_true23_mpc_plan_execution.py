...
def run(args):
    request_path, trace_path, plans_path = [
        args.run / name for name in ("request.json", "trace.npz", "plans.json")
    ]
    ...
    for name, mask in (
        ...
        summaries[name] = dict(
            ...
            root_position_error_p95_max_m=np.percentile(
                np.linalg.norm(current[:, :3], axis=1), [95, 100]
            ).tolist(),
            root_rotation_error_p95_max_rad=np.percentile(
                np.linalg.norm(current[:, 3:6], axis=1), [95, 100]
            ).tolist(),
            generalized_velocity_error_p95_max=np.percentile(
                np.max(np.abs(current[:, 29:]), axis=1), [95, 100]
            ).tolist(),
