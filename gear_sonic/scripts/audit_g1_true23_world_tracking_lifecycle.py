...
def audit_world_measurements(trace, completed, span_start, source_qpos29):
    ...
    shapes = {
        "desired_position_w": (completed, 3), "measured_position_w": (completed, 3),
        "error_m": (completed,), "failure": (completed,), "reference_q0": (completed,),
    ...
    if any(trace["world_tracking_" + key].dtype != np.int64
           for key in ("reference_q0", "common_step_counter")):
        raise ValueError("world reference and control counters must be int64")
    np.testing.assert_array_equal(
        trace["world_tracking_reference_q0"], span_start + 9 + np.arange(completed)
    )
...
def main():
    ...
    if (report["kind"] != "native23_world_tracking_full_lifecycle_diagnostic_v1"
    ...
    for record in report["cases"]:
        ...
        cases.append(dict(name=name, trace_audit=row, world_tracking=world,
    ...
    result = dict(kind="native23_world_tracking_saved_trace_audit_v1", inputs=inputs, cases=cases,
