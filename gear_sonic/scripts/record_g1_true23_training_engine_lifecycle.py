"""Uninterrupted SIM-only SONIC control in the actual MJLab/Warp plant.

Mirror the CPU diagnostic's source timing, startup history, action codec and
range guard. Four unique motions occupy32 identical-origin lanes (eight exact
replicas each); replicas are checked, not counted as additional evidence.
Training episode predicates are recorded without automatic reset or promotion.
Stopped/finished lanes are torque-disabled and excluded from subsequent traces.
...
def verify_replicas(value, case_index):
    group = np.asarray(value)[case_index * REPLICAS : (case_index + 1) * REPLICAS]
    if len(group) != REPLICAS or not np.isfinite(group).all():
        raise ValueError("invalid simulation replica group")
    if not np.array_equal(group, np.repeat(group[:1], REPLICAS, axis=0)):
        raise ValueError("identical-origin replicas diverged; single-row trace is insufficient")
    return group[0].copy()
...
def main(precision, precision_guard):
    ...
    cfg = make_causal_multimotion_v14_env_cfg(motion_file=str(output / "motion.npz"), num_envs=32, play=False)
    ...
    def lanes():
        return torch.as_tensor(np.repeat([case["active"] for case in cases], REPLICAS), device=env.device)
    ...
    try:
        ...
        env.sim.reset()
        action.reset()
        q = np.repeat(np.stack([c["cpu"]["qpos"][0] for c in cases]), REPLICAS, axis=0).astype(np.float32)
        v = np.repeat(np.stack([c["cpu"]["qvel"][0] for c in cases]), REPLICAS, axis=0).astype(np.float32)
        ...
        for index, case in enumerate(cases):
            group = slice(index * REPLICAS, (index + 1) * REPLICAS)
        ...
        for frame in range(1, 10):
            ...
            for name, (a, b) in HISTORY_TERMS.items():
                width = (b - a) // 10
                values = np.repeat(np.stack([row[a : a + width] for row in per_case]), REPLICAS, axis=0)
        ...
        write(
            ...
            dict(
                ...
                four_unique_trajectories_with_eight_verified_exact_replicas=True,
        ...
        for control in range(max(c["span"]["timeline"]["total_requested_controls"] for c in cases)):
            ...
            with torch.inference_mode():
                ...
                raw = actor(actor_obs).cpu().numpy()
                token = actor.core.encode(actor_obs["tokenizer"]).cpu().numpy()
            numpy_obs = {key: value.detach().cpu().numpy().copy() for key, value in actor_obs.items()}
            applied = np.zeros((32, 23), dtype=np.float32)
            for index, case in enumerate(cases):
                if not case["active"]:
                    continue
                if control != case["completed"]:
                    raise ValueError("active motion skipped a control")
                state_q, state_v = verify_replicas(qpos, index), verify_replicas(qvel, index)
                values = {key: verify_replicas(value, index) for key, value in numpy_obs.items()}
                proposal = verify_replicas(raw, index)
                encoded = verify_replicas(token, index)
            ...
            for index, case in enumerate(cases):
                if case["active"]:
                    case["trace"]["target23"].append(verify_replicas(targets, index))
            if not any(c["active"] for c in cases):
                break
            for substep in range(10):
                ...
                for index, case in enumerate(cases):
                    if not case["active"]:
                        continue
                    first = index * REPLICAS
                    sq, sv = verify_replicas(after_q, index), verify_replicas(after_v, index)
                    for key, value in (
                        ("physics_pre_qpos", verify_replicas(before_q, index)),
                        ("physics_pre_qvel", verify_replicas(before_v, index)),
                        ("physics_post_qpos", sq),
                        ("physics_post_qvel", sv),
                        ("requested_torque23", verify_replicas(requested, index)),
                        ("applied_torque23", verify_replicas(torque, index)),
                    ...
                    if (
                        ...
                        action.invalid_actuation[first : first + REPLICAS] = True
            env.episode_length_buf += lanes().long()
            env.common_step_counter += 1
            # Observe the unchanged training predicates at their original stale
            # pre-final-forward phase. They are not silently reset or called passes.
            ...
            for index, case in enumerate(cases):
                ...
                first = index * REPLICAS
                case["completed"] += 1
                case["trace"]["qpos"].append(verify_replicas(after_q, index))
                case["trace"]["qvel"].append(verify_replicas(after_v, index))
    ...
    report = dict(
        kind="native23_training_engine_full_lifecycle_diagnostic_v1",
        cases=reports,
        inputs=inputs,
        ...
        exact_replicas_per_motion=8,
        replicas_are_not_independent_robustness_trials=True,
        hardware_authorized=False,
        deployment_ready=False,
