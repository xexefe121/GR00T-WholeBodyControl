...
from gear_sonic.trl.mjlab import native23_compact_actor
...
def agent_config():
    ...
    config.actor = RslRlModelCfg(
        class_name="gear_sonic.trl.mjlab.native23_compact_actor:CompactNative23Actor",
...
def audit_reward(row, rewards, stored, value, gamma):
    base = row["weighted_base_components"].sum(-1)
    expected = (
        row["base_reward"] + row["shaping_reward"] + row["world_quality_bonus"] + row["foot_precision_bonus"]
    )
    errors = {
        "base": float((base - row["base_reward"]).abs().max()),
        "returned": float((expected - rewards.cpu()).abs().max()),
        "stored": float((stored.cpu() - rewards.cpu() - gamma * value.cpu() * row["timeouts"]).abs().max()),
    }
    if max(errors.values()) > 3e-5:
            ...
            with (args.output / "metrics.jsonl").open("x") as metrics_stream:
                for update in range(1, updates + 1):
                    ...
                    metric = dict(
                        ...
                        learning_rate=alg.learning_rate,
                        action_std_mean=float(actor.distribution.bounded_std.mean().cpu()),
