"""Restore force/contact feasibility over every supplied native23 motion frame.
...
from gear_sonic.utils import g1_true23_force_restoration, g1_true23_force_trajectory, g1_true23_reference_support
...
def bind_identity(identities, path):
    path = Path(path).resolve(strict=True)
    actual = geometry.sha256(path)
    if str(path) in identities and identities[str(path)] != actual:
        raise ValueError("force refinement must not replace a previously pinned source identity")
...
def validate_manifest(manifest):
    if manifest.get("kind") != "g1_true23_stance_candidate_manifest_v1" or any(
        ...
        raise ValueError("force refinement needs an unaccepted full stance-candidate manifest")
    motions = manifest.get("motions")
    if not isinstance(motions, list) or not motions or any(not isinstance(row, dict) for row in motions):
        raise ValueError("force refinement requires a nonempty motion corpus")
    names = [row.get("name") for row in motions]
    if any(
        ...
        raise ValueError("force refinement motion names must be unique safe labels")
    return motions
...
def main():
    ...
    parser.add_argument("--qp-maximum-iterations", type=int, default=30000)
    ...
    for path in [
        ...
        *[
            ...
            for module in (g1_true23_force_restoration, g1_true23_force_trajectory, g1_true23_reference_support)
        ],
    ...
    summary = {
        "kind": "g1_true23_complete_force_refinement_v1",
        "argv": sys.argv,
