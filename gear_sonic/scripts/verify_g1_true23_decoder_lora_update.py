...
from gear_sonic.trl.mjlab.native23_decoder_lora_runner import validate_decoder_lora_checkpoint
from gear_sonic.utils.g1_true23_decoder_lora_checkpoint import construct_actor, validate_semantics
from gear_sonic.utils.g1_true23_generalist_corpus import sha256_file


def summarize_state_changes(before, after, exploration):
    pose_keys = {"pose_lora_a", "pose_lora_b"} | {
        f"decoder_lora_{factor}.{i}" for factor in ("a", "b") for i in range(8)
    }
    ...
    result["checks"].update(
        ...
        initial_all_tail_b_exact_zero=all(
            not bool(torch.count_nonzero(before[f"decoder_lora_b.{i}"])) for i in range(8)
        ),
...
def audit(initial_path, trained_path, warm_start_path, source_checkpoint_path):
    ...
    from gear_sonic.utils import g1_true23_decoder_lora_checkpoint as reader
    ...
    validate_decoder_lora_checkpoint(initial, actor=actor, lineage=initial["lineage"])
    validate_decoder_lora_checkpoint(trained, actor=actor, lineage=initial["lineage"], minimum_update_count=1)
    ...
    checks.update(
        ...
        == ["root_conditioner", "decoder_adapters", "exploration", "critic"],
    ...
    return dict(
        kind="native23_decoder_lora_original_intent_actual_update_audit_v1",
        checks=checks,
