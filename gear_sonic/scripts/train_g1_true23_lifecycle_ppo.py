...
def read_initial_actor(path):
    """Read the checked format before comparing it with the actual source core."""
    peek = torch.load(path, map_location="cpu", weights_only=True)
    if not isinstance(peek, dict) or not isinstance(peek.get("adapter_contract"), dict):
        raise ValueError("initial actor lacks a frozen-platform contract")
    return load_frozen_platform_lora_checkpoint(path, expected_contract=peek["adapter_contract"])


def bind_standing_inputs(initialization, retention, bind):
    """Bind descriptor file fields; neither descriptor is a path-to-hash map."""
    bind(initialization["report_path"], initialization["report_sha256"])
    bind(initialization["adapter_path"], initialization["adapter_file_sha256"])
    bind(retention["teacher_report_path"], retention["teacher_report_sha256"])
    for episode in retention["episodes"]:
...
def main(argv=None):
    ...
    prior = read_initial_actor(source_checkpoint)
    ...
    bind_standing_inputs(standing_descriptor, standing_inputs, bind)
    ...
    with ieee_training_precision() as (precision, guard):
        ...
        contract = dict(
            ...
            standing_initialization=standing_descriptor,
            standing_retention=standing_inputs,
