...
def run(args):
    ...
    request=dict(kind="native23_whole_body_inverse_dynamics_qp_prototype",clip=args.clip,probe=args.probe,
                 ...
                 dynamics="M*qacc + bias - passive - measured_DOF_friction = joint_torque + sum(J_contact.T*force)",
    ...
    for control in range(requested):
        ...
        for substep in range(10):
            ...
            if failure: break
        if failure and substep == 0 and torque is None:
            break
        mujoco.mj_forward(model,data)
    ...
    attempted_controls=completed
    completed=len(trace["physics_torque"])//10
    ...
    result=dict(probe_completed=completed==requested and failure is None,completed_controls=completed,
                attempted_controls=attempted_controls,partial_physics_substeps=len(trace["physics_torque"])%10,
