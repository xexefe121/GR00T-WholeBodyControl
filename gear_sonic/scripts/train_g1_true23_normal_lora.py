"""Bounded fresh normal-core native23 SIM training, never a low-latency resume."""
...
def run(args):
    ...
    if not 1 <= args.updates <= 100 or not 4 <= args.num_envs <= 128 or not 8 <= args.rollout_steps <= 16:
        raise ValueError("normal fresh run limited to100updates,4..128envs,8..16steps")
    ...
    gradients, inputs = [], []
    outcome = dict(
        kind="native23_normal_actual_simulator_training_smoke_v1",
        completed=False,
        simulator_updates=0,
    ...
    try:
        with ieee_training_precision() as (precision, guard):
            ...
            resolved.update(
                schema="g1_true23_normal_core_received_920ms_training_v1",
...
def main():
    ...
    for name in ("spec", "spans", "sim-config", "warm-start", "source-checkpoint", "preflight", "run-dir"):
