"""Bounded existing-PICO normal SONIC training; simulation only, never actuation.

Uses the already verified normal actor/reward/physics implementation. Dataset
ownership, longer fixed training budget, milestones and sampled input telemetry
are explicit differences from the historical PICO-excluded100-update launcher.
"""
...
def run(args):
    ...
    if args.run_dir.exists():
        raise FileExistsError("existing-PICO trainer refuses overwrite or resume")
    ...
    if args.updates == 500:
        ...
        if (
            ...
            or smoke.get("simulator_updates") != 2
    ...
    recipe.write_runtime(
        ...
        dict(
            ...
            milestones=[0, 2] if args.updates == 2 else [0, 100, 250, 500],
            **FLAGS,
        ),
    )
    ...
    outcome = dict(
        kind="native23_existing_pico_bounded_training_v1",
        completed=False,
        simulator_updates=0,
        bank_sha256=sha256_file(args.bank_report),
        pico_used_for_training=True,
    ...
    try:
        with recipe.ieee_training_precision() as (precision, guard):
            resolved = recipe._resolved_training_config(
                agent_cfg=agent,
                mode="smoke" if args.updates == 2 else "bounded_existing_pico_training",
            ...
            resolved.update(
                schema="g1_true23_existing_pico_normal_training_v1",
                task="Unitree-G1-Native23-ExistingPICO-OriginalSONIC-Received920ms",
                normal_adaptation=recipe.normal_training_contract(),
            ...
            print("PICO: constructing unchanged nominal native23 physics", flush=True)
            ...
            runner = recipe.Native23NormalLoraRunner(
                wrapped,
                asdict(agent),
                str(args.run_dir),
                "cuda:0",
            ...
            with torch.no_grad():
                ...
                semantic = fixed["tokenizer"][..., 1:]
                base = actor.core.decoder(
                    torch.cat(
                        (actor.core.encode(semantic), actor.core.codec.encode_proprioception(fixed["policy"])), -1
                    )
                )
                if not torch.equal(mean_before, base.cpu()):
                    raise ValueError("new-bank initial actor differs from frozen normal base")
            ...
            with (args.run_dir / "training.log").open("x") as log_stream:
                ...
                def log(**values):
                    ...
                    if completed in (100, 250):
                        runner._save_numbered_checkpoint()
                    if completed == 1 or completed % 25 == 0 or completed == args.updates:
                        progress = dict(
                ...
                print(f"PICO: training {args.updates} continuous updates", flush=True)
            ...
            final = runner._numbered_checkpoint_path(args.updates)
            recipe.validate_normal_checkpoint(
