"""Separate retarget-reference residual PPO; explicit v2 bank, fresh base parity.

Current implemented action family is unchanged frozen BFM plus 23 bounded
residual targets. Direct arm control/leg-only outputs/barrier torque are not
silently added. A candidate using those requires its own exact action contract
and CPU/GPU parity before this prepared path is launched.
"""
...
from gear_sonic.utils.g1_true23_intent_retarget_bank import (
    KIND as BANK_KIND, RESIDUAL_KIND as KIND, validate_bank, install_reference_loader,
)
...
from gear_sonic.utils.g1_true23_bfm_residual import residual_contract as base_residual_contract
...
def residual_contract():
    return dict(**{**base_residual_contract(), "kind": KIND}, native_reference_pairing=BANK_KIND,
                action_family="frozen_bfm_all23_plus_bounded_residual23",
                direct_arm_control=False, extra_barrier_torque=False,
                original29_tasks_and_vr21_immutable=True)
...
def run(args):
    ...
    bank_path = args.bank_report
    bank = validate_bank(bank_path)
    reference_adapter = install_reference_loader(bank_path)
    ...
    manifest = dict(kind=KIND, contract=residual_contract(), bank_sha256=sha256_file(bank_path),
                    ...
                    source_files={str(p): sha256_file(p) for p in (
                        ...
                        ROOT / "gear_sonic/trl/mjlab/g1_true23_bfm_residual_actor.py",
                        ROOT / "gear_sonic/utils/g1_true23_intent_retarget_bank.py")}, **FLAGS)
    ...
    try:
        with recipe.ieee_training_precision() as (precision, guard):
            ...
            if args.resume:
                ...
                if saved["manifest"]["bank_sha256"] != manifest["bank_sha256"]:
                    raise ValueError("v2 resume requires the exact same retarget reference bank")
...
def main():
    ...
    parser.add_argument("--describe", action="store_true", help="validate bank and print prepared contract; no simulator or training")
    ...
    if args.describe:
        ...
        print(json.dumps(dict(contract=residual_contract(), bank_sha256=sha256_file(args.bank_report),
                              files=bank["files"], training_launched=False), indent=2, allow_nan=False))
    else:
        run(args)
