"""Trace immutable full-support task-space PPO models 0 and 1."""
...
from gear_sonic.utils.g1_true23_sonic_task_space_ppo_full_support_checkpoint_trace import (
    OUTPUT_FILENAME,
...
def _bind_local_runtime_sources(root: Path) -> dict[str, str]:
    ...
    if Path(src.__file__).resolve() != expected_src or Path(mjlab_module.__file__).resolve() != expected_mjlab:
        raise RuntimeError("full-support checkpoint trace local runtime binding mismatch")
...
def _configure_simulator_runtime(root: Path) -> dict[str, str]:
    if os.environ.get("WORLD_SIZE", "1") != "1":
        raise RuntimeError("full-support checkpoint trace requires WORLD_SIZE=1")
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("full-support checkpoint trace requires fixed visible CUDA device 0")
...
def _expected_output(repository_root: Path) -> Path:
    contract = load_trace_contract(repository_root)
...
def _validate_output(repository_root: Path, parent_run_dir: Path, output: Path) -> Path:
    ...
    if (
        ...
        raise ValueError(f"full-support checkpoint trace output must be exact sibling {expected}")
    return requested
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "preflight":
        report = trace_preflight(root, args.parent_run_dir)
        print(json.dumps(report, indent=2, sort_keys=True))  # noqa: T201
        return 0 if report.get("ready") is True else 1
    try:
        output = _validate_output(root, args.parent_run_dir, args.output_json)
    except Exception as error:
        print(f"full-support checkpoint trace output rejected: {type(error).__name__}: {error}")  # noqa: T201
        return 1
    if os.path.lexists(output):
        print(f"full-support checkpoint trace refused overwrite: {output}")  # noqa: T201
        return 1
    failure_stage = "input_resolution"
    provenance_snapshot_sha256: str | None = None
    try:
        inputs = resolve_trace_inputs(root, args.parent_run_dir)
        ...
        try:
            ...
            print(  # noqa: T201
                "full-support checkpoint trace failure persistence failed: "
                f"{type(persistence_error).__name__}: {persistence_error}"
            )
        print(f"full-support checkpoint trace failed: {type(error).__name__}: {error}")  # noqa: T201
        return 1
