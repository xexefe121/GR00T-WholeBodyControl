...
It adds pose-conditioned low-rank adaptation to the root branch without changing
source references, observation timing, rewards, physical limits or physics.
"""

from pathlib import Path

from gear_sonic.scripts import train_g1_true23_original_intent as intent
from gear_sonic.trl.mjlab.native23_pose_lora_actor import ARCHITECTURE, TRAINABLE
...
def main(argv=None):
    ...
    def feedback(args, curriculum):
        ...
        result.update(
            ...
            checkpoint_pattern="pose_lora_model_N.pt",
            pose_lora_training=training_contract(),
        )
        return result
    ...
    def dispatch(remaining):
        ...
        def install_hooks(args, inputs, curriculum, precision, guard):
            ...
            native23_root_feedback_runner.Native23RootFeedbackRunner = Native23PoseLoraRunner
            try:
            ...
            def agent():
                cfg = previous_agent()
                cfg.actor.class_name = "gear_sonic.trl.mjlab.native23_pose_lora_actor:True23PoseLoraActorModel"
                cfg.experiment_name = "sonic_native23_pose_lora_original_intent_simulation"
                return cfg
            ...
            def resolved(*values, **kwargs):
                ...
                result["native23_pose_lora"] = training_contract()
