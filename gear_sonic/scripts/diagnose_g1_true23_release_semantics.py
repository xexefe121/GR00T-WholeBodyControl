"""Simulation-only SONIC joint-surgery/reference ablation; no weight updates.
...
Default weights are the untouched row-trimmed release. An explicitly provided
root-feedback manifest selects existing trained weights without relabeling them.
"""
...
MODES = (
    "causal_past",
    "released_future_preview",
    "causal_past_virtual_source",
    "released_future_preview_virtual_source",
)
CASES = {
    "nominal": (),
...
class ReferenceAblation:
    def __init__(
        self, motion, mode, virtual_vr=None, action_convention="native_tanh", case="nominal", root_feedback=False
    ):
        if mode not in MODES:
            raise ValueError("unknown reference ablation")
        self.motion, self.mode = motion, mode
        if action_convention not in ("native_tanh", "released_bounded_linear", SOURCE_ACTION_CONVENTION):
            raise ValueError("unknown action convention")
        self.action_convention = action_convention
        if case not in CASES:
            raise ValueError("unknown declared force case")
        self.case, self.pulses = case, CASES[case]
        self.root_adapter = RootFeedbackRuntimeAdapter(self.pulses) if root_feedback else None
        self.virtual_vr = virtual_vr
        if mode.endswith("virtual_source") and (
            virtual_vr is None or virtual_vr.shape != (len(motion["joint_pos"]), 21)
        ):
    ...
    def transform_history(self, history):
        if self.action_convention == SOURCE_ACTION_CONVENTION:
    ...
    def infer(self, policy, encoder, history, *, control_index, **state):
        ...
        elif self.action_convention == SOURCE_ACTION_CONVENTION:
            raw, projection = source_scaled_precompensation(raw)
            self.projection_delta_rad.append(projection)
        return raw, decoder
    ...
    def external_force_world(self, step):
        ...
        for pulse in self.pulses:
            if pulse.start_substep <= step < pulse.stop_substep:
                force[:] = pulse.force_world_n
        return force

    def contract(self):
        return dict(
            kind="native23_sonic_simulation_reference_ablation_v2",
            mode=self.mode,
            reference_anchor="buffered_q9",
            decoder994_history_unchanged=self.action_convention != SOURCE_ACTION_CONVENTION,
...
def main(argv=None):
    ...
    parser.add_argument(
        "--action-convention",
        choices=("native_tanh", "released_bounded_linear", SOURCE_ACTION_CONVENTION),
        default="native_tanh",
    )
    parser.add_argument(
        "--profiles", choices=PROFILES, nargs="+", default=["native_model"]
    )
    parser.add_argument("--cases", choices=tuple(CASES), nargs="+", default=["nominal"])
    parser.add_argument(
        "--root-feedback-manifest",
        type=Path,
        help="Optional existing trained root policy; explicitly not zero-training release",
    )
    ...
    inputs = {
        ...
        for path in (
            ...
            root / "gear_sonic/utils/g1_true23_generalist_benchmark.py",
            root / "gear_sonic/scripts/audit_g1_true23_source_action_codec.py",
    ...
    if args.root_feedback_manifest is not None:
        inputs[str(args.root_feedback_manifest.resolve(strict=True))] = sha256_file(args.root_feedback_manifest)
    write_json(output / "started.json", dict(inputs=inputs, timeline=timeline, **FLAGS))
    rows = []
    with ieee_training_precision() as (precision, guard):
        core = None
        if args.root_feedback_manifest is None:
            core = (
            ...
            identity = core.artifact_contract()
            ...
            manifest = json.loads(args.root_feedback_manifest.read_text())
            ...
            if compatibility is not None:
                ...
                expected = release_compatibility_contract(
                    sha256_file(assets / "gear_sonic/data/robots/g1/g1_29dof.xml"), args.action_convention
                )
                if (
                    compatibility != expected
                    or args.modes != ["causal_past_virtual_source"]
                    or args.action_convention != compatibility["action_convention"]
                ):
                    raise ValueError(
                        "trained release-compatible actor requires matching causal geometry/action runtime"
                    )
            policy, identity = load_root_feedback_pair(
                args.root_feedback_manifest, session_options=options, expected_release_compatibility=compatibility
            )
            for path in identity["component_paths"].values():
                inputs[path] = sha256_file(Path(path))
            print("Loaded existing trained root-feedback policy; no weight changes in this ablation.", flush=True)
        for profile, mode, case in (
            (profile, mode, case) for profile in args.profiles for mode in args.modes for case in args.cases
        ):
            guard()
            adapter = ReferenceAblation(
                motion, mode, virtual_vr, args.action_convention, case, root_feedback=core is None
            )
            ...
            if adapter.root_adapter is not None:
            ...
            row = dict(
                ...
                linear_projection_joint_fraction=float(np.mean(np.abs(projection) > 1e-6))
                if projection.size
                else 0.0,
            )
        ...
        write_json(
            output / "report.json",
            dict(
                kind="native23_sonic_semantic_ablation_v2",
                rows=rows,
                inputs=inputs,
                policy=identity,
                timeline=timeline,
                precision=precision,
                learned_parameter_updates_in_this_experiment=0,
                untouched_row_trimmed_release_used=core is not None,
                paired_initial_states_and_model_equal=True,
