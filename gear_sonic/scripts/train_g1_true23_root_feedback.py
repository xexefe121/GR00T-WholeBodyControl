...
def feedback_training_contract(args, curriculum):
    ...
    return {
        ...
        "experiment_class": "local_regression_only" if args.mode == "regression" else args.mode,
...
def main(argv=None):
    ...
    inputs = generalist.validate_training_inputs(
        ...
        smoke_only=args.mode in {"smoke", "preflight", "regression"},
    )
    inputs["experiment_class"] = "local_regression_only" if args.mode == "regression" else args.mode
