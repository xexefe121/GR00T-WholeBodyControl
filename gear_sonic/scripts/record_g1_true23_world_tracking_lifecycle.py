"""Uninterrupted SIM-only world-tracking candidate in the actual MJLab/Warp plant.
...
def verified_update_checkpoint(run, update):
    ...
    if (
        update.get("kind") != "native23_world_tracking_actual_update_audit_v1"
        or update.get("passed") is not True
        or update.get("completed_updates") != 100
        or update.get("actual_transitions") != 204800
        or update.get("initial_actor_and_critic_equal_matched_bounded_predecessor") is not True
    ...
    path = (Path(run) / "train/checkpoints/world_tracking_model_100.pt").resolve()
...
def captured_world_record(env):
    ...
    if (not capture or len(capture) != env.common_step_counter
    ...
    return {
        ...
        for key, value in capture[-1].items() if key != "common_step_counter"
    }
...
def main(precision, precision_guard):
    ...
    report = dict(
        kind="native23_world_tracking_full_lifecycle_diagnostic_v1",
        additional_training_failure=world_tracking.termination_contract(),
