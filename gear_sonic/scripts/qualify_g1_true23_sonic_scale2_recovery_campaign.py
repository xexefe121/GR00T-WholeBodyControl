"""Run q250 scale-2 mixed-controller recovery qualification campaign."""
...
from gear_sonic.utils.g1_true23_sonic_scale2_recovery_qualification_campaign import (
    CampaignRequest,
    preflight,
    run_campaign,
...
def _parser() -> argparse.ArgumentParser:
    ...
    for name in ("preflight", "qualify"):
        ...
        command.add_argument(
            "--output",
            type=Path,
            default=Path("artifacts/g1_true23/g1_true23_sonic_scale2_recovery_campaign_q250_v1.json"),
        )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if os.path.lexists(request.output_path):
        raise FileExistsError(f"scale2 campaign output exists: {request.output_path}")
    try:
        report = run_campaign(request, progress=_progress)
    except Exception as error:
        report = {
            "schema_version": 1,
            "kind": "g1_true23_sonic_scale2_recovery_qualification_campaign_failure_v1",
            "campaign_qualified": False,
            "error": {
                "type": type(error).__name__,
                "detail_sha256": __import__("hashlib").sha256(str(error).encode()).hexdigest(),
            },
        ...
        descriptor = os.open(request.output_path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o644)
