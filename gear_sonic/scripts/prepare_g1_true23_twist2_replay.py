...
class NumpyOnlyUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        ...
        if module in ("numpy.core.multiarray", "numpy._core.multiarray") and name in ("_reconstruct", "scalar"):
            # The recordings were pickled under the NumPy 2.x module layout
            # (numpy._core.multiarray); NumPy 1.x exposes numpy.core.multiarray.
            # Resolve whichever this interpreter provides, still restricted to
            # the two multiarray callables above.
            try:
                from numpy._core import multiarray as _multiarray
            except ImportError:
                from numpy.core import multiarray as _multiarray
            return getattr(_multiarray, name)
