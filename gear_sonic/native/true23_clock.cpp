...
struct Observation {
  ...
  std::atomic<int> id{-1}, fault{0};
  std::atomic<int64_t> published{0};
  // Legacy382 prefix; coherent sensor57 and exact last-applied23 extension.
  std::atomic<double> values[462];
  Observation() {for(auto& x:values)x.store(0.,std::memory_order_relaxed);}
};
struct Result {
  std::atomic<unsigned> version{0};
  std::atomic<int> id{-1};
  std::atomic<int64_t> start{0}, finish{0};
  std::atomic<double> target[23];
  Result() {for(auto& x:target)x.store(0.,std::memory_order_relaxed);}
};
static_assert(std::atomic<double>::is_always_lock_free,"Shared snapshots require lock-free doubles");
static_assert(std::atomic<int64_t>::is_always_lock_free,"Shared snapshots require lock-free timestamps");
struct Shared {
...
void clock_wait_until(int64_t deadline) { wait_ns(deadline); }
void clock_set_limit_brake(int enabled) { limit_brake=enabled!=0; }
size_t clock_shared_size() { return sizeof(Shared); }
int clock_snapshot_protocol_version() { return 1; }
int clock_sensor_observation_width() { return 462; }
void clock_initialize(void *p) { new(p) Shared(); }
void clock_stop(void *p) {
  auto *s=static_cast<Shared*>(p);s->stop.store(1);
  syscall(SYS_futex,&s->generation,FUTEX_WAKE,1,nullptr,nullptr,0);
}
void clock_latch_fault(void *p) { static_cast<Shared*>(p)->fault.store(1); }
int clock_wait_observation(void *p,int last,int64_t timeout_ns) {
  auto *s=static_cast<Shared*>(p);
  if(s->stop.load())return 0;
  int seen=s->generation.load(std::memory_order_acquire);
  if(seen>last+1)return 1;
  timeout_ns=std::clamp(timeout_ns,int64_t(1),int64_t(20000000));
  timespec timeout{0,timeout_ns};
  syscall(SYS_futex,&s->generation,FUTEX_WAIT,seen,&timeout,nullptr,0);
  return !s->stop.load() && s->generation.load(std::memory_order_acquire)>last+1;
}
int clock_read_observation_count(void *p,int *id,int *fault,int64_t *published,double *values,int count) {
  auto &x=static_cast<Shared*>(p)->observation;
  const unsigned version=x.version.load(std::memory_order_acquire);
  if(version&1)return 0;
  *id=x.id.load(std::memory_order_relaxed);*fault=x.fault.load(std::memory_order_relaxed);
  *published=x.published.load(std::memory_order_relaxed);
  for(int i=0;i<count;++i)values[i]=x.values[i].load(std::memory_order_relaxed);
  std::atomic_thread_fence(std::memory_order_acquire);
  return x.version.load(std::memory_order_relaxed)==version;
}
int clock_read_observation(void *p,int *id,int *fault,int64_t *published,double *values) {
  return clock_read_observation_count(p,id,fault,published,values,382);
}
int clock_read_sensor_observation(void *p,int *id,int *fault,int64_t *published,double *values) {
  return clock_read_observation_count(p,id,fault,published,values,462);
}
int clock_publish_observation_count(void *p,int id,int fault,int64_t published,const double *values,int count) {
  auto &x=static_cast<Shared*>(p)->observation;
  // Single writer. A preempted reader can never block physical publication.
  // Atomic payloads avoid a C++ data race while the version rejects torn reads.
  x.version.fetch_add(1,std::memory_order_acq_rel);
  std::atomic_thread_fence(std::memory_order_release);
  x.id.store(id,std::memory_order_relaxed);x.fault.store(fault,std::memory_order_relaxed);
  x.published.store(published,std::memory_order_relaxed);
  for(int i=0;i<count;++i)x.values[i].store(values[i],std::memory_order_relaxed);
  x.version.fetch_add(1,std::memory_order_release);
  auto *s=static_cast<Shared*>(p);s->generation.store(id+1,std::memory_order_release);
  syscall(SYS_futex,&s->generation,FUTEX_WAKE,1,nullptr,nullptr,0);
  return 1;
}
int clock_publish_observation(void *p,int id,int fault,int64_t published,const double *values) {
  return clock_publish_observation_count(p,id,fault,published,values,382);
}
int clock_publish_sensor_observation(void *p,int id,int fault,int64_t published,const double *values) {
  return clock_publish_observation_count(p,id,fault,published,values,462);
}
int clock_publish_result(void *p,int standing,int id,int64_t start,int64_t finish,const double *target) {
  ...
  x.version.fetch_add(1,std::memory_order_acq_rel);
  std::atomic_thread_fence(std::memory_order_release);
  x.id.store(id,std::memory_order_relaxed);x.start.store(start,std::memory_order_relaxed);
  for(int i=0;i<23;++i)x.target[i].store(target[i],std::memory_order_relaxed);
  x.finish.store(now_ns(),std::memory_order_relaxed);
  x.version.fetch_add(1,std::memory_order_release);return 1;
}
int clock_read_result(void *p,int standing,int *id,int64_t *start,int64_t *finish,double *target) {
  auto *s=static_cast<Shared*>(p);auto &x=standing?s->standing:s->active;
  const unsigned version=x.version.load(std::memory_order_acquire);
  if(version&1)return 0;
  *id=x.id.load(std::memory_order_relaxed);*start=x.start.load(std::memory_order_relaxed);
  *finish=x.finish.load(std::memory_order_relaxed);
  for(int i=0;i<23;++i)target[i]=x.target[i].load(std::memory_order_relaxed);
  std::atomic_thread_fence(std::memory_order_acquire);
  return x.version.load(std::memory_order_relaxed)==version;
}
...
int clock_run(void *memory,mjModel *m,mjData *d,int controls,int64_t epoch,
  ...
  double current[23],prior[23]{},history[300]{},begin[59],incoming[23]{},observed[462]{};
  const int imu=mj_name2id(m,mjOBJ_SITE,"imu_in_pelvis");
  const int gyro=mj_name2id(m,mjOBJ_SENSOR,"imu-pelvis-angular-velocity");
  const int accel=mj_name2id(m,mjOBJ_SENSOR,"imu-pelvis-linear-acceleration");
  const bool sensors=imu>=0 && gyro>=0 && accel>=0 && m->opt.integrator==mjINT_EULER;
  ...
  for(int step=0;step<controls*10 && !shared->stop.load();++step) {
    ...
    for(int j=0;j<23;++j) {
      double torque=kp[j]*(current[j]-d->qpos[j+7])-kd[j]*d->qvel[j+6];
      if(limit_brake) {
        const double low=m->jnt_range[2*(j+1)],high=m->jnt_range[2*(j+1)+1];
        const double band=std::min(.1,.2*(high-low));
        const double penetration=d->qpos[j+7]-std::clamp(d->qpos[j+7],low+band,high-band);
        const double outward=penetration*d->qvel[j+6]>0?d->qvel[j+6]:0.;
        torque-=100*penetration+2*outward;
      }
      d->ctrl[j]=std::clamp(torque,-effort[j],effort[j]);
      targets[step*23+j]=current[j];torques[step*23+j]=d->ctrl[j];
    }
    // Euler sensordata/site transforms describe the pre-integration sample.
    // Publish q/dq and timestamp from that same instant, never current root GT
    // relabelled as an older sensor sample. Sensor age at boundary is2 ms.
    if(step%10==9 && sensors) {
      observed[382]=d->time;
      std::copy(d->qpos+7,d->qpos+30,observed+383);
      std::copy(d->qvel+6,d->qvel+29,observed+406);
    }
    mj_step(m,d);int64_t finish=now_ns();
    if(step%10==9) {
      if(sensors) {
        mju_mat2Quat(observed+429,d->site_xmat+9*imu);
        std::copy(d->sensordata+m->sensor_adr[gyro],d->sensordata+m->sensor_adr[gyro]+3,observed+433);
        std::copy(d->sensordata+m->sensor_adr[accel],d->sensordata+m->sensor_adr[accel]+3,observed+436);
      } else {for(int j=382;j<439;++j)observed[j]=NAN;}
      std::copy(current,current+23,observed+439);
    }
    expected_time+=.002;
    ...
    if(step%10==9 && control+1<controls) {
      ...
      if(!clock_publish_sensor_observation(memory,control+1,shared->fault.load(),now_ns(),observed))++publications_missed;
