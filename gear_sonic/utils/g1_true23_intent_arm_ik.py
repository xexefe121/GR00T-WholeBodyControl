...
class Native23ArmIK:
    ...
    def __init__(self, model, *, position_weight=1., orientation_weight=.03,
                 posture_weight=.002, max_nfev=60, joint_margin=0.):
        ...
        if not np.isfinite(joint_margin) or joint_margin < 0 or np.any(2 * joint_margin >= self.upper - self.lower):
            raise ValueError("joint margin must leave a finite nonempty arm interval")
        self.joint_margin = float(joint_margin)
        self.lower += self.joint_margin
        self.upper -= self.joint_margin
