...
def validate_semantics(checkpoint):
    ...
    if (
        ...
        or resolved.get("native23_decoder_lora") != training_contract()
        ...
        or feedback.get("decoder_lora_training") != training_contract()
        ...
        or agent.get("algorithm", {}).get("use_clipped_value_loss") is not False
        or agent.get("algorithm", {}).get("clip_param") != 0.2
    ):
    ...
    required = {
        ...
        "gear_sonic/scripts/train_g1_true23_decoder_lora.py",
...
class DecoderLoraCPUActor(OriginalIntentCPUActor):
    ...
    def infer(self, encoder267, history930, root_feedback9):
        ...
        with torch.inference_mode():
            ...
            output = decoder_conditioned_forward(
...
def load_cpu_actor(checkpoint_path, *, warm_start_path, source_checkpoint_path):
    ...
    validate_decoder_lora_checkpoint(checkpoint, actor=actor, lineage=checkpoint["lineage"])
    actor.load_training_artifact(checkpoint["actor"])
    if sha256_file(path) != digest:
        raise ValueError("checkpoint changed during read")
    identity = dict(
        kind="native23_decoder_lora_original_intent_pytorch_cpu_research_reader_v1",
    ...
    return DecoderLoraCPUActor(actor), identity, semantics
