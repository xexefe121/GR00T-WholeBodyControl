"""Closed-loop adapter for round2 seed835 counterexample SONIC candidate."""
...
from gear_sonic.utils import (
    g1_true23_sonic_seed835_counterexample_round2_bc_last_affine as fit,
    g1_true23_sonic_student_closed_loop_qualification as student,
)

CANDIDATE_MANIFEST_RELATIVE_PATH = Path(
    "artifacts/g1_true23/sonic_seed835_counterexample_round2_bc_last_affine_v1.manifest.json"
)
CANDIDATE_MANIFEST_SHA256 = "c2112088fdcaee58ca848285adb833bd4413f29369dbae21e34b47d698ec73af"
CANDIDATE_DECODER_SHA256 = "c9795977810999659fd3c24d287fa6cf3b73ed25566031d21c360810baea7459"
...
EXTRA_NAMES = tuple(f"seed835_counterexample_round2_{name}" for name in INPUT_NAMES)
...
def _adapter_scope(root: Path, seed: int) -> Iterator[None]:
    ...
    def specs(
        ...
        if tuple(result) != extended:
            raise RuntimeError("round2 candidate frozen input inventory drift")
        return result
...
def run_qualification(*, repository_root: Path, output: Path, runtime_seed: int) -> Mapping[str, Any]:
    ...
    if fit.nominal_fit.sha256_file(manifest) != CANDIDATE_MANIFEST_SHA256:
        raise ValueError("round2 candidate manifest hash mismatch")
    ...
    with _adapter_scope(root, runtime_seed):
        ...
        report["seed835_counterexample_round2_candidate"] = {
