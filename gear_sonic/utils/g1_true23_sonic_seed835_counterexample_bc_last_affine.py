"""Targeted final-affine BC on exact seed835 student counterexample states."""
...
from gear_sonic.utils import (
    ...
    g1_true23_sonic_seed835_failure_prefix_dagger as counterexample,
)
...
SCHEMA_VERSION = 1
CONTRACT_KIND = "g1_true23_sonic_seed835_counterexample_bc_last_affine_contract_v1"
MANIFEST_KIND = "g1_true23_sonic_seed835_counterexample_bc_last_affine_manifest_v1"
CONTRACT_RELATIVE_PATH = Path(
    "gear_sonic/config/sim_validation/g1_true23_sonic_seed835_counterexample_bc_last_affine_v1.json"
)
CONTRACT_SHA256 = "2140f61a7005a8c99a612abdaea901331b7039a9576bfa0087d015e0703024ad"
NOMINAL_ROWS = 4080
SEGMENT_ROWS = 510
COUNTEREXAMPLE_ROWS = 89
TOTAL_ROWS = 5189
DECODER_DIM = 994
...
def load_contract(root: Path) -> Mapping[str, Any]:
    ...
    if nominal_fit.sha256_file(path) != CONTRACT_SHA256:
        raise ValueError("counterexample BC contract hash mismatch")
    ...
    if (
        ...
        != {
            ...
            "seed835_student_counterexample": COUNTEREXAMPLE_ROWS,
            "total": TOTAL_ROWS,
        }
        or fit.get("only_final_affine_may_change") is not True
        or fit.get("counterexample_first50_weight") != 16.0
        or fit.get("counterexample_last39_weight") != 64.0
        or boundaries.get("final_supervised_counterexample_attempt") is not True
        or boundaries.get("hardware_authorized") is not False
    ):
        raise ValueError("counterexample BC contract semantic drift")
    return body
...
def validate_candidate_manifest_fields(manifest: Mapping[str, Any], contract: Mapping[str, Any]) -> None:
    ...
    if claimed != nominal_fit._sha256_bytes(nominal_fit._canonical_bytes(payload)):  # noqa: SLF001
        raise ValueError("counterexample candidate payload hash mismatch")
    ...
    if (
        ...
        raise ValueError("counterexample candidate manifest semantic drift")


def _load_counterexample(npz: Path, manifest: Path) -> dict[str, np.ndarray]:
    body = _mapping(json.loads(manifest.read_text(encoding="utf-8")), "counterexample manifest")
    if (
        body.get("kind") != counterexample.MANIFEST_KIND
        or body.get("artifact", {}).get("npz_sha256") != nominal_fit.sha256_file(npz)
        or body.get("rows")
        != {"total": 89, "student_on_policy": 89, "teacher_actuated": 0, "q9_first": 9, "q9_last": 97}
    ):
    ...
    counterexample._validate_arrays(arrays)  # noqa: SLF001
...
def _load_inputs(root: Path, paths: Mapping[str, Path]) -> tuple[dict[str, np.ndarray], ...]:
    ...
    failure = _load_counterexample(paths["counterexample_npz"], paths["counterexample_manifest"])
    return nominal, intervention, seed835, failure
...
def _weights(contract: Mapping[str, Any]) -> np.ndarray:
    ...
    value = np.concatenate(
        (
            ...
            np.full(50, float(fit["counterexample_first50_weight"]), dtype=np.float64),
            np.full(39, float(fit["counterexample_last39_weight"]), dtype=np.float64),
        )
    )
    value *= TOTAL_ROWS / float(value.sum())
    if value.shape != (TOTAL_ROWS,) or not math.isclose(float(value.mean()), 1.0, abs_tol=1e-14):
        raise RuntimeError("counterexample fit weights invalid")
    return value
...
def preflight(request: nominal_fit.FitRequest) -> Mapping[str, Any]:
    ...
    expected_rows = (NOMINAL_ROWS, SEGMENT_ROWS, SEGMENT_ROWS, COUNTEREXAMPLE_ROWS)
    if any(
        data["decoder994"].shape != (rows, DECODER_DIM) for data, rows in zip(datasets, expected_rows, strict=True)
    ):
        raise ValueError("counterexample fit input shape drift")
    return {
        "ready": True,
...
def run_fit(request: nominal_fit.FitRequest) -> nominal_fit.FitOutcome:
    ...
    slices = {
        ...
        "counterexample": slice(5100, 5189),
        "counterexample_last39": slice(5150, 5189),
    }
    metrics = {
        name: {
    ...
    for name, key in (
        ("counterexample", "maximum_counterexample_all_rmse_ratio_to_base"),
        ("counterexample_last39", "maximum_counterexample_last39_rmse_ratio_to_base"),
    ):
