"""Versioned ankle-feedforward SIM fork; never a hardware controller.

Original benchmark preserved unchanged. This fork adds explicit ankle torque
before original total effort clipping and logs it separately from position
commands. Other benchmark helpers remain a frozen copy for replay consistency.
...
from gear_sonic.utils.g1_true23_ankle_feedforward import native_pd_feedforward_numpy, validate_feedforward
...
def run_reference_diagnostic(
    *,
    ...
    if profile != "native_model" or training_model_counterfactual is not None:
        raise ValueError("ankle feedforward benchmark requires unchanged native model/gains")
    if runtime_adapter is None or not hasattr(runtime_adapter, "current_actuator_feedforward23"):
        raise ValueError("ankle feedforward benchmark requires its explicit actuator adapter")
    ...
    try:
        for transition in range(requested):
            ...
            feedforward = validate_feedforward(runtime_adapter.current_actuator_feedforward23())
            arrays.setdefault("feedforward_torque23", []).append(feedforward.copy())
            for _ in range(physics.decimation):
                ...
                torque, applied, invalid, excess_cost = native_pd_feedforward_numpy(
                    target.astype(np.float64), data.qpos[7:], data.qvel[6:], actuation, feedforward
                )
                if invalid:
    ...
    report["kind"] = "native23_explicit_bounded_ankle_feedforward_reference_diagnostic_v1"
    report["actuator_contract"]["pd"] = "kp*(held_target-measured_q)-kd*measured_dq+held_ankle_feedforward"
    report["actuator_contract"]["feedforward_torque_logged_separately"] = True
    report["actuator_contract"]["effort_saturation"] = (
        "clip_TOTAL_pd_plus_feedforward_to_original_native_effort_each_substep"
    )
    if runtime_adapter is not None:
