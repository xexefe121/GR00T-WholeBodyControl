"""Collect round3 seed835 pure-student failure prefix with exact teacher labels."""
...
from gear_sonic.utils import (
    g1_true23_sonic_nominal_dagger_cutoff50_collection as collection_base,
    g1_true23_sonic_seed835_counterexample_round2_student_qualification as adapter,
...
CONTRACT_RELATIVE_PATH = Path(
    "gear_sonic/config/sim_validation/g1_true23_sonic_seed835_failure_prefix_dagger_round3_v1.json"
)
CONTRACT_SHA256 = "f85750db4bdc274b6b1bbe0ce21d10e28264a4f5871cfcb9e585820d616a4c58"
CONTRACT_KIND = "g1_true23_sonic_seed835_failure_prefix_dagger_round3_contract_v1"
MANIFEST_KIND = "g1_true23_sonic_seed835_failure_prefix_dagger_round3_manifest_v1"
...
SOURCE_PATHS = (
    ...
    Path("gear_sonic/scripts/collect_g1_true23_sonic_seed835_failure_prefix_dagger_round3.py"),
)
...
def load_contract(root: Path) -> Mapping[str, Any]:
    ...
    if recovery.sha256_file(path) != CONTRACT_SHA256:
        raise ValueError("round3 failure-prefix contract hash mismatch")
    ...
    if (
        ...
        or rows.get("q9_last") != 474
        or admission.get("dynamic_recovery_mode") != MODE
        or admission.get("expected_terminal_transition") != 465
        or admission.get("expected_terminal_q9") != 474
        or body.get("boundaries", {}).get("hardware_authorized") is not False
    ):
        raise ValueError("round3 failure-prefix contract semantic drift")
    ...
    for name, raw_entry in inputs.items():
        ...
        if candidate.is_symlink() or not candidate.is_file() or recovery.sha256_file(candidate) != entry["sha256"]:
            raise ValueError(f"round3 failure-prefix input mismatch: {name}")
    parent = _mapping(
    ...
    if (
        ...
        or first_done.get("transition") != 465
        or first_done.get("q9_before") != 474
        or first_done.get("termination_names") != ["ee_body_pos"]
    ):
        raise ValueError("round3 failure-prefix parent evidence drift")
    return body
...
def _request(root: Path) -> recovery.RecoveryRequest:
    return recovery.RecoveryRequest(
        ...
        output=Path("artifacts/g1_true23/.seed835-round3-prefix-inner-unused.json"),
        mode=MODE,
    )


def preflight(request: collection_base.CollectionRequest) -> Mapping[str, Any]:
    root = request.root
    load_contract(root)
    with _scope(root):
...
def _stack(rows: Mapping[int, Mapping[str, np.ndarray]], names: tuple[str, ...]) -> dict[str, np.ndarray]:
    if tuple(sorted(rows)) != tuple(range(ROWS)):
        raise ValueError("round3 failure-prefix row continuity mismatch")
    return {name: np.ascontiguousarray(np.stack([rows[index][name] for index in range(ROWS)])) for name in names}


def collect(request: collection_base.CollectionRequest) -> tuple[dict[str, np.ndarray], Mapping[str, Any]]:
    root = request.root
    receipt = preflight(request)
    contract = load_contract(root)
    admission = _mapping(contract["admission"], "admission")
    expected_transition = int(admission["expected_terminal_transition"])
    expected_q9 = int(admission["expected_terminal_q9"])
    expected_terms = list(admission["expected_termination_names"])
    ...
    if (
        ...
        or first_done.get("transition") != expected_transition
        or first_done.get("q9_before") != expected_q9
        or first_done.get("termination_names") != expected_terms
        ...
        raise RuntimeError("round3 failure-prefix rollout reproduction failed")
    inference = _stack(
        inference_rows,
        (
            "student_encoder267",
    ...
    if any(controllers[index] != "student" for index in range(ROWS)):
        raise RuntimeError("round3 failure-prefix contains nonstudent behavior")
...
def _validate_arrays(arrays: Mapping[str, np.ndarray]) -> None:
    ...
    if set(arrays) != {*widths, "q9"}:
        raise ValueError("round3 failure-prefix array key mismatch")
    for name, width in widths.items():
        value = arrays[name]
        if value.shape != (ROWS, width) or value.dtype != np.float32 or not bool(np.isfinite(value).all()):
            raise ValueError(f"round3 failure-prefix invalid array: {name}")
    if arrays["q9"].dtype != np.int64 or not np.array_equal(arrays["q9"], np.arange(9, 9 + ROWS, dtype=np.int64)):
        raise ValueError("round3 failure-prefix q9 mismatch")
    if not np.array_equal(arrays["decoder994"], np.concatenate((arrays["token64"], arrays["policy930"]), axis=1)):
        raise ValueError("round3 failure-prefix decoder concat mismatch")
    if float(np.max(np.abs(arrays["teacher_label_raw_native23"]))) >= 10.0:
        raise ValueError("round3 teacher label requires clipping")


def publish(
    request: collection_base.CollectionRequest,
    arrays: Mapping[str, np.ndarray],
    ...
    if any(os.path.lexists(path) for path in (npz, manifest)):
        raise FileExistsError("round3 failure-prefix output exists")
        ...
        "artifact": {
            ...
            "array_schema": {
                name: {
                    ...
                    "sha256": collection_base._array_sha(value),  # noqa: SLF001
                }
    ...
    body["manifest_payload_sha256"] = collection_base._sha256_bytes(  # noqa: SLF001
        collection_base._canonical_bytes(body)  # noqa: SLF001
    )

    def write_manifest(path: Path) -> None:
        with path.open("wb") as stream:
            stream.write(collection_base._canonical_bytes(body))  # noqa: SLF001
            stream.flush()
            os.fsync(stream.fileno())
