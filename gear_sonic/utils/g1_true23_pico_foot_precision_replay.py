...
from gear_sonic.trl.mjlab.native23_pico_foot_precision_runner import validate_checkpoint, training_contract
...
class PicoFootPrecisionPolicy:
    ...
    def __init__(self, checkpoint_path, *, warm_start_path, source_checkpoint_path):
        ...
        validate_checkpoint(checkpoint, actor=self.actor, lineage=self.lineage)
    ...
    def identity(self):
        ...
        return dict(
            kind="native23_pico_foot_precision_cpu_diagnostic_identity_v1",
            training_objective=training_contract(),
...
class PicoFootPrecisionAdapter(ReleasedCoreAdapter):
    ...
    def infer(self, policy, encoder, history, *, control_index, **state):
        if type(policy) is not PicoFootPrecisionPolicy:
    ...
    def contract(self):
        return {
            **super().contract(),
            "kind": "native23_pico_foot_precision_guarded_cpu_adapter_v1",
            "training_objective": training_contract(),
