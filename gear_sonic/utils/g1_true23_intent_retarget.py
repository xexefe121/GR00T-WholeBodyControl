...
class Native23LegIK:
    ...
    def solve_with_height(self, qpos, target_positions, target_rotations, posture_qpos=None,
        ...
        step box intersected with physical ranges. A second current-source seed
        uses the same box to escape a straight-knee local minimum. Infeasible
        foot goals remain visible; source time and goals are never altered.
        """
        result=np.asarray(qpos,dtype=np.float64).copy()
        posture=result if posture_qpos is None else np.asarray(posture_qpos)
        ...
        source_initial=np.r_[result[7:19],desired_height]
        initial=source_initial.copy()
        ...
        source_initial=np.clip(source_initial,np.nextafter(lower,upper),np.nextafter(upper,lower))
        starts=[("previous_pose" if previous is not None else "current_source",initial)]
        if previous is not None and np.max(np.abs(source_initial-initial))>1e-9:
            starts.append(("current_source",source_initial))
        solutions=[least_squares(lambda x:evaluate(x)[0],start,jac=lambda x:evaluate(x)[1],
                                 bounds=(lower,upper),max_nfev=self.max_nfev,ftol=1e-9,xtol=1e-9,gtol=1e-9)
                   for _,start in starts]
        selected=min(range(len(solutions)),key=lambda index:solutions[index].cost)
        ...
        self.last_solution_info=dict(success=bool(solution.success),status=int(solution.status),
                                     ...
                                     selected_start=starts[selected][0],selected_index=selected,
                                     total_nfev=sum(int(s.nfev) for s in solutions),
                                     failed_attempt_count=sum(not bool(s.success) for s in solutions),
                                     attempts=[dict(start=name,success=bool(s.success),status=int(s.status),
                                                    cost=float(s.cost),nfev=int(s.nfev),message=str(s.message))
                                               for (name,_),s in zip(starts,solutions)])
...
def run(clip, output, max_frames=None):
    ...
    records={key:[] for key in ("qpos","body_pos_w","body_quat_w","foot_position_error",
                                ...
                                "leg_solver_optimality","leg_solver_attempt_failures","leg_nfev_total",
                                "leg_selected_initialization","arm_solver_success","arm_orientation_error")}
    failures=[]
    ...
    for frame in range(count):
        ...
        if legs.last_solution_info["failed_attempt_count"] or not arm_result.success.all():
        ...
        values=dict(qpos=qpos,body_pos_w=nd.xpos[1:].copy(),body_quat_w=nd.xquat[1:].copy(),
                    ...
                    leg_solver_attempt_failures=legs.last_solution_info["failed_attempt_count"],
                    leg_nfev_total=legs.last_solution_info["total_nfev"],
                    leg_selected_initialization=legs.last_solution_info["selected_index"],
    ...
    report=dict(clip=clip,frames=count,full_original_timeline=count==len(motion["joint_pos"]),
                ...
                leg_initialization_candidates="previous_pose_and_current_source_seed_inside_identical_step_bounds",
                leg_source_initialization_selected_frames=int(np.sum(arrays["leg_selected_initialization"]==1)),
                leg_solver_failure_count=int(np.sum(~arrays["leg_solver_success"])),
                leg_solver_attempt_failure_count=int(np.sum(arrays["leg_solver_attempt_failures"])),
                ...
                elapsed_s=time.perf_counter()-started,kind="torso_orientation_feet_se3_bounded_height_causal_leg_arm_ik_reference_v3_multistart",
