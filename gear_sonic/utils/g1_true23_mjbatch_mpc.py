...
from gear_sonic.utils.g1_true23_mjbatch_model import Native23Feasibility
...
class Native23Tracker(Planner):
    ...
    def __init__(
        ...
        *,
        ...
        all_joint_limit_margin=None,
        all_joint_limit_weight=None,
        relative_foot_weight=0.0,
        fd_epsilon=EPS,
        hard_feasibility=False,
    ):
        super().__init__(
            servo_model, horizon, 10, num_threads=threads, fd_epsilon=fd_epsilon,
            feasibility=Native23Feasibility(servo_model, contract) if hard_feasibility else None,
        )
        self.contract, self.motion = contract, motion
        self.start = 10
        if not np.isfinite(relative_foot_weight) or relative_foot_weight < 0:
            raise ValueError("relative-foot weight must be finite and nonnegative")
        self.relative_foot_weight = float(relative_foot_weight)
        ...
        self.all_joint_limit_override = None
        if (all_joint_limit_margin is None) != (all_joint_limit_weight is None):
            raise ValueError("all-joint margin and weight must be specified together")
        if all_joint_limit_margin is not None:
            if ankle_limit_margin is not None or ankle_limit_weight is not None:
                raise ValueError("all-joint and ankle-only limit overrides are mutually exclusive")
            values = np.asarray([all_joint_limit_margin, all_joint_limit_weight], float)
            if not np.isfinite(values).all() or np.any(values <= 0):
                raise ValueError("all-joint margin and weight must be finite and positive")
            if np.any(2 * all_joint_limit_margin >= self.hi - self.lo):
                raise ValueError("all-joint margin consumes a full native range")
            self.limit_margin, self.limit_weight = map(float, values)
            self.all_joint_limit_override = dict(
                joint_names=list(contract["joint_names"]),
                native_joint_indices=list(range(23)),
                interior_margin_rad=self.limit_margin,
                weight=self.limit_weight,
    ...
    def residual(self, t, features):
        ...
        if self.relative_foot_weight:
            residual = np.concatenate(
                (residual, relative_foot_residual(positions, ref_positions, weight=self.relative_foot_weight)),
                axis=-1,
            )
        return residual
