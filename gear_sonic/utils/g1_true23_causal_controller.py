...
class CausalController:
    def __init__(self,actor,contract,**receiver_options):
        ...
        with np.load(actor.with_suffix('.normalization.npz'),allow_pickle=False) as z:
            ...
            self.task_closure_stand=bool(z['task_closure_stand'].item()) if 'task_closure_stand' in z else False
        ...
        if width==1749:
            ...
            self.body_goal=ReceivedBodyGoal(contract,task_closure_stand=True) if self.task_closure_stand else ReceivedBodyGoal(contract)
        elif width!=1323:raise ValueError('unsupported causal controller input width')
        elif self.task_closure_stand:raise ValueError('task closure requires the direct body controller')
    ...
    def command(self,qpos,qvel,now):
        ...
        return ControllerCommand(targets,dict(mode=self.receiver.mode,epoch=self.receiver.gate.epoch,
            ...
            received_samples=len(self.receiver.samples),future_reference_frames=0,
            motion_fraction=float(features[-1]) if self.body_goal is not None else 1.,
            task_closure_stand=self.task_closure_stand,qualified=False))
