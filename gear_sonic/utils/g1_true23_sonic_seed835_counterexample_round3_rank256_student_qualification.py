"""Closed-loop adapter for rank256 seed835 counterexample SONIC candidate."""
...
from gear_sonic.utils import (
    g1_true23_sonic_seed835_counterexample_round3_rank256_bc_last_affine as fit,
    g1_true23_sonic_student_closed_loop_qualification as student,
)

CANDIDATE_MANIFEST_RELATIVE_PATH = Path(
    "artifacts/g1_true23/sonic_seed835_counterexample_round3_rank256_bc_last_affine_v1.manifest.json"
)
CANDIDATE_MANIFEST_SHA256 = "ad8a779e7867c88fdd6e9d60b6e85584a24f30368c32908b27f2274378c0b213"
CANDIDATE_DECODER_SHA256 = "d1a20a098ccc3328ab188c081b5cefa642425bda9e7d06711a599a66531279ca"
...
INPUT_NAMES = (
    ...
    "round1_counterexample_npz",
    "round1_counterexample_manifest",
    "round2_counterexample_npz",
    "round2_counterexample_manifest",
    "round3_counterexample_npz",
    "round3_counterexample_manifest",
    "base_decoder",
    "base_manifest",
    "bootstrap_npz",
    "bootstrap_manifest",
    "rejected_rank42_manifest",
)
EXTRA_NAMES = tuple(f"seed835_rank256_{name}" for name in INPUT_NAMES)
...
def _adapter_scope(root: Path, seed: int) -> Iterator[None]:
    ...
    def specs(
        ...
        if tuple(result) != extended:
            raise RuntimeError("rank256 candidate frozen input inventory drift")
        return result
...
def run_qualification(*, repository_root: Path, output: Path, runtime_seed: int) -> Mapping[str, Any]:
    ...
    if fit.nominal_fit.sha256_file(manifest) != CANDIDATE_MANIFEST_SHA256:
        raise ValueError("rank256 candidate manifest hash mismatch")
    ...
    with _adapter_scope(root, runtime_seed):
        ...
        report["seed835_counterexample_round3_rank256_candidate"] = {
