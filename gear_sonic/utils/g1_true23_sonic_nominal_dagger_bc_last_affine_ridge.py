"""Final-affine SONIC refit on nominal plus cutoff50 intervention rows."""
...
from gear_sonic.utils import (
    g1_true23_sonic_nominal_dagger_cutoff50_collection as intervention,
...
SCHEMA_VERSION = 1
CONTRACT_KIND = "g1_true23_sonic_nominal_dagger_bc_last_affine_ridge_contract_v1"
MANIFEST_KIND = "g1_true23_sonic_nominal_dagger_bc_last_affine_ridge_manifest_v1"
CONTRACT_RELATIVE_PATH = Path(
    "gear_sonic/config/sim_validation/g1_true23_sonic_nominal_dagger_bc_last_affine_ridge_v1.json"
)
CONTRACT_SHA256 = "1f3ab5686ee668649bc4af30d87c7e2c4d20f4fc00fec2b44b953ddad9687378"
NOMINAL_ROWS = 4080
INTERVENTION_ROWS = 510
TOTAL_ROWS = 4590
ACTION_DIM = 23
HIDDEN_DIM = 512
DECODER_DIM = 994
...
def load_contract(root: Path) -> Mapping[str, Any]:
    ...
    if nominal_fit.sha256_file(path) != CONTRACT_SHA256:
        raise ValueError("nominal DAgger BC contract hash mismatch")
    ...
    if (
        ...
        != {
            ...
            "student_on_policy_shadow": 50,
            "teacher_actuated_recovery": 460,
            "total": TOTAL_ROWS,
        }
        or fit.get("only_final_affine_may_change") is not True
        or fit.get("fixed_ridge_lambda") != 0.0001
        or fit.get("student_on_policy_shadow_weight") != 16.0
        or fit.get("teacher_actuated_recovery_weight") != 4.0
        or boundaries.get("support_qualified") is not False
        or boundaries.get("hardware_authorized") is not False
    ):
        raise ValueError("nominal DAgger BC contract semantic drift")
    return body
...
def validate_candidate_manifest_fields(manifest: Mapping[str, Any], contract: Mapping[str, Any]) -> None:
    ...
    if claimed != nominal_fit._sha256_bytes(nominal_fit._canonical_bytes(payload)):  # noqa: SLF001
        raise ValueError("nominal DAgger candidate payload hash mismatch")
    ...
    if (
        ...
        or lineage != {name: entry["sha256"] for name, entry in inputs.items()}
        ...
        raise ValueError("nominal DAgger candidate manifest semantic drift")


def _load_intervention(npz: Path, manifest: Path) -> tuple[dict[str, np.ndarray], Mapping[str, Any]]:
    body = json.loads(manifest.read_text(encoding="utf-8"))
    if (
        body.get("kind") != intervention.MANIFEST_KIND
    ...
    payload = dict(body)
    claimed = payload.pop("manifest_payload_sha256", None)
    if claimed != intervention._sha256_bytes(intervention._canonical_bytes(payload)):  # noqa: SLF001
    ...
    intervention._validate_arrays(arrays)  # noqa: SLF001
    for name, value in arrays.items():
        expected = body["artifact"]["array_schema"][name]
        if expected != {
            "shape": list(value.shape),
            "dtype": str(value.dtype),
            "sha256": intervention._array_sha(value),  # noqa: SLF001
        }:
...
def _weights(contract: Mapping[str, Any]) -> np.ndarray:
    ...
    value = np.concatenate(
        (
            ...
            np.full(50, float(fit["student_on_policy_shadow_weight"]), dtype=np.float64),
            np.full(460, float(fit["teacher_actuated_recovery_weight"]), dtype=np.float64),
        )
    )
    value *= TOTAL_ROWS / float(value.sum())
    if value.shape != (TOTAL_ROWS,) or not math.isclose(float(value.mean()), 1.0, abs_tol=1e-14):
        raise RuntimeError("union fit weights invalid")
    return value
...
def preflight(request: nominal_fit.FitRequest) -> Mapping[str, Any]:
    ...
    intervention_arrays, _ = _load_intervention(paths["intervention_npz"], paths["intervention_manifest"])
    if nominal["decoder994"].shape != (NOMINAL_ROWS, DECODER_DIM):
        raise ValueError("nominal input shape drift")
    if intervention_arrays["decoder994"].shape != (INTERVENTION_ROWS, DECODER_DIM):
    ...
    return {
        ...
        "student_on_policy_shadow_rows": 50,
        "teacher_actuated_recovery_rows": 460,
...
def run_fit(request: nominal_fit.FitRequest) -> nominal_fit.FitOutcome:
    ...
    nominal_prefix = Path(str(paths["nominal_npz"])[: -len(".npz")])
    nominal, _ = load_training_candidate(CollectionRequest(root, nominal_prefix))
    dagger, _ = _load_intervention(paths["intervention_npz"], paths["intervention_manifest"])
    decoder994 = np.concatenate((nominal["decoder994"], dagger["decoder994"]), axis=0)
    labels = np.concatenate((nominal["teacher_label_raw_native23"], dagger["teacher_label_raw_native23"]), axis=0)
    ...
    with nominal_fit._numeric_runtime():  # noqa: SLF001
        ...
        _, candidate_bytes = nominal_fit.base_bc.export_final_affine_model(
            base_model,
    ...
    nominal_base = _metrics(base_prediction[:NOMINAL_ROWS], labels[:NOMINAL_ROWS])
    nominal_candidate = _metrics(prediction[:NOMINAL_ROWS], labels[:NOMINAL_ROWS])
    intervention_base = _metrics(base_prediction[NOMINAL_ROWS:], labels[NOMINAL_ROWS:])
    intervention_candidate = _metrics(prediction[NOMINAL_ROWS:], labels[NOMINAL_ROWS:])
    shadow_base = _metrics(
        base_prediction[NOMINAL_ROWS : NOMINAL_ROWS + 50], labels[NOMINAL_ROWS : NOMINAL_ROWS + 50]
    )
    shadow_candidate = _metrics(
        prediction[NOMINAL_ROWS : NOMINAL_ROWS + 50], labels[NOMINAL_ROWS : NOMINAL_ROWS + 50]
    )
    ...
    if (
        intervention_candidate["rmse"]
        > float(gates["maximum_intervention_rmse_ratio_to_base"]) * intervention_base["rmse"]
    ):
        issues.append("intervention_rmse_gate_failed")
    if shadow_candidate["rmse"] > float(gates["maximum_student_shadow_rmse_ratio_to_base"]) * shadow_base["rmse"]:
        issues.append("student_shadow_ratio_gate_failed")
    if shadow_candidate["rmse"] > float(gates["maximum_student_shadow_rmse"]):
        issues.append("student_shadow_absolute_gate_failed")
    if nominal_candidate["rmse"] > nominal_base["rmse"] + float(gates["maximum_nominal_rmse_regression"]):
    ...
    report = {
        ...
        "fit": {
            ...
            "weights": {
                ...
                "nominal_h10": 1.0,
                "student_shadow": 16.0,
                "teacher_recovery": 4.0,
            },
        },
        "metrics": {
            "nominal_base": nominal_base,
            "nominal_candidate": nominal_candidate,
            "intervention_base": intervention_base,
            "intervention_candidate": intervention_candidate,
            "student_shadow_base": shadow_base,
            "student_shadow_candidate": shadow_candidate,
            "bootstrap_base": bootstrap_base_metrics,
            "bootstrap_candidate": bootstrap_candidate_metrics,
            "all_candidate": all_metrics,
        },
        "export": {
