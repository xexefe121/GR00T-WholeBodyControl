...
DT = 0.02
JOINT_TARGET_MARGIN_RAD = 0.0065
JOINT_LIMIT_BRAKE_LOOKAHEAD_S = 0.060
# The override is deliberately much smaller than the old 0.517 rad endpoint
# jump.  It is a control-period target increment, not an actuator-rate limit.
JOINT_LIMIT_BRAKE_STEP_RAD = 0.100
HORIZON = 8
...
class ReceivedSample:
    ...
    def __post_init__(self):
        # Cache only immutable, already-received reference geometry. The measured
        # pose feedback and both neural networks still run every control tick.
        rotation = _quaternion_matrix(self.packet.fields["body_quat_w"][0])
        self.root_yaw = np.arctan2(rotation[1, 0], rotation[0, 0])
        self.heading = np.array(
            [
                [np.cos(self.root_yaw), -np.sin(self.root_yaw), 0.0],
                [np.sin(self.root_yaw), np.cos(self.root_yaw), 0.0],
                [0.0, 0.0, 1.0],
            ]
        )
        self.local_positions = np.vstack((np.zeros(3), self.privileged[1:73].reshape(24, 3)))


def received_goal(policy, samples, measured_qpos, position_gain, yaw_gain):
    """Cached evaluation of the referee's v2 correction, with identical math."""
    started = time.perf_counter()
    states = np.stack([sample.state for sample in samples])
    privileged = np.stack([sample.privileged for sample in samples])
    if position_gain or yaw_gain:
        actual_rotation = _quaternion_matrix(measured_qpos[3:7])
        actual_yaw = np.arctan2(actual_rotation[1, 0], actual_rotation[0, 0])
        yaw_error = np.arctan2(np.sin(samples[0].root_yaw - actual_yaw), np.cos(samples[0].root_yaw - actual_yaw))
        omega = np.array([0.0, 0.0, np.clip(yaw_gain * yaw_error, -0.8, 0.8)])
        delta_world = position_gain * (samples[0].packet.fields["body_pos_w"][0] - measured_qpos[:3])
        delta_world[2] = 0.0
        magnitude = np.linalg.norm(delta_world)
        delta_world *= min(1.0, 0.6 / max(magnitude, 1e-8))
        actual_heading = np.array(
            [
                [np.cos(actual_yaw), -np.sin(actual_yaw), 0.0],
                [np.sin(actual_yaw), np.cos(actual_yaw), 0.0],
                [0.0, 0.0, 1.0],
            ]
        )
        angular_delta = np.tile(omega, 25)
        for offset, sample in enumerate(samples):
            original_velocity = sample.packet.fields["body_lin_vel_w"][0]
            local_delta = (
                actual_heading.T @ (original_velocity + delta_world) - sample.heading.T @ original_velocity
            )
            privileged[offset, 223:298] += (local_delta + np.cross(omega, sample.local_positions)).reshape(-1)
            privileged[offset, 298:373] += angular_delta
            states[offset, -3:] += omega
    encoded = time.perf_counter()
    with torch.inference_mode():
        device = getattr(policy, "device", torch.device("cpu"))
        latent = policy.backward(
            torch.from_numpy(states).to(device), torch.from_numpy(privileged).to(device)
        ).mean(0, keepdim=True)
        goal = 16 * F.normalize(latent, dim=-1)
    completed = time.perf_counter()
    return goal, (encoded - started) * 1000, (completed - encoded) * 1000
...
class PacketGate:
    ...
    def receive(self, packet, now, *, source_time_offset=0.0):
        if self.fault is not None:
            self.ignored += 1
            return False
        try:
            if not isinstance(packet, Packet):
                raise PacketRejected("packet_type")
            ...
            if not math.isfinite(source_time_offset):
                raise PacketRejected("packet_source_clock_offset_nonfinite")
            ...
            source_age = now - self.epoch_started_at - (packet.source_time + source_time_offset)
            if source_age < -1e-7:
...
class TeleopClockAlignment:
    """Maps a publisher epoch onto the local policy clock without changing gate limits.

    ``Packet.source_time`` remains the canonical sequence clock required by
    :class:`PacketGate`.  The reception offset maps it to the policy clock for
    both queued delivery and the gate's source-age calculation.
    """
    ...
    def reset_for_epoch(self, epoch):
        self.epoch = epoch
        self.offset = None

    def observe(self, packet, now, gate):
        """Anchor on first valid timestamp received in each gate epoch.

        Returns the packet source timestamp expressed in the local policy
        clock, or ``None`` when the timestamp is malformed.  The latter still
        reaches PacketGate and is rejected by its unchanged validation.
        """
        if self.epoch != gate.epoch:
            self.reset_for_epoch(gate.epoch)
        if self.offset is None:
            try:
                source_time = float(packet.source_time)
                if not math.isfinite(source_time):
                    return None
                offset = float(now) - source_time
            except (TypeError, ValueError, OverflowError):
                return None
            self.offset = offset
            self.anchors.append({"epoch": gate.epoch, "offset": offset, "received_at": float(now),
                                 "source_time": source_time})
        try:
            return float(packet.source_time) + self.offset
        except (TypeError, ValueError, OverflowError):
            return None

    def gate_source_time_offset(self, gate):
        """Offset for PacketGate's epoch-relative source-age expression."""
        if self.offset is None:
            return None
        return self.offset - gate.epoch_started_at


class GateAdmissionAudit:
    """Count every false PacketGate.receive result with an actionable reason."""

    def __init__(self):
        self.rejected = 0
        self.reasons = {}

    def receive(self, gate, packet, now, *, source_time_offset=0.0):
        rejected_before = gate.rejected
        accepted = gate.receive(packet, now, source_time_offset=source_time_offset)
        return self.record(gate, accepted, rejected_before)

    def record(self, gate, accepted, rejected_before):
        if accepted:
            return True
        self.rejected += 1
        if gate.rejected != rejected_before:
            reason = gate.fault["reason"] if gate.fault is not None else "packet_rejected_without_fault"
        elif gate.fault is not None:
            reason = "gate_fault_latched:" + gate.fault["reason"]
        else:
            reason = "packet_epoch_mismatch"
...
class BFMStreamSimulator:
    ...
    def __init__(
        ...
        *,
        ...
        sensor_step=None,
        record_trace=True,
        brake_step_rad=JOINT_LIMIT_BRAKE_STEP_RAD,
    ):
        ...
        # This final, model-derived target envelope belongs at the same point
        # in the command path as a physical robot's PD target safety layer.
        # The value exceeds the measured PICO lower-limit overshoot (6.397 mrad)
        # by 0.103 mrad.
        self.target_margin = np.zeros(model.nu)
        self.target_margin[model.joint("left_ankle_roll_joint").id - 1] = JOINT_TARGET_MARGIN_RAD
        if not np.isfinite(brake_step_rad) or brake_step_rad <= 0:
            raise ValueError("brake_step_rad must be a positive finite value")
        self.brake_step_rad = float(brake_step_rad)
        self._previous_target = None
        self._brake_engagement_controls = 0
        self._brake_target_step_max_rad = 0.0
        ...
        # Optional sensor-only observation hook.  The established stream
        # evaluator leaves these unset and therefore retains its exact prior
        # simulator-state feedback behaviour.
        self.sensor_feedback = sensor_feedback
        self.sensor_step = sensor_step
        # The normal evaluator keeps its complete forensic trace.  A live
        # consumer only reports scalar safety/tracking results, so it can
        # retain just the source errors required for those report fields.
        self.record_trace = record_trace
        self._source_joint_errors = []
        self._source_root_error_norms = []
        self._range_excess_max = 0.0
        self._range_excess_controls = 0
        self._velocity_ratio_max = 0.0
        self._effort_ratio_max = 0.0
    ...
    def receive(self, packet, now, *, source_time_offset=0.0):
        accepted = self.gate.receive(packet, now, source_time_offset=source_time_offset)
    ...
    def step(self, now, profiler=None):
        started = time.perf_counter()
        ...
        observed = self.sensor_feedback() if self.sensor_feedback is not None else {
            "joint_q": self.data.qpos[7:],
            "joint_dq": self.data.qvel[6:],
            "imu_quat_wxyz": self.data.qpos[3:7],
            "gyro_body": self.data.qvel[3:6],
            "root_position": self.data.qpos[:3],
        }
        feedback_qpos = np.r_[observed["root_position"], observed["imu_quat_wxyz"], observed["joint_q"]]
        sensed, terms = state_and_terms(
            observed["joint_q"],
            observed["joint_dq"],
            observed["imu_quat_wxyz"],
            observed["gyro_body"],
            self.action,
            self.contract["default_q"],
        )
        history = self.history.before_update(terms)
        goal, feedback_ms, backward_ms = received_goal(
            self.policy, samples, feedback_qpos, self.position_gain, self.yaw_gain
        )
        actor_started = time.perf_counter()
        if profiler is not None:
            profiler.record("feature_construction_goal_transform", actor_started - window_ready)
        ...
        actor_finished = time.perf_counter()
        if profiler is not None:
            profiler.record("bfm_inference", actor_finished - actor_started)
        self.action = raw * 5.0
        target = (
            self.contract["default_q"]
        ...
        safe_low = limits[:, 0] + self.target_margin
        safe_high = limits[:, 1] - self.target_margin
        if np.any(safe_low >= safe_high):
            raise ValueError("joint target safety margin exhausts a model range")
        # Preserve the established hard target clamp when there is no measured
        # approach to a limit.  The interior envelope is engaged only by the
        # predictive safety event below, so clips with no limit approach retain
        # their exact commanded targets.
        policy_clipped = np.clip(target, limits[:, 0], limits[:, 1])
        clipped = policy_clipped.copy()
        # A target can be inside its range while an already-moving joint still
        # crosses the hard stop during the next control interval.  On that
        # event, move only the protected target inward by one bounded control
        # increment.  The old endpoint projection was a 0.5-rad bang-bang
        # jump.  This layer also constrains an engaged target relative to the
        # previously emitted target, so engagement itself cannot make a larger
        # target step.  It changes neither policy inputs nor PD gains, effort
        # limits, or the physics model.
        predicted_q = self.data.qpos[7:] + self.data.qvel[6:] * JOINT_LIMIT_BRAKE_LOOKAHEAD_S
        protected = self.target_margin > 0.0
        toward_low = protected & (self.data.qvel[6:] < 0.0) & (predicted_q < safe_low)
        toward_high = protected & (self.data.qvel[6:] > 0.0) & (predicted_q > safe_high)
        brake_engaged = toward_low | toward_high
        previous_target = self._previous_target
        if np.any(toward_low):
            inward = np.minimum(policy_clipped[toward_low] + self.brake_step_rad, safe_high[toward_low])
            if previous_target is not None:
                inward = np.clip(
                    inward,
                    previous_target[toward_low] - self.brake_step_rad,
                    previous_target[toward_low] + self.brake_step_rad,
                )
            clipped[toward_low] = inward
        if np.any(toward_high):
            inward = np.maximum(policy_clipped[toward_high] - self.brake_step_rad, safe_low[toward_high])
            if previous_target is not None:
                inward = np.clip(
                    inward,
                    previous_target[toward_high] - self.brake_step_rad,
                    previous_target[toward_high] + self.brake_step_rad,
                )
            clipped[toward_high] = inward
        brake_target_step = 0.0
        if previous_target is not None and np.any(brake_engaged):
            brake_target_step = float(np.max(np.abs(clipped[brake_engaged] - previous_target[brake_engaged])))
            if brake_target_step > self.brake_step_rad + 1e-12:
                raise RuntimeError("bounded brake emitted an oversized target step")
        self._previous_target = clipped.copy()
        self._brake_engagement_controls += int(np.any(brake_engaged))
        self._brake_target_step_max_rad = max(self._brake_target_step_max_rad, brake_target_step)
        excess = effort = velocity = 0.0
        physics_started = time.perf_counter()
        for _ in range(self.physics.decimation):
            pre_sensor_state = None
            if self.sensor_step is not None:
                pre_sensor_state = {
                    "timestamp_s": float(self.data.time),
                    "joint_q": self.data.qpos[7:].copy(),
                    "joint_dq": self.data.qvel[6:].copy(),
                    "imu_quat_wxyz": self.data.qpos[3:7].copy(),
                }
            substep_started = time.perf_counter() if profiler is not None else None
            torque = (
            ...
            if profiler is not None:
                profiler.record("physics_substeps_500hz", time.perf_counter() - substep_started)
            if self.sensor_step is not None:
                self.sensor_step(pre_sensor_state, self.data)
            logging_started = time.perf_counter() if profiler is not None else None
            if self.record_trace:
            ...
            if profiler is not None:
                profiler.record("per_step_logging_array_appending", time.perf_counter() - logging_started)
        physics_finished = time.perf_counter()
        self.controls += 1
        self._range_excess_max = max(self._range_excess_max, excess)
        self._range_excess_controls += int(excess > 0.0)
        self._velocity_ratio_max = max(self._velocity_ratio_max, velocity)
        self._effort_ratio_max = max(self._effort_ratio_max, effort)
        ...
        if not self.record_trace:
            if source_sample is not None:
                reference = source_sample.packet.fields
                self._source_joint_errors.append(self.data.qpos[7:] - reference["joint_pos"])
                self._source_root_error_norms.append(
                    float(np.linalg.norm(self.data.qpos[:3] - reference["body_pos_w"][0]))
                )
        ...
        values = {
            ...
            "feedback_qpos": feedback_qpos.copy(),
            "state": sensed,
            "history": history,
            ...
            "brake_engaged": bool(np.any(brake_engaged)),
            "brake_target_step_rad": brake_target_step,
            "mode": self.mode.value,
            ...
            "reference_qpos": np.r_[
                samples[0].packet.fields["body_pos_w"][0],
                samples[0].packet.fields["body_quat_w"][0],
                samples[0].packet.fields["joint_pos"],
            ],
            "range_excess": excess,
            "velocity_ratio": velocity,
            "effort_ratio": effort,
            ...
            "reference_window_ms": (window_ready - started) * 1000,
            "goal_feedback_ms": feedback_ms,
            "backward_ms": backward_ms,
            "actor_ms": (actor_finished - actor_started) * 1000,
            "physics_ms": (physics_finished - physics_started) * 1000,
            "diagnostics_ms": (time.perf_counter() - physics_finished) * 1000,
        }
        logging_started = time.perf_counter() if profiler is not None else None
    ...
    def arrays(self):
        if not self.record_trace:
            raise RuntimeError("this simulator was configured without a full trace")
        return {name: np.asarray(values) for name, values in self.trace.items()}

    def source_metrics(self):
        if self.record_trace:
            arrays = self.arrays()
            mask = arrays["source_sequence"] >= 0
            if not np.any(mask):
                return None
            error = arrays["qpos"][1:][mask, 7:] - arrays["reference_qpos"][mask, 7:]
            root_error = arrays["qpos"][1:][mask, :3] - arrays["reference_qpos"][mask, :3]
            return {
                "source_controls": int(np.count_nonzero(mask)),
                "leg_rmse": float(np.sqrt(np.mean(error[:, :12] ** 2))),
                "arm_rmse": float(np.sqrt(np.mean(error[:, 13:] ** 2))),
                "root_p95": float(np.percentile(np.linalg.norm(root_error, axis=-1), 95)),
            }
        if not self._source_joint_errors:
            return None
        error = np.asarray(self._source_joint_errors)
        return {
            "source_controls": len(error),
            "leg_rmse": float(np.sqrt(np.mean(error[:, :12] ** 2))),
            "arm_rmse": float(np.sqrt(np.mean(error[:, 13:] ** 2))),
            "root_p95": float(np.percentile(self._source_root_error_norms, 95)),
        }

    def report(self):
        arrays = self.arrays() if self.record_trace else None
        return {
            "kind": "bfmzero_received_stream_simulation_v1",
            "mode": self.mode.value,
            "controls": self.controls,
            "physics_steps": len(self.trace["physics_torque"]) if self.record_trace else self.controls * self.physics.decimation,
            ...
            "range_excess_max_rad": float(arrays["range_excess"].max()) if self.record_trace and self.controls else (self._range_excess_max if self.controls else None),
            "range_excess_controls": int(np.count_nonzero(arrays["range_excess"] > 0)) if self.record_trace else self._range_excess_controls,
            "effort_ratio_max": float(arrays["effort_ratio"].max()) if self.record_trace and self.controls else (self._effort_ratio_max if self.controls else None),
            "brake_step_rad": self.brake_step_rad,
            "brake_engagement_controls": int(np.count_nonzero(arrays["brake_engaged"])) if self.record_trace else self._brake_engagement_controls,
            "brake_target_step_max_rad": float(arrays["brake_target_step_rad"].max()) if self.record_trace and self.controls else self._brake_target_step_max_rad,
            "velocity_ratio_max": float(arrays["velocity_ratio"].max()) if self.record_trace and self.controls else (self._velocity_ratio_max if self.controls else None),
            "events": [*self.events, *self.gate.events],
            "ground_truth_pose_feedback": self.sensor_feedback is None,
