...
def solve_task_step(problem, current, residual, jacobian):
    ...
    # the whole objective by frame count is a common positive scalar, not a
    # task reweighting. Preserve and audit the original absolute rows as well.
    temporal_origin = temporal @ current.ravel()
    rotation_origin = rotation_bound @ current.ravel()
    ...
    if solution is None:
        return None, report
    absolute_solution = np.r_[current.ravel() + solution[:n], solution[n:]]
    actual = matrix @ absolute_solution
    violation = max(
        np.maximum(absolute_lower - actual, 0).max(initial=0),
        np.maximum(actual - absolute_upper, 0).max(initial=0),
    )
    report["independent_original_absolute_row_violation"] = float(violation) if np.isfinite(violation) else None
    if not np.isfinite(violation) or violation > 1e-8:
        report.update(status="original_absolute_row_audit_failed", accepted=False)
        return None, report
    return absolute_solution[:n].reshape(current.shape), report
