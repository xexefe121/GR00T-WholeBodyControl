"""Strict frozen-decoder original-intent snapshots for offline CPU research only.
...
from gear_sonic.trl.mjlab.native23_frozen_decoder_actor import ACTOR_KIND, TRAINABLE, True23FrozenDecoderActorModel
from gear_sonic.trl.mjlab.native23_frozen_decoder_runner import (
    CHECKPOINT_HEADER,
    MULTIPLIERS,
    PROFILE,
    training_contract,
    validate_frozen_decoder_checkpoint,
)
...
def validate_semantics(checkpoint):
    ...
    if (
        ...
        or resolved.get("native23_frozen_decoder") != training_contract()
        ...
        or feedback.get("frozen_decoder_training") != training_contract()
    ...
    required = {
        ...
        "gear_sonic/scripts/train_g1_true23_frozen_decoder.py",
...
def construct_actor(checkpoint, semantics, *, warm_start_path, source_checkpoint_path):
    ...
    return True23FrozenDecoderActorModel(
...
class FrozenDecoderCPUActor(OriginalIntentCPUActor):
    """Reuse the exact CPU mean/trace machinery with a stricter actor type."""

    def __init__(self, actor):
        if not isinstance(actor, True23FrozenDecoderActorModel):
            raise TypeError("frozen-decoder CPU reader requires its distinct actor")
...
def load_cpu_actor(checkpoint_path, *, warm_start_path, source_checkpoint_path):
    ...
    validate_frozen_decoder_checkpoint(checkpoint, actor=actor, lineage=checkpoint["lineage"])
    actor.load_training_artifact(checkpoint["actor"])
    if sha256_file(path) != digest:
        raise ValueError("checkpoint changed during read")
    identity = dict(
        kind="native23_frozen_decoder_original_intent_pytorch_cpu_research_reader_v1",
    ...
    return FrozenDecoderCPUActor(actor), identity, semantics
