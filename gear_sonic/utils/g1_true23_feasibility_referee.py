...
def inspect_native_segment(model, data, targets, contract: Mapping,
    ...
    # MuJoCo advances time through repeated binary64 addition. Multiplication
    # by the total step count differs by >1e-10 after 120.74s at dt=.002.
    # Accumulate independently from the initial time, never from post-step data;
    # this preserves reset/extra/missing-step detection without false drift.
    expected_native_time = start_time
    ...
    maximum = dict(joint_excess_rad=0., speed_ratio=0., effort_ratio=0.,
                   tilt_rad=0., clock_error_seconds=0., ideal_clock_difference_seconds=0.)
    ...
    def assess(step):
        ...
        clock_error = abs(float(private.time) - expected_native_time)
        ideal_clock_difference = abs(float(private.time) - (start_time + step * .002))
        reasons = []
        ...
        current = dict(joint_excess_rad=excess, speed_ratio=speed, effort_ratio=effort_ratio,
                       ...
                       ideal_clock_difference_seconds=ideal_clock_difference)
    ...
    report = dict(
        ...
        clock_expectation='independent repeated binary64 addition of .002 from initial time; never resynchronized to observed time',
    )
    trace = None
    if retain_trace:
