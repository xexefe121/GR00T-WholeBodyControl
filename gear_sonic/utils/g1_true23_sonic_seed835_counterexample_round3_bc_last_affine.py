"""Final targeted final-affine BC fit on seed835 student counterexample states."""
...
from gear_sonic.utils import (
    ...
    g1_true23_sonic_seed835_failure_prefix_dagger_round3 as round3_counterexample,
)
...
SCHEMA_VERSION = 1
CONTRACT_KIND = "g1_true23_sonic_seed835_counterexample_round3_bc_last_affine_contract_v1"
MANIFEST_KIND = "g1_true23_sonic_seed835_counterexample_round3_bc_last_affine_manifest_v1"
CONTRACT_RELATIVE_PATH = Path(
    "gear_sonic/config/sim_validation/g1_true23_sonic_seed835_counterexample_round3_bc_last_affine_v1.json"
)
CONTRACT_SHA256 = "bcbd8d9806e970f2b46ac34c4e696b333d8205c6f0cbf065b1c578c389bb0af3"
NOMINAL_ROWS = 4080
SEGMENT_ROWS = 510
ROUND1_ROWS = 89
ROUND2_ROWS = 168
...
def load_contract(root: Path) -> Mapping[str, Any]:
    ...
    if nominal_fit.sha256_file(path) != CONTRACT_SHA256:
        raise ValueError("round3 counterexample BC contract hash mismatch")
    ...
    if (
        ...
        != {
            ...
            "round3_student_counterexample": ROUND3_ROWS,
            "total": TOTAL_ROWS,
        }
        or fit.get("only_final_affine_may_change") is not True
        or fit.get("round3_first416_weight") != 8.0
        or fit.get("round3_last50_weight") != 64.0
        or boundaries.get("final_supervised_counterexample_attempt") is not True
        or boundaries.get("hardware_authorized") is not False
    ):
        raise ValueError("round3 counterexample BC contract semantic drift")
    return body
...
def validate_candidate_manifest_fields(manifest: Mapping[str, Any], contract: Mapping[str, Any]) -> None:
    ...
    if claimed != nominal_fit._sha256_bytes(nominal_fit._canonical_bytes(payload)):  # noqa: SLF001
        raise ValueError("round3 counterexample candidate payload hash mismatch")
    ...
    if (
        ...
        raise ValueError("round3 counterexample candidate manifest semantic drift")


def _load_inputs(root: Path, paths: Mapping[str, Path]) -> tuple[dict[str, np.ndarray], ...]:
    earlier = round2_fit._load_inputs(root, paths)  # noqa: SLF001
    round3 = round2_fit._load_counterexample(  # noqa: SLF001
        paths["round3_counterexample_npz"],
        paths["round3_counterexample_manifest"],
        kind=round3_counterexample.MANIFEST_KIND,
        rows={
            "total": 466,
            "student_on_policy": 466,
            "teacher_actuated": 0,
            "q9_first": 9,
            "q9_last": 474,
        },
        validator=round3_counterexample._validate_arrays,  # noqa: SLF001
    )
    return *earlier, round3
...
def _weights(contract: Mapping[str, Any]) -> np.ndarray:
    ...
    value = np.concatenate(
        (
            ...
            np.full(416, float(fit["round3_first416_weight"]), dtype=np.float64),
            np.full(50, float(fit["round3_last50_weight"]), dtype=np.float64),
        )
    )
    value *= TOTAL_ROWS / float(value.sum())
    if value.shape != (TOTAL_ROWS,) or not math.isclose(float(value.mean()), 1.0, abs_tol=1e-14):
        raise RuntimeError("round3 counterexample fit weights invalid")
    return value
...
def preflight(request: nominal_fit.FitRequest) -> Mapping[str, Any]:
    ...
    expected_rows = (NOMINAL_ROWS, SEGMENT_ROWS, SEGMENT_ROWS, ROUND1_ROWS, ROUND2_ROWS, ROUND3_ROWS)
    if any(
        data["decoder994"].shape != (rows, DECODER_DIM) for data, rows in zip(datasets, expected_rows, strict=True)
    ):
        raise ValueError("round3 counterexample fit input shape drift")
    return {
        "ready": True,
        "contract_sha256": CONTRACT_SHA256,
        "total_rows": TOTAL_ROWS,
        "round3_counterexample_rows": ROUND3_ROWS,
