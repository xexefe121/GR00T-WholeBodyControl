"""Closed-loop adapter for the round2 nominal-plus-intervention SONIC student."""
...
CANDIDATE_MANIFEST_RELATIVE_PATH = Path(
    "artifacts/g1_true23/sonic_nominal_dagger_round2_bc_last_affine_ridge_v2.manifest.json"
)
CANDIDATE_MANIFEST_SHA256 = "4fafafe58bf589058c3e91e7c4e57343d149efec08c9e50e820433b350acadd4"
CANDIDATE_DECODER_SHA256 = "703aa2002eac1d6dc6bcca100ffc45bf2d3c3945ef0f3d76ac843a4e51e0900a"
...
EXTRA_NAMES = tuple(f"round2_{name}" for name in INPUT_NAMES)
...
def _adapter_scope(root: Path, seed: int) -> Iterator[None]:
    ...
    def specs(
        ...
        if tuple(result) != extended:
            raise RuntimeError("round2 frozen input inventory drift")
        return result
...
def run_qualification(*, repository_root: Path, output: Path, runtime_seed: int) -> Mapping[str, Any]:
    ...
    with _adapter_scope(root, runtime_seed):
        ...
        report["round2_nominal_dagger_candidate"] = {
