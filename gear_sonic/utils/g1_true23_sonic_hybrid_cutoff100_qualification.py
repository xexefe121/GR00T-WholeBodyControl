"""Qualify an earlier q9=109 teacher handoff after cutoff140 failed seed611."""
...
from gear_sonic.utils import (
    g1_true23_sonic_hybrid_cutoff140_qualification as cutoff140,
    g1_true23_sonic_nominal_dagger_student_qualification as adapter,
    g1_true23_sonic_student_teacher_recovery as recovery,
)

CONTRACT_RELATIVE_PATH = Path(
    "gear_sonic/config/sim_validation/g1_true23_sonic_hybrid_cutoff100_qualification_v1.json"
)
CONTRACT_SHA256 = "31012c1af892e6fdeed0c9d8427a6165e8198751cca1d0eaa4767308659110bd"
CONTRACT_KIND = "g1_true23_sonic_hybrid_cutoff100_qualification_contract_v1"
ALLOWED_RUNTIME_SEEDS = cutoff140.ALLOWED_RUNTIME_SEEDS
SOURCE_PATHS = (
    CONTRACT_RELATIVE_PATH,
    Path("gear_sonic/utils/g1_true23_sonic_hybrid_cutoff100_qualification.py"),
    Path("gear_sonic/scripts/qualify_g1_true23_sonic_hybrid_cutoff100.py"),
)
...
def _read_bound_json(root: Path, entry: Mapping[str, Any], context: str) -> Mapping[str, Any]:
    ...
    if path.is_symlink() or not path.is_file() or recovery.sha256_file(path) != entry["sha256"]:
        raise ValueError(f"cutoff100 qualification input mismatch: {context}")
...
def load_contract(root: Path) -> Mapping[str, Any]:
    ...
    if recovery.sha256_file(path) != CONTRACT_SHA256:
        raise ValueError("cutoff100 qualification contract hash mismatch")
    ...
    if (
        ...
        or runtime.get("mode") != "cutoff100"
        or runtime.get("total_transitions") != 510
        or runtime.get("student_transitions") != 100
        or runtime.get("teacher_transitions") != 410
        or runtime.get("student_last_q9") != 108
        or runtime.get("teacher_first_q9") != 109
        or runtime.get("allowed_seeds") != list(ALLOWED_RUNTIME_SEEDS)
        or decision.get("stop_if_cutoff100_fails_seed611") is not True
        ...
        raise ValueError("cutoff100 qualification contract semantic drift")
    ...
    for name, raw_entry in inputs.items():
        ...
        if candidate.is_symlink() or not candidate.is_file() or recovery.sha256_file(candidate) != entry["sha256"]:
            raise ValueError(f"cutoff100 qualification input mismatch: {name}")

    prior_pass = _read_bound_json(root, _mapping(inputs["known_seed_cutoff140_pass"], "prior pass"), "prior pass")
    prior_failure = _read_bound_json(
        root, _mapping(inputs["seed611_cutoff140_failure"], "prior failure"), "prior failure"
    )
    failure_done = _mapping(prior_failure.get("first_done"), "prior failure first_done")
    failure_ee = _mapping(failure_done.get("ee_body_position_scalar_evidence"), "prior failure terminal ee")
    if (
        prior_pass.get("verdict") != "mixed_controller_recovery_passed"
        or prior_pass.get("recovered_to_original_q9_518_boundary") is not True
        or prior_pass.get("controller_counts") != {"student": 140, "teacher": 370}
        or prior_failure.get("verdict") != "mixed_controller_recovery_failed"
        or prior_failure.get("recovered_to_original_q9_518_boundary") is not False
        or prior_failure.get("controller_counts") != {"student": 140, "teacher": 139}
        or failure_done.get("q9_before") != 287
        or failure_done.get("termination_names") != ["ee_body_pos"]
        or failure_ee.get("dominant_absolute_z_error_body") != "right_wrist_roll_rubber_hand"
    ):
        raise ValueError("cutoff100 decision evidence drift")
    return body
...
def _qualification_scope(root: Path, runtime_seed: int) -> Iterator[None]:
    with cutoff140._qualification_scope(root, runtime_seed), _source_scope():  # noqa: SLF001
        yield
...
def preflight(*, repository_root: Path, output: Path, runtime_seed: int) -> Mapping[str, Any]:
    ...
    if os.path.lexists(request.output_path):
        raise FileExistsError("cutoff100 qualification output exists")
...
def run_qualification(*, repository_root: Path, output: Path, runtime_seed: int) -> Mapping[str, Any]:
    ...
    with _qualification_scope(root, runtime_seed):
        ...
        report["hybrid_qualification"] = {
            ...
            "controller": "student_q9_9_through_108_then_exact_selected_teacher_q9_109_through_518",
