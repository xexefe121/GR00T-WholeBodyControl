...
from gear_sonic.scripts.qualify_g1_true23_sonic_nominal_dagger_lowrank_student import _parser
from gear_sonic.utils import (
    g1_true23_sonic_nominal_dagger_lowrank_last_block_bc_v2 as fit,
    g1_true23_sonic_nominal_dagger_lowrank_student_qualification as adapter,
...
def test_adapter_scope_patches_and_restores_globals() -> None:
    old_seed = student.FIXED_SEED
    old_names = student.FROZEN_INPUT_NAMES
    with adapter._adapter_scope(REPO_ROOT, 20260805):  # noqa: SLF001
        assert student.FIXED_SEED == 20260805
        assert student.BC_MANIFEST_KIND == fit.MANIFEST_KIND
        assert student.BC_CONTRACT_SHA256 == fit.CONTRACT_SHA256
        assert student.FROZEN_INPUT_NAMES == old_names + adapter.EXTRA_NAMES
