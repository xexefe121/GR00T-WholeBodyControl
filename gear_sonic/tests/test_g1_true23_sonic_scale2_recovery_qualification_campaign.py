...
from gear_sonic.utils import g1_true23_sonic_scale2_recovery_qualification_campaign as campaign
...
def test_contract_reuses_exact_twenty_seed_vector_cohort() -> None:
    ...
    campaign._verify_prerequisites(REPOSITORY_ROOT, contract)  # noqa: SLF001
    ...
    assert contract["disturbance"]["global_apply_transition"] == 291
    assert contract["disturbance"]["apply_q9"] == 300


def test_request_output_is_repository_contained() -> None:
    request = campaign.CampaignRequest(
        REPOSITORY_ROOT,
        Path("artifacts/g1_true23/scale2_campaign_test.json"),
    )
...
def _passing_probe(spec: object) -> dict[str, object]:
    ...
    return {
        ...
        "impulse_apply_transition": None if scenario == "nominal" else 291,
        "impulse_apply_q9": None if scenario == "nominal" else 300,
        ...
        "stable_streak_end_transition": None if scenario == "nominal" else 295,
        "recovery_time_s": None if scenario == "nominal" else 0.1,
        "recovered": None if scenario == "nominal" else True,
    }


def test_q250_pass_report_satisfies_nominal_run_assessor() -> None:
    contract = campaign.load_contract(REPOSITORY_ROOT)
    ...
    report = copy.deepcopy(dict(prerequisites["q250"]))
    ...
    assert record["published_teacher_label_count"] == 0


def test_probe_recovery_requires_five_stable_steps_within_two_seconds() -> None:
    contract = campaign.load_contract(REPOSITORY_ROOT)
    campaign._verify_prerequisites(REPOSITORY_ROOT, contract)  # noqa: SLF001
    ...
    probe.impulse_apply_transition = 291
    probe.impulse_apply_q9 = 300
    probe.qvel_write_exact = True
    probe.readback_error = 0.0
    probe.metric_values = [0.2] * 510
    result = probe.report()
    assert result["recovered"] is True
    assert result["stable_streak_end_transition"] == 295
