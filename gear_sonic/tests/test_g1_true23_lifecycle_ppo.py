...
def test_launcher_supplies_required_checkpoint_contract(monkeypatch):
    from gear_sonic.scripts import train_g1_true23_lifecycle_ppo as script

    contract = {"kind": "test-frozen-contract"}
    monkeypatch.setattr(script.torch, "load", lambda *args, **kwargs: {"adapter_contract": contract})

    def checked(path, *, expected_contract):
        assert path == "initial.pt" and expected_contract is contract
        return {"verified": True}

    monkeypatch.setattr(script, "load_frozen_platform_lora_checkpoint", checked)
    assert script.read_initial_actor("initial.pt") == {"verified": True}


def test_launcher_binds_descriptor_paths_without_treating_metadata_as_files():
    from gear_sonic.scripts import train_g1_true23_lifecycle_ppo as script

    initialization = dict(
        kind="initialization",
        report_path="fit.json",
        report_sha256="fit-hash",
        adapter_path="standing.pt",
        adapter_file_sha256="adapter-hash",
        hardware_authorized=False,
    )
    retention = dict(
        kind="retention",
        teacher_report_path="teacher.json",
        teacher_report_sha256="teacher-hash",
        train_rows=1500,
        episodes=[dict(path=f"episode{i}.npz", sha256=f"hash{i}") for i in range(3)],
    )
    calls = []
    script.bind_standing_inputs(initialization, retention, lambda path, digest: calls.append((path, digest)))
    assert calls == [
        ("fit.json", "fit-hash"),
        ("standing.pt", "adapter-hash"),
        ("teacher.json", "teacher-hash"),
        ("episode0.npz", "hash0"),
        ("episode1.npz", "hash1"),
        ("episode2.npz", "hash2"),
    ]
