...
from gear_sonic.scripts.train_g1_true23_decoder_lora import validate_frozen_recipe
from gear_sonic.tests.test_native23_original_intent_contract import fresh
from gear_sonic.trl.mjlab.native23_decoder_lora_runner import CHECKPOINT_HEADER
from gear_sonic.trl.mjlab.native23_original_intent_actor import ACTOR_KIND as OLD_KIND
from gear_sonic.utils.g1_true23_decoder_lora_checkpoint import DecoderLoraCPUActor, validate_semantics
...
def test_old_reader_rejects_new_header_and_non_actor_cannot_infer():
    ...
    with pytest.raises(TypeError, match="distinct actor"):
        DecoderLoraCPUActor(nn.Linear(2, 2))
...
def test_first_affine_reader_rejects_decoder_wide_header():
    from gear_sonic.utils.g1_true23_pose_lora_checkpoint import validate_semantics as first_reader

    with pytest.raises(ValueError, match="header"):
        first_reader({"header": CHECKPOINT_HEADER})


def test_new_header_cannot_relabel_first_affine_actor():
    from gear_sonic.trl.mjlab.native23_pose_lora_actor import ACTOR_KIND as FIRST_KIND

    with pytest.raises(ValueError, match="old/relabelled"):
        validate_semantics({"header": CHECKPOINT_HEADER, "actor": {"contract": {"kind": FIRST_KIND}}})
