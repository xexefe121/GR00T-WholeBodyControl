...
from gear_sonic.trl.mjlab.native23_frozen_decoder_actor import ACTOR_KIND, True23FrozenDecoderActorModel
...
def test_gradient_flows_through_decoder_without_changing_its_weights(actor):
    ...
    try:
        ...
        assert [len(g["params"]) for g in groups] == [1, 1, 2]
...
def lineage(tmp_path_factory):
    directory = tmp_path_factory.mktemp("frozen_decoder_lineage")
    manifests = {}
    ...
    return build_mjlab_training_lineage(
        WARM,
        resolved_config={"native23_frozen_decoder": training_contract()},
...
def runner_fixture(actor, lineage, directory):
    runner = object.__new__(Native23FrozenDecoderRunner)
...
def test_new_snapshot_roundtrip_old_reader_and_resume_rejected(actor, lineage, tmp_path):
    ...
    assert path.name == "frozen_decoder_model_2.pt"
    runner.save(path)
    saved = torch.load(path, map_location="cpu", weights_only=True)
    assert saved["header"] == CHECKPOINT_HEADER
    validate_frozen_decoder_checkpoint(saved, actor=actor, lineage=lineage)
...
def test_snapshot_tampering_rejected(actor, lineage, tmp_path, tamper):
    ...
    with pytest.raises(ValueError):
        validate_frozen_decoder_checkpoint(payload, actor=actor, lineage=lineage)
