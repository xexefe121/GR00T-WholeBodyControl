...
from gear_sonic.trl.mjlab.native23_decoder_lora_actor import ACTOR_KIND, True23DecoderLoraActorModel
...
def actor():
    return True23DecoderLoraActorModel(
        observations(),
...
def test_zero_step_exact_released_mean_and_honest_contract(actor):
    ...
    assert contract["decoder_adapter"]["trainable_parameters"] == 507792
    assert contract["decoder_adapter"]["linear_module_indices"] == list(range(0, 18, 2))
    assert len(actor.parameter_groups()["decoder_adapters"]) == 18
    assert not contract["full_decoder_checkpoint_relabelling_allowed"]


def test_tail_initialization_preserves_global_rng(actor):
    from gear_sonic.trl.mjlab.native23_decoder_lora_actor import initialize_tail_parameters

    before = torch.random.get_rng_state()
    a, b = initialize_tail_parameters([actor.core.decoder.module[i] for i in range(2, 18, 2)])
    assert torch.equal(before, torch.random.get_rng_state())
    for expected, actual in zip(actor.decoder_lora_a, a, strict=True):
        assert torch.equal(expected, actual)
    assert all(not torch.count_nonzero(v) for v in b)


def test_each_tail_layer_receives_real_gradient(actor):
    before = actor.export_training_artifact()
    try:
        optimizer = torch.optim.Adam(actor.parameter_groups()["decoder_adapters"], lr=1e-4)
        obs = observations()
        obs["root_feedback"].zero_()
        for step in range(2):
            actor.zero_grad(set_to_none=True)
            actor(obs).square().mean().backward()
            for a, b in zip(actor.decoder_lora_a, actor.decoder_lora_b, strict=True):
                assert torch.isfinite(b.grad).all() and torch.count_nonzero(b.grad)
                assert bool(torch.count_nonzero(a.grad)) == bool(step)
            optimizer.step()
...
def test_cpu_reader_executes_tail_and_old_first_layer_reader_rejects(actor):
    import numpy as np

    from gear_sonic.utils.g1_true23_decoder_lora_checkpoint import DecoderLoraCPUActor
    ...
    try:
        with torch.no_grad():
            original = actor(obs).numpy()[0].copy()
            actor.decoder_lora_b[-1].fill_(0.025)
            expected = actor(obs).numpy()[0].copy()
        inputs = [obs[key].numpy()[0].copy() for key in ("tokenizer", "policy", "root_feedback")]
        actual, _ = DecoderLoraCPUActor(actor).infer(*inputs)
        assert np.array_equal(expected, actual)
        assert not np.array_equal(original, actual)
        with pytest.raises(TypeError):
            PoseLoraCPUActor(actor)
    finally:
        actor.load_training_artifact(before)
...
def test_gradient_flows_through_decoder_without_changing_its_weights(actor):
    ...
    try:
        ...
        assert [len(g["params"]) for g in groups] == [1, 18, 1, 2]
...
def test_cpu_reader_executes_nonzero_pose_branch_not_legacy_root_only(actor):
    ...
    try:
        ...
        inputs = [obs[key].numpy()[0].copy() for key in ("tokenizer", "policy", "root_feedback")]
        actual, trace = DecoderLoraCPUActor(actor).infer(*inputs)
...
def lineage(tmp_path_factory):
    directory = tmp_path_factory.mktemp("decoder_lora_lineage")
    manifests = {}
    ...
    return build_mjlab_training_lineage(
        WARM,
        resolved_config={"native23_decoder_lora": training_contract()},
...
def runner_fixture(actor, lineage, directory):
    runner = object.__new__(Native23DecoderLoraRunner)
...
def test_new_snapshot_roundtrip_old_reader_and_resume_rejected(actor, lineage, tmp_path):
    ...
    assert path.name == "decoder_lora_model_2.pt"
    runner.save(path)
    saved = torch.load(path, map_location="cpu", weights_only=True)
    assert saved["header"] == CHECKPOINT_HEADER
    validate_decoder_lora_checkpoint(saved, actor=actor, lineage=lineage)
...
def test_snapshot_tampering_rejected(actor, lineage, tmp_path, tamper):
    ...
    with pytest.raises(ValueError):
        validate_decoder_lora_checkpoint(payload, actor=actor, lineage=lineage)
