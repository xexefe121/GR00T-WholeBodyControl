...
from gear_sonic.utils import g1_true23_sonic_seed835_failure_prefix_dagger_round3 as collector

REPOSITORY_ROOT = Path(__file__).resolve().parents[2]


def test_contract_binds_round3_failure_boundary() -> None:
    contract = collector.load_contract(REPOSITORY_ROOT)
    assert contract["rows"]["total"] == 466
    assert contract["rows"]["q9_last"] == 474
...
def test_array_validator_exact_q9() -> None:
    ...
    arrays = {name: np.zeros((466, width), dtype=np.float32) for name, width in widths.items()}
    arrays["q9"] = np.arange(9, 475, dtype=np.int64)
